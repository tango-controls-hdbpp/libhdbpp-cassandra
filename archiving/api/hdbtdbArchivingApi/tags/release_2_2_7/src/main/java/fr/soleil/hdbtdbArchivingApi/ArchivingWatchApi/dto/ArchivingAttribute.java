...
public class ArchivingAttribute {
	...
	private static Hashtable<String, Timestamp> lastInserts = new Hashtable<String, Timestamp>();
	...
	private void parse() {
		...
		try {
			...
			String dev = dom + Tools.TANGO_SEPARATOR + fam
					+ Tools.TANGO_SEPARATOR + mem;
	...
	/**
	 * An attribute is called "determined" if its OK/KO status is known with
	 * certainty. Conversely, it is called "undetermined" if an error (SQL
	 * problem or other) has prevented determining this status with certainty.
	 *
