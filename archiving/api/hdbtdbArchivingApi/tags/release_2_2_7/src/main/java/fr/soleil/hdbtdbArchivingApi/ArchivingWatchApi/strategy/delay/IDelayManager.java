...
import fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.devicelink.Warner;
 ...
 * <UL>
 * <LI>The cycle: lasts for macroPeriod (cf. device properties), and covers all
 * controlled attributes
 * <LI>The step: a subdivision of the cycle, it is the atomic control unit
 * <LI>Timescales bigger than macroPeriod: references to older Control Results
 * are stored, for example the most recent CR having KO attributes
 * </UL>
 *
 * @author CLAISSE
 */
public interface IDelayManager extends Warner {
	...
	/**
	 * The code for reading the latest complete full cycle, or (if there is
	 * none) the latest control step
	 */
	...
	/**
	 * The code for reading the latest control result added to the curren cycle
	 * (coincides with READ_LATEST_COMPLETE_STEP)
	 */
	...
	/**
	 * The code for reading the current rollover control result (rollover
	 * meaning that each time a step's attributes are added, they override the
	 * same attributes' older result )
	 */
	 ...
	 */
	public boolean addControlResult(ControlResult controlResult,
	 ...
	 *
	 * @param readType
	 *            The type of CR one wishes to read
	...
	public Hashtable chooseAttributesSubsetForCurrentStep(
			Hashtable filteredAttributes) throws DevFailed;
