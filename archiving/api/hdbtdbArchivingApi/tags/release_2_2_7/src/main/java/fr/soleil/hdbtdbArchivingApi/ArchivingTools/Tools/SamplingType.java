...
//
// Description:  (Claisse Laurent) - 29 août 2006
//
// $Author:  $
//
// $Revision: $
 ...
 * 
 */
public class SamplingType implements java.io.Serializable {
	private short type;
	...
	private SamplingType(short _type, String _oracleFormat,
			String _mySqlFormat, String _label) {
		this.type = _type;
	...
	public static SamplingType[] getChoices() {
		// Basic types
		SamplingType defaultSampling = SamplingType
				.getSamplingType(SamplingType.ALL);
		SamplingType everySecondSampling = SamplingType
				.getSamplingType(SamplingType.SECOND);
		SamplingType everyMinuteSampling = SamplingType
				.getSamplingType(SamplingType.MINUTE);
		SamplingType everyHourSampling = SamplingType
				.getSamplingType(SamplingType.HOUR);
		SamplingType everyDaySampling = SamplingType
				.getSamplingType(SamplingType.DAY);
	...
	public String getDescription() {
		...
		ret += CRLF;
		ret += "getAdditionalFilteringFactor: "
	...
	public static SamplingType getSamplingTypeByLabel(String typeName)
		...
		if (invalidType) {
			throw new IllegalArgumentException(
					"SamplingType.getSamplingTypeByLabel ( " + typeName
							+ " ): Invalid typeName.");
		}
	...
	public SamplingType cloneSamplingType() {
		SamplingType newSamplingType = SamplingType.getSamplingType(this
				.getType());
	...
	public String getOneLevelHigherFormat(boolean b) {
		...
		SamplingType oneLevelHigherSamplingType = SamplingType
				.getSamplingType(oneLevelHigherType);
	...
	public String getAdditionalFilteringClause(boolean isMySQL, String field) {
		...
		if (isMySQL) {
			...
			String to_char = "DATE_FORMAT(" + field + ", "
					+ typeOfAdditionalFiltering + ")";
			String to_number = to_char;
			return ", FLOOR(" + to_number + "/"
					+ this.additionalFilteringFactor + ")";
		} else {
			String typeOfAdditionalFiltering;
			switch (this.type) {
			case SamplingType.SECOND:
