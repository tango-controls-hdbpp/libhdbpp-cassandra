...
public abstract class ArchivingManagerApiRef implements IArchivingManagerApiRef {
    ...
    private static Map<String, AttributeInfo> attributeToDeviceInfo = new Hashtable<String, AttributeInfo>();
     ...
     *
     */
    public ArchivingManagerApiRef(int type) {
        this.m_type = type;
    }
    ...
    public String getStatus(String attributeName, boolean historic) throws ArchivingException {
        ...
        try {
            String archiverName = "";
            if (m_DataBase != null) {
                archiverName = m_DataBase.getMode().getDeviceInCharge(attributeName);
            }
        ...
        try {
            ...
            for (int i = 0; i < devicesList.length; i++) {
                if (TangoAccess.deviceLivingTest(devicesList[i])) {
                    runningDevicesListVector.add(((facility) ? "//" + dbase.get_tango_host() + "/"
                            : "")
                            + devicesList[i]);
                }
                else {
                    notRunningDevicesListVector.add(((facility) ? "//" + dbase.get_tango_host()
                            + "/" : "")
                            + devicesList[i]);
                }
            }
            ...
            for (int i = 0; i < notRunningDevicesListVector.size(); i++) {
                ...
                System.out
                        .println(db + "Archiver (NOT EXPORTED) " + m_dbNotExportedArchiverList[i]);
            }
     ...
     * 
     * @param historic true if historical archiving, false otherwise.
     * @param archivingMessConfig An ArchivingMessConfig object (a list of AttributeLightMode
     *            object)
     * @throws ArchivingException
     */
    public void ArchivingStart(ArchivingMessConfig archivingMessConfig) throws ArchivingException {
        ...
        Vector<String> goodAttributeList = new Vector<String>();
        // The badAttributesList will be used to add a FaultingAttribute for
        // each attribute that causes an ArchivingException
        Vector<String> badAttributeList = new Vector<String>();
        // Check the support type, format and writable...
        Enumeration<String> attributeNameList = archivingMessConfig.getAttributeListKeys();
        while (attributeNameList.hasMoreElements()) {
            ...
            try {
                if (checkAttributeSupport(attribute)) {
                    ...
                    String desc = "The archiving of the attribute named '" + attribute + "'"
                            + "was not launched ! : \r\n";
                    ArchivingException archivingException = new ArchivingException(message, reason,
                            ErrSeverity.WARN, desc, "");
                    archivingStartException.addStack(message, reason, ErrSeverity.WARN, desc, "",
                            archivingException);
        ...
        if (goodAttributeList.size() == 0) {
            ...
            if (badAttributeList.size() > 0) {
                archivingException = new AttributesArchivingException(reason, reason,
                        ErrSeverity.ERR, reason, "ArchivingManagerApi");
                ((AttributesArchivingException) archivingException).getFaultingAttributes().addAll(
                        archivingStartException.getFaultingAttributes());
            }
            else {
                archivingException = new ArchivingException(reason, reason, ErrSeverity.ERR,
                        reason, "ArchivingManagerApi");
            }
        ...
        LoadBalancedList loadBalancedList = selectArchiversTask(archivingMessConfig.isGrouped(),
                archivingMessConfig.getAttributeToDedicatedArchiverHasthable(), finalAttributeList);
        // Sending order to archivers
        Enumeration<String> my_archivers = loadBalancedList.getArchivers();
        ...
        while (my_archivers.hasMoreElements()) {
            ...
            if (assignedAttributes.length > 0) {
                ...
                try {
                    ...
                    String[] messConfigArray = archiver_archivingMessConfig.toArray();
     ...
     */
    private String[] checkRegistration(String[] attributeNameList) throws ArchivingException {
        String tangoHost = TangoAccess.getTangoHost();
        Vector<String> myNewListV = new Vector<String>();
        for (int i = 0; i < attributeNameList.length; i++) {
            ...
            try {
                ...
                // data
                // type
                attributeHeavy.setData_format(attributeInfo.data_format.value()); // ****************
                // Attribute
                // data
                // format
                ...
                String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : "
                        + GlobalConst.DEV_UNREACH_EXCEPTION + " (" + attributeName + ")";
                String reason = GlobalConst.TANGO_COMM_EXCEPTION;
     ...
     * @throws ArchivingException
     */
    private static void throwArchivingStartException(ArchivingException archivingStartException,
            boolean needsThrow, int badAttributeListSize) throws ArchivingException {
        if (needsThrow) {
            String message = GlobalConst.ARCHIVING_ERROR_PREFIX;
            String reason = "Failed while executing ArchivingManagerApi.ArchivingStart() method...";

            if (badAttributeListSize != -1) {
                String desc = "Impossible to launch the archiving of " + badAttributeListSize
                        + " attributes ! : \r\n";
                ArchivingException aef = new ArchivingException(message, reason, ErrSeverity.PANIC,
                        desc, "ArchivingManagerApi");
                archivingStartException.addStack(message, reason, ErrSeverity.PANIC, desc,
                        "ArchivingManagerApi", aef);
            }
    ...
    private static void throwArchivingStopException(ArchivingException archivingStopException,
            boolean needsThrow, int badAttributeListSize) throws ArchivingException {
        if (needsThrow) {
            String message = GlobalConst.ARCHIVING_ERROR_PREFIX;
            ...
            if (badAttributeListSize != -1) {
                String desc = "Impossible to stop the archiving of " + badAttributeListSize
                        + " attributes ! : \r\n";
                ArchivingException aef = new ArchivingException(message, reason, ErrSeverity.PANIC,
                        desc, "ArchivingManagerApi");
                archivingStopException.addStack(message, reason, ErrSeverity.PANIC, desc,
                        "ArchivingManagerApi", aef);
            }
     ...
     * 
     * @param historic true if historical archiving, false otherwise.
     * @param attributeNameList the list of attributes (The archiving of each element of this list
    ...
    public void ArchivingStop(String[] attributeNameList) throws ArchivingException {
        ...
        if (!IsArchivedStop(attributeNameList))
            throw new ArchivingException(GlobalConst.ALREADY_ARCHIVINGSTOP,
                    GlobalConst.ALREADY_ARCHIVINGSTOP, ErrSeverity.WARN,
        ...
        AttributesArchivingException archivingStopException = new AttributesArchivingException();
        boolean initCauseAlreadyDoneInException = false;
        IGenericMode genericMode = m_DataBase.getMode();
            ...
            try {
                ...
                if (archiverName == "" || !TangoAccess.deviceLivingTest(archiverName))
                    throw new ArchivingException(GlobalConst.ARC_UNREACH_EXCEPTION,
                            GlobalConst.ARC_UNREACH_EXCEPTION, ErrSeverity.WARN,
                            "Archiver name is empty or the given device is not alive", this
                                    .getClass().getName());
                ...
                if (isArchived(attributeName)) {
                     ...
                     */
                    int[] tfw = m_DataBase.getAttribute().getAtt_TFW_Data(attributeName);
                ...
                String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : ";
                String reason = "Failed while executing ArchivingManagerApi.ArchivingStop() method...";
                StringBuffer descBuffer = new StringBuffer(GlobalConst.ARCHIVING_ERROR_PREFIX);
                descBuffer.append(" : ");
                ...
                archivingStopException.addStack(message, reason, ErrSeverity.WARN, desc, "",
                        archivingException);
                archivingStopException.getFaultingAttributes().add(
                        new FaultingAttribute(attributeName, desc));
                needsThrow = true;
                badAttributesCount++;
            }
                ...
                String desc = descBuffer.toString();
                descBuffer = null;
                archivingStopException.addStack(ae.getMessage(), ae.getLastExceptionReason(),
                        ErrSeverity.WARN, desc, "", ae);
                archivingStopException.getFaultingAttributes().add(
                        new FaultingAttribute(attributeName, desc));
                needsThrow = true;
                badAttributesCount++;
                ...
                String reason = "Failed while executing ArchivingManagerApi.ArchivingStop() method...";
                StringBuffer descBuffer = new StringBuffer(GlobalConst.ARCHIVING_ERROR_PREFIX);
                descBuffer.append(" : Unexpected exception");
                descBuffer.append(" (Archiver: ");
                descBuffer.append(archiverName);
                descBuffer.append(") - error class:");
                descBuffer.append(e.getClass().getSimpleName());
                descBuffer.append(" - error message:");
                descBuffer.append(e.getMessage());
                String desc = descBuffer.toString();
                descBuffer = null;
                ArchivingException archivingException = new ArchivingException(message, reason,
                        ErrSeverity.PANIC, desc, "ArchivingManagerApi");
    ...
    public void ArchivingStopConf(String[] attributeNameList) throws ArchivingException {
        ...
        try {
            ...
            String message = GlobalConst.ERROR_ARCHIVINGSTOP + " : "
                    + "Problems talking to the archiver " + archiverName + " for the attribute "
                    + attributeName;
            String reason = GlobalConst.TANGO_COMM_EXCEPTION;
            String desc = "Failed while executing ArchivingManagerApi.ArchivingStopConf() method...";

            ArchivingException toBeThrown = new ArchivingException(message, reason,
                    ErrSeverity.WARN, desc, "", devFailed);
     ...
     * @return the mode
     */
    public Mode GetArchivingMode(String attributeName) throws ArchivingException {
        // if (isRegistred(attributeName)) {
        // return m_DataBase.getMode().getCurrentArchivingMode(attributeName);
        // }
        // Avoid 2 requests
        Mode mode = m_DataBase.getMode().getCurrentArchivingMode(attributeName);
        ...
        if (mode == null || (mode.toArray().length == 0)) {
            ...
            throw new ArchivingException(buffer.toString(), buffer.toString(), ErrSeverity.WARN,
                    "Failed while executing ArchivingManagerApiRef.getArchivingMode(), the \""
                            + attributeName + "\" attribute didn't have an archiving mode", this
                            .getClass().getName());
        }
        return mode;
    }
     ...
     */
    protected boolean isRegistred(String attributeName) throws ArchivingException {
        if (m_DataBase != null) {
     ...
     * @throws ArchivingException
     */
    private boolean IsArchivedStop(String[] attributeNameList) throws ArchivingException {
        for (int i = 0; i < attributeNameList.length; i++) {
    ...
    private static void setAttributeInfoInArchivingMess(ArchivingMessConfig archivingMessConfig)
            throws ArchivingException {
    ...
    private LoadBalancedList selectArchiversTask(boolean grouped,
            Hashtable attributeToChosenArchiverHasthable, String[] attributeNameList)
            throws ArchivingException {
        Hashtable attributeToDedicatedArchiver = getAttributesToDedicatedArchiver(false);// CLA
        // 10/11/06

        LoadBalancedList loadBalancedList = new LoadBalancedList(DbUtils.getHdbTdbType(m_DataBase
                .getDbConn().getDbType()) == ConfigConst.HDB);
        ...
        for (int i = 0; i < attributeNameList.length; i++) {
            ...
            archiver = m_DataBase.getMode().getArchiverForAttribute(currentAttributeName);
            // System.out.println (
     ...
     */
    public static AttributeInfo getAttributeInfo(String attributeName, boolean doRefresh)
            throws DevFailed {
    ...
    private static AttributeInfo getAttributeInfoFromTheDevice(String attributeName)
            throws DevFailed {
        int index = attributeName.lastIndexOf("/");
     ...
     * @throws ArchivingException
     */
    private void connectArchivingDatabase(String mdbhost, String mdbname, String muser,
            String mpassword, boolean rac, boolean harvestableCnx, boolean autoCommit)
            throws ArchivingException {
        if (m_DataBase == null) {
            // Host
            String host = mdbhost;
            ...
            m_DataBase = DataBaseManagerFactory.getDataBaseManager(m_type);
            m_DataBase.setDbConn(ConnectionFactory.connect_auto(host, name, schema, user, pass,
                    m_type, rac, harvestableCnx, autoCommit));
            is_connected = true;
        }
    }
    ...
    public void ArchivingConfigure(String user, String password) throws ArchivingException {
        ...
        ArchivingConfigure(user, password, false);
    }

    public void ArchivingConfigure(String user, String password, boolean rac)
            throws ArchivingException {

        ArchivingConfigureWithoutArchiverListInit("", "", user, password, rac, false, false);
     ...
     * @throws ArchivingException
     */
    public void ArchivingConfigure(String host, String name, String user, String password,
            boolean rac, boolean harvestableCnx, boolean autoCommit) throws ArchivingException {

        ArchivingConfigureWithoutArchiverListInit(host, name, user, password, rac, harvestableCnx,
                autoCommit);
    ...
    public void ArchivingConfigureWithoutArchiverListInit(String user, String password)
            throws ArchivingException {

        ArchivingConfigureWithoutArchiverListInit("", "", user, password, false, false, false);

    }

    public void ArchivingConfigureWithoutArchiverListInit(String host, String name, String user,
            String password, boolean rac, boolean harvestableCnx, boolean autoCommit)
            throws ArchivingException {
        // System.out.println(
        ...
        // Database connection
        connectArchivingDatabase(host, name, user, password, rac, harvestableCnx, autoCommit);
    }
     ...
     */
    public static Vector<AttributeLightMode> stoppingVector(String[] my_light_attributs) {
        Vector<AttributeLightMode> myResultVect = new Vector<AttributeLightMode>();
        int index = 0;
    ...
    public static boolean checkAttributeSupport(String attributeName) throws ArchivingException {
        try {
            ...
            return AttributeSupport.checkAttributeSupport(attributeName, data_type, data_format,
                    writable);
        }
        catch (DevFailed devFailed) {
            String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : "
                    + GlobalConst.TANGO_COMM_EXCEPTION + " or " + GlobalConst.ATT_UNREACH_EXCEPTION;
            ...
            String desc = "Failed while retrieving informations for the attribute named '"
                    + attributeName + "'";
            throw new ArchivingException(message, reason, ErrSeverity.PANIC, desc, DevFailed.class
                    .getName(), devFailed);
        }
    }
    ...
    public Hashtable getAttributesToDedicatedArchiver(boolean refreshArchivers)
        ...
        for (int i = 0; i < exportedArchiversNameList.length; i++) {
            ...
            Archiver currentExportedArchiver = Archiver.findArchiver(currentExportedArchiverName,
        ...
        for (int i = 0; i < notExportedArchiversNameList.length; i++) {
            ...
            Archiver currentNotExportedArchiver = Archiver.findArchiver(
                    currentNotExportedArchiverName, DbUtils.getHdbTdbType(m_DataBase.getDbConn()
        ...
        Archiver.setAttributesToDedicatedArchiver(DbUtils.getHdbTdbType(m_DataBase.getDbConn()
     ...
     *
     */
    private static void initArchivers(LoadBalancedList loadBalancedList, boolean grouped,
            String[] referenceArchiverList) throws ArchivingException {
