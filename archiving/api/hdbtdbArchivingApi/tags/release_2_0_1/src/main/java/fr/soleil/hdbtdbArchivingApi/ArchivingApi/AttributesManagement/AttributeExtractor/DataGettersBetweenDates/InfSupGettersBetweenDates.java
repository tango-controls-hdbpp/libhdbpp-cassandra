...
public class InfSupGettersBetweenDates extends DataExtractor {
	 ...
	 * @param at
	 */
	public InfSupGettersBetweenDates(IDBConnection con, IAdtAptAttributes at) {
		super(con, at);
	...
	private DevVarDoubleStringArray getAttScalarDataInfThanBetweenDates(String att_name , int upper_value , String time_0 , String time_1 , int writable) throws ArchivingException
	{
		...
		String fields = ro_fields ?
				( dbConn.getDbUtils().toDbTimeFieldString(ConfigConst.TAB_SCALAR_RO[ 0 ]) + ", " + ConfigConst.TAB_SCALAR_RO[ 1 ] ) :
					( dbConn.getDbUtils().toDbTimeFieldString(ConfigConst.TAB_SCALAR_RW[ 0 ]) + ", " + ConfigConst.TAB_SCALAR_RW[ 1 ] + ", " + ConfigConst.TAB_SCALAR_RW[ 2 ] ); // if (writable == AttrWriteType._READ_WITH_WRITE || writable == AttrWriteType._READ_WRITE)
				String selectField = ro_fields ? ConfigConst.TAB_SCALAR_RO[ 1 ] : ConfigConst.TAB_SCALAR_RW[ 1 ];
				String dateClause = ConfigConst.TAB_SCALAR_RO[ 0 ] + " BETWEEN " + dbConn.getDbUtils().toDbTimeString(time_0.trim()) + " AND " + dbConn.getDbUtils().toDbTimeString(time_1.trim());
				// My statement
