...
public class AttributeMembers extends AttributesData {
	...
	public String[] get_members() throws ArchivingException {
		...
		String sqlStr;
		sqlStr = "SELECT DISTINCT " + ConfigConst.TAB_DEF[6] + " FROM "
	...
	public int get_membersCount() throws ArchivingException {
		...
		// Create and execute the SQL query string
		String sqlStr;
		sqlStr = "SELECT COUNT(DISTINCT " + ConfigConst.TAB_DEF[6] + ") FROM "
				+ dbConn.getSchema() + "." + ConfigConst.TABS[0];
		sqlStr = sqlStr + " WHERE " + ConfigConst.TAB_DEF[6] + " IS NOT NULL";
	 ...
	 * @throws ArchivingException
	 */
	public String[] get_members(String domain_name, String family_name)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		if (dbConn == null)
			return null;
		...
		sqlStr = "SELECT DISTINCT " + ConfigConst.TAB_DEF[6] + " FROM "
				+ dbConn.getSchema() + "." + ConfigConst.TABS[0];
		sqlStr = sqlStr + " WHERE (";
		sqlStr = sqlStr + ConfigConst.TAB_DEF[4] + " LIKE " + "'"
				+ domain_name.replace('*', '%') + "'";
		sqlStr = sqlStr + " AND ";
		sqlStr = sqlStr + ConfigConst.TAB_DEF[5] + " LIKE " + "'"
				+ family_name.replace('*', '%') + "'";
		sqlStr = sqlStr + " AND ";
	...
	public String[] get_members_by_criterion(String domain_name,
			String family_name, String member_regexp) throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		if (dbConn == null)
			return null;
		...
		sqlStr = sqlStr + ConfigConst.TAB_DEF[5] + " LIKE " + "'"
				+ family_name.replace('*', '%') + "'";
		sqlStr = sqlStr + " AND ";
		sqlStr = sqlStr + ConfigConst.TAB_DEF[6] + " LIKE " + "'"
				+ member_regexp.replace('*', '%') + "'";
		sqlStr = sqlStr + " ) ";
		// CLA 25/07/06
	 ...
	 * @throws ArchivingException
	 */
	public int get_membersCount(String domain_name, String family_name)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		if (dbConn == null)
			return 0;
