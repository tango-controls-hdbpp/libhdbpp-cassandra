...
/*	public  static IDBConnection connect_auto(int type) throws ArchivingException
	{
		...
		catch ( ArchivingException e )
		{
			...
			String reason = "Failed while executing ConnectionFactory.connect_auto()YYYY method...";
		...
		{
			String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : " + "CONNECTION FAILED !";
			String reason = "Failed while executing ConnectionFactory.connect_auto() XXX method...";
	...
	public  static IDBConnection connect_auto(String host, String name, String schema, String user, String pass, int type, boolean rac) throws ArchivingException
	{
		...
		if(dbConn == null){
			...
			try
			{
				...
				dbConn.connect(rac);
				System.out.println("ConnectionFactory.connect_auto (Oracle): " + user + "@" + name + " has been done.");
			}
			catch ( ArchivingException e )
			{
				initDbConn(type);
				String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " Database Connection FAILED";
				...
				String desc = "Oracle database Connection FAILED with the following parameters : \n\t\tuser = "
					+ user +" password = " + pass + " rac = " + rac + " host = " + host+ " name = "  + name;
				...
				try
				{
					...
					// try MySQL;
					dbConn.connect(false);
					System.out.println("ConnectionFactory.connect_auto (MySql): " + dbConn.getUser() + "@" + dbConn.getName() + " has been done.");
				}
				catch ( ArchivingException e1 )
				{
					...
					desc = "MySql database Connection FAILED with the following parameters : \n\t\tuser = "
						+ user + " password = " + pass + " host = " + host+ " name = "  + name;

					archivingException.addStack(message , reason , ErrSeverity.PANIC , desc , "" , e1);
					//System.out.println(desc);
					throw archivingException;
				}
			}
