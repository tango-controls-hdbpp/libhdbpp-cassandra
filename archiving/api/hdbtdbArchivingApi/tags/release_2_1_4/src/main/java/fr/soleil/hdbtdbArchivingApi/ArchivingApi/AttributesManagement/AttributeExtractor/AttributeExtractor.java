...
public class AttributeExtractor extends DataExtractor {
     ...
     * data beetwen two dates, for a given scalar attribute.
     * 
     * @param argin
     ...
     */
    public Vector<ImageData> getAttPartialImageDataBetweenDates(String[] argin,
        ...
        if (dbConn == null || att == null || dbUtils == null) return null;
        ...
        if (samplingType.hasSampling()) {
            ...
            String format = dbUtils.getFormat(samplingType);// isMySQL ?
                                                            // samplingType.getMySqlFormat
                                                            // () :
                                                            // samplingType.getOracleFormat
                                                            // ();

            String selectField_0 = dbUtils.toDbTimeFieldString(
                    ConfigConst.TAB_IMAGE_RO[0], format);
            String selectField_1 = SamplingType.AVERAGING_NORMALISATION + " ( "
                    + ConfigConst.TAB_IMAGE_RO[1] + " ) ";
            String selectField_2 = SamplingType.AVERAGING_NORMALISATION + " ( "
                    + ConfigConst.TAB_IMAGE_RO[2] + " ) ";
            fields = selectField_0 + ", " + selectField_1 + ", "
                    + selectField_2;

            whereClause = ConfigConst.TAB_IMAGE_RO[0] + " BETWEEN "
            ...
            fields = (dbUtils.toDbTimeFieldString(ConfigConst.TAB_IMAGE_RO[0])
                    + ", " + ConfigConst.TAB_IMAGE_RO[1] + ", " + ConfigConst.TAB_IMAGE_RO[2]);
            whereClause = ConfigConst.TAB_IMAGE_RO[0] + " BETWEEN "
                    + dbUtils.toDbTimeString(time_0.trim()) + " AND "
                    + dbUtils.toDbTimeString(time_1.trim());
            query = "SELECT " + fields + " FROM " + tableName + " WHERE " + "("
        ...
        // System.out.println (
        // "CLA/getAttPartialImageDataBetweenDates/query/"+query+"/" );
        ...
        try {
            ...
            // System.out.println (
            // "CLA/getAttPartialImageDataBetweenDates/AFTER/"+new Timestamp
            // (System.currentTimeMillis()) );
            Vector<ImageData> ret = new Vector<ImageData>();
            ...
            while (rset.next()) {
                ...
                data
                        .setHistoric(DbUtils.getHdbTdbType(dbConn.getDbType()) == ConfigConst.HDB);

                ret.add(data);
                // System.out.println (
                // "CLA/getAttPartialImageDataBetweenDates/next/date|"+date+"|"
                // );
            }
    ...
    public double[][] getAttImageDataForDate(String att_name, String time)
        ...
        IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
        IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
        if (dbConn == null || dbUtils == null) return null;
        ...
        // System.out.println (
        // "CLA/AttributeExtractor/getAttImageDataForDate/query/"+query+"/" );
