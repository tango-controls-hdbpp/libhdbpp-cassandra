 ...
 * @author CLAISSE 
 */
public class TdbModeControllerByLastInsertComparison extends TdbModeControllerAdapter 
{
    ...
    protected int controlPeriodicMode ( ModePeriode modeP , TdbSpec spec , ArchivingAttribute attr ) throws DevFailed 
    {
        if ( modeP == null )
        {
            Tools.trace ( "TdbModeControllerByLastInsertComparison/controlPeriodicMode/modeP == null!/for attribute|"+attr.getCompleteName()+"|" , Warnable.LOG_LEVEL_ERROR );
...
//        	 Retrieve the couple max(time), value
        	_timeValue = attributesReader.getTimeValueNullOrNotOfLastInsert(_completeName,_dataFormat,_writable);
            ...
            ISaferPeriodCalculator _saferPeriodCalculator = SaferPeriodCalculatorFactory.getCurrentImpl();
            int _saferPeriod = _saferPeriodCalculator.getSaferPeriod ( modeP.getPeriod () + _exportPeriod );
        ...
        {
            Tools.trace ( "TdbModeControllerByRecordCount/controlPeriodicMode/failed calling getRecordCount for attribute|"+_completeName+"|" , e , Warnable.LOG_LEVEL_WARN );
            return IModeController.CONTROL_FAILED;
        }
        catch(Exception e)
        {
        	Tools.trace ( "TdbModeControllerByRecordCount/controlPeriodicMode/failed calling for attribute|"+_completeName+"|" , e , Warnable.LOG_LEVEL_WARN );
        	return IModeController.CONTROL_FAILED;
        }       
