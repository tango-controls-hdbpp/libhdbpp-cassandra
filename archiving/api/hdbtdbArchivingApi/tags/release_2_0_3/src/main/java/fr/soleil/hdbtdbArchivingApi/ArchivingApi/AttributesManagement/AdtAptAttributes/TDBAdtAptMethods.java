...
public class TDBAdtAptMethods {
     ...
     */
    public  int[] getAtt_TFW_Data_By_Id(int id, AdtAptAttributes att) throws ArchivingException
    {
        ...
        String table_1 = att.dbConn.getSchema() + "." + ConfigConst.TABS[ 0 ];
        ...
        try
        {
        	conn = att.dbConn.getConnection();
            preparedStatement = conn.prepareStatement(get_TFW_DataQuery);
            att.dbConn.setLastStatement(preparedStatement);
        ...
        catch ( SQLException e )
        {
            ...
            throw new ArchivingException(message , reason , ErrSeverity.WARN , desc , att.getClass().getName() , e);
        }
        finally
		{
			try {
				...
				if(conn!=null)att.dbConn.closeConnection(conn);
			} catch (SQLException e) {
				att.dbConn.closeConnection(conn);
