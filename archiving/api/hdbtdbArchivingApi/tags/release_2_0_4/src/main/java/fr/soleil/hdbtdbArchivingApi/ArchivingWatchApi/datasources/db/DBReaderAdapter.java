...
import fr.soleil.hdbtdbArchivingApi.ArchivingApi.ModesManagement.GenericMode;
...
public abstract class DBReaderAdapter implements IDBReader 
{
	...
	public int getRecordCount(String completeName) throws DevFailed 
	{
		...
		try
		{
			res = database.getAttribute().getAttRecordCount ( completeName, database.getDbUtil() );
		}
		catch ( Exception e )
		{
	...
	public Timestamp getTimeOfLastInsert(String completeName) throws DevFailed 
	{
		...
		try
		{
			ret = database.getDbUtil().getTimeOfLastInsert ( completeName , true, database.getDbConn(), database.getAttribute() );
		}
	...
	public String[] getAdminReport()throws DevFailed {
		String[] adminReport = null;
		try {
			this.openConnectionIfNecessary ( false );
			this.database.getDbUtil().startWatcherReport(database.getDbConn());
			int nbArchived = this.database.getMode().getCurrentArchivedAttCount();
			int nbOkAttributes = this.database.getDbUtil().getAttributesCountOkOrKo(database.getDbConn(), true);
			int nbKoAttributes = this.database.getDbUtil().getAttributesCountOkOrKo(database.getDbConn(), false);
			String[] listKoAttributes = this.database.getDbUtil().getKoAttributes(database.getDbConn());
			String[] listOfPartitions = this.database.getDbUtil().getListOfPartitions(database.getDbConn());
			String[] listOfJobsStatus = this.database.getDbUtil().getListOfJobStatus(database.getDbConn());
			String[] listOfJobsErrors = this.database.getDbUtil().getListOfJobErrors(database.getDbConn());
			...
			int result_size = 3 + koAttrSize + PartitionsSize + jobErrorsSize + jobStatusSize;
	...
	public String[] getTimeValueNullOrNotOfLastInsert(String completeName, int dataFormat, int writable) throws DevFailed 
	{
		...
		try
		{
			if(isHistoric()){
				...
				ret = database.getDbUtil().getTimeValueNullOrNotOfLastInsert ( completeName, dataFormat, writable, partition , database.getDbConn(), database.getAttribute());
				if(ret != null && ret[2]!=null){
					setLastPartition(ret[2]);
				}
			}
			else {
				ret = database.getDbUtil().getTimeValueNullOrNotOfLastInsert ( completeName, dataFormat, writable, null , database.getDbConn(), database.getAttribute());
			}
	...
	public boolean isLastValueNull(String completeName) throws DevFailed 
	{
		...
		try
		{
			...
			boolean ret = database.getDbUtil().isLastDataNull ( completeName, database.getDbConn(), database.getAttribute() );
		...
		if ( ( !this.isReady ) || forceOpening )
		{
			...
			try
			{
				if(host.equals("") && name.equals("")){
					...
					this.manager.ArchivingConfigure( host, name, user , pwd  );
				}
			}
			catch ( Exception e )
			{
