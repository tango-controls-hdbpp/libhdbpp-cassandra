...
//				m_DataBase.getAttribute().CreateAttributeTableIfNotExist(attributeName, m_DataBase.getExtractor().getDataGetters(), m_DataBase.getDbUtil());
	 ...
	 * @throws ArchivingException
	 */
	private void connectArchivingDatabase(String mdbhost , String mdbname, String muser , String mpassword) throws ArchivingException
	{
		if ( m_DataBase == null )
		{
			// Host
			...
			if ( pass.equals("") )
				...
				m_DataBase.setDbConn(ConnectionFactory.connect_auto(host, name, schema, user, pass, type ));
				is_connected = true;
		}
	}
...
//		System.out.println( "ArchivingConfigure/historic/"+historic);
		...
		ArchivingConfigureWithoutArchiverListInit("","", user ,  password );
...
//		System.out.println( "ArchivingConfigureWithoutArchiverListInit/historic/"+historic+"/user/"+user);
		...
		// Database connection
		connectArchivingDatabase(host, name,user , password);
	}
