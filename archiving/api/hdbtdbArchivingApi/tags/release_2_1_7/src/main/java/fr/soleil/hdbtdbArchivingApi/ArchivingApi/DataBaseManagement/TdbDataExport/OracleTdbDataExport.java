...
public class OracleTdbDataExport extends TdbDataExport {
	...
	/* (non-Javadoc)
	 * @see fr.soleil.hdbtdbArchivingApi.ArchivingApi.DataBaseManagement.DbDataExport.IDataExport#exportToDB_Image(java.lang.String, java.lang.String, java.lang.String, int)
	 */
	public void exportToDB_Data(String remoteDir, String fileName,
		...
		StringBuffer query = new StringBuffer().append("{call ").append(dbConn.getSchema()).append(".crcontrolfile(?, ?, ?)}");
		...
		try
		{
			...
			System.out.println("OracleTdbDataExport/exportToDB_Data BEFORE PS crcontrolfile: "+ new Timestamp(System.currentTimeMillis()).toString());
			callableStatement.executeUpdate();
			System.out.println("OracleTdbDataExport/exportToDB_Data AFTER PS crcontrolfile: "+ new Timestamp(System.currentTimeMillis()).toString());
		}
	...
	public void forceDatabaseToImportFile(String tableName) throws ArchivingException
	{
		...
		StringBuffer query = new StringBuffer().append("{call ").append(dbConn.getSchema()).append(".force_load_data(?)}");
		...
		try
		{
			...
			try
			{
				System.out.println("CLA/OracleTdbDataExport/forceDatabaseToImportFile BEFORE PS force_load_data: "+ new Timestamp(System.currentTimeMillis()).toString());
				callableStatement.executeUpdate();
				ConnectionCommands.close(callableStatement);
				dbConn.closeConnection(conn);
				System.out.println("CLA/OracleTdbDataExport/forceDatabaseToImportFile AFTER PS force_load_data: "+ new Timestamp(System.currentTimeMillis()).toString());
			}
			catch ( SQLException e )
			{
				e.printStackTrace ();
