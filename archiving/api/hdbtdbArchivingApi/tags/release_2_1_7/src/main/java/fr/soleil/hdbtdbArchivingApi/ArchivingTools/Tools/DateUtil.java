...
public class DateUtil {
    ...
    private static final java.util.GregorianCalendar calendar        = new java.util.GregorianCalendar();
    private static final java.text.SimpleDateFormat  genFormatFR     = new java.text.SimpleDateFormat(
                                                                             FR_DATE_PATTERN);
     ...
     */
    public synchronized static String milliToString(long milli, String pattern) {
        Date date;
        calendar.setTimeInMillis(milli);
        date = calendar.getTime();
        if (pattern.equals(FR_DATE_PATTERN)) return genFormatFR.format(date);
        else return genFormatUS.format(date);
    }
     ...
     */
    public synchronized static long stringToMilli(String date)
            throws ArchivingException {
        ...
        try {
            ...
            try {
                ...
                String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : "
                        + GlobalConst.DATE_PARSING_EXCEPTION;
                String reason = GlobalConst.DATE_PARSING_EXCEPTION;
                String desc = "Failed while executing DateUtil.stringToMilli() method...";
                archivingException = new ArchivingException(
                        message,
                        reason,
                        ErrSeverity.WARN,
                        desc,
                        "fr.soleil.hdbtdbArchivingApi.ArchivingTools.Tools.DateUtil",
                        e1);
     ...
     * @throws ArchivingException
     */
    public synchronized static String stringToDisplayString(String date)
            throws ArchivingException {
    ...
    public synchronized static String oracleToMySQL(String string) {
        try {
