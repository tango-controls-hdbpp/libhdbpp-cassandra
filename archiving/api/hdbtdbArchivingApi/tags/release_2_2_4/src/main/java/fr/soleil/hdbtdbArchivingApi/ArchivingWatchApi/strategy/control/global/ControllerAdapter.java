...
public abstract class ControllerAdapter implements IController {
	...
	public ControlResult control(boolean doArchiverDiagnosis) throws DevFailed {
		...
		if (this.attributesToControl == null
				|| this.attributesToControl.size() == 0) {
		...
		while (enumeration.hasMoreElements()) {
			...
			ModeData nextModeData = (ModeData) this.attributesToControl
			...
			ControlResultLine line = new ControlResultLine(nextAttr,
					nextModeData, result);
			ret.addLine(line);
		}
	...
	public void doSomethingAboutActionResult(int actionResult) {
		...
		case IController.NO_ACTION_NEEDED:
			Tools
					.trace(
							"doSomethingAboutActionResult/No action needed being undertaken",
		...
		case IController.ACTION_FAILED:
			Tools.trace("doSomethingAboutActionResult/The action failed !",
	...
	public static int doRetry(ControlResult control) {
		Hashtable<String, Archiver> errorArchivers = control.getErrorArchivers();
		if (errorArchivers == null) {
			return IController.NO_ACTION_NEEDED;
		}

		Enumeration<String> koArchivers = errorArchivers.keys();
		while (koArchivers.hasMoreElements()) {
			...
			try {
				...
				proxy.command_inout_asynch(GLOBAL_RETRY_COMMAND_NAME, argin,
						true);
			} catch (DevFailed e) {
				Tools.trace("ControllerAdapter/doRetry/DevFailed for archiver|"
						+ nextArchiver + "| (" + htKOAttributess.size()
						+ " attributes)", e, Warnable.LOG_LEVEL_WARN);
				return IController.ACTION_FAILED;
			} catch (Throwable e) {
				Tools.trace("ControllerAdapter/doRetry/Exception for archiver|"
						+ nextArchiver + "| (" + htKOAttributess.size()
						+ " attributes)", e, Warnable.LOG_LEVEL_ERROR);
				return IController.ACTION_FAILED;
			}
		}
