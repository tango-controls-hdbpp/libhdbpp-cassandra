...
/**
 * The Main class in standalone mode, it is also used to store device properties
 * in device mode.
 * 
 * @author CLAISSE
 */
public class ArchivingWatch {
	...
	/**
	 * @param args
	 *            The launch parameters in standalone mode
	 */
	public static void main(String[] args) {
		if (args == null || args.length != 7) {
			Tools
					.trace(
							"Incorrect arguments. Correct syntax: java ArchivingWatch HDBuser HDBpassword TDBuser TDBpassword sleepDuration(s) doArchiverDiagnosis(true/false) doRetry(true/false)",
		...
		try {
			...
			Tools
					.trace(
							"Incorrect macroPeriod parameter. Must be a duration in seconds",
							Warnable.LOG_LEVEL_FATAL);
			System.exit(1);
		}
		...
		boolean doArchiverDiagnosis = Tools
				.stringToBoolean(doArchiverDiagnosis_s);
	...
	/**
	 * @param defaultSafetyPeriod
	 *            The defaultSafetyPeriod to set.
	 */
	public static void setDefaultSafetyPeriod(String _defaultSafetyPeriod) {
		...
		SaferPeriodCalculatorFactory
				.setUserDefinedDefaultMode(_defaultSafetyPeriod);
	}
	...
	/**
	 * @param doStartOnInitDevice
	 *            The doStartOnInitDevice to set.
	 */
