...
public  class AttributeExtractor extends DataExtractor {
	 ...
	 * @param at
	 */
	public AttributeExtractor(IDBConnection con, IAdtAptAttributes at) {
		super(con, at);
    ...
    public Vector getAttPartialImageDataBetweenDates(String[] argin ,  SamplingType samplingType) throws ArchivingException
    {
        ...
        {            
            ...
            String format = dbConn.getDbUtils().getFormat(samplingType);//isMySQL ? samplingType.getMySqlFormat () : samplingType.getOracleFormat ();
            
            String selectField_0 = dbConn.getDbUtils().toDbTimeFieldString( ConfigConst.TAB_IMAGE_RO[ 0 ] , format );
            ...
            whereClause = ConfigConst.TAB_IMAGE_RO[ 0 ] + " BETWEEN " + dbConn.getDbUtils().toDbTimeString(time_0.trim()) + " AND " + dbConn.getDbUtils().toDbTimeString(time_1.trim());
            groupByClause = " GROUP BY " +dbConn.getDbUtils().toDbTimeFieldString(ConfigConst.TAB_IMAGE_RO[ 0 ], format);
            orderByClause = " ORDER BY " +dbConn.getDbUtils().toDbTimeFieldString(ConfigConst.TAB_IMAGE_RO[ 0 ], format);
            
            query = "SELECT " + fields + " FROM " + tableName + " WHERE " + whereClause + groupByClause + orderByClause;;            
        }
        else
        {
            fields = ( dbConn.getDbUtils().toDbTimeFieldString(ConfigConst.TAB_IMAGE_RO[ 0 ]) + ", " + ConfigConst.TAB_IMAGE_RO[ 1 ] + ", " + ConfigConst.TAB_IMAGE_RO[ 2 ] );
            whereClause = ConfigConst.TAB_IMAGE_RO[ 0 ] + " BETWEEN " + dbConn.getDbUtils().toDbTimeString(time_0.trim()) + " AND " + dbConn.getDbUtils().toDbTimeString(time_1.trim());
