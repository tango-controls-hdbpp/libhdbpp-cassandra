...
public class AttributeFamilies extends AttributesData {
	...
	public AttributeFamilies(int type) {
		super(type);
	...
	public String[] get_families() throws ArchivingException {
		...
		String sqlStr;
		sqlStr = "SELECT DISTINCT " + ConfigConst.TAB_DEF[5] + " FROM "
	...
	public int get_familiesCount() throws ArchivingException {
		...
		// Create and execute the SQL query string
		String sqlStr;
		sqlStr = "SELECT COUNT(DISTINCT " + ConfigConst.TAB_DEF[5] + ") FROM "
				+ dbConn.getSchema() + "." + ConfigConst.TABS[0];
		sqlStr = sqlStr + " WHERE " + ConfigConst.TAB_DEF[5] + " IS NOT NULL";
	...
	public String[] get_families_by_criterion(String domain_name,
			String family_regexp) throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		if (dbConn == null)
			return null;
	...
	public int get_familiesCount(String domain_name) throws ArchivingException {
		...
		if (domain_name.trim().equals("*")) {
			sqlStr = sqlStr + "(" + ConfigConst.TAB_DEF[4] + " LIKE '%' OR "
					+ ConfigConst.TAB_DEF[4] + " IS NULL)";
		} else {
			sqlStr = sqlStr + ConfigConst.TAB_DEF[4] + " LIKE " + "'"
					+ domain_name.replace('*', '%') + "'";
		}
