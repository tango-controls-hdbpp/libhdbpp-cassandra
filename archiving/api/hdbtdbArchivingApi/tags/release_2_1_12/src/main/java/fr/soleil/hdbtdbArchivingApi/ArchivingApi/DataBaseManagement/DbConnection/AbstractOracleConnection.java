...
public abstract class AbstractOracleConnection extends DBConnection {
	...
	//private PoolDataSource poolDS = null;
	...
	public void connect(boolean rac) throws ArchivingException {
		...
		try {
			...
			String url = rac ? "jdbc:oracle:oci8:@" + getName()
					: ConfigConst.DRIVER_ORACLE + ":@" + getHost() + ":"
							+ ConfigConst.ORACLE_PORT + ":" + getName(); // pilote
			// Oracle;
			String driver = rac ? "oracle.jdbc.driver.OracleDriver"
					: ConfigConst.DRIVER_ORACLE;
			setDriver(driver);
			// setM_dataSource(PoolConnectionFactory.getInstance(getDbType())
			// .getPoolInstance(url, getUser(), getPasswd()));
			if (m_dataSource == null) {
				Connection conn = null;
				try {
					String minPoolSize, maxPoolSize;
					String inactivityTimeout;
					String classDevice;

					// Used to test the Cnx, otherwise the cnx pb will be visible at the first GetConnection method call
					conn = DriverManager.getConnection(url, getUser(), getPasswd());

					classDevice = getClassDeviceName();

					minPoolSize = GetConf.readStringInDB(classDevice, MIN_POOL_SIZE_PROPERTY);
					maxPoolSize = GetConf.readStringInDB(classDevice, MAX_POOL_SIZE_PROPERTY);
					inactivityTimeout = GetConf.readStringInDB(classDevice, INACTIVITY_TIMEOUT_SIZE_PROPERTY);
					...
					cacheProps.setProperty("InactivityTimeout", inactivityTimeout); // "7200");
					cacheProps.setProperty("AbandonedConnectionTimeout", "900"); // seconds
					cacheProps.setProperty("MaxStatementLimit", "5"); // for one
					...
					String desc = "Failed while connecting to the Oracle archiving database with url = "
							+ url;
					throw new ArchivingException(message, reason,
	...
	// The call to the alterSession method implies the getConnection() overwrite.
	...
	// String url = "";
	// String ons = "";
	// short minPoolSize, maxPoolSize;
	// short inactivityTimeout;
	// try {
	// String classDevice;
	...
	// poolDS.setInactiveConnectionTimeout((int) inactivityTimeout);
	// poolDS.setInitialPoolSize((int) minPoolSize);
	// poolDS.setMinPoolSize((int) minPoolSize);
	// poolDS.setMaxPoolSize((int) maxPoolSize);
	// poolDS.setMaxStatements(10);
	// // Libérer les connexions invalides
