...
//	private String last_partition = null;
	 ...
	 *
	 * @see
	 * archwatch.datasources.in.IArchivedAttributesReader#getArchivedAttributes
	 * ()
	 */
	public Hashtable<String, ModeData> getArchivedAttributes() throws DevFailed {
		...
		try {
			...
			Tools
					.trace(
							"DBReaderAdapter/getArchivedAttributes/problem calling getCurrentArchivedAtt(): ",
							e, Warnable.LOG_LEVEL_ERROR);
		...
		Hashtable<String, ModeData> ret = new Hashtable<String, ModeData>(numberOfAttributes);
	 ...
	 */
	public ModeData getModeDataForAttribute(String completeName)
			throws DevFailed {
		this.openConnectionIfNecessary(false);
		...
		try {
			...
			Tools
					.trace(
							"DBReaderAdapter/getModeDataForAttribute/problem calling getArchiverForAttribute ( "
									+ completeName + " ): ", e,
		...
		try {
			...
			if (mode.getModeP() == null) {
				Tools
						.trace(
								"DBReaderAdapter/getModeDataForAttribute/mode.getModeP() == null for attribute|"
										+ completeName + "|",
								Warnable.LOG_LEVEL_ERROR);
			...
			Tools
					.trace(
							"DBReaderAdapter/getModeDataForAttribute/problem calling getModeForAttribute ( "
									+ completeName + " ): ", e,
							Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	...
	/*
	 * //CLA 22/03/07 The DevFailed should be thrown public ModeData
	 * getModeDataForAttribute ( String completeName ) throws DevFailed {
	 * this.openConnectionIfNecessary ( false );
	 *
	 * ModeData ret = new ModeData ();
	 *
	 * try { String archiver = this.getArchiverForAttribute ( completeName );
	 * ret.setArchiver ( archiver ); } catch ( Throwable e ) { Tools.trace (
	 * "DBReaderAdapter/getModeDataForAttribute/problem calling getArchiverForAttribute ( "
	 * + completeName +" ): " , e , Warnable.LOG_LEVEL_ERROR );
	 * ret.setIncomplete ( true ); }
	 *
	 * try { Mode mode = this.getModeForAttribute ( completeName ); ret.setMode
	 * ( mode );
	 *
	 * if ( mode.getModeP () == null ) { Tools.trace (
	 * "DBReaderAdapter/getModeDataForAttribute/mode.getModeP() == null for attribute|"
	 * + completeName +"|" , Warnable.LOG_LEVEL_ERROR ); ret.setIncomplete (
	 * true ); } } catch ( Throwable e ) { Tools.trace (
	 * "DBReaderAdapter/getModeDataForAttribute/problem calling getModeForAttribute ( "
	 * + completeName +" ): " , e , Warnable.LOG_LEVEL_ERROR );
	 * //Tools.throwDevFailed ( e ); ret.setIncomplete ( true ); }
	 *
	 * return ret; }
	 */
	 ...
	 * @throws ArchivingException
	 */
	private String getArchiverForAttribute(String completeName)
			throws DevFailed {
		// System.out.println (
		// "getArchiverForAttribute/completeName|"+completeName+"|" );
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/getArchiverForAttribute/problem calling getDeviceInCharge ( "
							+ completeName + " ): ", e,
					Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	...
	public Mode getModeForAttribute(String completeName) throws DevFailed {
		...
		try {
			...
			Tools
					.trace(
							"DBReaderAdapter/getModeForAttribute/problem calling getCurrentArchivingMode ( "
									+ completeName + " , true ): ", e,
							Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	...
	public void closeConnection() throws DevFailed {
		try {
			...
			Tools.trace(
					"DBReaderAdapter/closeConnection/problem calling close ()",
					e, Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	 ...
	 *
	 * @see
	 * archwatch.datasource.in.IArchivedAttributesReader#getRecordCount(java
	 * .lang.String)
	 */
	public int getRecordCount(String completeName) throws DevFailed {
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/getRecordCount/problem calling getAttRecordCount ( "
							+ completeName + " ): ", e,
					Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	...
	public Timestamp getTimeOfLastInsert(String completeName) throws DevFailed {
		...
		try {
			...
			if (ae.getCause() != null && ae.getCause() instanceof SQLException) {
				...
				Tools.trace(
						"DBReaderAdapter/getTimeOfLastInsert/the table doesn't exist for attribute|"
								+ completeName + "|SQL state|" + state
								+ "|SQL code|" + code + "|", ae,
				...
				Tools.trace(
						"DBReaderAdapter/getTimeOfLastInsert/problem calling getTimeOfLastInsert ( "
								+ completeName + " ): ", ae,
						Warnable.LOG_LEVEL_ERROR);
	...
	public int getAttributeId(String completeName) throws DevFailed {
		...
		int ret = -1;
		try {
			ret = database.getAttribute().getIds().getBufferedAttID(
					completeName);
		} catch (ArchivingException ae) {
			if (ae.getCause() != null && ae.getCause() instanceof SQLException) {
				SQLException sqlex = (SQLException) ae.getCause();
				...
				int code = sqlex.getErrorCode();
				Tools.trace(
						"DBReaderAdapter/getAttributeId/the table doesn't exist for attribute|"
								+ completeName + "|SQL state|" + state
								+ "|SQL code|" + code + "|", ae,
						Warnable.LOG_LEVEL_INFO);
				...
				Tools.trace(
						"DBReaderAdapter/getAttributeId/problem calling getTimeOfLastInsert ( "
								+ completeName + " ): ", ae,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(ae);
			}
	...
	public void startArchivingReport() throws DevFailed {
		...
		try {
			...
			// int nbArchived =
			// this.database.getMode().getCurrentArchivedAttCount();
			// int nbOkAttributes =
			// this.database.getDbUtil().getAttributesCountOkOrKo( true);
			// int nbKoAttributes =
			// this.database.getDbUtil().getAttributesCountOkOrKo( false);
			// String[] listKoAttributes =
			// this.database.getDbUtil().getKoAttributes();
			//
			// int koAttrSize = listKoAttributes==null
			// ?0:listKoAttributes.length;
	...
	public String[] getDatabaseReport() throws DevFailed {
		...
		try {
			this.openConnectionIfNecessary(false);
			this.database.getDbUtil().startWatcherReport();
			String[] listOfPartitions = this.database.getDbUtil()
					.getListOfPartitions();
			String[] listOfJobsStatus = this.database.getDbUtil()
					.getListOfJobStatus();
			String[] listOfJobsErrors = this.database.getDbUtil()
					.getListOfJobErrors();

			int PartitionsSize = listOfPartitions == null ? 0
					: listOfPartitions.length;
			int jobStatusSize = listOfJobsStatus == null ? 0
					: listOfJobsStatus.length;
			int jobErrorsSize = listOfJobsErrors == null ? 0
					: listOfJobsErrors.length;
	...
	public String[] getTimeValueNullOrNotOfLastInsert(String completeName,
			int dataFormat, int writable) throws DevFailed {
		this.openConnectionIfNecessary(false);

		String ret[] = null;
		try {
//			if (isHistoric()) {
//				int last_month = getLastPartition() == null ? -1 : Integer
//						.parseInt(getLastPartition().substring(
//								getLastPartition().indexOf("M") + 1));
//				int actual_month = new Timestamp(System.currentTimeMillis())
//						.getMonth() + 1;
//				String partition = last_month == actual_month ? getLastPartition()
//						: null;
//				ret = database.getDbUtil().getTimeValueNullOrNotOfLastInsert(
//						completeName, dataFormat, writable, partition);
//				if (ret != null && ret[2] != null) {
//					setLastPartition(ret[2]);
//				}
//			} else {
				ret = database.getDbUtil().getTimeValueNullOrNotOfLastInsert(
						completeName, dataFormat, writable);
//			}
			...
			if (ae.getCause() != null && ae.getCause() instanceof SQLException) {
				...
				Tools
						.trace(
								"DBReaderAdapter/getTimeValueNullOrNotOfLastInsert/the table doesn't exist for attribute|"
										+ completeName
										+ "|SQL state|"
										+ state
										+ "|SQL code|" + code + "|", ae,
				...
				Tools
						.trace(
								"DBReaderAdapter/getTimeValueNullOrNotOfLastInsert/problem calling getTimeValueNullOrNotOfLastInsert ( "
										+ completeName + " ): ", ae,
								Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(ae);
			}
	...
	/*
	 * public Timestamp now () throws DevFailed { //throwDummyException();
	 *
	 * this.openConnectionIfNecessary ( false );
	 *
	 * try { return database.now (); } catch ( Exception e ) { Tools.trace (
	 * "DBReaderAdapter/now/problem calling now ()" , e ,
	 * Warnable.LOG_LEVEL_ERROR ); Tools.throwDevFailed ( e ); } return null; }
	 */
	...
	private void throwDummyException(int threshold) throws DevFailed {
		System.out.println("CLA/throwDummyException/threshold/" + threshold
				+ "/cpt/" + cpt);
		cpt++;
		if (cpt >= threshold) {
			Exception e = new Exception(
					"CLA/dummy exception throwDummyException-----------------------------------------------------------------------------");
			Tools.throwDevFailed(e);
		}
	}
	...
	private void alwaysThrowDummyException() throws DevFailed {
		Exception e = new Exception(
				"CLA/dummy exception alwaysThrowDummyException-----------------------------------------------------------------------------");
		Tools.throwDevFailed(e);
	}
	...
	public int getRecordCount(String completeName, int period) throws DevFailed {
		...
		// Tools.trace ( "DBReaderAdapter/completeName|" + completeName +
		// "|period|" + period , Warnable.LOG_LEVEL_DEBUG );
		...
		// Tools.trace ( "DBReaderAdapter/now|" + after_s + "|now-period|" +
		// before_s , Warnable.LOG_LEVEL_DEBUG );
		...
		try {
			DbData dbData = database.getExtractor()
			...
			if (dbData == null || dbData.getData_timed() == null) {
				Tools
						.trace(
								"DBReaderAdapter/case ( dbData == null || dbData.getData() == null )",
			...
			if (ae.getCause() != null && ae.getCause() instanceof SQLException) {
				...
				int code = sqlex.getErrorCode();
				// Tools.trace (
				// "DBReaderAdapter/getRecordCount/the table doesn't exist for attribute|"+
				// completeName +"|SQL state|"+state+"|SQL code|"+code+"|" , ae
				// , Warnable.LOG_LEVEL_INFO );
				...
				Tools.trace(
						"DBReaderAdapter/getRecordCount/problem calling getAttDataBetweenDates ( "
								+ completeName + " , " + before_s + " , "
								+ after_s + " )", ae, Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(ae);
			}
		} catch (Exception e) {
			Tools.trace(
					"DBReaderAdapter/getRecordCount/problem calling getAttDataBetweenDates ( "
							+ completeName + " , " + before_s + " , " + after_s
							+ " )", e, Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
		return 0;
	...
	private void throwDummyException(String completeName) throws DevFailed {
		if (completeName.equals("tango/tangotest/1/short_scalar")) {
			Exception e = new Exception(
					"CLA/dummy exception for short_scalar-----------------------------------------------------------------------------");
			Tools.throwDevFailed(e);
		}
	}
	 ...
	 *
	 * @see
	 * archwatch.datasource.in.IArchivedAttributesReader#isScalar(java.lang.
	 * String)
	 */
	public boolean isScalar(String completeName) throws DevFailed {
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/isScalar/problem calling getAttDataFormat ( "
							+ completeName + " )", e, Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
	...
	public int getAttFormatCriterion(String completeName) throws DevFailed {
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/getAttFormatCriterion/problem calling getAttDataFormat ( "
							+ completeName + " )", e, Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
		return -1;
	...
	public int getAttWritableCriterion(String completeName) throws DevFailed {
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/getAttWritableCriterion/problem calling getAttDataWritable ( "
							+ completeName + " )", e, Warnable.LOG_LEVEL_ERROR);
			Tools.throwDevFailed(e);
		}
		return -1;
	 ...
	 *
	 * @see
	 * archwatch.datasources.db.IArchivedAttributesReader#getArchiverLoad(java
	 * .lang.String, int)
	 */
	public int getArchiverLoad(String archiverName, int loadType)
		...
		case IDBReader.LOAD_ALL:
			try {
				...
				Tools.trace(
						"DBReaderAdapter/getArchiverLoad/problem calling get_charge ( "
								+ archiverName + " )", e,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(e);
			}
		case IDBReader.LOAD_SCALAR:
			try {
				...
				Tools.trace(
						"DBReaderAdapter/getArchiverLoad/problem calling get_scalar_charge ( "
								+ archiverName + " )", e,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(e);
			}
		case IDBReader.LOAD_SPECTRUM:
			try {
				...
				Tools.trace(
						"DBReaderAdapter/getArchiverLoad/problem calling get_spectrum_charge ( "
								+ archiverName + " )", e,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(e);
			}
		case IDBReader.LOAD_IMAGE:
			try {
				...
				Tools.trace(
						"DBReaderAdapter/getArchiverLoad/problem calling get_image_charge ( "
								+ archiverName + " )", e,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(e);
			}
		default:
			Tools.throwDevFailed(new IllegalArgumentException(
					"Expected LOAD_ALL(0), LOAD_SCALAR(1), LOAD_SPECTRUM(2), LOAD_IMAGE(3) got "
							+ loadType + " instead."));
		}
	 ...
	 *
	 * @see
	 * archwatch.datasources.db.IArchivedAttributesReader#isAlive(java.lang.
	 * String)
	 */
	public boolean isAlive(String archiverName) throws DevFailed {
		...
		try {
			...
			Tools.trace(
					"DBReaderAdapter/isAlive/problem calling deviceLivingTest ( "
							+ archiverName + " )", ae, Warnable.LOG_LEVEL_INFO);
			return false;
		}
	}
	 ...
	 *
	 * @see
	 * archwatch.datasources.db.IArchivedAttributesReader#getStatus(java.lang
	 * .String)
	 */
	public String getDeviceStatus(String attributeName) throws DevFailed {
		...
		try {
			...
			Tools
					.trace(
							"DBReaderAdapter/getDeviceStatus/problem calling getDeviceStatus ( "
									+ attributeName + " )", ae,
							Warnable.LOG_LEVEL_INFO);
	...
	public boolean isDBConnectionAlive() {
		try {
			...
			Tools
					.trace(
							"DBReaderAdapter/isDBConnectionAlive/The minimum request failed!!! Trying to reopen the DB connection.",
							e, Warnable.LOG_LEVEL_ERROR);
			...
			try {
				...
				Tools
						.trace(
								"DBReaderAdapter/isDBConnectionAlive/The DB connection can't be reopened!!!",
								e1, Warnable.LOG_LEVEL_FATAL);
				return false;
			}
		}
	...
	public boolean isLastValueNull(String completeName) throws DevFailed {
		...
		// System.out.println (
		// "DBReaderAdapter/isLastValueNull/completeName|"+completeName+"|" );
		try {
			// System.out.println (
			// "DBReaderAdapter/isLastValueNull/before database.isLastDataNull"
			// );
			boolean ret = database.getDbUtil().isLastDataNull(completeName);
			// System.out.println (
			// "DBReaderAdapter/isLastValueNull/after database.isLastDataNull/ret|"+ret+"|"
			// );
			return ret;
		} catch (ArchivingException ae) {
			// System.out.println (
			// "DBReaderAdapter/isLastValueNull/ArchivingException" );
			...
			if (ae.getCause() != null && ae.getCause() instanceof SQLException) {
				...
				int code = sqlex.getErrorCode();
				Tools.trace(
						"DBReaderAdapter/isLastValueNull/the table doesn't exist for attribute|"
								+ completeName + "|SQL state|" + state
								+ "|SQL code|" + code + "|", ae,
						Warnable.LOG_LEVEL_INFO);
				...
				Tools.trace(
						"DBReaderAdapter/isLastValueNull/problem calling getAttDataLast_n ( "
								+ completeName + " ): ", ae,
						Warnable.LOG_LEVEL_ERROR);
				Tools.throwDevFailed(ae);
			}
		...
		// System.out.println (
		// "DBReaderAdapter/isLastValueNull/Shouldn't be here!!!!!!!" );
		return true;// this line is never reached since throwDevFailed ALWAYS
					// throws even if the compilator doesn't know that
	}
	...
	public void openConnectionIfNecessary(boolean forceOpening)
		...
		if ((!this.isReady) || forceOpening) {
			...
			boolean rac = this.isHistoric() ? ArchivingWatch.isHDBRac()
					: ArchivingWatch.isTDBRac();
			Tools.trace(
					"DBReaderAdapter/openConnection/trying to connect with parameters ( "
							+ HDBuser + " , " + HDBpassword + " , " + TDBuser
							+ " , " + TDBpassword + " ): ",
					Warnable.LOG_LEVEL_DEBUG);
			...
			try {
				if (host.equals("") && name.equals("")) {
					this.manager.ArchivingConfigureWithoutArchiverListInit(user, pwd);
				} else {
					this.manager.ArchivingConfigureWithoutArchiverListInit(host, name, user, pwd, rac, false,false);
				}
			} catch (Exception e) {
				Tools
						.trace(
								"DBReaderAdapter/openConnection/problem calling ArchivingManagerApi.ArchivingConfigure ( "
										+ HDBuser
										+ " , "
										+ HDBpassword
										+ " , "
