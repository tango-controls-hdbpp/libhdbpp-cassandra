...
public class ControlResultLine {
	 ...
	 * 
	 * @param _attr
	 *            The attribute this line is all about
	 * @param _modeData
	 *            The attribute's archiving mode description
	 * @param _result
	 *            The result of the archiving controls on each mode
	 */
	public ControlResultLine(ArchivingAttribute _attr, ModeData _modeData,
			int[] _result) {
		super();
	 ...
	 * <LI>If necessary adds additionnal data such as the lastInsert and
	 * lastInsert dates
	 * </UL>
	 * 
	 * @throws DevFailed
	...
	public boolean build() throws DevFailed {
		...
		if (isDetermined) {
			if (controlOkButValueIsNull) {
				this.attribute
						.setArchivingStatus(IModeController.CONTROL_OK_BUT_VALUE_IS_NULL);
			} else if (controlOkButValueMightBeNull) {
				this.attribute
						.setArchivingStatus(IModeController.CONTROL_OK_BUT_VALUE_MIGHT_BE_NULL);
			} else {
				this.attribute
						.setArchivingStatus(isOK ? IModeController.CONTROL_OK
								: IModeController.CONTROL_KO);
			}
		} else {
		...
		// Tools.trace (
		// "ControlResultLine/build/completeName|"+completeName+"|isDetermined|"+isDetermined+"|isOK|"+isOK
		...
		if (modeData != null && !modeData.isIncomplete()) {
			...
			// Tools.trace (
			// "ControlResultLine/build/modeData != null/archiver|"+archiver+"|period|"+period
			// , Warnable.LOG_LEVEL_DEBUG );
		} else {
			// Tools.trace ( "ControlResultLine/build/modeData == null" ,
			// Warnable.LOG_LEVEL_DEBUG );
			archiver = "THIS";
		}

		boolean isArchivingOK = (this.attribute.getArchivingStatus() == IModeController.CONTROL_OK);
		isArchivingOK = isArchivingOK
				|| (this.attribute.getArchivingStatus() == IModeController.CONTROL_OK_BUT_VALUE_IS_NULL);
		isArchivingOK = isArchivingOK
				|| (this.attribute.getArchivingStatus() == IModeController.CONTROL_OK_BUT_VALUE_MIGHT_BE_NULL);

		IDBReader hdbReader = DBReaderFactory.getCurrentImpl();

		this.attribute.setArchiver(archiver);
		this.attribute.setId(hdbReader.getAttributeId(completeName));
		...
		if (!isArchivingOK) {
			...
			try {
				if (hdbReader == null) {
					...
					Timestamp lastInsertRequest = dbProxy
							.getLastInsertRequest(completeName);
				...
				e.printStackTrace();
				Tools.trace(
						"ControlResultLine/build/failed to load the lastInsert date for attrtibute|"
								+ this.getAttribute().getCompleteName() + "|",
						e, Warnable.LOG_LEVEL_WARN);
			}
		}
	...
	public String getReport() {
		...
		int id = attr.getId();
		return formatReportLine(completeName, id, archiver, isArchivingOK,
				isDetermined, period, lastInsert, lastInsertRequest,
				deviceStatus);
	}
	...
	/**
	 * Returns true if it couldn't be determined (due to eg. an SQL error)
	 * whether this attribute's line has a null most recent value or not
	 * 
	 * @return
	 */
	...
	private String formatReportLine(String completeName, int id,
			String archiver, boolean isArchivingOK, boolean isDetermined,
			int period, Timestamp lastInsert, Timestamp lastInsertRequest,
			String deviceStatus) {
		...
		IDbProxy dbProxy = ArchivingWatch.getDbProxy();
		if (dbProxy != null && isDetermined) // HDBarchiver internal controls
												// mode
		{
