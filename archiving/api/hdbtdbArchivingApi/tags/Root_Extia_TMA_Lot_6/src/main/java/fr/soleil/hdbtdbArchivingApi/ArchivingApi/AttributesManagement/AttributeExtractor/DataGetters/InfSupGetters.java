...
public class InfSupGetters extends DataExtractor {
	...
	public DbData getAttDataInfThan(String[] argin) throws ArchivingException {
		...
		case AttrDataFormat._SCALAR:
			dvdsa = getAttScalarDataInfThan(att_name, upper_value, dbData
					.getWritable());
	...
	public int getAttDataInfThanCount(String[] argin) throws ArchivingException {
		...
		case AttrDataFormat._SCALAR:
			valuesCount = getAttScalarDataInfThanCount(att_name, upper_value,
					writable);
			break;
		case AttrDataFormat._SPECTRUM:
	...
	public DbData getAttDataSupThan(String[] argin) throws ArchivingException {
		...
		switch (dbData.getData_format()) { // [0 - > SCALAR] (1 - > SPECTRUM] [2
											// - > IMAGE]
		case AttrDataFormat._SCALAR:
			dvdsa = getAttScalarDataSupThan(att_name, upper_value, dbData
					.getWritable());
			dbData.setData(dvdsa);
			break;
		case AttrDataFormat._SPECTRUM:
	...
	public int getAttDataSupThanCount(String[] argin) throws ArchivingException {
		...
		switch (data_format) { // [0 - > SCALAR] (1 - > SPECTRUM] [2 - > IMAGE]
		case AttrDataFormat._SCALAR:
			valuesCount = getAttScalarDataSupThanCount(att_name, lower_value,
					writable);
			break;
		case AttrDataFormat._SPECTRUM:
			methods.makeDataException(data_format, "Scalar", "Spectrum");
	...
	/**
	 * <b>Description : </b> Returns data that are lower than the given value x
	 * OR higher than the given value y.
	 * 
	 * @param argin
	 ...
	 * @throws ArchivingException
	 */
	public DbData getAttDataInfOrSupThan(String[] argin)
			throws ArchivingException {
		IAdtAptAttributes att = AdtAptAttributesFactory.getInstance(arch_type);
		if (att == null)
			return null;
		...
		switch (dbData.getData_format()) { // [0 - > SCALAR] (1 - > SPECTRUM] [2
											// - > IMAGE]
		case AttrDataFormat._SCALAR:
			dvdsa = getAttScalarDataInfOrSupThan(att_name, lower_value,
					upper_value, dbData.getWritable());
			dbData.setData(dvdsa);
			break;
		case AttrDataFormat._SPECTRUM:
			methods.makeDataException(dbData.getData_format(), "Scalar",
	...
	/**
	 * <b>Description : </b> Returns the number of data lower than the given
	 * value x OR higher than the given value y.
	 * 
	 * @param argin
	 *            The attribute's name, the lower limit and the upper limit
	 * @return The number of scalar data lower than the given value x OR higher
	 *         than the given value y, associated with their corresponding
	 *         timestamp <br>
	 * @throws ArchivingException
	 */
	public int getAttDataInfOrSupThanCount(String[] argin)
			throws ArchivingException {
		IAdtAptAttributes att = AdtAptAttributesFactory.getInstance(arch_type);
		if (att == null)
			return 0;
		...
		switch (data_format) { // [0 - > SCALAR] (1 - > SPECTRUM] [2 - > IMAGE]
		case AttrDataFormat._SCALAR:
			valuesCount = getAttScalarDataInfOrSupThanCount(att_name,
					lower_value, upper_value, writable);
			break;
		case AttrDataFormat._SPECTRUM:
			methods.makeDataException(data_format, "Scalar", "Spectrum");
		case AttrDataFormat._IMAGE:
	...
	/**
	 * <b>Description : </b> Returns data that are highter than the given value
	 * x OR lower than the given value y.
	 * 
	 * @param argin
	 *            The attribute's name, the lower limit and the upper limit
	 * @return The scalar data for the specified attribute<br>
	 * @throws ArchivingException
	 */
	public DbData getAttDataSupAndInfThan(String[] argin)
			throws ArchivingException {
		IAdtAptAttributes att = AdtAptAttributesFactory.getInstance(arch_type);
		if (att == null)
			return null;
		...
		switch (dbData.getData_format()) { // [0 - > SCALAR] (1 - > SPECTRUM] [2
											// - > IMAGE]
		case AttrDataFormat._SCALAR:
			dvdsa = getAttScalarDataSupAndInfThan(att_name, lower_value,
					upper_value, dbData.getWritable());
			dbData.setData(dvdsa);
			break;
		case AttrDataFormat._SPECTRUM:
	 ...
	 * @throws ArchivingException
	 */
	public int getAttDataSupAndInfThanCount(String[] argin)
			throws ArchivingException {
		int valuesCount = 0;
		IAdtAptAttributes att = AdtAptAttributesFactory.getInstance(arch_type);
		if (att == null)
		...
		switch (data_format) { // [0 - > SCALAR] (1 - > SPECTRUM] [2 - > IMAGE]
		case AttrDataFormat._SCALAR:
			valuesCount = getAttScalarDataSupAndInfThanCount(att_name,
					lower_value, upper_value, writable);
			break;
		case AttrDataFormat._SPECTRUM:
			methods.makeDataException(data_format, "Scalar", "Spectrum");
	 ...
	 * 
	 */
	private DevVarDoubleStringArray getAttScalarDataInfThan(String att_name,
			int upper_value, int writable) throws ArchivingException {
		...
		String query = "SELECT " + fields + " FROM " + tableName + " WHERE "
				+ selectField + " < " + upper_value + " ORDER BY time";

		return methods.getAttScalarDataCondition(att_name, query, ro_fields,
				dbConn);
	 ...
	 * 
	 */
	private DevVarDoubleStringArray getAttScalarDataSupThan(String att_name,
			int lower_value, int writable) throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
		if (dbConn == null || dbUtils == null)
			return null;
		...
		String tableName = dbConn.getSchema() + "."
				+ dbUtils.getTableName(att_name);
		String query = "SELECT " + fields + " FROM " + tableName + " WHERE ("
				+ selectField + " > " + lower_value + ") ORDER BY time";
	 ...
	 * 
	 */
	public DevVarDoubleStringArray getAttScalarDataInfOrSupThan(
			String att_name, int lower_value, int upper_value, int writable)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
		if (dbConn == null || dbUtils == null)
	 ...
	 * 
	 */
	public DevVarDoubleStringArray getAttScalarDataSupAndInfThan(
			String att_name, int lower_value, int upper_value, int writable)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
	 ...
	 * 
	 */
	private int getAttScalarDataInfThanCount(String att_name, int upper_value,
	 ...
	 * 
	 */
	public int getAttScalarDataSupThanCount(String att_name, int lower_value,
			int writable) throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
		if (dbConn == null || dbUtils == null)
	 ...
	 * 
	 */
	public int getAttScalarDataInfOrSupThanCount(String att_name,
			int lower_value, int upper_value, int writable)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
		if (dbConn == null || dbUtils == null)
	 ...
	 * 
	 */
	public int getAttScalarDataSupAndInfThanCount(String att_name,
			int lower_value, int upper_value, int writable)
			throws ArchivingException {
		IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
		IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
