...
public class Mode {
     ...
     * @see #Mode().
     */
    public Mode(ModePeriode mp, ModeAbsolu ma, ModeRelatif mr, ModeSeuil ms,
            ModeCalcul mc, ModeDifference md, ModeExterne me) {
        mode_p = mp;
        mode_a = ma;
        mode_r = mr;
        mode_s = ms;
     ...
     */
    public Mode(ModePeriode mp, ModeAbsolu ma, ModeRelatif mr, ModeSeuil ms,
            ModeCalcul mc, ModeDifference md, ModeExterne me, TdbSpec ts) {
        mode_p = mp;
        mode_a = ma;
        mode_r = mr;
        mode_s = ms;
    ...
    public Mode(String[] mode_array) {
        ...
        while (index < my_array_size && mode_array[index] != null) {
            if (mode_array[index].equals(Tag.MODE_P)) {
                // Mode P�riodique
                mode_p = new ModePeriode(Integer
                        .parseInt(mode_array[index + 1]));
                index = index + 2;
            }
            else if (mode_array[index].equals(Tag.MODE_A)) {
                // Mode Absolu
                mode_a = new ModeAbsolu(
                        Integer.parseInt(mode_array[index + 1]), Double
                ...
                // Mode Relatif
                mode_r = new ModeRelatif(Integer
                        .parseInt(mode_array[index + 1]), Double
                        .parseDouble(mode_array[index + 2]), Double
                        .parseDouble(mode_array[index + 3]), Boolean
                        .parseBoolean(mode_array[index + 4]));
                index = index + 5;
                ...
                mode_s = new ModeSeuil(Integer.parseInt(mode_array[index + 1]),
                        Double.parseDouble(mode_array[index + 2]), Double
                                .parseDouble(mode_array[index + 3]));
                index = index + 4;
            }
            else if (mode_array[index].equals(Tag.MODE_C)) {
                // Mode Calcul
                mode_c = new ModeCalcul(
                        Integer.parseInt(mode_array[index + 1]), Integer
                                .parseInt(mode_array[index + 2]), Integer
                                .parseInt(mode_array[index + 3]));
                // Warning : there is one more field for this mode type
                index = index + 5;
            }
            else if (mode_array[index].equals(Tag.MODE_D)) {
                // Mode Diff�rent
                mode_d = new ModeDifference(Integer
                        .parseInt(mode_array[index + 1]));
                index = index + 2;
            }
                ...
                tdbSpec = new TdbSpec(Long.parseLong(mode_array[index + 1]),
                        Long.parseLong(mode_array[index + 2]));
                index = index + 3;
            }
            else {
                index++;
    ...
    public String[] toArray() {
        ...
        if (this.getModeA() != null) {
            ...
            index++;
            mode_array[index] = Boolean
                    .toString(this.getModeA().isSlow_drift());
            index++;
        ...
        if (this.getModeR() != null) {
            ...
            index++;
            mode_array[index] = Double
                    .toString(this.getModeR().getPercentInf());
            index++;
            mode_array[index] = Double
                    .toString(this.getModeR().getPercentSup());
            index++;
            mode_array[index] = Boolean
                    .toString(this.getModeR().isSlow_drift());
            index++;
        }
        if (this.getModeT() != null) {
            ...
            index++;
            mode_array[index] = Double.toString(this.getModeT()
                    .getThresholdInf());
            index++;
            mode_array[index] = Double.toString(this.getModeT()
                    .getThresholdSup());
        ...
        if (this.getModeC() != null) {
            ...
            index++;
            mode_array[index] = Double
                    .toString(this.getModeC().getTypeCalcul());
            index++;
        ...
        if (this.getTdbSpec() != null) {
            ...
            index++;
            mode_array[index] = Long.toString(this.getTdbSpec()
                    .getExportPeriod());
            index++;
            mode_array[index] = Long.toString(this.getTdbSpec()
                    .getKeepingPeriod());
    ...
    public int getArraySizeSmall() {
        int my_size = 0;
        if (this.getModeP() != null) my_size = my_size + 2;
        if (this.getModeA() != null) my_size = my_size + 5;
        if (this.getModeR() != null) my_size = my_size + 5;
        if (this.getModeT() != null) my_size = my_size + 4;
        if (this.getModeC() != null) my_size = my_size + 5;
        if (this.getModeD() != null) my_size = my_size + 2;
        if (this.getModeE() != null) my_size = my_size + 1;
        if (this.getTdbSpec() != null) my_size = my_size + 3;
        return my_size;
    }
        ...
        if (mode_d != null)
            if (mode_d.getPeriod() < HDB_PERIOD_MIN_VALUE)
                throw generateException("On Difference mode  : archiving period to low !!"
                        + "("
                        + (mode_d.getPeriod() / 1000)
                        + "< "
                        + (HDB_PERIOD_MIN_VALUE / 1000) + " s" + ")");
    }

    private void checkModeForTDB() throws ArchivingException {
        if (mode_p != null)
    ...
    public void checkMode(String attributeName, int period)
            throws ArchivingException {
        if (ShortPeriodAttributesManager.isShortPeriodAttribute(attributeName)) {
            int attributePeriod = ShortPeriodAttributesManager
                    .getPeriodFromShortAttributeName(attributeName);
            if (period / 1000 < attributePeriod) {
                throw generateException("Archiving mode  : archiving period to low !!"
                        + "("
                        + attributeName
                        + "'s period must be greater or equal to "
                        + attributePeriod
                        + " s" + ")");
            }
        }
