//+======================================================================
// $Source: /cvsroot/tango-cs/tango/api/java/fr/soleil/TangoArchiving/ArchivingTools/Tools/ArchivingConfig.java,v $
//
...
public class ArchivingConfig {
	...
	private Hashtable attribute_list = null; // This list contains
												// AttributeLight objects types

	/**
	 * Default constructor Creates a new instance of ArchivingConfig
	 */
	...
	public ArchivingConfig(String[] archivingConfigArray) {
		...
		try {
			...
			for (int i = index; i < archivingConfigArray.length; i = i + 4) {
				...
				attributeLight
						.setAttribute_complete_name(archivingConfigArray[i]);
				attributeLight.setData_type(Integer
						.parseInt(archivingConfigArray[i + 1]));
				attributeLight.setData_format(Integer
						.parseInt(archivingConfigArray[i + 2]));
				attributeLight.setWritable(Integer
						.parseInt(archivingConfigArray[i + 3]));
				add(attributeLight);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * Sets the ArchivingConfig's <I>list of attributes</I> included in the
	 * context.
	 * 
	 * @param attributeName
	...
	public AttributeLight getAttribute(String attributeName) {
		AttributeLight attributeLight = (AttributeLight) attribute_list
				.get(attributeName);
	...
	public void setAttributeList(ArrayList attributeList) {
		for (int i = 0; i < attributeList.size(); i++) {
			AttributeLight attributeLight = (AttributeLight) attributeList
					.get(i);
			attribute_list.put(attributeLight.getAttribute_complete_name(),
					attributeLight);
		}
	}
	...
	public String[] toArray() {
		...
		try {
			...
			int size = 2 + modeArraySize + attribute_list.size()
					* AttributeLight.HDB_ATTRIBUTE_LIGHT_LENGTH;
	...
	public String toString() {
		...
		hdbConfigString.append("\tdevice_in_charge = ")
				.append(device_in_charge).append("\r\n");
		// Write the mode configuration
		hdbConfigString.append("\tmode.toString() = \r\n").append(
				mode.toString()).append("\r\n");
		...
		while (my_attribute_list.hasMoreElements()) {
			...
			hdbConfigString.append("[").append(i++).append("] : \r\n").append(
					AttributeLight.toString()).append("\r\n");
		}
