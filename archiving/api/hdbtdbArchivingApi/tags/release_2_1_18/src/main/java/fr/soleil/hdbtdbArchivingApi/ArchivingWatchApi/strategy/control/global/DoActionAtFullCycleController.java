 ...
 * @author CLAISSE
 */
public class DoActionAtFullCycleController extends ControllerAdapter implements
		IController {
	...
	public int useControlResult(ControlResult control) {
		// System.out.println (
		// "CLA/BasicController/useControlResult/control.isFullCycle ()/"+control.isFullCycle
		// () );
		...
		if (!control.isFullCycle())// CLA 28/07/06 We don't want to call actions
									// such as doRetry at every step, just at
									// the end of a cycle
		{
			return IController.NO_ACTION_NEEDED;
		}
		...
		try {
			...
			/*
			 * Timestamp startDate = control.getStartDate (); Timestamp endDate
			 * = control.getEndDate ();
			 */
