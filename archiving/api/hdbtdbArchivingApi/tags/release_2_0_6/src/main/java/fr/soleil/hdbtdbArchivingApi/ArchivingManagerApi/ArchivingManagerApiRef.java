...
public abstract class ArchivingManagerApiRef implements IArchivingManagerApiRef {
	...
	/*
	 * (non-Javadoc)
	 * @see fr.soleil.hdbtdbArchivingApi.ArchivingManagerApi.IArchivingManagerApiRef#getDbHost()
	 */
	 ...
	 * 
	 */
	public   void initDbArchiverList () throws ArchivingException
	{
		try
		{
			String m_dbDevicePattern = geDevicePattern() ;
			String m_dbClassDevice = getClassDevice();//isHistoric ? m_hdbClassDevice : m_tdbClassDevice;
	...
	public void ArchivingStart(ArchivingMessConfig archivingMessConfig) throws ArchivingException
	{
		...
		LoadBalancedList loadBalancedList = selectArchiversTask(archivingMessConfig.isGrouped(),archivingMessConfig.getAttributeToDedicatedArchiverHasthable ()  , finalAttributeList);
		...
		for ( int i = 0 ; i < attributeNameList.length ; i++ )
		{
			...
			try
			{
				...
				if ( !isRegistred(attributeName) )
				{
						...
						m_DataBase.getAttribute().registerAttribute(attributeHeavy, m_DataBase.getDbUtil());
				}
//				m_DataBase.getAttribute().CreateAttributeTableIfNotExist(attributeName, m_DataBase.getExtractor().getDataGetters(), m_DataBase.getDbUtil());
				myNewListV.add(attributeName);
				m_DataBase.getAttribute().CreateAttributeTableIfNotExist(attributeName, attributeHeavy, m_DataBase.getExtractor().getDataGetters(), m_DataBase.getDbUtil());
			}
			catch ( DevFailed devFailed )
			{
	...
	public void ArchivingStopConf(String[] attributeNameList) throws ArchivingException
	{
		...
		try
		{
			for ( int i = 0 ; i < attributeNameList.length ; i++ )
			{
				...
				if ( !m_DataBase.getMode().getDeviceInCharge(attributeName).equals("") )
				{
					if ( myStoppingConf.containsKey(archiverName) || !TangoAccess.deviceLivingTest(archiverName) )
					{
					}
	...
	protected boolean isRegistred(String attributeName)
	{
		try
		{
			if ( m_DataBase != null )
			{
				if ( m_DataBase.getAttribute().isRegisteredADT(attributeName) )
					return true;
			}
		}
		catch ( ArchivingException e )
	 ...
	 * @throws ArchivingException
	 */
	private LoadBalancedList selectArchiversTask(boolean grouped , Hashtable attributeToChosenArchiverHasthable ,
			String[] attributeNameList) throws ArchivingException
			{
		Hashtable attributeToDedicatedArchiver = getAttributesToDedicatedArchiver ( false );//CLA 10/11/06

		LoadBalancedList loadBalancedList = new LoadBalancedList (DbUtils.getHdbTdbType(m_DataBase.getDbConn().getDbType())== ConfigConst.HDB);
	 ...
	 */
	public static AttributeInfo getAttributeInfo(String attributeName, boolean doRefresh) throws DevFailed
	{
	 ...
	 * @throws ArchivingException
	 */
	private void connectArchivingDatabase(String mdbhost , String mdbname, String muser , String mpassword, boolean rac) throws ArchivingException
	{
		if ( m_DataBase == null )
		{
			// Host
			...
			try
			{
				...
				if ( schema.equals("") )
					schema = getSchema();//( historic ) ? ConfigConst.HDB_SCHEMA_NAME : ConfigConst.TDB_SCHEMA_NAME;
			}
			catch ( ArchivingException e )
			{
				System.err.println(e.toString());
			...
			String user = muser;
			if ( user.equals("") )
				user = getUser();//( historic ) ? ConfigConst.HDB_BROWSER_USER : ConfigConst.TDB_BROWSER_USER;

			String pass = mpassword;
			if ( pass.equals("") )
				pass = getPassword();//( historic ) ? ConfigConst.HDB_BROWSER_PASSWORD : ConfigConst.TDB_BROWSER_PASSWORD;

				m_DataBase = DataBaseManagerFactory.getDataBaseManager(type);
				m_DataBase.setDbConn(ConnectionFactory.connect_auto(host, name, schema, user, pass, type, rac ));
				is_connected = true;
		}
	}
...
//		System.out.println( "ArchivingConfigure/historic/"+historic);
	 ...
	 * @throws ArchivingException
	 */
	public void ArchivingConfigure(String host , String name, String user , String password, boolean rac ) throws ArchivingException
	{
//		System.out.println( "ArchivingConfigure/historic/"+historic);

		ArchivingConfigureWithoutArchiverListInit(host, name, user ,  password, rac );

		// The created lists are useful during the start archiving action 
		initDbArchiverList ( );
	}

	public void ArchivingConfigureWithoutArchiverListInit(String host , String name, String user , String password, boolean rac) throws ArchivingException
	{
//		System.out.println( "ArchivingConfigureWithoutArchiverListInit/historic/"+historic+"/user/"+user);
	 ...
	 * 
	 */
	public  Hashtable getAttributesToDedicatedArchiver (boolean refreshArchivers ) throws ArchivingException
	{
		if ( refreshArchivers )
		{
			initDbArchiverList ();
		...
		for ( int i = 0 ; i < exportedArchiversNameList.length ; i ++ )
		{
			...
			Archiver currentExportedArchiver = Archiver.findArchiver ( currentExportedArchiverName, DbUtils.getHdbTdbType(m_DataBase.getDbConn().getDbType())== ConfigConst.HDB);
		...
		for ( int i = 0 ; i < notExportedArchiversNameList.length ; i ++ )
		{
			...
			Archiver currentNotExportedArchiver = Archiver.findArchiver ( currentNotExportedArchiverName, DbUtils.getHdbTdbType(m_DataBase.getDbConn().getDbType())== ConfigConst.HDB );
		...
		Archiver.setAttributesToDedicatedArchiver (DbUtils.getHdbTdbType(m_DataBase.getDbConn().getDbType())== ConfigConst.HDB,  ret );
		return ret;
	}
	 ...
	 * 
	 */
	private static void initArchivers(LoadBalancedList loadBalancedList,boolean grouped,String[] referenceArchiverList) throws ArchivingException 
	{
