...
public class OracleAdtAptAttributes extends AdtAptAttributes {
	...
	public OracleAdtAptAttributes(IDBConnection c) {
	...
	protected String getRequest(String att_name) {
		...
		str = str + "to_char(" + ConfigConst.TAB_DEF[ ConfigConst.TAB_DEF.length - 1 ] + ")";
		str = str +
	...
	public void registerAttribute(AttributeHeavy attributeHeavy,
			IDbUtils dbUtils) throws ArchivingException {
		// TODO Auto-generated method stub
			...
			String myStatement = "{call " + dbConn.getSchema() + ".crtab (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}";
			try
			{
				//slim
			...
			{
				ConnectionCommands.rollback(conn, dbUtils);
				String message = "";
            ...
            {
            	try {
      				if(cstmt_register_in_hdb!=null)ConnectionCommands.close(cstmt_register_in_hdb);
	...
	public void CreateAttributeTableIfNotExist(String attributeName,
			AttributeHeavy attr, DataGetters extractor, IDbUtils dbUtils) throws ArchivingException {
		// TODO Auto-generated method stub
