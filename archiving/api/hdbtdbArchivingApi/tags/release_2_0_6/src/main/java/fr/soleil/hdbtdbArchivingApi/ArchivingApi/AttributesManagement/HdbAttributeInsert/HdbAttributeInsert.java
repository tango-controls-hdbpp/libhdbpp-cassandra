...
public abstract class HdbAttributeInsert implements IHdbAttributeInsert{
	...
	public HdbAttributeInsert(IDBConnection con, IAdtAptAttributes at) {
		// TODO Auto-generated constructor stub
		this.dbConn = con;
		this.att = at;

	}
	abstract protected void insert_SpectrumData_RO_DataBase(StringBuffer query, StringBuffer tableName, StringBuffer tableFields, int dim_x,
			Timestamp timeSt,  Double[] dvalue, StringBuffer valueStr, String att_name) throws ArchivingException;
	abstract protected void insert_SpectrumData_RW_DataBase(StringBuffer query, StringBuffer tableName, StringBuffer tableFields, int dim_x,
			Timestamp timeSt, Double[] dvalueWrite, Double[] dvalueRead, StringBuffer valueWriteStr, StringBuffer valueReadStr,
			String att_name) throws ArchivingException;
	abstract protected void insert_ImageData_RO_DataBase(StringBuffer query, StringBuffer tableName, StringBuffer tableFields, int dim_x,
			int dim_y, Timestamp timeSt, Double[][] dvalue, StringBuffer valueStr, String att_name) throws  ArchivingException;
	...
	public void insert_ScalarData(ScalarEvent scalarEvent) throws ArchivingException
	{
		...
		try
		{
			...
			_tableFields = (_writable == AttrWriteType._READ || _writable == AttrWriteType._WRITE ) ?
					...
					_tableName = new StringBuffer().append(dbConn.getSchema()).append(".").append(dbConn.getDbUtils().getTableName(_attributeName, att));
...
//							Fill the value field
							ConnectionCommands.prepareSmtForInsertScalar1ValNotNull(preparedStatement,_dataType,_value,2);
						}
						else
						{
							ConnectionCommands.prepareSmtForInsertScalar2ValNotNull(preparedStatement, _dataType,_value,2);
						}
					}
					else // if ( _value != null )
					{
						ConnectionCommands.prepareSmtForInsertScalarNullValue(preparedStatement,_writable,_dataType,2);
					}
	...
	public void insert_SpectrumData_RO(SpectrumEvent_RO hdbSpectrumEvent_RO) throws ArchivingException
	{
		...
		StringBuffer tableName = new StringBuffer().append(dbConn.getSchema()).append(".").append(dbConn.getDbUtils().getTableName(att_name, att));
...
//			dbConn.connect();
		...
		insert_SpectrumData_RO_DataBase(query, tableName, tableFields, dim_x, timeSt, dvalue, valueStr, att_name);
...
//			dbConn.connect();
		...
		insert_SpectrumData_RW_DataBase(query, tableName, tableFields, dim_x, timeSt, dvalueWrite, dvalueRead, valueWriteStr, valueReadStr, att_name);
...
//		}
		...
		insert_ImageData_RO_DataBase(query, tableName, tableFields, dim_x, dim_y, timeSt, dvalue, valueStr, att_name);
		//Close the connection with the database
//		if ( dbConn.isAutoConnect() )
//		{
//			dbConn.close();
