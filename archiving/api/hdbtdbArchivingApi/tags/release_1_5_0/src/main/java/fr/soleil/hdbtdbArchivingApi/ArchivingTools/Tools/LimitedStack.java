...
public class LimitedStack extends Vector
{
    ...
    public boolean containsDate ( long date , ILogger logger )  
    {
        ...
        while ( enumer.hasMoreElements () )
        {
            ...
            if ( matchFound )
            {
            	logger.trace ( ILogger.LEVEL_DEBUG , "New date = " + date + " | Existing date = " + nextOld.longValue () );
                return true;
            }
        }
...
//            		 "\n old date :" + new Timestamp(nextOld.longValue ()).toString());
            if ( matchFound )
            {
            	logger.trace ( ILogger.LEVEL_DEBUG , "New date = " + date + " | Previous date = " + nextOld.longValue () );
                return true;
            }
        }
        return false;
