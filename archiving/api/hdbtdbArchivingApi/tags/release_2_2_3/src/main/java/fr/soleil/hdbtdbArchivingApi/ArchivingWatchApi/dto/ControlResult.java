...
public class ControlResult {
	...
	private Hashtable<String, ControlResultLine> lines;
	private boolean isBuilt = false;

	private Hashtable<String, Archiver> errorArchivers;
	private Hashtable<String, Domain> errorDomains;
	private Hashtable<String, ArchivingAttribute> errorAttributes;
	private Hashtable<String, ArchivingAttributeSubName> errorSubAttributes;

	private Hashtable<String, ArchivingAttribute> okAttributes;
	...
	public ControlResult() {
		super();
		this.lines = new Hashtable<String, ControlResultLine>();
		...
		this.errorArchivers = new Hashtable<String, Archiver>();
		this.errorDomains = new Hashtable<String, Domain>();
		this.errorSubAttributes = new Hashtable<String, ArchivingAttributeSubName>();
		this.errorAttributes = new Hashtable<String, ArchivingAttribute>();
		this.okAttributes = new Hashtable<String, ArchivingAttribute>();
	...
	public void setAllArchivingAttributes(Hashtable<String, ModeData> list) {
		if (list == null) {
			return;
		}
	...
	public void setLines(Vector[] listOfControlledAttributes) throws DevFailed {
		...
		this.setNumberOfArchivingAttributes(koAttributes.size()
				+ okAttributes.size());
	...
	private void addList(Vector controlledAttributes, boolean areOK) {
		...
		if (controlledAttributes != null && controlledAttributes.size() != 0) {
			...
			for (int i = 0; i < numberOfKOAttributes; i++) {
				String nextCompleteName = (String) controlledAttributes
						.elementAt(i);
				// System.out.println (
				...
				ControlResultLine line = new ControlResultLine(attr,
						nextModeData, result);
				this.addLine(line);
			}
		}
	...
	public int getFinalSize() {
		IChoosingStrategy choosingStrategy = ChoosingStrategyFactory
				.getCurrentImpl();
	...
	private String getAttributeSubNameReport() {
		...
		if (this.errorSubAttributes != null
				&& this.errorSubAttributes.size() != 0) {
			buff
					.append("**************************************ATTRIBUTE NAMES WITH AT LEAST 1 KO ATTRIBUTE*****************************************");
			buff.append(Tools.CRLF);

			List<ArchivingAttributeSubName> list = new Vector<ArchivingAttributeSubName>();
			list.addAll(this.errorSubAttributes.values());
			Collections.sort(list, this.archivingAttributeSubNamesComparator);
			Iterator<ArchivingAttributeSubName> it = list.iterator();

			while (it.hasNext()) {
				ArchivingAttributeSubName nextArchivingAttributeSubName = (ArchivingAttributeSubName) it
						.next();
				if (nextArchivingAttributeSubName != null) {
	...
	private String getDomainsReport() {
		...
		if (this.errorDomains != null && this.errorDomains.size() != 0) {
			...
			List<Domain> list = new Vector<Domain>();
	...
	private String getAttributesReport() {
		...
		List<ControlResultLine> list = new Vector<ControlResultLine>();
		list.addAll(this.lines.values());
		Collections.sort(list, this.linesComparator);
		Iterator<ControlResultLine> it = list.iterator();
	...
	public String getReportHeader() throws DevFailed {
		...
		String endDate = this.getEndDate() == null ? "NOT YET" : this
				.getEndDate()
				+ "";
		buff.append("Cycle started at: " + this.getStartDate()
				+ ". Current time: "
				+ new Timestamp(System.currentTimeMillis())
				+ ". Cycle completed at: " + endDate);
		buff.append(Tools.CRLF);

		buff.append("Completed cycles: " + this.getCycles()
				+ ". Completed steps: " + this.getSteps());
		buff.append(Tools.CRLF);

		buff.append("Global result: "
				+ ControlResult.getAssociatedLabel(this.getCode()));
		buff.append(Tools.CRLF);

		buff
				.append("Attributes recorded in DB when the current fixed cycle started: "
						+ this.getNumberOfArchivingAttributes());
		buff.append(Tools.CRLF);

		buff.append("Control attempts in the current rollover cycle: "
				+ this.getNumberOfControlledAttributes());
		buff.append(Tools.CRLF);

		buff
				.append("Successful control attempts in the current rollover cycle: "
						+ this.getNumberOfAttributesControlledWithSuccess());
		buff.append(Tools.CRLF);

		buff.append("Number of KO attributes: "
				+ this.getNumberOfKoAttributes());
		buff.append(Tools.CRLF);

		buff.append("Number of undetermined attributes: "
				+ this.getNumberOfUndeterminedAttributes());
		buff.append(Tools.CRLF);

		buff
				.append("Number of attributes with null values: "
						+ this
	...
	public synchronized boolean isAttributeCorrectlyArchived(String completeName) {
		ControlResultLine nextLine = (ControlResultLine) this.lines
				.get(completeName);
		int attributeStatus = nextLine.getAttribute().getArchivingStatus();

		boolean ret = (attributeStatus == IModeController.CONTROL_OK);
		ret = ret
				|| (attributeStatus == IModeController.CONTROL_OK_BUT_VALUE_MIGHT_BE_NULL);
		ret = ret
				|| (attributeStatus == IModeController.CONTROL_OK_BUT_VALUE_IS_NULL);
		return ret;
	}
	...
	private static String getAssociatedLabel(int _code) throws DevFailed {
		...
		default:
			Tools.throwDevFailed(new IllegalArgumentException(
					"Expected either of " + NOT_READY + "," + ALL_OK + ","
							+ ALL_KO + "," + PARTIAL_KO + ","
							+ NO_ATTRIBUTES_TO_CONTROL
							+ " as a parameter. Received " + _code
							+ " instead."));
		}
		return null;
	}
	 ...
	 * @throws DevFailed
	 */
	public void build(boolean doArchiverDiagnosis, boolean _isFullCycle)
			throws DevFailed {
		...
		// this.getNumberOfKoAttributes() );
		int _code = this.buildGlobalCode(_numberOfControlledAttributes, this
				.getNumberOfAttributesControlledWithSuccess(), this
				.getNumberOfKoAttributes());
		this.setCode(_code);
		...
		Hashtable<String, Archiver> _errorArchivers;
		try {
			_errorArchivers = this.buildArchivers(this.errorArchivers,
					doArchiverDiagnosis);
		} catch (Exception e) {
			// Tools.throwDevFailed ( e );
			Tools.trace("ControlResult/failed to build the archivers report!",
					e, Warnable.LOG_LEVEL_ERROR);
			_errorArchivers = new Hashtable<String, Archiver>();
		}
		this.setErrorArchivers(_errorArchivers);

		Hashtable<String, Domain> _errorDomains = this
				.buildDomains(this.errorDomains);
		this.setErrorDomains(_errorDomains);

		Hashtable<String, ArchivingAttributeSubName> _errorSubAttributes = this
				.buildSubAttributes(this.errorSubAttributes);
		this.setErrorSubAttributes(_errorSubAttributes);

		this.isBuilt = true;
	}

	private Hashtable<String, ArchivingAttributeSubName> buildSubAttributes(
			Hashtable<String, ArchivingAttributeSubName> _errorSubAttributes) {
		Enumeration<String> errorSubAttributesKeys = _errorSubAttributes.keys();
	...
	private Hashtable<String, Domain> buildDomains(
			Hashtable<String, Domain> _errorDomains) {
		Enumeration<String> errorDomainsKeys = _errorDomains.keys();
	...
	private void buildLines(Hashtable<String, ControlResultLine> _lines)
			throws DevFailed {
		Enumeration<String> _enum = _lines.keys();
		...
		while (_enum.hasMoreElements()) {
			...
			if (!isArchivingOK) {
				if (modeData != null && !modeData.isIncomplete()) {
					this.errorArchivers.put(modeData.getArchiver(), null);
				}

				this.errorDomains.put(attr.getDomain(), null);
				this.errorSubAttributes.put(attr.getAttributeSubName(), null);
	...
	private Hashtable<String, Archiver> buildArchivers(
			Hashtable<String, Archiver> _errorArchivers,
			boolean doArchiverDiagnosis) // throws DevFailed, ArchivingException
	{
		// Tools.trace ( "ControlResult/buildArchivers/START" ,
		// Warnable.LOG_LEVEL_DEBUG );

		Enumeration<String> errorArchiversKeys = _errorArchivers.keys();
		while (errorArchiversKeys.hasMoreElements()) {
			...
			if (doArchiverDiagnosis) {
				...
				try {
					...
					Tools
							.trace(
									"ControlResult/buildArchivers/failed calling isAlive for archiver|"
											+ nextKey + "|", e,
				...
				if (isAlive) {
					...
					try {
						spectrumLoad = attributesReader.getArchiverLoad(
								nextKey, IDBReader.LOAD_SPECTRUM);
						ok = false;
					} catch (DevFailed e) {
						Tools
								.trace(
					...
					try {
						imageLoad = attributesReader.getArchiverLoad(nextKey,
								IDBReader.LOAD_IMAGE);
						ok = false;
					} catch (DevFailed e) {
						Tools
								.trace(
	 ...
	 */
	private int buildGlobalCode(int _numberOfControlledAttributes,
			int _numberOfControlledAttributesWithSuccess,
			int _numberOfKoAttributes) {
	 ...
	 */
	public synchronized Hashtable<String, Archiver> getErrorArchivers() {
	 ...
	 */
	private synchronized void setErrorArchivers(
			Hashtable<String, Archiver> errorArchivers) {
	 ...
	 */
	public synchronized Hashtable<String, Domain> getErrorDomains() {
		return errorDomains;
	}
	 ...
	 */
	private synchronized void setErrorDomains(
			Hashtable<String, Domain> errorDomains) {
	...
	public void addLines(ControlResult controlResult2) {
		Hashtable<String, ControlResultLine> lines2 = controlResult2.lines;
		Enumeration<String> keys = lines2.keys();
		while (keys.hasMoreElements()) {
			String nextKey = (String) keys.nextElement();
			ControlResultLine nextLine = (ControlResultLine) lines2
					.get(nextKey);
	...
	public synchronized ControlResult cloneControlResult() {
		...
		ret.lines = (Hashtable<String, ControlResultLine>) this.lines.clone();
		ret.code = this.code;
		...
		ret.errorArchivers = (Hashtable<String, Archiver>) this.errorArchivers
				.clone();
		ret.errorAttributes = (Hashtable<String, ArchivingAttribute>) this.errorAttributes
				.clone();
		ret.errorDomains = (Hashtable<String, Domain>) this.errorDomains
				.clone();
		ret.errorSubAttributes = (Hashtable<String, ArchivingAttributeSubName>) this.errorSubAttributes
				.clone();
		ret.okAttributes = (Hashtable<String, ArchivingAttribute>) this.okAttributes
				.clone();
		...
		ret.m_numberOfAttributesControlledWithSuccessWithNullValue = this.m_numberOfAttributesControlledWithSuccessWithNullValue;
	 ...
	 *         are empty.
	 */
	public Hashtable<String, ArchivingAttributeSubName> getErrorSubAttributes() {
	...
	private void setErrorSubAttributes(
			Hashtable<String, ArchivingAttributeSubName> errorSubAttributes) {
	 ...
	 */
	private void setNumberOfUndeterminedAttributes(
			int numberOfUndeterminedAttributes) {
	...
	private void setNumberOfAttributesControlledWithSuccessWithNullValue(
			int numberOfAttributesControlledWithSuccessWithNullValue) {
		this.m_numberOfAttributesControlledWithSuccessWithNullValue = numberOfAttributesControlledWithSuccessWithNullValue;
	}
	...
	public void setSteps(int _steps) {
		System.out.println("CLA/ControlResult/" + this.hashCode()
				+ "/setSteps/_steps/" + _steps);
		this.steps = _steps;
	}

	public ControlResultLine[] sort() {
		ControlResultLine[] ret = new ControlResultLine[this.lines.size()];
		Iterator<ControlResultLine> it = this.lines.values().iterator();
		int i = 0;
		...
		List<ControlResultLine> list = Arrays.asList(ret);
		Collections.sort(list, this.linesComparator);
	...
	public void removeOldLines(Set<String> namesOfAttributesToControl)// CLA
	// 14/12/06
	{
		// System.out.println (
		// "CLA/removeOldLines/namesOfAttributesToControl|"+namesOfAttributesToControl.size()
		// );
		Enumeration<String> it = this.lines.keys();
		while (it.hasMoreElements()) {
			String nextName = (String) it.nextElement();
			ControlResultLine next = (ControlResultLine) this.lines
					.get(nextName);
