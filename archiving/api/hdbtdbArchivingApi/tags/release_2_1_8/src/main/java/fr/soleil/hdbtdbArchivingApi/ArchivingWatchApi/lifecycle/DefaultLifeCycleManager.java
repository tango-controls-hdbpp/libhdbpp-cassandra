...
public abstract class DefaultLifeCycleManager extends Thread implements LifeCycleManager
{
	 ...
	 */
	public void process ( long macroPeriod , boolean doArchiverDiagnosis, short numberOfKOForAlarmState )
	{
	    ...
	    while ( true )
	    {
	        try
	        {
                ...
                processStep ( macroPeriod , doArchiverDiagnosis,numberOfKOForAlarmState );
	...
	public void processStep ( long macroPeriod , boolean doArchiverDiagnosis, short numberOfKOForAlarmState ) throws DevFailed
    {
	    IDelayManager delayManager = DelayManagerFactory.getCurrentImpl ();
        ...
        if ( currentResult != null )
        {
            ...
            if ( currentResult.hasErrors () )
            {
                ...
                //System.out.println("=====>SPJZ ======>Number of Ko ATT = " + currentResult.getNumberOfKoAttributes());
                int numberOfKo = currentResult.getNumberOfKoAttributes();

                if(numberOfKo >= numberOfKOForAlarmState && !previousWasInAlarmState) {
                		this.warnWatcherAlarm ();
                		previousWasInAlarmState = true;
                }else if(numberOfKo < numberOfKOForAlarmState && previousWasInAlarmState){
                	this.warnWatcherOn ();
                	previousWasInAlarmState = false;
                }
            }
    ...
    public void run()
    {
        try
        {
            ...
            this.process ( macroPeriod , doArchiverDiagnosis,  ArchivingWatch.getMinNumberOfKoForAlarmState());
        }
        catch (Throwable t)
        {
            t.printStackTrace();
