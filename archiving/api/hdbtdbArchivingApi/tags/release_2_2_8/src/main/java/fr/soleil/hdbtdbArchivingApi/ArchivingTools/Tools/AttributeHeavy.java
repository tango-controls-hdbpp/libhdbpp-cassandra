 ...
 * @see fr.soleil.hdbtdbArchivingApi.ArchivingTools.Tools.ArchivingEvent
 */
public class AttributeHeavy extends AttributeLightMode {
	...
	private String optional_properties = ""; // ****************
												// OPTIONAL_PROPERTIES
	 ...
	 * 
	 * @param argin
	 *            an array that contains the AttributeHeavy's registration time,
	 *            full name, device name, associated domain name, associated
	 *            family name, associated member name, attribute name, data type
	 *            parameter, data format parameter, writable parameter,
	 *            max_dim_x value, max_dim_y value, level value, belonging
	 *            control system name, archivable value, substitue name,
	 *            description, label, unit, standard unit, display unit, format,
	 *            min value, max value, min alarm, max amarm, optional
	 *            properties, target, collector and mode.
	 */
	public AttributeHeavy(String[] argin) {
		...
		setOptional_properties(argin[26]); // ****************
											// OPTIONAL_PROPERTIES
	 ...
	 *            the AttributeHeavy's <I>name</I> (full name).
	 */
	...
	/**
	 * Returns the AttributeHeavy's <I>type property</I>
	 * (DevShort/DevLong/DevDouble)
	 * 
	 * @return the AttributeHeavy's <I>type property</I>
	 *         (DevShort/DevLong/DevDouble)
	 */
	...
	/**
	 * Sets the AttributeHeavy's <I>type property</I>
	 * (DevShort/DevLong/DevDouble)
	 * 
	 * @param data_type
	 *            the AttributeHeavy's <I>type property</I>
	 *            (DevShort/DevLong/DevDouble)
	 */
	...
	/**
	 * Returns the AttributeHeavy's <I>data format property</I>
	 * (SCALAR/SPECTRUM/IMAGE)
	 * 
	 * @return the AttributeHeavy's <I>data format property</I>
	 *         (SCALAR/SPECTRUM/IMAGE)
	 */
	...
	/**
	 * Sets the AttributeHeavy's <I>data format property</I>
	 * (SCALAR/SPECTRUM/IMAGE)
	 * 
	 * @param data_format
	 *            the AttributeHeavy's <I>data format property</I>
	 *            (SCALAR/SPECTRUM/IMAGE)
	 */
	...
	/**
	 * Returns the AttributeHeavy's <I>writable property</I>
	 * (READ/READ_WRITE/READ_WITH_WRITE).
	 * 
	 * @return the AttributeHeavy's <I>writable property</I>
	 *         (READ/READ_WRITE/READ_WITH_WRITE).
	 */
	...
	/**
	 * Sets the AttributeHeavy's <I>writable property</I>
	 * (READ/READ_WRITE/READ_WITH_WRITE).
	 * 
	 * @param writable
	 *            the AttributeHeavy's <I>writable property</I>
	 *            (READ/READ_WRITE/READ_WITH_WRITE).
	 */
	...
	/**
	 * Returns an array representation of the object <I>AttributeHeavy</I>. In
	 * this order, array fields are :
	 * <ol>
	 ...
	 * <li>some <I>optional_properties</I>
	 * <p/>
	 * <li>the <I>target</I> (TANGO device in charge of inserting datum into the
	 * database)
	...
	public String[] toArray() {
		...
		att_tab[0] = getRegistration_time().toString(); // ****************
														// Attribute
														// registration
														// timestamp
		att_tab[1] = getAttribute_complete_name(); // **************** The whole
													// attribute name
													// (device_name +
													// attribute_name)
		att_tab[2] = getAttribute_device_name(); // **************** name of the
													// belonging device.

		att_tab[3] = getDomain(); // **************** domain to which the
									// attribute is associated
		att_tab[4] = getFamily();
		att_tab[5] = getMember();

		att_tab[6] = getAttribute_name(); // **************** attribute name
		att_tab[7] = Integer.toString(getData_type()); // ****************
														// Attribute data type
		att_tab[8] = Integer.toString(getData_format()); // ****************
															// Attribute data
															// format
		att_tab[9] = Integer.toString(getWritable()); // ****************
														// Attribute read/write
														// type
		att_tab[10] = Integer.toString(getMax_dim_x()); // ****************
														// Attribute Maximum X
														// dimension
		att_tab[11] = Integer.toString(getMax_dim_y()); // ****************
														// Attribute Maximum Y
														// dimension
		att_tab[12] = Integer.toString(getLevel()); // ****************
													// Attribute display level
		att_tab[13] = getCtrl_sys(); // **************** Control system to which
										// the attribute belongs
		att_tab[14] = Integer.toString(getArchivable()); // ****************
															// archivable
															// (Property that
															// precises whether
															// the attribute is
															...
															// it is "always"
															// archivable
		att_tab[15] = Integer.toString(getSubstitute()); // ****************
															// substitute
		/* APT */
		...
		att_tab[26] = getOptional_properties(); // ****************
												// OPTIONAL_PROPERTIES
	...
	public String toString() {
		...
		buf.append("\tRegistration Time .........."
				+ getRegistration_time().toString() + "\n"); // ****************
																// Attribute
																// registration
																// timestamp
		buf.append("\tAttribute_complete_name ...."
				+ getAttribute_complete_name() + "\n"); // **************** The
		...
		buf.append("\tAttribute name ............." + getAttribute_name()
				+ "\n"); // **************** attribute name
		buf.append("\tData type property ........."
				+ Integer.toString(getData_type()) + "\n"); // ****************
															// Attribute data
															// type
		buf.append("\tData format property ......."
				+ Integer.toString(getData_format()) + "\n"); // ****************
																// Attribute
																// data format
		buf.append("\tWritable property .........."
				+ Integer.toString(getWritable()) + "\n"); // ****************
															// Attribute
															// read/write type
		buf.append("\tMaximum X dimension ........"
				+ Integer.toString(getMax_dim_x()) + "\n"); // ****************
															// Attribute Maximum
															// X dimension
		buf.append("\tMaximum X dimension ........"
				+ Integer.toString(getMax_dim_y()) + "\n"); // ****************
															// Attribute Maximum
															// Y dimension
		buf.append("\tDisplay level .............."
		...
		buf.append("\tCtrl system ................" + getCtrl_sys() + "\n"); // ****************
																				...
																				// attribute
																				// belongs
		buf.append("\tArchivable  ................"
				+ Integer.toString(getArchivable()) + "\n"); // ****************
																// archivable
																// (Property
																// that precises
																// whether the
		...
		buf.append("\tSubstitute ................."
				+ Integer.toString(getSubstitute()) + "\n"); // ****************
																// substitute
		...
		buf.append("\tUnit ......................." + getUnit() + "\n"); // ****************
																			// UNIT
		buf
				.append("\tStandard Unit .............." + getStandard_unit()
						+ "\n"); // **************** STANDARD_UNIT
	...
	public boolean equals(Object o) {
		...
		if (level != attributeHeavy.level)
			return false;
		if (max_dim_x != attributeHeavy.max_dim_x)
			return false;
		if (max_dim_y != attributeHeavy.max_dim_y)
			return false;
		if (substitute != attributeHeavy.substitute)
			return false;
		if (!attribute_device_name.equals(attributeHeavy.attribute_device_name))
			return false;
		if (!ctrl_sys.equals(attributeHeavy.ctrl_sys))
			return false;
		if (description != null ? !description
				.equals(attributeHeavy.description)
				: attributeHeavy.description != null)
			return false;
		if (display_unit != null ? !display_unit
				.equals(attributeHeavy.display_unit)
				: attributeHeavy.display_unit != null)
			return false;
		if (!domain.equals(attributeHeavy.domain))
			return false;
		if (!family.equals(attributeHeavy.family))
			return false;
		if (format != null ? !format.equals(attributeHeavy.format)
				: attributeHeavy.format != null)
			return false;
		if (label != null ? !label.equals(attributeHeavy.label)
				: attributeHeavy.label != null)
			return false;
		if (max_alarm != null ? !max_alarm.equals(attributeHeavy.max_alarm)
				: attributeHeavy.max_alarm != null)
			return false;
		if (max_value != null ? !max_value.equals(attributeHeavy.max_value)
				: attributeHeavy.max_value != null)
			return false;
		if (!member.equals(attributeHeavy.member))
			return false;
		if (min_alarm != null ? !min_alarm.equals(attributeHeavy.min_alarm)
				: attributeHeavy.min_alarm != null)
			return false;
		if (min_value != null ? !min_value.equals(attributeHeavy.min_value)
				: attributeHeavy.min_value != null)
			return false;
		if (optional_properties != null ? !optional_properties
				.equals(attributeHeavy.optional_properties)
				: attributeHeavy.optional_properties != null)
			return false;
		if (!registration_time.equals(attributeHeavy.registration_time))
			return false;
		if (standard_unit != null ? !standard_unit
				.equals(attributeHeavy.standard_unit)
				: attributeHeavy.standard_unit != null)
			return false;
		if (unit != null ? !unit.equals(attributeHeavy.unit)
				: attributeHeavy.unit != null)
			return false;
	...
	public int hashCode() {
		...
		result = 29 * result
				+ (standard_unit != null ? standard_unit.hashCode() : 0);
		result = 29 * result
				+ (display_unit != null ? display_unit.hashCode() : 0);
		...
		result = 29
				* result
				+ (optional_properties != null ? optional_properties.hashCode()
						: 0);
		return result;
	}
}
