 ...
 */
package fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.delay;
...
public class DefaultDelayManager implements IDelayManager {
	...
	/**
	 * A ControlResult that is built progressively during the first cycle, then
	 * (when it is complete), each newer control result for a given atribute
	 * overrides the control result for the same given atribute that was
	 * computed in the previous cycle.
	 */
	protected ControlResult rollOverResult;

	/**
	 * The current position in the list of attributes to control, ranging from 0
	 * to the total number of attributes to control
	 */
	protected int currentIndex;

	/**
	 * The Hashtable that stores the list of attributes to control for the next
	 * step
	 */
	protected Hashtable<String, ModeData> archivedAttributes = null;
	/**
	...
	/**
	 * Used to directly control the number of attributes controlled in a step:
	 * minimum value
	 */
	...
	/**
	 * A reference to the device that needs to be warned when the first control
	 * step is completed (so that it can go from the INIT state the ON state)
	 */
	...
	private boolean isFullCycle() {
		boolean isFullCycle = this.currentResult == null ? false
				: this.currentResult.isComplete();
	 ...
	 *
	 * @see
	 * archwatch.strategy.delay.IDelayManager#addControlResult(archwatch.dto
	 * .ControlResult)
	 */
	public boolean addControlResult(ControlResult partialControlResult,
			Set<String> namesOfAttributesToControl) {
		this.setSizeOfLatestAddition(partialControlResult.getCurrentSize());
		this.rollOverResult.removeOldLines(namesOfAttributesToControl);// CLA
																		// 14/12/06
																		// to
																		// avoid
																		// keeping
	...
	private void completeCycle(ControlResult partialControlResult) {
		...
		this.latestCompleteResult = this.currentResult == null ? null
				: this.currentResult.cloneControlResult();
	...
	public Hashtable chooseAttributesSubsetForCurrentStep(Hashtable in)
			throws DevFailed {
		...
		if (in == null) {
			Tools
					.trace(
							"DefaultDelayManager/chooseAttributesSubsetForCurrentStep/in == null !!!!",
							Warnable.LOG_LEVEL_WARN);
		}

		IChoosingStrategy choosingStrategy = ChoosingStrategyFactory
				.getCurrentImpl();
		Hashtable filteredAttributes = choosingStrategy
				.filterAttributesToControl(in);
		int numberOfAttributesToControl = filteredAttributes.size();
		int totalNumberOfAttributes = choosingStrategy
				.getNumberOfAttributesToControl();
		/*
		 * if ( numberOfAttributesToControl != totalNumberOfAttributes ) {
		 * System.out.println (
		 * "CLA/DefaultDelayManager/numberOfAttributesToControl/"
		 * +numberOfAttributesToControl
		 * +"/totalNumberOfAttributes/"+totalNumberOfAttributes ); }
		 */

		long macroPeriod = ArchivingWatch.getMacroPeriod();
		int numberOfAttributesByStep = this.getSizeOfSteps(
				numberOfAttributesToControl, macroPeriod);
		// System.out.println (
		// "CLA/DefaultDelayManager/numberOfAttributesByStep/"+numberOfAttributesByStep
		// );

		int numberOfAttributesNotControlledYet = numberOfAttributesToControl
				- this.currentIndex;
		// System.out.println (
		// "CLA/DefaultDelayManager/numberOfAttributesToControl/"+numberOfAttributesToControl+"/currentIndex/"+currentIndex+"/numberOfAttributesNotControlledYet/"+numberOfAttributesNotControlledYet
		// );
		...
		for (int i = 0; i < this.currentIndex; i++) {
			try {
				...
				Tools
						.trace(
								"DefaultDelayManager/chooseAttributesSubsetForCurrentStep/suspect NoSuchElementException.",
								Warnable.LOG_LEVEL_WARN);
				int inSize = in == null ? -1 : in.size();
				// Tools.trace (
				// "inSize/"+inSize+"/numberOfAttributesToControl/"+numberOfAttributesToControl+"/totalNumberOfAttributes/"+totalNumberOfAttributes+"/numberOfAttributesByStep/"+numberOfAttributesByStep+"/"
				// , Warnable.LOG_LEVEL_WARN );
				Tools.trace("inSize/" + inSize
						+ "/numberOfAttributesToControl/"
						+ numberOfAttributesToControl
						+ "/numberOfAttributesByStep/"
						+ numberOfAttributesByStep + "/",
						Warnable.LOG_LEVEL_WARN);
				return ret;
			}
		...
		for (int i = 0; i < numberOfAttributesByStep; i++) {
			try {
				...
				// System.out.println (
				// "----------CLA/DefaultDelayManager/key|"+key );
				ModeData val = attributesReader.getModeDataForAttribute(key);

				if (val.getMode().getModeP() == null) {
					Tools
							.trace(
									"DefaultDelayManager/chooseAttributesSubsetForCurrentStep/modeP == null!/for attribute|"
											+ key + "|",
	...
	/**
	 * Returns the ControlResult of the latest complete cycle if available,
	 * otherwise returns the result of the latest constrol step.
	 *
	 * @return The ControlResult of the latest complete cycle if available,
	 *         otherwise returns the result of the latest constrol step.
	 */
	 ...
	 */
	public Hashtable<String, ModeData> reloadArchivingAttributesIfNecessary() throws DevFailed {
		if (this.isNewCycle) {
			// System.out.println (
			// "DefaultDelayManager/getArchivingAttributesForCurrentStep/isNewCycle"
			// );
			IDBReader archivedAttributesReader = DBReaderFactory
					.getCurrentImpl();

			Hashtable<String, ModeData> _archivedAttributes = new Hashtable<String, ModeData>();
			try {
				_archivedAttributes = archivedAttributesReader
						.getArchivedAttributes();
			} catch (DevFailed e) {
				Tools
						.trace(
								"DefaultDelayManager/getArchivingAttributesForCurrentStep/failed calling getArchivedAttributes!",
			...
			System.out
					.println("DefaultDelayManager/getArchivingAttributesForCurrentStep/!isNewCycle");
		}
	 ...
	 *
	 * @see
	 * archwatch.strategy.delay.IDelayManager#getSizeOfAttributesPackets(int,
	 * long)
	 */
	public int getSizeOfSteps(int totalNumberOfAttributes, long macroPeriod) {
		System.out
				.println("DefaultDelayManager/getSizeOfFullAttributesPackets/0");
	 ...
	 *
	 * @param totalNumberOfAttributes
	 *            The total number of attributes in a cycle
	 * @param macroPeriod
	...
	private int getNumberOfPackets(int totalNumberOfAttributes, long macroPeriod) {
		int sizeOfAttributesPackets = this.getSizeOfSteps(
				totalNumberOfAttributes, macroPeriod);
		int numberOfPackets = (int) Math.ceil((totalNumberOfAttributes * 1.0)
				/ (sizeOfAttributesPackets * 1.0));
	 ...
	 *
	 * @see
	 * archwatch.strategy.delay.IDelayManager#getTimeBetweenAttributesPackets
	 * (int, long)
	 */
	public long getTimeBetweenSteps(int totalNumberOfAttributes,
			long macroPeriod) {
		int numberOfPackets = this.getNumberOfPackets(totalNumberOfAttributes,
				macroPeriod);
	...
	/**
	 * @param sizeOfLatestAddition
	 *            The sizeOfLatestAddition to set.
	 */
	 ...
	 */
	public synchronized ControlResult getControlResult(int readType)
			throws DevFailed {
		...
		default:
			Tools
					.throwDevFailed(new IllegalArgumentException(
							"Expected either LATEST_ADDITION (30),LATEST_COMPLETE_CYCLE(10),LATEST_COMPLETE_CYCLE_OR_STEP_IF_CYCLE_NOT_AVAILABLE(11),LATEST_COMPLETE_STEP(20), got "
									+ readType + " instead."));
		}
	...
	/**
	 * Returns a ControlResult refreshed at every control step, by overriding
	 * the previous result of the attributes that were just controlled
	 *
	 * @return A ControlResult refreshed at every control step, by overriding
	 *         the previous result of the attributes that were just controlled
	 */
	...
	/**
	 * Warns the device it should go into the ON state, if it hasn't been warned
	 * already
	 */
	...
	/**
	 * Returns the ControlResult for the latest complete full cycle having KO
	 * attributes
	 *
	...
	/**
	 * @param latestBadCompleteResult
	 *            The latestBadCompleteResult to set.
	 */
	public synchronized void setLatestBadCompleteResult(
			ControlResult latestBadCompleteResult) {
		this.latestBadCompleteResult = latestBadCompleteResult;
		this.latestBadCompleteResult.setEndDate(new Timestamp(System
				.currentTimeMillis()));
	}
	 ...
	 * (non-Javadoc)
	 *
	 * @seefr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.devicelink.Warner#
	 * getWatcherToWarn()
	 */
