...
public class HDBOracleAttributeInsert extends HdbAttributeInsert /*
	...
	try {
	    maxInsertionCounter = GetConf.readShortInDB("HdbArchiver", COMMIT_PERIOD_IN_COUNTER,
     ...
     */
    @Override
    protected synchronized void insertScalarDataInDB(final StringBuffer _query,
	    final String _attributeName, final int _writable, final Timestamp _timeSt,
	    final Object _value, final int _dataType) throws ArchivingException {
	...
	if (insertionCounter >= maxInsertionCounter) {
		...
		try {
		    logger.trace(ILogger.LEVEL_INFO, "Commiting connection ...");
		    connectionForInsertScalar.commit();
		    logger.trace(ILogger.LEVEL_INFO, "Connection commited successfully.");
		} catch (final SQLException e) {
		    logger.trace(ILogger.LEVEL_ERROR,
			    "HDBOracleAttributeInsert : Error occured while commiting connection."
				    + e);
		}
		logger.trace(ILogger.LEVEL_INFO, "Closing connection ...");
		dbConn.closeConnection(connectionForInsertScalar);
		logger.trace(ILogger.LEVEL_INFO, "Connection closed successfully.");
		connectionForInsertScalar = null;
	    } else {
		logger.trace(ILogger.LEVEL_INFO, "Connection is NULL...");
	    }
	    insertionCounter = 0;
	}
	...
	try {
		...
		logger.trace(ILogger.LEVEL_INFO, "Get a new Connection");
		insertionCounter = 0;
	    }
	} catch (final SQLException e) {
	    connectionForInsertScalar = null;
	    ...
	    final String desc = "Failed while executing HDBOracleAttributeInsert.insertScalarData()method getConnection part ...";
	...
	String oracleMess = "";
	try {
	    preparedStatement = connectionForInsertScalar.prepareStatement(_query.toString());
	    ...
	    message += " ( attribut = " + _attributeName + " / timestamp = " + _timeSt + " )";
	    final String reason = GlobalConst.INSERT_FAILURE;
	    final String desc = "Failed while executing HDBOracleAttributeInsert.insertScalarData()method insert part ...";
	    throw new ArchivingException(message, reason, ErrSeverity.WARN, desc, this.getClass()
		    .getName(), e);
	} finally {
	    ConnectionCommands.close(preparedStatement);
		...
		if (oracleMess.trim().equalsIgnoreCase("Closed Connection")
			|| oracleMess.trim().equalsIgnoreCase("IO Error: Connection timed out")) {
    ...
    protected void insert_ImageData_RO_DataBase(StringBuffer query, final StringBuffer tableName,
	    final StringBuffer tableFields, final int dim_x, final int dim_y,
	    final Timestamp timeSt, final Double[][] dvalue, final StringBuffer valueStr,
	...
	query = new StringBuffer().append("{call ").append(dbConn.getSchema()).append(
		".ins_im_1val (?, ?, ?, ?, ?)}");
    ...
    protected void insert_SpectrumData_RO_DataBase(StringBuffer query,
	    final StringBuffer tableName, final StringBuffer tableFields, final int dim_x,
	    final Timestamp timeSt, final Double[] dvalue, final StringBuffer valueStr,
	    final String att_name) throws ArchivingException {
	// TODO Auto-generated method stub
	final IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
    ...
    protected void insert_SpectrumData_RW_DataBase(StringBuffer query,
	    final StringBuffer tableName, final StringBuffer tableFields, final int dim_x,
	    final Timestamp timeSt, final Double[] dvalueWrite, final Double[] dvalueRead,
	    final StringBuffer valueWriteStr, final StringBuffer valueReadStr, final String att_name)
