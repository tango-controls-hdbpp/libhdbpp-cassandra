...
public class Archiver {
	 ...
	 */
	public Archiver() {
		this.KOAttributes = new Hashtable<String, ArchivingAttribute>();
	...
	public String getReport() {
		...
		String isLiving = this.isDoDiagnosis() ? "" + this.isLiving()
				: "Not tested";
		buff.append("Is device alive? " + isLiving);
		buff.append(Tools.CRLF);
		buff.append("Status: " + this.getStatus());
		buff.append(Tools.CRLF);
		buff.append("Total load: " + this.getTotalLoad() + ". Scalar load: "
				+ this.getScalarLoad() + ". Spectrum load: "
				+ this.getSpectrumLoad() + ". Image load: "
				+ this.getImageLoad());
		...
		List<ArchivingAttribute> list = new Vector<ArchivingAttribute>();
		list.addAll(this.KOAttributes.values());
		Collections.sort(list, this.comparator);
		Iterator<ArchivingAttribute> it = list.iterator();
	 ...
	 */
	public void buildKOAttributes(Hashtable<String, ArchivingAttribute> _errorAttributes) {
		if (_errorAttributes == null || _errorAttributes.size() == 0) {
			if (_errorAttributes == null) {
				// Tools.trace (
				// "Archiver/buildKOAttributes/_errorAttributes == null" ,
				// Warnable.LOG_LEVEL_DEBUG );
			} else if (_errorAttributes.size() == 0) {
				// Tools.trace (
				// "Archiver/buildKOAttributes/_errorAttributes.size () == 0" ,
				// Warnable.LOG_LEVEL_DEBUG );
			}
		...
		Enumeration<String> keys = _errorAttributes.keys();
		while (keys.hasMoreElements()) {
			...
			// , Warnable.LOG_LEVEL_DEBUG );
			if (attributeArchiver != null
					&& attributeArchiver.equals(this.name)) {
				// Tools.trace (
	 ...
	 *            complete names and which values are ArchivingAttribute objects
	 */
	public void setKOAttributes(Hashtable<String, ArchivingAttribute> attributes) {
