...
public class HdbLifeCycleManager extends DefaultLifeCycleManager {
	...
	protected void startFactories(boolean doRetry) throws DevFailed {
		...
		ChoosingStrategyFactory
				.getImpl(ChoosingStrategyFactory.SCALAR_AND_SPECTRUM_ONLY);

		if (doRetry) {
			// ControllerFactory.getImpl (
			// ControllerFactory.RETRY_EVERY_THIRD_STEP );
			ControllerFactory
					.getImpl(ControllerFactory.RETRY_EVERY_LARGE_AMOUNT_OF_STEPS);
		} else {
			ControllerFactory
					.getImpl(ControllerFactory.DO_ACTION_AT_FULL_CYCLE);
		}

		ModeControllerFactory
				.getImpl(ModeControllerFactory.HDB_BY_LAST_INSERT_COMPARISON);
