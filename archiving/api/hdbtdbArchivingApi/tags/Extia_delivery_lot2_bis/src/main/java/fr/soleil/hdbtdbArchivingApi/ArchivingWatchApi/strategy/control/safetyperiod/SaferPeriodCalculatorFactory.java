package fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.control.safetyperiod;
...
public class SaferPeriodCalculatorFactory {
	 ...
	 */
	public static ISaferPeriodCalculator getImpl(int typeOfImpl)
			throws DevFailed {
		if (SaferPeriodCalculatorFactory.userDefinedDefaultMode == null) {
			switch (typeOfImpl) {
			case ABSOLUTE:
				currentImpl = new AbsoluteSaferPeriodCalculator(
						AbsoluteSaferPeriodCalculator.DEFAULT_AMOUNT,
						AbsoluteSaferPeriodCalculator.DEFAULT_TYPE);
				break;

			case RELATIVE:
				currentImpl = new RelativeSaferPeriodCalculator(
						RelativeSaferPeriodCalculator.DEFAULT_MULTIPLIER);
				break;

			default:
				Tools
						.throwDevFailed(new IllegalArgumentException(
								"Expected either DUMMY_TYPE (1) or SLOW_BASIC_TYPE (2) or FAST_BASIC_TYPE (3) or FAST_BASIC_TYPE_2 (4) or DEBUG_TYPE(5) or FAST_BASIC_TYPE_3(6), got "
										+ typeOfImpl + " instead."));
			}
		} else {
			currentImpl = SaferPeriodCalculatorFactory
					.getImpl(SaferPeriodCalculatorFactory.userDefinedDefaultMode);
		}
	...
	/**
	 * Instantiates and return the ISaferPeriodCalculator singleton of the
	 * Absolute variety
	 * 
	 * @param amount
	 *            The integer amount of time units to add to the period
	 * @param type
	 *            The type of time units to add to the period; must be either
	 *            ADD_SECONDS, ADD_MINUTES, or ADD_HOURS
	 * @return The required implementation
	 * @throws DevFailed
	 */
	public static AbsoluteSaferPeriodCalculator getAbsoluteImpl(int amount,
			int type) throws DevFailed {
	 ...
	 * 
	 * @param multiplier
	 *            The multiplier by which to multiply the period
	 * @return The required implementation
	 * @throws DevFailed
	 */
	public static RelativeSaferPeriodCalculator getRelativeImpl(
			double multiplier) throws DevFailed {
	 ...
	 *            The description of the type of implementation; can be either
	 *            of two types:
	 *            <UL>
	 *            <LI>absolute/[seconds|minutes|hours]/[integer amount];
	 *            ex:absolute/minutes/10 means that saferPeriod=period+10
	 *            minutes
	 *            <LI>relative/[float amount] ex: relative/2.5 means that
	 *            saferPeriod=period*2.5
	 *            </UL>
	 * @return The required implementation
	 */
	public static ISaferPeriodCalculator getImpl(String modeDescription)
			throws DevFailed {
		if (modeDescription == null) {
			return SaferPeriodCalculatorFactory
					.getImpl(SaferPeriodCalculatorFactory.ABSOLUTE);
		}

		StringTokenizer st = new StringTokenizer(modeDescription,
				SaferPeriodCalculatorFactory.MODE_DESCRIPTION_SEPARATOR);
		try {
			int type = -1;
			...
			case ABSOLUTE:
				try {
					...
					int typeOfAddition = AbsoluteSaferPeriodCalculator
							.getTypeOfAddition(typeOfAddition_s);
					int amount = Integer.parseInt(amount_s);

					return getAbsoluteImpl(amount, typeOfAddition);
				} catch (Throwable t) {
					throwIllegalArgumentException("Illegal mode description: "
							+ modeDescription);
				}
			case RELATIVE:
				try {
	...
	static void throwIllegalArgumentException(String invalidValue)
			throws DevFailed {
		IllegalArgumentException iae = new IllegalArgumentException(
				"Illegal mode description: " + invalidValue);
	 ...
	 * 
	 * @param userDefinedDefaultMode
	 *            The userDefinedDefaultMode to set.
	 */
