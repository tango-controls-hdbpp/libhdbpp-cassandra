...
public class DBTools {
	 ...
	 * @throws DevFailed
	 */
	public static DevFailed getAsDevFailed(Throwable exception)
			throws DevFailed {
	...
	public static String getCompleteMessage(DevFailed t) {
		...
		if (errors != null && errors.length != 0) {
			...
			for (int i = 0; i < errors.length; i++) {
				DevError error = errors[i];
				message.append("desc/" + error.desc + "/origin/" + error.origin
						+ "/reason/" + error.reason);
				message.append(DBTools.CRLF);
			}
	...
	public static boolean areTimestampsEqual(long newTime, long oldTime,
			ILogger logger) {
		try {
			if (newTime == oldTime) {
				return true;
			...
			// System.out.println (
			// "CLA/areTimestampsEqual/BEFORE/oldTimeS/"+oldTimeS+"/newTimeS/"+newTimeS+"/"
			// );
			...
			// System.out.println (
			// "CLA/areTimestampsEqual/AFTER/oldTimeS/"+oldTimeS+"/newTimeS/"+newTimeS+"/ret/"+ret+"/"
			// );
			return ret;
		} catch (Exception e) {
			e.printStackTrace();
