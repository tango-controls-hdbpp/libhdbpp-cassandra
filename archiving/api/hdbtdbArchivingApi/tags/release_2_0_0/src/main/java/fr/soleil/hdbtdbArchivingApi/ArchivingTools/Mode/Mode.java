...
public class Mode
{
	 ...
	 * Default constructor
	 *
	 * @see #Mode(fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModePeriode mp, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeAbsolu ma, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeRelatif mr, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeSeuil ms, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeCalcul mc, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeDifference md, fr.soleil.hdbtdbArchivingApi.ArchivingTools.Mode.ModeExterne me ).
	 */
	public Mode()
	{
	}
		...
		while ( index < my_array_size && mode_array[ index ]!=null)
		{
			...
			else if ( mode_array[ index ].equals(Tag.MODE_A) )
			{
				...
				mode_a = new ModeAbsolu(Integer.parseInt(mode_array[ index + 1 ]) ,
				                        Double.parseDouble(mode_array[ index + 2 ]) , Double.parseDouble(mode_array[ index + 3 ]), Boolean.parseBoolean(mode_array[ index + 4 ]));
				index = index + 5;
			}
	...
	public String[] toArray()
	{
		...
		if ( this.getModeA() != null )
		{
			...
			index++;
			mode_array[ index ] = Boolean.toString(this.getModeA().isSlow_drift());
			index++;
		...
		if ( this.getModeR() != null )
		{
			...
			index++;
			mode_array[ index ] = Boolean.toString(this.getModeR().isSlow_drift());
			index++;
		}
