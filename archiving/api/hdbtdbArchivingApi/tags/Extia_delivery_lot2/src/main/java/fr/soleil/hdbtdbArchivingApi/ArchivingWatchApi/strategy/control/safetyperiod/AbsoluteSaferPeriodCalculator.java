...
/**
 * An implementation that computes the safer period by adding a constant amount
 * of time to the theoretical archiving period.
 * 
 * @author CLAISSE
 */
public class AbsoluteSaferPeriodCalculator implements ISaferPeriodCalculator {
	...
	AbsoluteSaferPeriodCalculator(int _amount, int _type) throws DevFailed {
		...
		default:
			Tools.throwDevFailed(new IllegalArgumentException(
					"Expected ADD_SECONDS(), ADD_MINUTES(2), ADD_HOURS(3) got "
							+ _type + "instead."));
		}
	 ...
	 * 
	 * @see
	 * fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.control.safetyperiod
	 * .ISaferPeriodCalculator#getSaferPeriod(int)
	 */
	 ...
	 * @see
	 * fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.control.safetyperiod
	 * .ISaferPeriodCalculator#getCalculationDescription()
	 */
	private String getCalculationDescription() {
		...
		String ret = "Safer period = theoric period + " + this.amount + " "
				+ label;
		return ret;
	}
	...
	static int getTypeOfAddition(String typeOfAddition_s) throws DevFailed {
		if (AbsoluteSaferPeriodCalculator.ADD_HOURS_LABEL
				.equals(typeOfAddition_s)) {
			return AbsoluteSaferPeriodCalculator.ADD_HOURS;
		} else if (AbsoluteSaferPeriodCalculator.ADD_MINUTES_LABEL
				.equals(typeOfAddition_s)) {
			return AbsoluteSaferPeriodCalculator.ADD_MINUTES;
		} else if (AbsoluteSaferPeriodCalculator.ADD_SECONDS_LABEL
				.equals(typeOfAddition_s)) {
			return AbsoluteSaferPeriodCalculator.ADD_SECONDS;
		} else {
			SaferPeriodCalculatorFactory
					.throwIllegalArgumentException(typeOfAddition_s);
		}
		return -1;
	}
