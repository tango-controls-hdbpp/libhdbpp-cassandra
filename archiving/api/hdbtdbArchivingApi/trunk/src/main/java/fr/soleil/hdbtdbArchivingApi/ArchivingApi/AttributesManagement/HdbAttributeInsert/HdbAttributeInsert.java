...
public abstract class HdbAttributeInsert implements IHdbAttributeInsert {
    ...
    public HdbAttributeInsert(final int type, final IAdtAptAttributes at) {
    ...
    abstract protected void insert_SpectrumData_RO_DataBase(StringBuffer query,
	    StringBuffer tableName, StringBuffer tableFields, int dim_x, Timestamp timeSt,
	    Double[] dvalue, StringBuffer valueStr, String att_name) throws ArchivingException;

    abstract protected void insert_SpectrumData_RW_DataBase(StringBuffer query,
	    StringBuffer tableName, StringBuffer tableFields, int dim_x, Timestamp timeSt,
	    Double[] dvalueWrite, Double[] dvalueRead, StringBuffer valueWriteStr,
	    StringBuffer valueReadStr, String att_name) throws ArchivingException;

    abstract protected void insert_ImageData_RO_DataBase(StringBuffer query,
	    StringBuffer tableName, StringBuffer tableFields, int dim_x, int dim_y,
	    Timestamp timeSt, Double[][] dvalue, StringBuffer valueStr, String att_name)
	...
	try {
	    ...
	    final int _writable = scalarEvent.getWritable();
	    ...
	    _tableFields = _writable == AttrWriteType._READ || _writable == AttrWriteType._WRITE ? new StringBuffer()
		    ...
		    : new StringBuffer().append(ConfigConst.TAB_SCALAR_RW[0]).append(", ").append(
			    ConfigConst.TAB_SCALAR_RW[1]).append(", ").append(
			    ConfigConst.TAB_SCALAR_RW[2]);

	    _tableName = new StringBuffer().append(dbConn.getSchema()).append(".").append(
		    dbUtils.getTableName(_attributeName));

	    _query = new StringBuffer();
	    _query.append("INSERT INTO ").append(_tableName).append(" (").append(_tableFields)
		    .append(")").append(" VALUES").append(" (").append(_selectFields).append(")");
	    final Timestamp _timeSt = new Timestamp(scalarEvent.getTimeStamp());
	    final Object _value = scalarEvent.getValue();
	    final int _dataType = scalarEvent.getData_type();
	    insertScalarDataInDB(_query, _attributeName, _writable, _timeSt, _value, _dataType);
    ...
    protected void insertScalarDataInDB(final StringBuffer _query, final String _attributeName,
	...
	try {
	    ...
	    logger.trace(ILogger.LEVEL_ERROR, "SqlException has been raised insertScalarData()");
	    String message = "";
	    ...
	    message += " ( attribut = " + _attributeName + " / timestamp = " + _timeSt + " )";
	    final String reason = GlobalConst.INSERT_FAILURE;
	    final String desc = "Failed while executing HdbAttributeInsert.insertScalarDataInDB() method...";
     ...
     *            value and the value.
     */
    public void insert_SpectrumData_RO(final SpectrumEvent_RO hdbSpectrumEvent_RO)
	    throws ArchivingException {
	final IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
	final IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
	if (dbConn == null) {
	...
	final String att_name = hdbSpectrumEvent_RO.getAttribute_complete_name().trim();
	final StringBuffer tableName = new StringBuffer().append(dbConn.getSchema()).append(".")
		.append(dbUtils.getTableName(att_name));
	final long time = hdbSpectrumEvent_RO.getTimeStamp();
	final Timestamp timeSt = new Timestamp(time);
	final int dim_x = hdbSpectrumEvent_RO.getDim_x();
	int data_type = -1;
	Double[] dvalue = null;
	Float[] fvalue = null;
	Integer[] lvalue = null;
	...
	// Create and execute the SQL query string
	// Build the query string
	final StringBuffer tableFields = new StringBuffer().append(ConfigConst.TAB_SPECTRUM_RO[0])
		.append(", ").append(ConfigConst.TAB_SPECTRUM_RO[1]).append(", ").append(
	...
	case TangoConst.Tango_DEV_STRING:
		...
		for (int i = 0; i < stvalue.length - 1; i++) {
		    valueStr.append(StringFormater.formatStringToWrite(stvalue[i])).append(
			    GlobalConst.CLOB_SEPARATOR).append(" ");
		}
		if (stvalue.length > 0) {
		    valueStr
			    .append(StringFormater.formatStringToWrite(stvalue[stvalue.length - 1]));
		}
	    }
	    break;
	}
	insert_SpectrumData_RO_DataBase(query, tableName, tableFields, dim_x, timeSt, dvalue,
		valueStr, att_name);
     ...
     * @throws ArchivingException
     */
    public void insert_SpectrumData_RW(final SpectrumEvent_RW hdbSpectrumEvent_RW)
	    throws ArchivingException {
	final IDBConnection dbConn = ConnectionFactory.getInstance(arch_type);
	final IDbUtils dbUtils = DbUtilsFactory.getInstance(arch_type);
	if (dbConn == null) {
	...
	final String att_name = hdbSpectrumEvent_RW.getAttribute_complete_name().trim();
	final StringBuffer tableName = new StringBuffer().append(dbConn.getSchema()).append(".")
		.append(dbUtils.getTableName(att_name));
	final long time = hdbSpectrumEvent_RW.getTimeStamp();
	final Timestamp timeSt = new Timestamp(time);
	final int dim_x = hdbSpectrumEvent_RW.getDim_x();
	...
	// Create and execute the SQL query string
	// Build the query string
	final StringBuffer tableFields = new StringBuffer().append(ConfigConst.TAB_SPECTRUM_RW[0])
		.append(", ").append(ConfigConst.TAB_SPECTRUM_RW[1]).append(", ").append(
			ConfigConst.TAB_SPECTRUM_RW[2]).append(", ").append(
			ConfigConst.TAB_SPECTRUM_RW[3]);
	insert_SpectrumData_RW_DataBase(query, tableName, tableFields, dim_x, timeSt, dvalueWrite,
		dvalueRead, valueWriteStr, valueReadStr, att_name);
     ...
     * @throws ArchivingException
     */
    public void insert_ImageData_RO(final ImageEvent_RO imageEvent_RO) throws SQLException,
	...
	final String att_name = imageEvent_RO.getAttribute_complete_name().trim();
	final StringBuffer tableName = new StringBuffer().append(dbConn.getSchema()).append(".")
		.append(dbUtils.getTableName(att_name));
	final long time = imageEvent_RO.getTimeStamp();
	final Timestamp timeSt = new Timestamp(time);

	final int dim_x = imageEvent_RO.getDim_x();
	final int dim_y = imageEvent_RO.getDim_y();
	...
	// Create and execute the SQL query string
	// Build the query string
	final StringBuffer tableFields = new StringBuffer().append(ConfigConst.TAB_IMAGE_RO[0])
		.append(", ").append(ConfigConst.TAB_IMAGE_RO[1]).append(", ").append(
	...
	insert_ImageData_RO_DataBase(query, tableName, tableFields, dim_x, dim_y, timeSt, dvalue,
		valueStr, att_name);
	// Close the connection with the database
	// if ( dbConn.isAutoConnect() )
	// {
