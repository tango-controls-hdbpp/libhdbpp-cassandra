...
public class ArchivingMessConfig {
	...
	private Hashtable<String, AttributeLightMode> attribute_list = null; // This list contains
												// AttributeLightMode objects
												// types

	private Hashtable<String, String> attributeToDedicatedArchiverHasthable;
	...
	private ArchivingMessConfig() {
		attribute_list = new Hashtable<String, AttributeLightMode>();
		attributeToDedicatedArchiverHasthable = new Hashtable<String, String>();
	}
	 ...
	 * @return
	 */
	static public ArchivingMessConfig creationWithFullInformation(
			String[] archivingMessConfigArray) {
	 ...
	 * @return
	 */
	static public ArchivingMessConfig creationWithoutFullInformation(
			String[] archivingMessConfigArray) {
	 ...
	 */
	private ArchivingMessConfig(String[] archivingMessConfigArray,
			boolean fullInformation) {
		/*
		 * for (int i = 0 ; i < archivingMessConfigArray.length ; i ++ ) {
		 * System.out.println (
		 * "ArchivingMessConfig/new/i|"+i+"|archivingMessConfigArray[i]|"
		 * +archivingMessConfigArray[i] ); }
		 */
		...
		if (fullInformation)
			addInAttListWithFullInformation(archivingMessConfigArray);
		else
			addInAttListWithoutFullInformation(archivingMessConfigArray);
	}

	private void addInAttListWithFullInformation(
			String[] archivingMessConfigArray) {
		/*
		...
		for (index = 1; index < archivingMessConfigArray.length;) {
			...
			// );
			int attributeLightModeSize = Integer
					.parseInt(archivingMessConfigArray[index]);
			// System.out.println (
			// "ArchivingMessConfig/new/attributeLightModeSize|"+attributeLightModeSize+"|"
			// );
			String[] attributeLightModeArray = new String[attributeLightModeSize];
			index++;
			// System.out.println ( "ArchivingMessConfig/new/index 1|"+index+"|"
			// );
			System.arraycopy(archivingMessConfigArray, index,
					attributeLightModeArray, 0, attributeLightModeSize);

			add(AttributeLightMode
					.creationWithFullInformation(attributeLightModeArray));

			index = index + attributeLightModeSize;
		}
	}

	private void addInAttListWithoutFullInformation(
			String[] archivingMessConfigArray) {
		/*
		 ...
		 * ortrue/false,2,"tango/tangotest/spjz_1/double_spectrum_ro",
		 * "tango/tangotest/spjz_1/double_scalar_ro",MODE_P,10000
		 *
		 * true/false,1,"tango/tangotest/spjz_2/double_spectrum_ro",MODE_P,2000,
		 * TDB_SPEC,300000,21600000,MODE_D,2000,TDB_SPEC,300000,21600000 or
		 * true/
		...
		for (index = 0; index < numberOfAttributes; index++) {
			...
			System.arraycopy(archivingMessConfigArray,
					(2 + numberOfAttributes), attributeLightModeArray, 1,
					modeSize);
			// attribute full_name copy
			attributeLightModeArray[0] = archivingMessConfigArray[index + 2];

			add(AttributeLightMode
					.creationWithoutFullInformation(attributeLightModeArray));
	 ...
	 */
	public Enumeration<String> getAttributeListKeys() {
		Enumeration<String> myEnumeration = attribute_list.keys();
	...
	public Collection<AttributeLightMode> getAttributeListValues() {
		Collection<AttributeLightMode> collection = attribute_list.values();
		return collection;
	}
	...
	public AttributeLightMode getAttribute(String attributeName) {
		AttributeLightMode attributeLightMode = (AttributeLightMode) attribute_list
				.get(attributeName);
...
//			AttributeLightMode attributeLightMode = (AttributeLightMode) attributeList
//					.get(i);
//			attribute_list.put(attributeLightMode.getAttribute_complete_name(),
//					attributeLightMode);
//		}
//	}
	...
	public void removeAll() {
		Enumeration<String> attributeKeys = attribute_list.keys();
	...
	public String[] toArray() {
		...
		Hashtable<String, String[]> hashtableArr = new Hashtable<String, String[]>();
		...
		while (attributeKeys.hasMoreElements()) {
			...
			// index initial + index spécifiant du tableau la taille
			// attributeLightModeArray + place du tableau
			archivingMessConfigSize = archivingMessConfigSize + 1
					+ attributeLightModeArray.length;
		...
		while (attributeKeys.hasMoreElements()) {
			String attributeKey = (String) attributeKeys.nextElement();
			String[] attributeLightModeArray = (String[]) hashtableArr
					.get(attributeKey);
			archivingMessConfigArray[i] = Integer
					.toString(attributeLightModeArray.length);
			i++;
			System
					.arraycopy(attributeLightModeArray, 0,
							archivingMessConfigArray, i,
							attributeLightModeArray.length);
	...
	public String toString() {
		...
		Enumeration<String> my_attribute_list = this.getAttributeListKeys();
		int i = 0;
		while (my_attribute_list.hasMoreElements()) {
			String my_attribute_name = (String) my_attribute_list.nextElement();
			AttributeLightMode attributeLightMode = (AttributeLightMode) attribute_list
					.get(my_attribute_name);
			sb.append("[").append(i++).append("] : \r\n").append(
					attributeLightMode.toString()).append("\r\n");
		}
		return sb.toString();
	}

	public Hashtable<String, String> getAttributeToDedicatedArchiverHasthable() {
		return this.attributeToDedicatedArchiverHasthable;
	}

	public void setDeviceInChargeForAttribute(
			AttributeLightMode attributeLightMode, String nextDedicatedArchiver) {
		boolean hasDedicatedArchiver = nextDedicatedArchiver != null
				&& !nextDedicatedArchiver.equals("");
		if (hasDedicatedArchiver) {
			this.attributeToDedicatedArchiverHasthable.put(attributeLightMode
					.getAttribute_complete_name(), nextDedicatedArchiver);
		}
	}

	public void filter(boolean isDedicated, String[] reservedAttributes,
			ILogger logger) {

		if (!isDedicated) {
			logger.trace(ILogger.LEVEL_DEBUG,
					"ArchivingMessConfig/filter/!isDedicated");
			return;
		}
		if (reservedAttributes == null || reservedAttributes.length == 0) {
			logger
					.trace(
							ILogger.LEVEL_DEBUG,
							"ArchivingMessConfig/filter/reservedAttributes == null || reservedAttributes.length == 0");
			return;
		}

		int numberOfReserved = reservedAttributes.length;
		Vector<String> reservedAttributesList = new Vector<String>(numberOfReserved);
		for (int i = 0; i < numberOfReserved; i++) {
			reservedAttributesList.add(reservedAttributes[i]);
			// System.out.println (
			// "CLA/filter/reservedAttributes [ i ]|"+reservedAttributes [ i ]
			// );
		}

		Enumeration<String> myEnumeration = this.attribute_list.keys();
		while (myEnumeration.hasMoreElements()) {
			String next = (String) myEnumeration.nextElement();
			// System.out.println ( "CLA/filter/next|"+next);
			logger.trace(ILogger.LEVEL_DEBUG,
					"ArchivingMessConfig/filter/next|" + next + "|");
			if (!reservedAttributesList.contains(next)) {
				String msg = "The following attribute is reserved |"
						+ next
						+ "| on a dedicated archiver. No archiving started on this archiver for this attribute.";
				logger.trace(ILogger.LEVEL_ERROR, msg);
				this.remove(next);
			} else {
				logger.trace(ILogger.LEVEL_DEBUG,
						"ArchivingMessConfig/filter/next|" + next + "| OK");
			}
		}
	}
}
