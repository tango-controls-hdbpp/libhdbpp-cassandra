//+======================================================================
//$Source: /cvsroot/tango-cs/tango/api/java/fr/soleil/TangoArchiving/ArchivingWatchApi/lifecycle/DefaultLifeCycleManager.java,v $
//
...
//
//-======================================================================
package fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.lifecycle;
...
import fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.control.global.ControllerFactory;
import fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.strategy.control.global.IController;
 ...
 * @author CLAISSE
 */
public abstract class DefaultLifeCycleManager extends Thread implements
		LifeCycleManager {
	...
	private static boolean previousWasInAlarmState = false;
	...
	public synchronized void applicationWillStart(boolean doRetry) {
		...
		try {
			...
			previousWasInAlarmState = false;
		} catch (DevFailed t) {
			Tools.trace("Error during applicationWillStart: ", t,
					Warnable.LOG_LEVEL_FATAL);
			t.printStackTrace();
			this.warnWatcherFault(t);
		}

		// Tools.trace ( ".....APPLICATION INITIALIZED" ,
	...
	public void process(long macroPeriod, boolean doArchiverDiagnosis,
		...
		while (true) {
			try {
				...
				int sleepDurationInSeconds = (int) Math
						.round(sleepDuration / 1000.0);
				Tools.trace("stepDuration/" + stepDuration + "/stepPeriod/"
						+ stepPeriod + "/sleepDuration/" + sleepDuration,
						Warnable.LOG_LEVEL_INFO);

				Timestamp nextStepTime = new Timestamp(afterStep
						+ sleepDuration);

				Tools.trace(".....ENDING CONTROL STEP. Controlled " + stepSize
						+ " attributes in " + stepDuration + " milliseconds.",
						Warnable.LOG_LEVEL_INFO);

				if (delayManager.isNewCycle()) {
					Tools.trace(".....ENDING CONTROL CYCLE. Controlled "
							+ finalSize + " attributes in "
							+ cycleComputationTime + " milliseconds.",
							Warnable.LOG_LEVEL_INFO);
					Tools.trace("", Warnable.LOG_LEVEL_INFO);
				}
				Tools.trace(".....WAITING FOR " + sleepDuration
						+ " milliseconds. Next control step: " + nextStepTime,
						Warnable.LOG_LEVEL_INFO);

				this.doSleep(sleepDuration);
			} catch (DevFailed t) {
				t.printStackTrace();
				Tools.trace("Error during process: ", t,
						Warnable.LOG_LEVEL_FATAL);
				this.warnWatcherFault(t);
			}
		}
	...
	public void processStep(long macroPeriod, boolean doArchiverDiagnosis,
			short numberOfKOForAlarmState) throws DevFailed {
		IDelayManager delayManager = DelayManagerFactory.getCurrentImpl();
		Hashtable<String, ModeData> archivedAttributes = delayManager
				.reloadArchivingAttributesIfNecessary();
		int numberOfArchivingAttributes = archivedAttributes == null ? 0
				: archivedAttributes.size();

		// Filters out unwanted attributes and positions the cursor to the next
		// position in the attributes list
		Hashtable stepArchivedAttributes = delayManager
				.chooseAttributesSubsetForCurrentStep(archivedAttributes);
		...
		boolean isFullCycle = delayManager.addControlResult(stepControlResult,
				archivedAttributes.keySet());

		ControlResult currentResult = delayManager
				.getControlResult(IDelayManager.READ_ROLLOVER);

		if (currentResult != null) {
			// CLA 14/12/06
			currentResult
					.setNumberOfArchivingAttributes(numberOfArchivingAttributes);
			...
			if (currentResult.hasErrors()) {
				if (currentResult.isComplete())// otherwise it isn't a
												// "complete" result
				{
					delayManager.setLatestBadCompleteResult(currentResult
							.cloneControlResult());
				}
				// System.out.println("=====>SPJZ ======>Number of Ko ATT = " +
				// currentResult.getNumberOfKoAttributes());
				int numberOfKo = currentResult.getNumberOfKoAttributes();

				if (numberOfKo >= numberOfKOForAlarmState
						&& !previousWasInAlarmState) {
					this.warnWatcherAlarm();
					previousWasInAlarmState = true;
				} else if (numberOfKo < numberOfKOForAlarmState
						&& previousWasInAlarmState) {
					this.warnWatcherOn();
					previousWasInAlarmState = false;
				}
	...
	public void run() {
		try {
			...
			boolean doArchiverDiagnosis = ArchivingWatch
					.isDoArchiverDiagnosis();
			boolean doRetry = ArchivingWatch.isDoRetry();
			boolean doStartOnInitDevice = ArchivingWatch
					.isDoStartOnInitDevice();
			...
			this.process(macroPeriod, doArchiverDiagnosis, ArchivingWatch
					.getMinNumberOfKoForAlarmState());
		} catch (Throwable t) {
			t.printStackTrace();
		}
	}
