...
public class Archiver {
	...
	public Color getAssociationColor(String attributeName) {
		...
		/*
		 * case Archiver.NON_DEDICATED_ARCHIVER_RIGHT: color = Color.BLACK;
		 * break;
		 * 
		 * case Archiver.NON_DEDICATED_ARCHIVER_WRONG: color = Color.ORANGE;
		 * break;
		 * 
	...
	private int hasReservedAttribute(String attributeName) {
		...
		if (this.reservedAttributes == null
				|| this.reservedAttributes.length == 0) {
			return nonDedicated(attributeName);
		}
		...
		int ret = hasAttribute ? DEDICATED_ARCHIVER_RIGHT
				: DEDICATED_ARCHIVER_WRONG;
		return ret;
	}

	private int nonDedicated(String attributeName) {
		Hashtable attributesToDedicatedArchiver = this.isHistoric ? attributesToHdbDedicatedArchiver
	 ...
	 */
	public static void setAttributesToDedicatedArchiver(boolean historic,
			Hashtable _deviceDefinedAttributesToDedicatedArchiver) {
		if (historic) {
			// System.out.println (
			// "CLA/Archiver/setAttributesToDedicatedArchiver/0" );
			attributesToHdbDedicatedArchiver = _deviceDefinedAttributesToDedicatedArchiver;
		} else {
			// System.out.println (
			// "CLA/Archiver/setAttributesToDedicatedArchiver/1" );
			attributesToTdbDedicatedArchiver = _deviceDefinedAttributesToDedicatedArchiver;
		}
	}

	private static Archiver getArchiver(String my_archiver, boolean _historic)
		...
		try {
			...
			DbDatum isDedicatedRawValue = archiverProxy
					.get_property(m_isDedicated);
			boolean isDedicated = false;

			DbDatum reservedAttributesRawValue = archiverProxy
					.get_property(m_reservedAttributes);
			...
			try {
				reservedAttributes = reservedAttributesRawValue
						.extractStringArray();
				/*
				 * int l = reservedAttributes == null ? 0 :
				 * reservedAttributes.length; for ( int i = 0 ; i < l ; i ++ ) {
				 * //System.out.println (
				 * "CLA/Archiver/getArchiver/i|"+i+"|reservedAttributes|"
				 * +reservedAttributes[i]+"|" ); }
				 */
			} catch (Exception e) {
				// do nothing, no such attribute or empty value
			}
	...
	/*
	 * public void trace () { if ( attributesToHdbDedicatedArchiver == null ) {
	 * return; } Enumeration enumeration = attributesToHdbDedicatedArchiver.keys
	 * (); while ( enumeration.hasMoreElements () ) { String nextAttribute =
	 * (String) enumeration.nextElement (); Archiver nextArchiver = (Archiver)
	 * attributesToHdbDedicatedArchiver.get ( nextAttribute );
	 * 
	 * System.out.println (
	 * "Archiver/trace/nextAttribute|"+nextAttribute+"|nextArchiver|"
	 * +nextArchiver.getName()+"|" ); }
	 * 
	 * }
	 */

	public static Archiver findArchiver(String name, boolean _historic)
			throws ArchivingException {
		if (name == null) {
			return null;
		}
