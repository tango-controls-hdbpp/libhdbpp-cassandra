//+======================================================================
//$Source: /cvsroot/tango-cs/tango/api/java/fr/soleil/TangoArchiving/ArchivingWatchApi/lifecycle/LifeCycleManager.java,v $
//
 ...
 * </UL>
 * Extends Warner so that the APIs can warn the associated device
 * (ArchivingWatcher) and use its Tango log methods.
 * 
 * @author CLAISSE
 */
public interface LifeCycleManager extends Warner {
	/**
	 * Called when the application will start. Connects to the database, loads
	 * resources, ..
	 * 
	 * @param doRetry
	 *            True if the watcher has to call a retry command on the KO
	 *            attributes it detects.
	 */
	public void applicationWillStart(boolean doRetry);

	/**
	 * Called when the application will stop. Closes the connection to the
	 * database, frees resources, ..
	 * 
	 * @param endParameters
	 *            Key/Value couples of parameters can be passed to this method.
	 *            Not used so far.
	 */
	public void applicationWillClose(Hashtable endParameters);

	/**
	 * Describes the infinite repetition of control cycles through time. A cycle
	 * is made of smaller atomic steps.
	 * 
	 * @param macroPeriod
	 *            The length of a full control cycle.
	 * @param doArchiverDiagnosis
	 *            True if the watcher has to investigate KO archivers
	 */
	...
	/**
	 * Returns a Runnable representation of this life cycle. Useful in device
