...
public class HdbTdbDbData extends DbData {
	...
	private static ArchivingException generateException(String cause,
			int cause_value, String name) {
		...
		case AttrDataFormat._SPECTRUM:
			...
			case AttrWriteType._WRITE:
				for (int i = 0; i < data_size; i++) {
					...
					// System.out.println (
					// "CLA/DbData/setData/this.getMax_x ()/"+this.getMax_x
					// ()+"/event.getDim_x()/"+event.getDim_x() );
					// added CLA 30/01/06
					...
					default:
						throw generateException(
								GlobalConst.DATA_TYPE_EXCEPTION, data_type,
								name);
					}
				}
			...
			case AttrWriteType._READ_WRITE:
				...
				break;
			default:
				throw generateException(GlobalConst.DATA_WRITABLE_EXCEPTION,
						writable, name);
			}
			break;
				...
				for (int i = 0; i < data_size; i++) {
					...
					case TangoConst.Tango_DEV_STATE:
						...
						if (event.getScalarValue() == null) {
							...
							ival[0] = new Integer((event.getScalarValue())
									.intValue());
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
				...
				for (int i = 0; i < data_size; i++) {
					...
					case TangoConst.Tango_DEV_STATE:
						...
						if (event.getScalarValueRW()[0] == null) {
							...
							ival[0] = new Integer(event.getScalarValueRW()[0]
									.intValue());
						}
						if (event.getScalarValueRW()[1] == null) {
							ival[1] = null;
						} else {
							ival[1] = new Integer(event.getScalarValueRW()[1]
									.intValue());
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
			...
			case AttrWriteType._READ:
				for (int i = 0; i < data_size; i++) {
					...
					case TangoConst.Tango_DEV_USHORT:
						...
						if (tempsval == null || tempsval.length == 0
								|| tempsval[0].length == 0) {
							sval = new Short[0];
						} else {
							sval = new Short[tempsval.length
									* tempsval[0].length];
							int svalindex = 0;
					...
					case TangoConst.Tango_DEV_ULONG:
						...
						if (tempival == null || tempival.length == 0
								|| tempival[0].length == 0) {
							ival = new Integer[0];
						} else {
							ival = new Integer[tempival.length
									* tempival[0].length];
							int ivalindex = 0;
					...
					case TangoConst.Tango_DEV_FLOAT:
						...
						if (tempfval == null || tempfval.length == 0
								|| tempfval[0].length == 0) {
							fval = new Float[0];
						} else {
							fval = new Float[tempfval.length
									* tempfval[0].length];
							int fvalindex = 0;
					...
					case TangoConst.Tango_DEV_BOOLEAN:
						...
						if (tempbval == null || tempbval.length == 0
								|| tempbval[0].length == 0) {
							bval = new Boolean[0];
						} else {
							bval = new Boolean[tempbval.length
									* tempbval[0].length];
							int bvalindex = 0;
					...
					case TangoConst.Tango_DEV_DOUBLE:
						...
						if (tempdval == null || tempdval.length == 0
								|| tempdval[0].length == 0) {
							dval = new Double[0];
						} else {
							dval = new Double[tempdval.length
									* tempdval[0].length];
							int dvalindex = 0;
					...
					case TangoConst.Tango_DEV_STRING:
						...
						if (tempstval == null || tempstval.length == 0
								|| tempstval[0].length == 0) {
							stval = new String[0];
						} else {
							stval = new String[tempstval.length
									* tempstval[0].length];
							int stvalindex = 0;
	...
	public void setData(DevVarDoubleStringArray devVarDoubleStringArray)
			throws ArchivingException {
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (data_type == TangoConst.Tango_DEV_STRING) {
				...
				time = new Long(
						DateUtil
								.stringToMilli((String) devVarDoubleStringArray.svalue[i]));
			}
			Boolean[] bval;
			Double[] dval;
			Float[] fval;
			...
			case AttrDataFormat._SCALAR:
				...
				case AttrWriteType._READ:
					...
					case TangoConst.Tango_DEV_USHORT:
						...
						if (Double.isNaN(devVarDoubleStringArray.dvalue[i])) {
							...
							sval[0] = new Short(
									(short) devVarDoubleStringArray.dvalue[i]);
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
						data[i].time = time;
					...
					case TangoConst.Tango_DEV_ULONG:
						...
						if (Double.isNaN(devVarDoubleStringArray.dvalue[i])) {
							...
							ival[0] = new Integer(
									(int) devVarDoubleStringArray.dvalue[i]);
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
						data[i].time = time;
					...
					case TangoConst.Tango_DEV_DOUBLE:
						...
						if (Double.isNaN(devVarDoubleStringArray.dvalue[i])) {
							long l = Double
									.doubleToRawLongBits(devVarDoubleStringArray.dvalue[i]);
							if (l == Double
									.doubleToRawLongBits(GlobalConst.NAN_FOR_NULL)) {
								dval[0] = null;
							} else {
								dval[0] = new Double(
					...
					case TangoConst.Tango_DEV_FLOAT:
						...
						if (Double.isNaN(devVarDoubleStringArray.dvalue[i])) {
							...
							fval[0] = new Float(
									(float) devVarDoubleStringArray.dvalue[i]);
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
						data[i].time = time;
					...
					case TangoConst.Tango_DEV_BOOLEAN:
						...
						if (Double.isNaN(devVarDoubleStringArray.dvalue[i])) {
							...
							bval[0] = new Boolean(
									(devVarDoubleStringArray.dvalue[i] == 0) ? false
											: true);
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
					...
					case TangoConst.Tango_DEV_STRING:
						stval = new String[1];
						if (devVarDoubleStringArray.svalue[i] == null
								|| "null"
										.equals(devVarDoubleStringArray.svalue[i])) {
							stval[0] = null;
						} else {
				...
				case AttrWriteType._READ_WITH_WRITE:
					...
					case TangoConst.Tango_DEV_STRING:
						...
						if (devVarDoubleStringArray.svalue[i + data_size] == null
								|| "null"
										.equals(devVarDoubleStringArray.svalue[i
												+ data_size])) {
							stval[1] = null;
						} else {
							stval[1] = devVarDoubleStringArray.svalue[i
									+ data_size];
						}
						data[i] = new NullableTimedData();
						data[i].data_type = data_type;
				...
				default:
					throw generateException(
							GlobalConst.DATA_WRITABLE_EXCEPTION, writable, name);
				}
				break;
			case AttrDataFormat._SPECTRUM:
			case AttrDataFormat._IMAGE:
