...
public class TdbModeControllerByRecordCount extends TdbModeControllerAdapter {
	...
	protected int controlPeriodicMode(ModePeriode modeP, TdbSpec spec,
		...
		if (modeP == null) {
			Tools
					.trace(
							"TdbModeControllerByRecordCount/controlPeriodicMode/modeP == null!/for attribute|"
		...
		int saferPeriod = saferPeriodCalculator.getSaferPeriod(period
				+ exportPeriod);
		// Tools.trace ( "TdbModeControllerByRecordCount/period|" + period +
		// "|exportPeriod|" + exportPeriod + "|saferPeriod|" + saferPeriod ,
		...
		int recordCount;
		try {
			recordCount = attributesReader.getRecordCount(completeName,
					saferPeriod);
		} catch (DevFailed e) {
			Tools
					.trace(
							"FastMode3Controller/controlPeriodicMode/failed calling getRecordCount for attribute|"
									+ completeName + "|", e,
		...
		if (recordCount > 0) {
			...
			/*
			 * try { if ( attributesReader.isLastValueNull ( completeName ) ) {
			 * return IModeController.CONTROL_OK_BUT_VALUE_IS_NULL; } else {
			 * return IModeController.CONTROL_OK; }
			 * 
			 * } catch ( DevFailed e ) { Tools.trace (
			 * "FastMode3Controller/controlPeriodicMode/failed calling isLastValueNull for attribute|"
			 * +completeName+"|" , e , Warnable.LOG_LEVEL_WARN ); return
			 * IModeController.CONTROL_OK_BUT_VALUE_MIGHT_BE_NULL; }
			 */
		} else {
