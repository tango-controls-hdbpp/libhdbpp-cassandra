...
public class HDBOracleAttributeInsert extends HdbAttributeInsert /*
	...
	private short _commitInCounter = DEFAULT_COMMIT_COUNTER;
	...
	public HDBOracleAttributeInsert(int type, IAdtAptAttributes at) {
		super(type, at);
		try {
			_commitInCounter = GetConf.readShortInDB("HdbArchiver",
					GetConf.COMMIT_PERIOD_IN_MINUTE, DEFAULT_COMMIT_COUNTER);
			// this.logger.trace(ILogger.LEVEL_INFO, "CommitPeriodInMinute = "
			// + _commitInCounter);
		} catch (ArchivingException e) {
			_commitInCounter = DEFAULT_COMMIT_COUNTER;
	 ...
	 * System.out.println("Harvesting connection ..."); // Boolean result =
	 * false;
	 *
	 * if (connectionHarvestForScalar != null) {
	 *
	 * try { // System.out.println("Commiting connection ...");
	 * this.logger.trace(ILogger.LEVEL_INFO, "Commiting connection ...");
	 * connectionHarvestForScalar.commit();
	 * this.logger.trace(ILogger.LEVEL_INFO,
	 * "Connection commited successfully."); } catch (SQLException e) {
	 * this.logger.trace(ILogger.LEVEL_ERROR,
	 * "HARVEST : Error occured while commiting connection." + e); }
	 *
	 * try { this.logger.trace(ILogger.LEVEL_INFO, "Closing connection ...");
	 * connectionHarvestForScalar.close(); this.logger.trace(ILogger.LEVEL_INFO,
	 * "Connection closed successfully."); } catch (SQLException e) {
	 * this.logger .trace(ILogger.LEVEL_ERROR,
	 * "HARVEST :Error occured while closing connection. " + e); return false; }
	 *
	 * // Reset connection connectionHarvestForScalar = null; } return true; }
	 */
	protected synchronized void insertScalarDataInDB(StringBuffer _query,
		...
		if (_insertionCounter >= _commitInCounter) {
			...
			if (connectionHarvestForScalar != null) {
				...
				dbConn.closeConnection(connectionHarvestForScalar);
				this.logger.trace(ILogger.LEVEL_INFO,
		...
		/*
		 * if (connectionHarvestForScalar == null ||
		 * connectionHarvestForScalar.isClosed()) { // connection = null; // in
		 * case of isClosed connectionHarvestForScalar = dbConn.getConnection();
		 * connectionHarvestForScalar.setAutoCommit(false);
		 *
		 * // Callback pour commiter et fermer la connexion lorsque le // pool
		 * va la récupérera (harvest) ((HarvestableConnection)
		 * connectionHarvestForScalar)
		 * .registerConnectionHarvestingCallback(this);
		 * this.logger.trace(ILogger.LEVEL_INFO, "Get New connection for att = "
		 * + _attributeName + " at time " + _timeSt); } // Travail avec la
		 * connexion en empêchant le pool de la // récupérer (harvest)
		...
		try {
			...
			/*
			 * if (connectionHarvestForScalar != null) { try {
			 * ((HarvestableConnection) connectionHarvestForScalar)
			 * .setConnectionHarvestable(true); } catch (SQLException e) {
			 * System.err
