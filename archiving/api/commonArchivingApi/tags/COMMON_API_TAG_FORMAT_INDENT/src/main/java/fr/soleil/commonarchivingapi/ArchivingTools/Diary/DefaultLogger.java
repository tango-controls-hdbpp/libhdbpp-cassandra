...
public class DefaultLogger extends AbstractLogger {
		...
		try {
			if (o instanceof String) {
				...
				for (int i = 0; i < stack.length; i++) {
					write(this.writer, "        at " + stack[i].toString(),
							true);
				}
			} else {
				// System.out.println (
	 ...
	 *             8 juil. 2005
	 */
	public static void write(PrintWriter pw, String s, boolean hasNewLine)
			throws Exception {
		if (hasNewLine) {
			pw.println(s);
		} else {
	...
	private boolean needsNewFile(DateHeure timeOfLastLog2, DateHeure now) {
		 ...
		 * +timeOfLastLog+
		 * "/timeOfLastLogToTheMinute/"+timeOfLastLogToTheMinute+"/nowToTheMinute/"
		 * +nowToTheMinute+"/areTheSameMinute/"+areTheSameMinute );
		 * 
		 * return ! areTheSameMinute;
		 */

		String timeOfLastLogToTheDay = timeOfLastLog2.getAnnee() + separator
				+ timeOfLastLog2.getMois() + separator
				+ timeOfLastLog2.getJour();
		String nowToTheDay = now.getAnnee() + separator + now.getMois()
				+ separator + now.getJour();
