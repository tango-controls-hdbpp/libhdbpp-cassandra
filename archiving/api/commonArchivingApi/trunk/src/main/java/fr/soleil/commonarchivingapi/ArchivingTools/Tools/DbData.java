...
public class DbData implements java.io.Serializable {
	...
	final NullableTimedData aNullableTimedData = timedData[timedData.length - 1];
	final Short[] sh_ptr = (Short[]) aNullableTimedData.value;
	...
	for (int i = 0; i < timedData.length; i++) {
		...
		case TangoConst.Tango_DEV_STATE:
			...
			for (int j = 0; j < timedData[i].value.length; j++) {
			    String value = null;
			    if (timedData[i].value[j] != null) {
				value = new String(TangoStateTranslation
