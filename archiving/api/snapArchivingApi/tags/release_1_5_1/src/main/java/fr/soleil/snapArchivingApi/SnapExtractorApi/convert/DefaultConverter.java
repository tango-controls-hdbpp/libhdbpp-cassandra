 ...
 */
package fr.soleil.snapArchivingApi.SnapExtractorApi.convert;
...
public class DefaultConverter implements IConverter {
	 ...
	 * 
	 * @see
	 * snapextractor.api.convert.IConverter#convert(fr.soleil.snapArchivingApi
	 * .SnapshotingTools.Tools.SnapAttributeExtract)
	 */
	public synchronized DbData convert(SnapAttributeExtract currentExtract)
			throws DevFailed {
		...
		dbData.setData_timed(timedAttrDatas);
	...
	private NullableTimedData[] convertData(SnapAttributeExtract currentExtract)
		...
		try {
			if (isScalar) {
				...
				if (hasBothReadAndWriteValues) {
					theTimedAttrData = buildDataSpectrumRW(value, dataType,
							when);
	 ...
	 * @return
	 */
	private NullableTimedData buildDataScalarRW(Object value, int dataType,
			long when) throws IllegalArgumentException {
		...
		case TangoConst.Tango_DEV_ULONG:
			Integer theDataL_R,
			theDataL_W;
			if (valueTab[0] == null || "null".equals(valueTab[0])
					|| "NaN".equalsIgnoreCase(valueTab[0])) {
				theDataL_R = null;
			} else {
				double theDataL_R_D = Double.parseDouble(valueTab[0]);
				theDataL_R = new Integer((int) Math.round(theDataL_R_D));
			}
			if (valueTab[1] == null || "null".equals(valueTab[1])
					|| "NaN".equalsIgnoreCase(valueTab[1])) {
				theDataL_W = null;
			} else {
		...
		default:
			throw new IllegalArgumentException(
					"DefaultConverter/Data type not supported: " + dataType);
		}
	...
	private NullableTimedData buildDataSpectrumRW(Object value, int dataType,
			long when) throws IllegalArgumentException {
		...
		if (valueTab[0] == null || "NaN".equalsIgnoreCase(valueTab[0] + "")
				|| "[NaN]".equalsIgnoreCase(valueTab[0] + "")) {
			return null;
		}
		if (valueTab[1] == null || "NaN".equalsIgnoreCase(valueTab[1] + "")
				|| "[NaN]".equalsIgnoreCase(valueTab[1] + "")) {
			return null;
		}
	...
	private NullableTimedData buildDataScalarR(Object value, int dataType,
			long when) throws IllegalArgumentException {
		NullableTimedData theTimedAttrData = null;
		...
		case TangoConst.Tango_DEV_ULONG:
			String valS = value + "";
			Integer theDataL;
			if (valS == null || "null".equals(valS)
					|| "NaN".equalsIgnoreCase(valS)) {
				theDataL = null;
			} else {
	...
	private NullableTimedData buildDataSpectrumR(Object value, int dataType,
			long when) throws IllegalArgumentException {
		if ("NaN".equalsIgnoreCase(value + "")
				|| "[NaN]".equalsIgnoreCase(value + "")) {
			return null;
	 ...
	 * (non-Javadoc)
	 * 
	 * @seefr.soleil.snapArchivingApi.SnapExtractorApi.convert.IConverter#
	 * castObjectTableToString(java.lang.Object[], boolean)
	 */
	public String[] castObjectTableToString(Object[] objects,
			boolean returnNullIfOneElementIsNull) {
