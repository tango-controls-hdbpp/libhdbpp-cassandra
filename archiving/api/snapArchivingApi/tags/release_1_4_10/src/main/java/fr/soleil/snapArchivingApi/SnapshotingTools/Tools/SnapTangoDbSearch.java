...
/**
 * This class wraps the TangoDbSearch class by generating an Exception dedicated
 * to the Snap Management
 * 
 * @author PIERREJOSEPH
 * 
 */
public class SnapTangoDbSearch {
	...
	public String[] getAttributesForClass(String classe, Vector devOfDomain)
		...
		try {
			return wrappedTangoDbSearch.getAttributesForClass(classe,
					devOfDomain);
		} catch (DevFailed e) {
			String message = GlobalConst.SNAPSHOTING_ERROR_PREFIX + " : "
					+ GlobalConst.DBT_EXCEPTION;
	...
	public Hashtable getClassAndDevices(String domain)
			throws SnapshotingException {
		try {
