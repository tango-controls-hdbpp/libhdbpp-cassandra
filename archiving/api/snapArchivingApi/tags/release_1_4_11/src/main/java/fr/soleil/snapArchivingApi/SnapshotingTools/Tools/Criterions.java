...
public class Criterions {
	...
	public Criterions(Condition[] conditions) {
		...
		if (conditions != null) {
			...
			for (int i = 0; i < nbOfConditions; i++) {
				...
				// The Conditions are referencedby their table's field
				// (columnName).
				if (conditionsHT.containsKey(columnName)) {
					currentColumnConditionsList = (Vector) conditionsHT
							.get(columnName);
				} else {
	...
	public String getContextClause() throws SnapshotingException {
		...
		if (conditions != null) {
			...
			if (conditions.length == 1) {
				...
				if (count == 0) {
					clause = clause + " WHERE " + table + "." + field[0]
							+ predicat;
					count++;
				} else {
					clause = clause + " AND " + table + "." + field[0]
							+ predicat;
				}
			} else {
				String message = "";
		...
		// l'ID_context.
		{
			...
			if (conditions != null) {
				for (int j = 0; j < conditions.length; j++) {
					...
					// Mise en forme de la requete SQL.
					if (count == 0) {
						clause = clause + " WHERE " + table + "." + field[i]
								+ predicat;
						count++;
					} else {
						clause = clause + " AND " + table + "." + field[i]
								+ predicat;
					}
				}
			}
	...
	public String getSnapshotClause() throws SnapshotingException {
		...
		if (conditions != null) {
			...
			if (conditions.length == 1) {
				...
				// Mise en forme de la requete SQL.
				if (count == 0) {
					clause = clause + " WHERE " + table + "." + field[1]
							+ predicat;
					count++;
				} else {
					clause = clause + " AND " + table + "." + field[1]
							+ predicat;
				}
			} else {
				String message = "";
	...
	public String toString() {
		...
		while (key.hasMoreElements()) {
			Vector columnConditionsList = (Vector) conditionsHT.get(key
					.nextElement());
