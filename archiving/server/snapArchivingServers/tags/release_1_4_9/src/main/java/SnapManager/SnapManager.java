...
//	=========================================================
	/**
	 * Execute command "SetEquipmentsWithCommand" on device.
	 * This command is used to set snapshot attribute's values as
	 * command argument to equipments.
	 * @param	argin	The command name and The snapshot id 
	 * from which equipments are set.
	 * @throws SnapshotingException 
	 */
//	=========================================================
	...
	public String[] set_equipments_with_command(String[] argin ) throws DevFailed
	{
		...
		try {
			...
			if(m_manager== null){
				m_manager = new SnapManagerImpl(dbUser, dbPassword);
			}
			...
			//add attributes test: 1-different devices 2-same type 3-same format
			if(validSnapshot(snapAttributeExtractArray)){
				SnapShot snapShot = SnapShot.getPartialSnapShot(argin , snap_id, snapAttributeExtractArray);
				if(snapShot == null)
					return null;
				//call command by every device proxy for each specific attribute
				ArrayList listAttributes = snapShot.getAttribute_List();
				 DeviceData device_data_out;
				 Vector<String> result = new Vector<String>();
				for (int i = 0; i < listAttributes.size(); i++) {
					DeviceProxy deviceProxy = new DeviceProxy((( SnapAttributeExtract ) listAttributes.get(i)).getdeviceName());
					DeviceData device_data = new DeviceData();
					snapAttributeExtractArray[i].insertsnapAttributeValue(device_data);
					device_data_out = deviceProxy.command_inout( command_name , device_data);
					String[] tmp = extractData(device_data_out, result);
					
					if( tmp!=null && tmp.length > 1) {
						
						String elt_res = command_name + "("+ (( SnapAttributeExtract ) listAttributes.get(i)).getAttribute_complete_name() + ") = ";
						...
						result.add( command_name + "("+ (( SnapAttributeExtract ) listAttributes.get(i)).getAttribute_complete_name() + ") = "
								+ device_data_out.extractAny().toString()) ;
					}
	...
	// TODO Auto-generated method stub
			...
			try {
				if(data_out.type().name().equals("DevVarDoubleArray")){
					...
					long[] res_array = data_out.extractULong64Array();
					data_array = new String[res_array.length];
					for (int i = 0; i < res_array.length; i++) {
						data_array[i] = String.valueOf(res_array[i]);
					}
	...
	private String[] toStringArray(Vector<String> vect) {
	// TODO Auto-generated method stub
		if(vect==null) return null;
		String[] res = new String[vect.size()];
		int counter = 0;
		for (Iterator iterator = vect.iterator(); iterator.hasNext();) {
			res[counter] = (String) iterator.next();
			counter++;
	...
	/*
	 * test if the attributes are from different devices and have the same format and the same type
	 */
