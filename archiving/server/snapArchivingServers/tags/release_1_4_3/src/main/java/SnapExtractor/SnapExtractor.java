...
//	=========================================================
	...
	private synchronized String[] get_attribute_value(SnapAttributeExtract snapAttributeExtract) {
		...
		try{
			...
			argout[0] = currentExtract.getReadValue().toString();
			argout[1] = currentExtract.getWriteValue().toString();
	...
	private DevVarLongStringArray createDynamicAttribute(DbData dbData /*added by CLA*/, String [] names) throws DevFailed
	{
		try
		{
			...
			if ( _2value )
			{
				...
				int dataMax_x = readDbData.getMax_x();
				int dataMax_y = readDbData.getMax_y();

				int dataFormat = readDbData.getData_format();
				int dataType = readDbData.getData_type();
				int dataWritable = readDbData.getWritable();

				int dataSize = readDbData.size();
				String dataName = readDbData.getName();

				TimedAttrData [] readTimedAttrDatas = readDbData.getDataAsTimedAttrData();
				int numberOfTimedAttrDataInRead = readTimedAttrDatas.length;
				for ( int i = 0 ; i < numberOfTimedAttrDataInRead ; i ++ )
				{
					TimedAttrData currentReadTimedAttrData = readTimedAttrDatas [i];

					int x = currentReadTimedAttrData.x;
					int y  = currentReadTimedAttrData.y;
					short [] sh_ptr = currentReadTimedAttrData.sh_ptr;				 
				}

				int dataMax_x_write = writeDbData.getMax_x();
				int dataMax_y_write = writeDbData.getMax_y();

				int dataFormatdataMax_x_write = writeDbData.getData_format();
				int dataTypedataMax_x_write = writeDbData.getData_type();
				int dataWritabledataMax_x_write = writeDbData.getWritable();

				int dataSizedataMax_x_write = writeDbData.size();
				String dataNamedataMax_x_write = writeDbData.getName();

				TimedAttrData [] writeTimedAttrDatas = writeDbData.getDataAsTimedAttrData();
				int numberOfTimedAttrDataInWritedata = writeTimedAttrDatas.length;
				for ( int i = 0 ; i < numberOfTimedAttrDataInWritedata ; i ++ )
				{
					TimedAttrData currentWriteTimedAttrData = writeTimedAttrDatas [i];

					int x = currentWriteTimedAttrData.x;
					int y  = currentWriteTimedAttrData.y;
					short [] sh_ptr = currentWriteTimedAttrData.sh_ptr;
				}
