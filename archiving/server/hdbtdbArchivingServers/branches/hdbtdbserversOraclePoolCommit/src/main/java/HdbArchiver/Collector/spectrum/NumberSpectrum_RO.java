...
public class NumberSpectrum_RO extends HdbCollector implements
		ISpectrumListener, ITangoArchiveListener {
	...
	private void addSourceForEvents(AttributeLightMode attributeLightMode)
		...
		try {
			...
			if (attribute == null) {
				System.out
						.println("\033[1;31mNumberSpectrum_RO.java: the attribute \""
								+ attributeLightMode
	...
	private void removeSourceForEvents(String attributeName) {
		Util.out2.println("NumberSpectrum_RO.removeSource");
		System.out
				.print("\033[0;42mremoving\033[0m source for \033[4mspectrum\033[0m [RO]: "
						+ attributeName + "\"...\t");
	...
	public void archive(TangoArchiveEvent event) {
		...
		try {
			...
			System.err.println("Spectrum_RO.java: archive : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ spectrumEvent_ro.getAttribute_complete_name()
					+ " values...");
	...
	public void spectrumChange(NumberSpectrumEvent event) {
		...
		SpectrumEvent_RO spectrumEvent_ro = new SpectrumEvent_RO();
		spectrumEvent_ro.setAttribute_complete_name(((INumberSpectrum) event
				.getSource()).getName());
	...
	public void processEventSpectrum(SpectrumEvent_RO snapSpectrumEvent_RO,
		...
		// lastValue
		{
			...
			if (try_number > 0) {
				Util.out2
						.println("NumberSpectrum_RO.processEventSpectrum : \r\n\ttry "
								+ (try_number) + "failed...");
