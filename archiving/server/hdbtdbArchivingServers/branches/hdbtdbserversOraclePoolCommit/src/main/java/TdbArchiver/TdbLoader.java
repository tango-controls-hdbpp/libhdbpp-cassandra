 ...
 * @author CLAISSE
 *
 * This class hasn't been tested since a long time ago and has been
 *  modified the 11 february 2010 due to dspath and dbpath management modification
 */
public class TdbLoader {
	...
	public static void main(String[] args) {
		if (args == null || args.length != 4) {
			TdbLoader
					.end(TdbLoader.FAIL,
							"Bad arguments. Usage: java TdbLoader workingPath dbPath tdbUser tdbPwd");
		}
	...
	private void verify() {
		...
		if (!this.workingDirectory.isDirectory()) {
			TdbLoader.end(TdbLoader.FAIL,
					"TdbLoader/verify/The working path isn't a directory|"
							+ this.workingPath + "|");
		}
		if (!this.workingDirectory.isDirectory()) {
			TdbLoader.end(TdbLoader.FAIL,
					"TdbLoader/verify/The working path can't be read|"
							+ this.workingPath + "|");
		}
		...
		if (this.filesToLoad == null || this.filesToLoad.length == 0) {
			TdbLoader.end(TdbLoader.NOTHING_TO_DO,
					"TdbLoader/verify/No .dat files to load in the working directory|"
							+ this.workingPath + "|");
		}
		...
		try {
			...
			this.manager.ArchivingConfigureWithoutArchiverListInit(this.tdbUser, this.tdbPwd);
			this.dataBaseApi = this.manager.getDataBase();
		} catch (ArchivingException e) {
			e.printStackTrace();
			TdbLoader.end(TdbLoader.FAIL,
					"TdbLoader/verify/can't connect to the TDB database with tdbUser|"
							+ this.tdbUser + "|tdbPwd|" + this.tdbPwd);
		}
	}
	...
	private void processFile(File file) {
		...
		try {
			...
			DbProxy dbProxy = new DbProxy(logger);
			// The two directories are the same here
			FileTools fileTools = new FileTools(tableName, dataFormat,
					writable, windowsDuration, logger, false, dbProxy,dbPath,dbPath);
			//fileTools.set_remoteFilePath(this.dbPath);

			ExportTask exportTask = fileTools.new ExportTask(fileName, false);
			exportTask.execute();

			((TDBDataBaseManager) this.manager.getDataBase()).getTdbExport()
	...
	private int[] getTypeFormatWritable(String tableName)
		...
		return ((ITDBAdtAptAttribute) this.dataBaseApi.getAttribute())
				.getAtt_TFW_Data_By_Id(id);
	}
