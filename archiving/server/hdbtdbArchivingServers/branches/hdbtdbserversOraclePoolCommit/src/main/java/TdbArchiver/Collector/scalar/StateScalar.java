...
public abstract class StateScalar extends TdbCollector implements
		IDevStateScalarListener {

	/**
	 *
	 */
	private static final long serialVersionUID = -2675583628918464918L;
	/**
	...
	public StateScalar(TdbModeHandler _modeHandler, String currentDsPath,
	...
	public void processEventScalar(ScalarEvent scalarEvent, int try_number) {
		...
		try {
			...
			if (this.isFirstValue) {
				 ...
				 * "StateScalar/processEventScalar/scalarEvent.getReadValue() == null/for attribute/"
				 * +scalarEvent.getAttribute_complete_name()); } if (
				 * getLastValue(scalarEvent) == null ) {
				 * super.m_logger.trace(ILogger.LEVEL_WARNING,
				 * "StateScalar/processEventScalar/getLastValue(scalarEvent) == null/for attribute/"
			 ...
			 *
			 * if ( valueCondition ) { //System.out.println(
			 * "StateScalar/processEventScalar/valueCondition TRUE/"+new
			 * Timestamp(scalarEvent.getTimeStamp()) ); ( ( FileTools )
			 * filesNames.get(scalarEvent.getAttribute_complete_name())
			 * ).processEventScalar(scalarEvent); } } else {
			 * //System.out.println(
			 * "StateScalar/processEventScalar/NO valueCondition REQUIRED/"+new
			 * Timestamp(scalarEvent.getTimeStamp()) ); ( ( FileTools )
			 * filesNames.get(scalarEvent.getAttribute_complete_name())
			 * ).processEventScalar(scalarEvent); }
			 */
	...
	public void removeSource(String attributeName) throws ArchivingException {
		...
		try {
			synchronized (attributeList) {
				 ...
				 * != null ) {
				 */
				IDevStateScalar attribute = (IDevStateScalar) attributeList
						.get(attributeName);
				if (attribute != null) {
	...
	public void errorChange(ErrorEvent errorEvent) {
		...
		try {
			processEventScalar(getNullValueScalarEvent(errorEvent,
					TangoConst.Tango_DEV_STATE, getWritableValue()), tryNumber);
		} catch (Exception e) {
			super.m_logger
					.trace(
							ILogger.LEVEL_ERROR,
