...
{
	private static final long serialVersionUID = 10047L;
	
	public BooleanScalar_WO(HdbModeHandler modeHandler)
	{
		super(modeHandler);
	}
	...
	public void booleanScalarChange(BooleanScalarEvent event)
	{
		...
		catch ( Exception exE )
		{
            ...
            String message = "Problem in BooleanScalar_WO/booleanScalarChange";
...
//	--------------------------------------------------------------------------//
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		catch(DevFailed f)
		{
			...
			print_exception(
					"\033[1;31mBooleanScalar_WO.archive.getValue() failed, caught DevFailed\033[0m",f);
		    return;
		}
		...
		try{
			...
			{
				System.out.println("\033[1;31mBooleanScalar_WO: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			...
			Boolean bvalue = attrib.extractBoolean();
