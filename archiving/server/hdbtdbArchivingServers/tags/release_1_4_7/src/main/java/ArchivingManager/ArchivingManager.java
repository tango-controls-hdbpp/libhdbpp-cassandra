//+============================================================================
// $Source: /cvsroot/tango-cs/tango/jserver/archiving/ArchivingManager/ArchivingManager.java,v $
//
// project :     Tango Device Server
//
// Description:	java source code for the ArchivingManager class and its commands.
...
// Modification of the ArchivingMessConfig object creation and so the ArchivingStart and ArchivingModif methods argin has been simplified.
//
...
// modified the ArchivingConfigure command and the init_device method to init the HDB and TDB connections separately
//
// Revision 1.11  2006/10/12 06:53:42  chinkumo
// Argin comment modification for the archivingStartHdb command.
// ==>modif précédemment perdue
//
// Revision 1.10  2006/10/11 08:30:32  ounsy
// modified the archiving_start commands to accept the same parameters as Archivers
//
...
// The ArchivingManager now use exclusively ArchivingMessConfig object to trigger archiving.
//
...
// The trouble with grouped strategy when launching an archiving sequence was fixed.
 ...
 * The frequency use for this kind of storage is usually small.
 * <Br><Br>
 * <Li><b>The Temporary archiving</b>, manage by TdbArchiver devices.<br>
 * This kind archiving allows to store in a database all the exported tango attributes.<br>
 * The stored values are kept in the database for a limited time ("Keeping Window" [4 hours - 24 hours]).
 * Before being stored into the database, values are first collected into files (timestamp, read value [, write value]).
 * Those files are periodically exported to the database. This export frequency must also be given ("Export Window" [5 seconds - 1 hour]).<br>
 * The frequency use for this kind of storage is usually high.
 * <Br><Br>
 * <br><b>ARCHIVING MODE</b><br>
 * An archiving sequence must be tuned to follow one (or several) of the defined archiving modes :<br>
 * <Li> The <b>Periodical mode</b>, this mode allows to archive following a defined frequency.<br>
 * <Li> The <b>Absolute mode</b>, this mode allows to archive a value only if : <br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <var>current_value</var> &gt; <var>previous_archived_value</var> + <var>&delta;<sub>1</sub></var> &nbsp;&nbsp;&nbsp;&nbsp; or <br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <var>current_value</var> &lt; <var>previous_archived_value</var> - <var>&delta;<sub>2</sub></var>.<br>
 ...
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; (<var>&micro;<sub>increase</sub></var> and <var>&micro;<sub>decrease</sub></var> are percentages)
 * <Li> The <b>Threshold mode</b>, this mode allows to archive a value only if :<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <var>current_value</var> &gt; <var>threshold_value<sub>sup</sub></var> &nbsp;&nbsp;&nbsp;&nbsp; or <br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <var>current_value</var> &lt; <var>threshold_value<sub>min</sub></var>.<br>
 * (like alarms work).<br>
 * <Br><Br>
 * <Li> The <b>On Calculation mode</b>, this mode allows to archive according to a user formula.<br>
 * <Li> The <b>On Difference mode</b>, this mode allows to archive when a value changes.<br>
 * <Li> The <b>External mode</b>, this mode allows to archive on the user demand.<br>
 * <br>
 * <br>
 * The 6 last mode must be associated to a periodic mode.
 * <Br><Br>
 *
 ...
 *	DevState.ALARM :	Either one of the two databases (HDB or TDB) is unreachable, or there is no archiver one of them.
 *	DevState.FAULT :	Either the two databases (HDB and TDB) are unreachable, or there is no archiver at all.
 */
//--------- End of States Description ----------


public class ArchivingManager extends DeviceImpl/*WithShutdownRunnable*/ implements TangoConst
{
	protected int state;
...
//=========================================================
    ...
    * @param   version The device version
	 */
//=========================================================
	ArchivingManager(DeviceClass cl , String s , String d, String version) throws DevFailed
	{
		super(cl , s , d);
		m_version = version;
		init_device();
...
//=========================================================
	public void init_device() throws DevFailed
	{
        ...
        try
        {
            if(hdbHost==null || hdbName==null)
            	this.manager.ArchivingConfigure(hdbUser , hdbPassword , true);
            else 
            	this.manager.ArchivingConfigure(hdbHost, hdbName, hdbUser , hdbPassword , true);
        }
        catch ( ArchivingException e )
        {
            e.printStackTrace ();
            get_logger().warn(e.toString() , e);
            //throw e.toTangoException(); CLA 30/10/06 AN EXCEPTION IS NO LONGER THROWN, AS BOTH DATABASES ARE INITIALIZED INDEPENDANTLY
        }
        try
        {
        	if(tdbHost==null || tdbName==null)
        		this.manager.ArchivingConfigure( tdbUser , tdbPassword , false);
        	else
        		this.manager.ArchivingConfigure(tdbHost, tdbName, tdbUser , tdbPassword , false);
        }
        catch ( ArchivingException e )
        {
            e.printStackTrace ();
		...
		if ( ( al = ( ArchivingManagerApi.getArchiverListSize(true) == 0 && ArchivingManagerApi.getArchiverListSize(false) == 0 ) ) ||
		     ( db = !( this.manager.is_db_connected(true) ) && !( this.manager.is_db_connected(false) ) ) )
		{
		...
		else if ( ( al = ( ArchivingManagerApi.getArchiverListSize(true) == 0 || ArchivingManagerApi.getArchiverListSize(false) == 0 ) ) ||
		          ( db = !( this.manager.is_db_connected(true) ) || !( this.manager.is_db_connected(false) ) ) )
		{
...
//===================================================================
	public void get_device_property() throws DevFailed
	{
		...
		else
		{
			//	Try to get value from class property
			DbDatum cl_prop = ds_class.get_class_property(dev_prop[ i ].name);
			if ( cl_prop.is_empty() == false ) hdbPassword = cl_prop.extractString();
		}
		...
		else
		{
			//	Try to get value from class property
			DbDatum cl_prop = ds_class.get_class_property(dev_prop[ i ].name);
			if ( cl_prop.is_empty() == false ) tdbPassword = cl_prop.extractString();
		}
...
//=========================================================
	/**
	 * Execute command "ArchivingConfigure" on device.
	 ...
	 *
	 * @param	argin	<ul> <li><var>argin</var>[<code>0</code>], the <em>user name</em> used to logg into the historical database. <li><var>argin</var>[<code>1</code>], the <em>password</em> used to logg into the historical database. <li><var>argin</var>[<code>2</code>], the <em>user name</em> used to logg into the temporary database. <li><var>argin</var>[<code>9</code>], the <em>password</em> used to logg into the temporary database.   </ul>
	 */
//=========================================================
	public void archiving_configure(String[] argin) throws DevFailed
	{
        ...
        /*try
        {
            ArchivingManagerApi.ArchivingConfigure(argin[ 0 ] , argin[ 1 ] , argin[ 2 ] , argin[ 3 ]);
        }
        catch ( ArchivingException e )
        {
            get_logger().warn(e.toString() , e);
        ...
        try
		{
            this.manager.ArchivingConfigure(argin[ 0 ] , argin[ 1 ] , true);
		}
		catch ( ArchivingException e )
		{
			get_logger().warn(e.toString() , e);
        ...
        try
        {
            this.manager.ArchivingConfigure(argin[ 2 ] , argin[ 3 ] , false);            
        }
        catch ( ArchivingException e )
        {
            get_logger().warn(e.toString() , e);
        ...
        {
            String desc = "Failed connecting to HDB";
            ArchivingException exceptionHDB = new ArchivingException (message , reason , ErrSeverity.PANIC , desc , "" );
            totale.addStack ( desc, exceptionHDB );
        }
        if ( tdbArchivingException != null )
        {
            String desc = "Failed connecting to TDB";
            ArchivingException exceptionTDB = new ArchivingException (message , reason , ErrSeverity.PANIC , desc , "" );
        ...
        if ( needsToThrow )
        {
            ...
            ArchivingException e = new ArchivingException (message , reason , ErrSeverity.PANIC , desc , "" );
            throw e.toTangoException();
        }
	}
...
//=========================================================
	public void archiving_start_hdb(String[] argin) throws DevFailed
	{
		...
		try
		{
            ArchivingMessConfig archivingMessConfig = ArchivingMessConfig.creationWithoutFullInformation(argin);
            this.manager.ArchivingStart(true , archivingMessConfig);
		}
		catch ( ArchivingException e )
		{
            e.printStackTrace ();
...
//=========================================================
	/**
	 * Execute command "ArchivingStartTdb" on device.
	 * Start an temporary archiving for the specified attributes, and following the specified mode.
	 *
...
//=========================================================
	public void archiving_start_tdb(String[] argin) throws DevFailed
	{
		...
		try
		{
            this.manager.ArchivingStart(false , archivingMessConfig);
		}
		catch ( ArchivingException e )
		{
			get_logger().warn(e.toString() , e);
...
//=========================================================
	        ...
	        (String[] argin) throws DevFailed
	{
		...
		try
		{
            this.manager.ArchivingStopConf(argin , true);
		}
		catch ( ArchivingException e )
		{
			get_logger().warn(e.toString() , e);
...
//=========================================================
	/**
	 * Execute command "ArchivingStopTdb" on device.
	 * Stops the temporary archiving for the given attributes.
	 *
	 * @param	argin	The attribute list.
	 */
//=========================================================
	public void archiving_stop_tdb
	        (String[] argin) throws DevFailed
	{
		...
		try
		{
            this.manager.ArchivingStopConf(argin , false);
		}
		catch ( ArchivingException e )
		{
			get_logger().warn(e.toString() , e);
...
//=========================================================
	        ...
	        (String[] argin) throws DevFailed
	{
		...
		try
		{
            this.manager.ArchivingStopConf(archivingMessConfig.getAttributeList() , true);
...
//=========================================================
	/**
	 * Execute command "ArchivingModifTdb" on device.
	 ...
	 */
//=========================================================
	public void archiving_modif_tdb
	        (String[] argin) throws DevFailed
	{
		...
		try
		{
            this.manager.ArchivingStopConf(archivingMessConfig.getAttributeList() , false);
...
//=========================================================
	/**
	 * Execute command "IsArchivedHdb" on device.
	 * Check the archiving state (historical archiving) for each attribute of the given list.
	 *
	 * @param	argin	The attribute list.
	 * @return	For each attribute of the given list...<br>   <ul> <li><code>1</code>, if the attribute is currently being archived (historical archiving) <li><code>0</code>, otherwise </ul>
	 */
//=========================================================
	public short[] is_archived_hdb
	        (String[] argin) throws DevFailed
	{
		...
		for ( int i = 0 ; i < argin.length ; i++ )
		{
			try
			{
				result = this.manager.isArchived(argin[ i ] , true);
			}
			catch ( ArchivingException e )
			{
				get_logger().warn(e.toString() , e);
...
//=========================================================
	/**
	 * Execute command "IsArchivedTdb" on device.
	 * Check the archiving state (temporary archiving) for each attribute of the given list.
	 *
	 * @param	argin	The attribute list.
	 * @return	For each attribute of the given list...<br>   <ul> <li><code>1</code>, if the attribute is currently being archived (temporary archiving) <li><code>0</code>, otherwise </ul>
	 */
//=========================================================
	public short[] is_archived_tdb
	        (String[] argin) throws DevFailed
	{
		short[] argout = new short[ 5 ];
		...
		for ( int i = 0 ; i < argin.length ; i++ )
		{
			try
			{
				result = this.manager.isArchived(argin[ i ] , false);
			}
			catch ( ArchivingException e )
			{
				get_logger().warn(e.toString() , e);
...
//=========================================================
	        ...
	        argin) throws DevFailed
	{
		...
		// ---Add your Own code to control device here ---
		Mode mode = ( this.manager.GetArchivingMode(argin , true) );
		argout = mode.toArray();
...
//=========================================================
	/**
	 * Execute command "GetArchivingModeTdb" on device.
	 ...
	 */
//=========================================================
	public String[] get_archiving_mode_tdb
	        (String
	        argin) throws DevFailed
	{
		String[] argout = new String[ 5 ];
		...
		// ---Add your Own code to control device here ---
		Mode mode = ( this.manager.GetArchivingMode(argin , false) );
		argout = mode.toArray();
...
//=========================================================
	/**
	 * Execute command "GetStatusHdb" on device.
...
//=========================================================
	        ...
	        (String[] argin) throws DevFailed
	{
		...
		for ( int i = 0 ; i < argin.length ; i++ )
			try
			{
				argout[ i ] = this.manager.getStatus(argin[ i ] , true);
			}
			catch ( ArchivingException e )
			{
				throw e.toTangoException();
...
//=========================================================
	/**
	 * Execute command "GetStatusTdb" on device.
...
//=========================================================
	        ...
	        (String[] argin) throws DevFailed
	{
		...
		for ( int i = 0 ; i < argin.length ; i++ )
			try
			{
				argout[ i ] = this.manager.getStatus(argin[ i ] , false);
			}
			catch ( ArchivingException e )
			{
				throw e.toTangoException();
...
//--------------------------------------------------------------------------
/* end of $Source: /cvsroot/tango-cs/tango/jserver/archiving/ArchivingManager/ArchivingManager.java,v $ */
