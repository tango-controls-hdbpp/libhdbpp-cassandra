...
public class StringScalar_RO extends StringScalar {
	...
	public void archive(TangoArchiveEvent event) {
		...
		try {
			...
			print_exception(
					"Error getting archive event value \033[1;31mStringScalar_RO.archive.getValue() failed, caught DevFailed\033[0m",
					f);
			return;
		...
		try {
			...
			print_exception("StringScalar_RO.archive() : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ scalarEvent.getAttribute_complete_name() + " values...",
					devFailed);
			...
			System.err.println("StringScalar_RO.archive() : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ scalarEvent.getAttribute_complete_name() + " values...");
			exE.printStackTrace();
