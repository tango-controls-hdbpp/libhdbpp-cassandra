...
{
	private static final long serialVersionUID = 800011L;
	...
	public void numberScalarChange(NumberScalarEvent event)
	{
		...
		try
		{
            ...
            /*long fixed = 1153484775366L;
            long l;
            double seuil= 0.7;
            if ( Math.random () < seuil )
            {
                System.out.println ( "NumberScalar_RW/numberScalarChange/fixed/"+new Timestamp (fixed) );
                l =fixed;
            }
            else
            {
                l =event.getTimeStamp();
            }
...
//			ELETTRA : Archiving Events 
//			--------------------------------------------------------------------------//
/* svalue[0] = new Short((short)event.getValue());
 * ivalue[0] = new Integer((int)event.getValue());
 * dvalue[0] = new Double(event.getValue());
 * fvalue[0] = new Float((float)event.getValue());
 */
//			--------------------------------------------------------------------------//
//			ELETTRA : Archiving Events 
//			--------------------------------------------------------------------------//
			switch ( scalarEvent.getData_type() )
			{
				...
				case TangoConst.Tango_DEV_USHORT:
					svalue = new Short[ 2 ];
                    svalue[0] = new Short((short)event.getNumberSource().getNumberScalarValue());
                    svalue[1] = new Short((short)event.getNumberSource().getNumberScalarSetPoint());
					scalarEvent.setValue(svalue);
					break;
				case TangoConst.Tango_DEV_LONG:
				case TangoConst.Tango_DEV_ULONG:
					ivalue = new Integer[ 2 ];
                    ivalue[0] = new Integer((int)event.getNumberSource().getNumberScalarValue());
                    ivalue[1] = new Integer((int)event.getNumberSource().getNumberScalarSetPoint());
					scalarEvent.setValue(ivalue);
					break;
				case TangoConst.Tango_DEV_FLOAT:
					fvalue = new Float[ 2 ];
                    fvalue[0] = new Float((float)event.getNumberSource().getNumberScalarValue());
                    fvalue[1] = new Float((float)event.getNumberSource().getNumberScalarSetPoint());
					scalarEvent.setValue(fvalue);
					break;
                case TangoConst.Tango_DEV_DOUBLE:
				default:
					dvalue = new Double[ 2 ];
                    dvalue[0] = new Double(event.getNumberSource().getNumberScalarValue());
                    dvalue[1] = new Double(event.getNumberSource().getNumberScalarSetPoint());
		...
		{
			System.err.println("NumberScalar_RW.numberScalarChange : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + scalarEvent.getAttribute_complete_name() + " values...");
...
//	--------------------------------------------------------------------------//
	 ...
	 * @since events
	 * @author giacomo
	 * Implements the boolean scalar archive events for the read only type
	 */
	public void archive(TangoArchiveEvent event)
	{
		...
		{
			System.out.println("NumberScalar_RW.java: Error getting archive event value for the event \033[1;31m"
		...
		{
			System.out.println("NumberScalar_RW.archive.getValue() failed, caught generic Exception, code failure");
			e.printStackTrace();
			return;
		}
		...
		try{
			...
			{
				System.out.println("\033[1;31mNumberScalar_RW.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			...
			switch ( scalarEvent.getData_type() )
			{
				case TangoConst.Tango_DEV_SHORT:				
					...
					scalarEvent.setValue(svalue);
					System.out.println("short (R): " + svalue[0] + " (W): " + svalue[1]);
					break;
				case TangoConst.Tango_DEV_USHORT:
					/* extractUShortArray() returns an int[]. Then it cannot be casted to
					 * Short[]. If we do this, a cast exception is thrown.
					 * We probably could convert int to short and then short to Short...
					 * But anyway we lose the `unsigned' information
					 */
					short ushvalue[] = attrib.extractShortArray();
					svalue[0] = (Short) ushvalue[0];
					svalue[1] = (Short) ushvalue[1];
					scalarEvent.setValue(svalue);
					System.out.println("unsigned short (R): " + svalue[0] + " (W): " + svalue[1]);
					break;
				case TangoConst.Tango_DEV_LONG:
					...
					scalarEvent.setValue(ivalue);
					System.out.println("long (R): " + ivalue[0] + " (W): " + ivalue[1]);
					break;
				case TangoConst.Tango_DEV_ULONG:
					...
					scalarEvent.setValue(ivalue);
					System.out.println("unsigned long (R): " + ivalue[0] + " (W): " + ivalue[1]);
					break;
				...
				case TangoConst.Tango_DEV_FLOAT:
					...
					scalarEvent.setValue(fvalue);
					System.out.println("float (R): " + fvalue[0] + " (W): " + fvalue[1]);
					break;
				case TangoConst.Tango_DEV_BOOLEAN:
					...
					System.out.println("boolean (Here?!?! :o ) (R): " + bvalue[0] + " (W): " + bvalue[1]);
					break;
				...
				default:
					...
					scalarEvent.setValue(dvalue);
					System.out.println("double (R): " + dvalue[0] + " (W): " + dvalue[1]);
					break;
			}
			processEventScalar(scalarEvent , tryNumber);
		...
		catch ( DevFailed devFailed )
		{
			print_exception("NumberScalar_RW.archive() : " + GlobalConst.ARCHIVING_ERROR_PREFIX + 
					"\r\n\t" + "Problem while reading " + scalarEvent.getAttribute_complete_name() +
					" values (DevFailed)...",
					devFailed);
			Object value = null;
			scalarEvent.setValue(value);
		...
		{
			System.err.println("NumberScalar_RW.archive : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + scalarEvent.getAttribute_complete_name() + " values (Exception)...");
			exE.printStackTrace();
			Object value = null;
			scalarEvent.setValue(value);
			processEventScalar(scalarEvent , tryNumber);
