...
//--------------------------------------------------------------------------//
public abstract class NumberScalar extends HdbCollector implements INumberScalarListener,
	...
	m_logger.trace(ILogger.LEVEL_INFO, "===> Entering " + this.getClass().getSimpleName()
		+ ".removeSource for " + attributeName);
	...
	try {
	    ...
	    m_logger.trace(ILogger.LEVEL_INFO, "===> Exiting " + this.getClass().getSimpleName()
		    + ".removeSource for " + attributeName);
	}
    }
	...
	// );
	if (!timeCondition) {
	    // System.out.println("processEventScalar - exit");
	    return;
	}
	...
	try {
	    processEventScalar(getNullValueScalarEvent(errorEvent, ((AAttribute) errorEvent
		    .getSource()).getTangoDataType(), getWritableValue()), tryNumber);
	} catch (final Exception e) {
