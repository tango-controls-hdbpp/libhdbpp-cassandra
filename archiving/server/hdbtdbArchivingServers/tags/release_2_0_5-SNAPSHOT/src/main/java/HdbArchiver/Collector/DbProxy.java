...
public class DbProxy implements IDbProxy
{
    ...
    public DbProxy()
	{
		try
		{
            this.manager =  ArchivingManagerApiRefFactory.getInstance(ConfigConst.HDB);
            this.manager.ArchivingConfigure(HdbArchiver.HdbArchiver.myDbHost , HdbArchiver.HdbArchiver.myDbName, HdbArchiver.HdbArchiver.myDbUser , HdbArchiver.HdbArchiver.myDbPassword, HdbArchiver.HdbArchiver.myRacConnection );
	...
	private void initCommitThread() throws ArchivingException 
    {
        ...
        if ( ! dbApi.getDbConn().getAutoCommit () )
        {
            CommitThread commitThread = new CommitThread ( getCommitPeriod(), dbApi );
            commitThread.start ();   
        }
    }
    ...
    private long getCommitPeriod() {
    	...
    	try{
    		return  GetConf.readLongInDB("HdbArchiver", GetConf.COMMIT_PERIOD_IN_MINUTE);
    	}
    	catch(ArchivingException e){
	...
	public void store(ScalarEvent scalarEvent) throws ArchivingException
	{
		...
		try
		{
            ...
            ((HDBDataBaseManager)this.manager.getDataBase()).getHdbAttributeInsert().insert_ScalarData(scalarEvent);
    ...
    public void store(SpectrumEvent_RO spectrumEvent_ro) throws ArchivingException
	{
		...
		try
		{
		    ...
		    ((HDBDataBaseManager)this.manager.getDataBase()).getHdbAttributeInsert().insert_SpectrumData_RO(spectrumEvent_ro);
	...
	public void store(SpectrumEvent_RW spectrumEvent_rw) throws ArchivingException
	{
		...
		try
		{
		    ...
		    ((HDBDataBaseManager)this.manager.getDataBase()).getHdbAttributeInsert().insert_SpectrumData_RW(spectrumEvent_rw);
	...
	public void store(ImageEvent_RO imageEvent_RO) throws SQLException , IOException, ArchivingException
	{
		...
		try
		{
		    ...
		    ((HDBDataBaseManager)this.manager.getDataBase()).getHdbAttributeInsert().insert_ImageData_RO(imageEvent_RO);
	...
	public void insertModeRecord(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase().getMode().insertModeRecord(attributeLightMode, this.manager.getDataBase().getAttribute());
	...
	public void updateModeRecord(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase().getMode().updateModeRecord(attributeLightMode.getAttribute_complete_name(), this.manager.getDataBase().getAttribute());
		}
		catch ( ArchivingException e )
		{
			Util.out2.println(e.toString());
	...
	public void updateModeRecord(String att_name) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase().getMode().updateModeRecord(att_name, this.manager.getDataBase().getAttribute());
		}
		catch ( ArchivingException e )
		{
			Util.out2.println(e.toString());
