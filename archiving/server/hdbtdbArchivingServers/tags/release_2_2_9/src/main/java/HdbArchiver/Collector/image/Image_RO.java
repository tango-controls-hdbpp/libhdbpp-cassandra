...
public class Image_RO extends HdbCollector implements IImageListener {
    ...
    public void processEventImage(final ImageEvent_RO snapImageEvent_RO, int try_number) {
	Util.out4.println("Image_RO.processEventImage");

	final boolean timeCondition = super.isDataArchivableTimestampWise(snapImageEvent_RO);
	if (!timeCondition) {
	    return;
	}
