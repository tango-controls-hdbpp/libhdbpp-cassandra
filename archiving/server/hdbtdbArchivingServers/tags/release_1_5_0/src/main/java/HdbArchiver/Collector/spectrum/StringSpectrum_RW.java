...
//	--------------------------------------------------------------------------//	
    private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			...
			if(attribute == null)
			{
				System.out.println("\033[1;31mStringSpectrum_RW.java: the attribute \"" +
						attributeLightMode.getAttribute_complete_name() +
		...
		{
			System.out.println("\033[1;31mStringStringSpectrum_RW.java: Failed to get the attribute from the AttributeFactory\033[0m");
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		{
			System.out.println("StringSpectrum_RW.java.archive.getValue() failed, caught generic Exception, code failure");
			e.printStackTrace();
			return;
		}
		try{
			...
			{
				System.out.println("\033[1;31mBooleanStringSpectrum_RW.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
		...
		catch ( DevFailed devFailed )
		{
			...
			//printException(GlobalConst.DATA_TYPE_EXCEPTION , AttrDataFormat._SCALAR , spectrumEvent_rw.getAttribute_complete_name() , devFailed);
...
//	--------------------------------------------------------------------------//	    	
	...
	{
        int tryNumber = DEFAULT_TRY_NUMBER;
        Util.out3.println("StringSpectrum_RW.errorChange : " + "Unable to read the attribute named " + errorEvent.getSource().toString());
        String[] value = null;
		SpectrumEvent_RW spectrumEvent_rw = new SpectrumEvent_RW();
    ...
    public void stringSpectrumChange (StringSpectrumEvent event)
	{
		...
		//System.out.println ( "CLA/Spectrum_RW/spectrumChange/value.length/"+value.length+"/" );
		SpectrumEvent_RW spectrumEvent_rw = new SpectrumEvent_RW();
