...
{
	private static final long serialVersionUID = 800012L;
	
	public NumberScalar_WO(HdbModeHandler modeHandler)
	{
		super(modeHandler);
	}
	...
	public void numberScalarChange(NumberScalarEvent event)
	{
		...
		try
		{
			...
			switch ( scalarEvent.getData_type() )
			{
				case TangoConst.Tango_DEV_SHORT:
					scalarEvent.setValue(new Short((short)event.getNumberSource().getNumberScalarSetPoint()));
					break;
				case TangoConst.Tango_DEV_USHORT:
					scalarEvent.setValue(new Short((short)event.getNumberSource().getNumberScalarSetPoint()));
					break;
				case TangoConst.Tango_DEV_LONG:
					scalarEvent.setValue(new Integer((int)event.getNumberSource().getNumberScalarSetPoint()));
					break;
				...
				case TangoConst.Tango_DEV_FLOAT:
					scalarEvent.setValue(new Float((float)event.getNumberSource().getNumberScalarSetPoint()));
					break;
				default:
					scalarEvent.setValue(new Double(event.getNumberSource().getNumberScalarSetPoint()));
					break;
		...
		{
			System.err.println("NumberScalar_WO.numberScalarChange : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + scalarEvent.getAttribute_complete_name() + " values...");
...
//	--------------------------------------------------------------------------//
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		catch(DevFailed f)
		{
			...
			System.out.println();
			print_exception("\033[1;31mNumberScalar_WO.archive.getValue() failed, caught DevFailed\033[0m",f);
		    return;
		}
		catch (Exception e) /* Shouldn't be reached */
		{
			System.out.println("NumberScalar_WO.archive.getValue() failed, caught generic Exception, code failure");
			e.printStackTrace();
			return;
		}
		...
		try{
			...
			{
				System.out.println("\033[1;31mNumberScalar_WO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			...
			{
				case TangoConst.Tango_DEV_SHORT:
					Short sval = new Short ((short) attrib.extractShort() );
				...
				case TangoConst.Tango_DEV_LONG:
					...
					int longval =  attrib.extractLong();
					scalarEvent.setValue(new Integer(longval) );
					System.out.println(longval);
					break;
				case TangoConst.Tango_DEV_ULONG:
					int longintval = attrib.extractLong();
					scalarEvent.setValue(new Integer ( longintval  ) ) ;
					System.out.println(longintval);
					break;
				case TangoConst.Tango_DEV_DOUBLE:
					Double dval = new Double( (double) attrib.extractDouble() );
					scalarEvent.setValue(new Double( dval) );
					System.out.println(dval);
					break;
				case TangoConst.Tango_DEV_FLOAT:
					Float floatval = new Float( (float) attrib.extractFloat() );
					scalarEvent.setValue(floatval);
					System.out.println(floatval);
					break;
				case TangoConst.Tango_DEV_BOOLEAN:
					scalarEvent.setValue(new Boolean( (boolean) attrib.extractBoolean() ) );
					break;
				default:
					scalarEvent.setValue(new Double(  (double) attrib.extractDouble() ) );
					break;
			}
			processEventScalar(scalarEvent , tryNumber);
