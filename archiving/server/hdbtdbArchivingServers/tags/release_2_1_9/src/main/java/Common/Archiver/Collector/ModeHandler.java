...
public abstract class ModeHandler {
	...
	public boolean isDataArchivable(ModesCounters attModeCounter,
			int data_type, Object currentEvent, Object previousEvent) {
		...
		case TangoConst.Tango_DEV_STRING:
			String currentEventString = currentEvent == null ? ""
					: (String) currentEvent;
			String previousEventString = previousEvent == null ? ""
		...
		case TangoConst.Tango_DEV_UCHAR:
			currentEventD = isCurrentNull ? -1 : ((Byte) currentEvent)
					.doubleValue();
			previousEventD = isPreviousNull ? -1 : ((Byte) previousEvent)
					.doubleValue();
			break;
		case TangoConst.Tango_DEV_LONG:
		case TangoConst.Tango_DEV_ULONG:
			currentEventD = isCurrentNull ? -1 : ((Integer) currentEvent)
					.doubleValue();
			previousEventD = isPreviousNull ? -1 : ((Integer) previousEvent)
					.doubleValue();
			break;
		case TangoConst.Tango_DEV_BOOLEAN:
			currentEventD = isCurrentNull ? -1 : (((Boolean) currentEvent)
					.booleanValue()) ? 1 : 0;
			previousEventD = isPreviousNull ? -1 : (((Boolean) previousEvent)
					.booleanValue()) ? 1 : 0;
			break;
		case TangoConst.Tango_DEV_SHORT:
		case TangoConst.Tango_DEV_USHORT:
			currentEventD = isCurrentNull ? -1 : ((Short) currentEvent)
					.doubleValue();
			previousEventD = isPreviousNull ? -1 : ((Short) previousEvent)
					.doubleValue();
			break;
		case TangoConst.Tango_DEV_FLOAT:
			currentEventD = isCurrentNull ? -1 : ((Float) currentEvent)
					.doubleValue();
			previousEventD = isPreviousNull ? -1 : ((Float) previousEvent)
					.doubleValue();
			break;
		case TangoConst.Tango_DEV_DOUBLE:
			currentEventD = isCurrentNull ? -1 : ((Double) currentEvent)
					.doubleValue();
			previousEventD = isPreviousNull ? -1 : ((Double) previousEvent)
					.doubleValue();
			break;
		default:
			try {
				...
				throw new IllegalArgumentException(m_myName
						...
						+ currentEvent.getClass().toString() + "/data_type/"
						+ data_type);
			}
			try {
				previousEventD = isPreviousNull ? -1 : ((Double) previousEvent)
				...
				throw new IllegalArgumentException(m_myName
						...
						+ previousEvent.getClass().toString() + "/data_type/"
						+ data_type);
			}
			break;
		}
		...
		if (isDataArchivableModeA(attModeCounter, isCurrentNull,
				isPreviousNull, currentEventD, previousEventD)) {
			result = true;
		}
		if (isDataArchivableModeR(attModeCounter, isCurrentNull,
				isPreviousNull, currentEventD, previousEventD)) {
			result = true;
		}
		...
		if (isDataArchivableModeC(attModeCounter, isCurrentNull,
				isPreviousNull, currentEventD, previousEventD)) {
			result = true;
		}
		if (isDataArchivableModeD(attModeCounter, isCurrentNull,
				isPreviousNull, currentEventD, previousEventD)) {
			result = true;
		}
		if (isDataArchivableModeE(attModeCounter, isCurrentNull,
				isPreviousNull, currentEventD, previousEventD)) {
			result = true;
		}
		return result;
	...
	private boolean isDataArchivableModeA(ModesCounters attModeCounter,
			boolean isCurrentNull, boolean isPreviousNull, double currentEvent,
		...
		if (attModeCounter.getCountModeA() < m_factorModeAbsolute) {
			...
			double lower_limit = ((double) previousEvent)
					- Math.abs(m_mode.getModeA().getValInf());
			double upper_limit = ((double) previousEvent)
	...
	private boolean isDataArchivableModeR(ModesCounters attModeCounter,
			boolean isCurrentNull, boolean isPreviousNull, double currentEvent,
			double previousEvent) {
		...
		if (attModeCounter.getCountModeR() < m_factorModeRelative) {
			...
			double percentInf = Math
					.abs(m_mode.getModeR().getPercentInf() / 100.0);
			double percentSup = Math
					.abs(m_mode.getModeR().getPercentInf() / 100.0);
			...
			if ((currentEvent < lower_limit) || (currentEvent > upper_limit)) {
				Util.out4.println(m_myName + ".isDataArchivableModeR \r\n\t"
						+ "\r\n\t" + "PercentInf() : "
						+ Math.abs(m_mode.getModeR().getPercentInf())
						+ "\r\n\t" + "PercentSup() : "
	...
	private boolean isDataArchivableModeT(ModesCounters attModeCounter,
			boolean isCurrentNull, double currentEvent) {
		...
		if (attModeCounter.getCountModeT() < m_factorModeThreshold) {
			...
			if ((currentEvent < (double) m_mode.getModeT().getThresholdInf())
					|| (currentEvent > (double) m_mode.getModeT()
							.getThresholdSup())) {
	...
	private boolean isDataArchivableModeD(ModesCounters attModeCounter,
			boolean isCurrentNull, boolean isPreviousNull, double currentEvent,
			double previousEvent) {
	...
	// todo faire la suite ...
	private boolean isDataArchivableModeC(ModesCounters attModeCounter,
			boolean isCurrentNull, boolean isPreviousNull, double currentEvent,
			double previousEvent) {
		return false;
	}

	private boolean isDataArchivableModeE(ModesCounters attModeCounter,
			boolean isCurrentNull, boolean isPreviousNull, double currentEvent,
			double previousEvent) {
		return false;
	}
	...
	private void initFactorWithRounding() {
		m_factorModePeriodic = (int) Math
				.round((m_mode.getModeP().getPeriod() * 1.0)
						/ (m_refresherInterval * 1.0));

		if (m_mode.getModeA() != null) {
			m_factorModeAbsolute = (int) Math.round((m_mode.getModeA()
					.getPeriod() * 1.0)
					/ (m_refresherInterval * 1.0));
		}

		if (m_mode.getModeR() != null) {
			m_factorModeRelative = (int) Math.round((m_mode.getModeR()
					.getPeriod() * 1.0)
					/ (m_refresherInterval * 1.0));
		}

		if (m_mode.getModeT() != null) {
			m_factorModeThreshold = (int) Math.round((m_mode.getModeT()
					.getPeriod() * 1.0)
					/ (m_refresherInterval * 1.0));
		}

		if (m_mode.getModeD() != null) {
			m_factorModeDifference = (int) Math.round((m_mode.getModeD()
					.getPeriod() * 1.0)
					/ (m_refresherInterval * 1.0));
		}

		if (m_mode.getModeC() != null) {
			m_factorModeCalcul = (int) Math.round((m_mode.getModeC()
					.getPeriod() * 1.0)
					/ (m_refresherInterval * 1.0));
		}
	}
	...
	private void initFactorWithoutRounding() {
		if (m_mode.getModeP().getPeriod() % m_refresherInterval == 0) {
			m_factorModePeriodic = m_mode.getModeP().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
		if ((m_mode.getModeA() != null)
				&& (m_mode.getModeA().getPeriod() % m_refresherInterval == 0)) {
			m_factorModeAbsolute = m_mode.getModeA().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
		if ((m_mode.getModeR() != null)
				&& (m_mode.getModeR().getPeriod() % m_refresherInterval == 0)) {
			m_factorModeRelative = m_mode.getModeR().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
		if ((m_mode.getModeT() != null)
				&& (m_mode.getModeT().getPeriod() % m_refresherInterval == 0)) {
			m_factorModeThreshold = m_mode.getModeT().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
		if ((m_mode.getModeC() != null)
				&& (m_mode.getModeC().getPeriod() % m_refresherInterval == 0)) {
			m_factorModeCalcul = m_mode.getModeC().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
		if ((m_mode.getModeD() != null)
				&& (m_mode.getModeD().getPeriod() % m_refresherInterval == 0)) {
			m_factorModeDifference = m_mode.getModeD().getPeriod()
					/ m_refresherInterval;
		} else {
			// Todo send an exception
		}
