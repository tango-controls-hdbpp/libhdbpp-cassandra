...
import HdbArchiver.Collector.Tools.commit.CommitThread;
...
{
	private Hashtable periods;
	private Hashtable counters;
	private Hashtable lastInserts;
	private Hashtable lastInsertRequests;
    
    private ArchivingManagerApi manager;
    
    private static final long COMMIT_PERIOD_IN_MINUTES = 1;
    ...
    public DbProxy()
	{
		try
		{
            ...
            this.manager.ArchivingConfigure(HdbArchiver.HdbArchiver.myDbHost , HdbArchiver.HdbArchiver.myDbName, HdbArchiver.HdbArchiver.myDbUser , HdbArchiver.HdbArchiver.myDbPassword , true);
			
			this.periods = new Hashtable ();
			this.counters = new Hashtable ();
			this.lastInserts = new Hashtable ();
			this.lastInsertRequests = new Hashtable ();
	...
	private void initCommitThread() 
    {
        DataBaseApi dbApi = this.manager.getDataBase ( true );
        if ( ! dbApi.getAutoCommit () )
        {
            CommitThread commitThread = new CommitThread ( COMMIT_PERIOD_IN_MINUTES , dbApi );
            commitThread.start ();   
        }
    }
	...
	{
        boolean ret = this.manager.is_db_connected(true);
        return ret;
	}
	...
	public void store(ScalarEvent scalarEvent) throws ArchivingException
	{
		...
		try
		{
            ...
            this.manager.getDataBase(true).insert_ScalarData(scalarEvent);
     ...
     * @param scalarEvent
     */
    private void updateCountersForAttributePreInsert(ArchivingEvent archivingEvent) 
    {
        String completeName = archivingEvent.getAttribute_complete_name ();
        this.lastInsertRequests.put ( completeName , new Long ( archivingEvent.getTimeStamp () ) );    
    }
	 ...
	 */
    private void updateCountersForAttributePostInsert ( ArchivingEvent archivingEvent ) 
    {
        String completeName = archivingEvent.getAttribute_complete_name ();
		...
		currentCounter ++;
		this.counters.put ( completeName , new Long ( currentCounter ) );
		
		this.lastInserts.put ( completeName , new Long ( archivingEvent.getTimeStamp () ) );
    }

    public void store(SpectrumEvent_RO spectrumEvent_ro) throws ArchivingException
	{
		Util.out4.println("DbProxy.store");
		try
		{
		    this.updateCountersForAttributePreInsert ( spectrumEvent_ro );
            this.manager.getDataBase(true).insert_SpectrumData_RO(spectrumEvent_ro);
	...
	public void store(SpectrumEvent_RW spectrumEvent_rw) throws ArchivingException
	{
		Util.out4.println("DbProxy.store SpectrumEvent_RW");
		try
		{
		    this.updateCountersForAttributePreInsert ( spectrumEvent_rw );
            this.manager.getDataBase(true).insert_SpectrumData_RW(spectrumEvent_rw);
	...
	/*public boolean hasBeenArchiving ( String completeName )
	{
	    Long currentCounter_I = (Long) this.counters.get ( completeName );
		long currentCounter = currentCounter_I == null ? 0 : currentCounter_I.longValue ();
		boolean ret = ( currentCounter != 0 );
		System.out.println ( "hasBeenArchiving/completeName/"+completeName+"/ret/"+ret );
		return ret;
	}*/
	...
	public boolean hasBeenArchiving ( String completeName ) throws DevFailed
	{
	    ...
	    {
	        saferPeriodCalculator = SaferPeriodCalculatorFactory.getImpl ( SaferPeriodCalculatorFactory.ABSOLUTE );    
	    }
	...
	public void store(ImageEvent_RO imageEvent_RO) throws SQLException , IOException, ArchivingException
	{
		Util.out4.println("DbProxy.store");
		try
		{
		    this.updateCountersForAttributePreInsert ( imageEvent_RO );
            this.manager.getDataBase(true).insert_ImageData_RO(imageEvent_RO);
	...
	public void insertModeRecord(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase(true).insertModeRecord(attributeLightMode);
	...
	public boolean isArchived(String att_name) throws ArchivingException
	{
		...
		try
		{
			return this.manager.getDataBase(true).isArchived(att_name);
		}
		catch ( ArchivingException e )
		{
			Util.out2.println(e.toString());
    ...
    public boolean isArchived(String att_name, String device_name) throws ArchivingException
    {
        ...
        try
        {
            return this.manager.getDataBase(true).isArchived(att_name, device_name);
        }
        catch ( ArchivingException e )
        {
            Util.out2.println(e.toString());
	...
	public void updateModeRecord(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase(true).updateModeRecord(attributeLightMode.getAttribute_complete_name());
		}
		catch ( ArchivingException e )
		{
			Util.out2.println(e.toString());
	...
	public void updateModeRecord(String att_name) throws ArchivingException
	{
		...
		try
		{
            this.manager.getDataBase(true).updateModeRecord(att_name);
		}
		catch ( ArchivingException e )
		{
			Util.out2.println(e.toString());
	...
	public Vector getArchiverCurrentTasks(String archiverName) throws ArchivingException
	{
		...
		boolean facility = ArchivingManagerApi.getHdbFacility();

		try
		{
			archiverCurrentTasks = this.manager.getDataBase(true).getArchiverCurrentTasks(( ( facility ) ? "//" + ( new Database() ).get_tango_host() + "/" : "" ) + archiverName , true);
    ...
    /* (non-Javadoc)
     * @see fr.soleil.hdbtdbArchivingApi.ArchivingWatchApi.datasources.device.IDbProxy#getLastInsert(java.lang.String)
     */
    public Timestamp getLastInsert(String completeName) throws DevFailed 
    {
        Timestamp ret = null;
        //System.out.println ( "getLastInsert/this.lastInserts.size()/"+this.lastInserts.size() );
