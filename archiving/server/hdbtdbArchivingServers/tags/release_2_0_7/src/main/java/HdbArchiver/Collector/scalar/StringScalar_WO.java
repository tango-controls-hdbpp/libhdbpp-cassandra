...
{
	private static final long serialVersionUID = 5000897766L; 
    ...
    public void stringScalarChange(StringScalarEvent event)
    {
        ...
        try
        {
            ...
            String value = new String( ( (IStringScalar) (event.getSource()) ).getStringSetPoint());
...
//  --------------------------------------------------------------------------//
    ...
    public void archive(TangoArchiveEvent event)
	{
		...
		{
			print_exception("Error getting archive event value \033[1;31mStringScalar_WO.archive.getValue() failed, caught DevFailed\033[0m", 
					f);
		    return;
		}
		...
		try{
			...
			{
				System.out.println("\033[1;31mStringScalar_WO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			System.out.println("\033[1;35marchive() [String Scalar WO]: [EVENT]: \033[1;32m" + proxy.name() + "/" + attrib.getName() + "\033[0m");
