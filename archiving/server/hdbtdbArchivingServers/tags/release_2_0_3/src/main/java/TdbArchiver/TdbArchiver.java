...
/**
 * Class Description:
 * This device is in charge of archiving Tango attributes to the Temporary Database, as known as TDB.
 ...
 * For each attribute, the values are then inserted into TDB every [base period], and if so required, when its value meets certain user-defined criterions.
 * The typical TDB archiving is higher frequency than HDB archiving. Thus, records are not directly inserted into TDB. Instead, they are written to a temp file, which is periodically imported by TDB.
 * 
...
public class TdbArchiver extends DeviceImpl/*WithShutdownRunnable*/ implements TangoConst, Warnable
{
    protected int state;
...
//--------- End of attributes data members ----------
    ...
    String dsPath;
    /**
     * Path used by the database service to access temporary data through the network.<br>
     * <p/>
     * <b>Default value : </b> C:\tango\TdbArchiver
     */
    String dbPath;
...
//=========================================================
     ...
     */
//=========================================================
    TdbArchiver(DeviceClass cl , String s , String d, String version) throws DevFailed
    {
        super(cl , s , d);
        m_version = version;
        init_device();
...
//=========================================================
    public synchronized void init_device() throws DevFailed
    {
        ...
        /*myCleanOldFiles = cleanOldFiles;
        myHasTriggeredExport = hasTriggeredExport;*/
        myHasDiary = hasDiary;
    ...
    {
        //System.out.println ( "CLA/startLoggingFactory/this.device_name/"+this.device_name+"/hashCode/"+this.hashCode () );        
        //ILogger _logger = LoggerFactory.getImpl ( LoggerFactory.DEFAULT_TYPE , this.device_name , this.dsPath , this.hasDiary );
    ...
    private void startArchiving(ArchivingMessConfig archivingMessConfig , boolean forceThreadedMode) throws DevFailed 
    {
        ...
        Thread startArchivingThread = new Thread ( startArchivingRunnable , "TDBStartArchivingThread" ); 
...
//===================================================================
    public void get_device_property() throws DevFailed
    {
        ...
        else
        {
            //  Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[ i ].name);
            if ( cl_prop.is_empty() == false ) dsPath = cl_prop.extractString();
        }
        ...
        else
        {
            //  Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[ i ].name);
            if ( cl_prop.is_empty() == false ) dbPath = cl_prop.extractString();
        }

        /*
        //  Extract Cleaner value
        if ( dev_prop[ ++i ].is_empty() == false )
            cleaner = dev_prop[ i ].extractBoolean();
        else
        {
            //  Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[ i ].name);
            if ( cl_prop.is_empty() == false ) cleaner = cl_prop.extractBoolean();
        }*/
        ...
        /*
        //    Extract CleanOldFiles value
        if ( dev_prop[ ++i ].is_empty() == false )
        {
            cleanOldFiles = dev_prop[ i ].extractBoolean();
        }
        else
        {
            //  Try to get value from class property
            ...
            if ( cl_prop.is_empty() == false )
            {
                cleanOldFiles = cl_prop.extractBoolean();
            }
        }

        //  Extract hasNewExport value
        if ( dev_prop[ ++i ].is_empty() == false )
        {
            hasTriggeredExport = dev_prop[ i ].extractBoolean();
        }
        else
        {
            //  Try to get value from class property
            ...
            if ( cl_prop.is_empty() == false )
            {
                hasTriggeredExport = cl_prop.extractBoolean();
            }
        }*/
        ...
        if ( dev_prop[ ++i ].is_empty() == false )
        {
            ...
            System.out.println ( "TdbArchiver/get_device_property/2/"+this.get_name()+"/isDedicated/"+isDedicated );
        }
        else
        {
            //  Try to get value from class property
                ...
                if ( att_support )
                {
                    ...
                    if ( this.dbProxy.isArchived(att_complete_name) )
                    {
                        if ( this.dbProxy.isArchived(att_complete_name, device_name) )
                        {
                            ...
                            {
                            	this.logger.trace ( ILogger.LEVEL_ERROR , "Failed while executing TdbArchiver.trigger_archive_conf() method : removeAllForAttribute");
                                ae.printStackTrace();
                                continue;
                            }
                    ...
                    try
                    {
                    	this.dbProxy.getDataBase().getAttribute().CreateAttributeTableIfNotExist(att_complete_name, this.dbProxy.getDataBase().getExtractor().getDataGetters(), this.dbProxy.getDataBase().getDbUtil());
...
//                      if the attribute is already archived, ignore it
                    ...
                    m_collector = collectorFactory.get(attributeLightMode,true,this.dbProxy);
            ...
            {
                this.logger.trace ( ILogger.LEVEL_ERROR , "ATTENTION=====Failed while executing TdbArchiver.trigger_archive_conf() method see exception " + e);
                e.printStackTrace();
            }
        }
...
//=========================================================
    public synchronized void trigger_archive_att(String[] argin) throws DevFailed
    {
        ...
        try
        {
            ...
            {
                attributeLightMode.getMode().checkMode(false);
                collector = collectorFactory.get(attributeLightMode,true,this.dbProxy);
        ...
        catch ( ArchivingException e )
        {
            ...
            String desc = ( att_support ) ?
                          "Unable to archive the attribute named " + attributeLightMode.getAttribute_complete_name() + "\r\n" + "Mode parameters should be checked..."
                          :
                          "Attribute named " + attributeLightMode.getAttribute_complete_name() + " have not supported type, format or writable property!"
                    ;
...
//=========================================================
    public synchronized void stop_archive_att(String[] argin) throws DevFailed
    {
        ...
        try
        {
            ...
            tdbCollector = collectorFactory.get(attributeLightMode,false,this.dbProxy);
            if ( tdbCollector != null )
            {
                tdbCollector.removeSource(attributeLightMode.getAttribute_complete_name());
            ...
            else// should only happen when attempting to call stop_archive_att on a non-longer-reserved attribute of a formerly (ie. at the time the archiving was started) dedicated archiver.
            {
                ...
                String desc = "Failed while executing TdbArchiver.stop_archive_att";
...
//=========================================================
    /**
     * Execute command "ExportData2Db" on device.
     * This command need an AttributeLightMode type object.
     * An AttributeLightMode type object encapsulate all the informations needed to found the Collector in charge of the archiving of the specified attribute
     * The informations needed are the type, the format, the writable property and the archiving mode
     *
...
//=========================================================
    public String export_data2_db(String[] argin) throws DevFailed
    {
        ...
        {
            System.out.println ( "export_data2_db/i|"+i+"|argin[i]|"+argin[i]+"|" );
        }*/
        // ---Add your Own code to control device here ---
        AttributeLightMode attributeLightMode = AttributeLightMode.creationWithFullInformation(argin);
        String tableName = ""; 
        ...
        try
        {
            TdbCollector collector = collectorFactory.get(attributeLightMode,false,this.dbProxy);
            if ( collector != null )
            {
                tableName = collector.exportFile2Db(attributeLightMode.getAttribute_complete_name(),false);   
            }
            else
            {
                String msg = "TdbArchiver/export_data2_db/getAttribute_complete_name|"+attributeLightMode.getAttribute_complete_name()+"| The collector is missing!";
...
//--------------------------------------------------------------------------
/* end of $Source: /cvsroot/tango-cs/tango/jserver/archiving/TdbArchiver/TdbArchiver.java,v $ */