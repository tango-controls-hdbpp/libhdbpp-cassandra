...
public abstract class AbsArchivingWatcher extends DeviceImpl implements
	...
	/**
	 * It defines the minimum number of KO attributes which put the device in ALARM state
	 * <b>Default value : </b> 100
	 */
...
//	--------- End of attributes data members ----------
	...
	// A l'avenir faire une interface avec la WatcherClass pour pouvoir utiliser la méthode
...
//	===================================================================
	protected void get_device_property() throws DevFailed
	{
		...
		if (dev_prop[++i].is_empty()== false) minNumberOfKoForAlarmState = dev_prop[i].extractShort();
		else
		{
			//	Try to get value from class property
			DbDatum	cl_prop = getClassProperty(dev_prop[i].name);
			if (cl_prop.is_empty() == false) minNumberOfKoForAlarmState = cl_prop.extractShort();
		}
		...
		get_logger().info("MinNumberOfKoForAlarmState = " + minNumberOfKoForAlarmState);
