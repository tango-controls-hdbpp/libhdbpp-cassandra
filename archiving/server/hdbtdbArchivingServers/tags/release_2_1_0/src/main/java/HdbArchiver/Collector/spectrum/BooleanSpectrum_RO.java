...
//	--------------------------------------------------------------------------//	
    private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			...
			if(attribute == null)
			{
				System.out.println("\033[1;31mSpectrum_RO.java: the attribute \"" +
						attributeLightMode.getAttribute_complete_name() +
		...
		{
			System.out.println("\033[1;31mSpectrum_RO.java: Failed to get the attribute from the AttributeFactory\033[0m");
		...
		System.out.print("boolean SPECTRUM [RO] attribute \"" + attribute.getNameSansDevice() + 
		"\"...\t");
		System.out.flush();
	...
	{
		Util.out2.println("BooleanSpectrum_RO.removeSource");
		System.out.print("boolean spectrum [RO]: removing source for \"" + attributeName + "\"...\t");
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		try{
			...
			{
				System.out.println("\033[1;31mBooleanSpectrum_RO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
		...
		{
			System.err.println("BooleanSpectrum_RO.java: archive() : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + spectrumEvent_ro.getAttribute_complete_name() + " values...");
		...
		{
			System.err.println("Boolean.Spectrum_RO.java: archive : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + spectrumEvent_ro.getAttribute_complete_name() + " values...");
			exE.printStackTrace();
			Object value = null;
			spectrumEvent_ro.setValue(value);
			processEventSpectrum(spectrumEvent_ro , tryNumber);
...
//    	--------------------------------------------------------------------------//	    	
	...
	public void processEventSpectrum(SpectrumEvent_RO snapSpectrumEvent_RO , int try_number)
	{
		...
		catch ( ArchivingException e )
		{
			...
			if ( try_number > 0 )
			{
				Util.out2.println("BooleanSpectrum_RO.processEventSpectrum : \r\n\ttry " + ( try_number ) + "failed...");
