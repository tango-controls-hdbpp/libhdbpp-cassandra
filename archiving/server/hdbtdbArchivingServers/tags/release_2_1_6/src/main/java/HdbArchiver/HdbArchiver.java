...
/* WithShutdownRunnable */implements TangoConst, Warnable {
    ...
    public void get_device_property() throws DevFailed {
        ...
        if (dev_prop[++i].is_empty() == false) dbHost = dev_prop[i]
                .extractString();
        else {
        ...
        if (dev_prop[++i].is_empty() == false) dbName = dev_prop[i]
                .extractString();
        else {
            // Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
        ...
        if (dev_prop[++i].is_empty() == false) dbUser = dev_prop[i]
                .extractString();
        else {
            // Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
        ...
        if (dev_prop[++i].is_empty() == false) dbPassword = dev_prop[i]
                .extractString();
        else {
            // Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
        ...
        if (dev_prop[++i].is_empty() == false) RacConnection = dev_prop[i]
                .extractBoolean();
        else {
            // Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
        ...
        if (dev_prop[++i].is_empty() == false) mySafetyPeriod = dev_prop[i]
                .extractString();
        else {
            // Try to get value from class property
            DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
    ...
    public void read_attr_hardware(Vector attr_list) {
        get_logger().info(
                "In read_attr_hardware for " + attr_list.size()
                        + " attribute(s)");
    ...
    public synchronized void trigger_archive_att(String[] argin)
        ...
        try {
            ...
            String desc = "Unable to archive the attribute named "
                    + attributeLightMode.getAttribute_complete_name();
            if (!att_support) desc = desc + "\r\n"
                    + "Type, format or writable property should be checked!";
            else if (!mode_valid) desc = desc + "\r\n"
                    + "Mode parameters should be checked...";
            else {
                desc = desc + "\r\n"
                        + "The attribute is unreachable... may be OFF or DOWN";
                // Correction des donn�es de la base : Cas o� les
                // �quipements ne tournent pas au red�marrage de l'archiver.
                try {
     ...
     */
    // =========================================================
    public synchronized boolean is_attribute_correctly_archived_internal(
