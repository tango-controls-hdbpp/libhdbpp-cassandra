...
/* WithShutdownRunnable */implements TangoConst {
     ...
     */
    // =========================================================
    ArchivingManager(final DeviceClass cl, final String s, final String version) throws DevFailed {
	super(cl, s);
	m_version = version;
	init_device();
    }
     ...
     */
    // =========================================================
    ArchivingManager(final DeviceClass cl, final String s, final String d, final String version)
	    throws DevFailed {
	super(cl, s, d);
	m_version = version;
	init_device();
	...
	try {
	    hdbManager = ArchivingManagerApiRefFactory.getInstance(ConfigConst.HDB);
	    if (hdbName == null) {
		hdbManager.ArchivingConfigure(hdbUser, hdbPassword);
	    } else {
		hdbManager.ArchivingConfigureWithoutArchiverListInit(hdbHost, hdbName, hdbUser,
			hdbPassword, hdbRacConnection, false, false);
	    }
	...
	try {
	    tdbManager = ArchivingManagerApiRefFactory.getInstance(ConfigConst.TDB);
	    if (tdbName == null) {
		tdbManager.ArchivingConfigure(tdbUser, tdbPassword);
	    } else {
		tdbManager.ArchivingConfigureWithoutArchiverListInit(tdbHost, tdbName, tdbUser,
			tdbPassword, tdbRacConnection, false, false);
	    }
	} catch (final ArchivingException e) {
	    e.printStackTrace();
	    get_logger().warn(e.toString(), e);
	...
	if ((al = hdbManager.getArchiverListSize() == 0 && tdbManager.getArchiverListSize() == 0)
		|| (db = !hdbManager.is_db_connected() && !tdbManager.is_db_connected())) {
	    ...
	    get_logger().warn(
		    "WARNING : " + (db ? " - One database is not connected ! " : "")
			    + (al ? " - One kind of archiver is not available ! " : ""));
	} else {
	...
	final String[] propnames = { "HdbHost", "HdbName", "TdbHost", "TdbName", "HdbUser",
	...
	try {
	    hdbManager.ArchivingConfigure(argin[0], argin[1]);
	} catch (final ArchivingException e) {
	    get_logger().warn(e.toString(), e);
	    // throw e.toTangoException(); CLA 30/10/06 AN EXCEPTION IS NO
	    // LONGER THROWN, AS BOTH DATABASES ARE INITIALIZED INDEPENDANTLY
	...
	try {
	    tdbManager.ArchivingConfigure(argin[2], argin[3]);
	} catch (final ArchivingException e) {
	    get_logger().warn(e.toString(), e);
	    // throw e.toTangoException(); CLA 30/10/06 AN EXCEPTION IS NO
	    // LONGER THROWN, AS BOTH DATABASES ARE INITIALIZED INDEPENDANTLY
	...
	if (needsToThrow) {
	    ...
	    final ArchivingException e = new ArchivingException(message, reason, ErrSeverity.PANIC,
		    desc, "");
	...
	for (int i = 0; i < argin.length; i++) {
	    try {
		argout[i] = hdbManager.getStatus(argin[i], true);
	    } catch (final ArchivingException e) {
		throw e.toTangoException();
	    }
	}
	...
	for (int i = 0; i < argin.length; i++) {
	    try {
		argout[i] = tdbManager.getStatus(argin[i], false);
	    } catch (final ArchivingException e) {
		throw e.toTangoException();
	    }
	}
