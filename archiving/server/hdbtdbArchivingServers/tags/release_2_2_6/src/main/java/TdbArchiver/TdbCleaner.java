...
public class TdbCleaner {
	...
	private void process() throws ArchivingException {
		...
		try {
			attributeList = ((TdbMode) this.dataBaseApi.getMode())
					.getArchivedAtt();
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected long computeRetentionPeriod() {
		...
		try {
			...
			String retentionValue = GetConf.readStringInDB("TdbArchiver",
					"RetentionPeriod");
			...
			if (type_s != "") {
				...
				if (timePeriod <= 0) {
					System.out
							.println("Invalid period : default value is used (3 days)");
					useDefault = true;
				}
			}
		} catch (ArchivingException e) {
			System.out
					.println("Properties is not used due to exception : default value is used (3 days)");
			// Nothing to do use default value
			useDefault = true;
		} catch (Exception e) {
			System.out.println("Exception raised " + e
					+ " default value is used (3 days)");
			useDefault = true;
		}
