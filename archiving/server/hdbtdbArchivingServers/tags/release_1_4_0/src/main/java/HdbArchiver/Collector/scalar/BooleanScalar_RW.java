...
public class BooleanScalar_RW extends BooleanScalar
{
	...
	public BooleanScalar_RW(HdbModeHandler modeHandler)
	{
		super(modeHandler);
	}
...
//			--------------------------------------------------------------------------//
/* In case of error the line can be modified as followed
 * 			boolean readvalue, setvalue;
 * 			readvalue = event.getValue();
 *			setvalue = ( ( IBooleanScalar ) event.getSource() ).getSetPoint();
 *          bvalue[0] = readvalue;
 *          bvalue[1] = setvalue;
...
//			--------------------------------------------------------------------------//
            ...
            bvalue[0] = new Boolean(( ( IBooleanScalar ) event.getSource() ).getValue());
            bvalue[1] = new Boolean(( ( IBooleanScalar ) event.getSource() ).getSetPoint());
		...
		catch ( Exception exE )
		{
            ...
            String message = "Problem in BooleanScalar_RW/booleanScalarChange";
...
//	--------------------------------------------------------------------------//
	public void archive(TangoArchiveEvent event)
	{
		...
		try{
			...
			{
				System.out.println("\033[1;31mBooleanScalar_RW: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			...
			bvalue_tmp = attrib.extractBooleanArray(); /* returns boolean[] */
			/* we need Boolean[] to store the correct data type */
			bvalue[0] = (Boolean) bvalue_tmp[0];	
			bvalue[1] = (Boolean) bvalue_tmp[1];
			scalarEvent.setValue(bvalue);
			
			System.out.println(proxy.name() + ": " +attrib.getName() + 
					"{boolean, RW} [\033[1;32mEVENT\033[0m]: " 
