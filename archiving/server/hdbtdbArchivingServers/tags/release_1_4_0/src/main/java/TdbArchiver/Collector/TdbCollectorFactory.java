...
public class TdbCollectorFactory
{
	...
	/**
	 * This method returns the instance of TdbCollector associated / associable to an attribute.
	 * In this method, attributes are not grouped by mode.
	 *
	 * @param attributeLightMode Attribute associated to the looked collector.
	 * @return the instance of TdbCollector associated / associable to an attribute.
	 */
	public TdbCollector get(AttributeLightMode attributeLightMode,boolean doCreateCollectorIfMissing,DbProxy dbProxy) throws ArchivingException
	{
	 ...
	 * @return the instance of TdbCollector associated / associable to an attribute.
	 */
	public TdbCollector create(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		TdbModeHandler modeHandler = new TdbModeHandler(attributeLightMode.getMode());
		TdbCollector collector = null;
    ...
    public synchronized void removeAllForAttribute(AttributeLightMode attributeLightMode) throws ArchivingException
    {
        ...
        m_logger.trace ( ILogger.LEVEL_INFO , "===> Entering removeAllForAttribute");
        synchronized ( tableCollector )//CLA 15/06/06
        {
            ...
            while( i < tableCollector.size() )
            {
                ...
                TdbCollector collector = ( TdbCollector ) tableCollector.get(superMode);
                
                if(collector != null)
                {
                	m_logger.trace ( ILogger.LEVEL_INFO , "===> i = " + i + ": Collector found : removeSource is requested ... ");
        ...
        this.m_logger.trace ( ILogger.LEVEL_INFO , "===> Exiting removeAllForAttribute");
    }
    ...
    /*public synchronized void removeAllForAttribute(String att_name) throws ArchivingException
    {
        synchronized ( tableCollector )//CLA 15/06/06
        {
            ...
            {
                SuperMode superMode = (SuperMode)keyIterator.next();//CLA -->Concurrent
                TdbCollector collector = ( TdbCollector ) tableCollector.get(superMode);
