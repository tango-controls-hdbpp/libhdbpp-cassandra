...
//					 informs the mother class that one new attribute must be removed
		...
		catch ( Exception e )
		{
			...
			String desc = "Failed while executing " + this.getClass().getSimpleName() + ".removeSource() method...";
		...
		try
		{
			...
			synchronized ( attributeList )
			{
				...
				//{
					...
					Util.out4.println("\t The attribute named " + att_name + " was hired to the Collector list...");
...
//					 informs the mother class that one new attribute must be managed
		...
		catch ( ConnectionException e )
		{
			...
			String reason = GlobalConst.TANGO_COMM_EXCEPTION;
			String desc = "Failed while executing " + this.getClass().getSimpleName() + ".addSource() method...";
			throw new ArchivingException(message , reason , ErrSeverity.WARN , desc , "" , e);
		}
	    catch ( Exception e )
	    {
...
//	--------------------------------------------------------------------------//	
	private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			...
			if(attribute == null)
			{
				System.out.println("\033[1;31mNumberScalar.java: the attribute \"" +
		...
		{
			System.out.println("\033[1;31mNumberScalar.java: Failed to get the attribute from the AttributeFactory\033[0m");
		...
		System.out.print("\033[1;42mregistering\033[0m the scalar attribute \"" + attribute.getNameSansDevice() + 
		"\"...\t");
		System.out.flush();
	...
	{
		System.out.print("number: \033[1;42mremoving\033[0m source for \"" + attributeName + "\"...\t");
...
//	--------------------------------------------------------------------------//	
	...
	public void processEventScalar(ScalarEvent scalarEvent , int try_number)
	{
        ...
        //System.out.println ( "processEventScalar/getAttribute_complete_name/"+scalarEvent.getAttribute_complete_name()+"/timeCondition/"+timeCondition );
        if ( ! timeCondition )
        {
	...
	public void errorChange(ErrorEvent errorEvent)
	{
	    ...
	    /* Must be change beacause the number scalar are not only double */
        try
        {
