...
public class StringScalar_RW extends StringScalar {
	private static final long serialVersionUID = 5000899893L; /*
															 * Recommended in
															 * java 5
															 */
	...
	public void archive(TangoArchiveEvent event) {
		...
		try {
			...
			System.out
					.println("\033[1;35marchive() [String Scalar RW]: [EVENT]: \033[1;32m"
			...
			System.out.println("\033[1;35m(R): " + svalue[0]
					+ "\033[0m, (Set):\033[1;36m " + svalue[1] + "\033[0m");
			scalarEvent.setValue(svalue); /* complete the scalarEvent */

			processEventScalar(scalarEvent, tryNumber);
		} catch (DevFailed devFailed) {
			print_exception("StringScalar_RW.archive() : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ scalarEvent.getAttribute_complete_name() + " values...",
					devFailed);
			...
			System.err.println("StringScalar_RW.archive() : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ scalarEvent.getAttribute_complete_name() + " values...");
			exE.printStackTrace();
