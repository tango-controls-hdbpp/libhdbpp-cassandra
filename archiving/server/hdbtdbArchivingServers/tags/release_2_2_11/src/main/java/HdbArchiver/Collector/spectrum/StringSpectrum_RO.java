...
public class StringSpectrum_RO extends HdbCollector implements IStringSpectrumListener,
	...
	try {
	    ...
	    System.out
		    .println("\033[1;31mStringStringSpectrum_RO.java: Failed to get the attribute from the AttributeFactory\033[0m");
	...
	try {
	    ...
	    System.out
		    .println("StringSpectrum_RO.java: archive(): Error getting archive event attribute value");
	    f.printStackTrace();
	    return;
	} catch (final Exception e) /* Shouldn't be reached */
	{
	    System.out
		    .println("StringSpectrum_RO.java.archive.getValue() failed, caught generic Exception, code failure");
	    e.printStackTrace();
	    return;
	}
	try {
		...
		System.out
			.println("\033[1;31mBooleanStringSpectrum_RO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
		return;
	    }
	    ...
	    System.err.println("BooleanStringSpectrum_RO.java: archive() : "
		    + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading "
		    + spectrumEvent_ro.getAttribute_complete_name() + " values...");
	    ...
	    System.err.println("Boolean.StringSpectrum_RO.java: archive : "
		    + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading "
		    + spectrumEvent_ro.getAttribute_complete_name() + " values...");
	    exE.printStackTrace();
	    final Object value = null;
    ...
    public void processEventSpectrum(final SpectrumEvent_RO snapSpectrumEvent_RO, int try_number) {
	final boolean timeCondition = super.isDataArchivableTimestampWise(snapSpectrumEvent_RO);
	if (!timeCondition) {
	    return;
	}
