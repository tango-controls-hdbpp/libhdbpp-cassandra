...
{
	public Image_RO(TdbModeHandler modeHandler)
	{
		super(modeHandler);
	}
		...
		try
		{
			synchronized ( attributeList )
			{
				...
				{*/
					...
					String table_name = super.dbProxy.getDataBase().getDbUtil().getTableName(attributeLightMode.getAttribute_complete_name(), super.dbProxy.getDataBase().getAttribute());
	...
	public void processEventImage(ImageEvent_RO imageEvent_ro , int try_number)
	{
        ...
        boolean timeCondition = super.isDataArchivableTimestampWise ( imageEvent_ro );
        if ( !timeCondition )
        {
            return;
        }
		...
		try
		{
			( ( FileTools ) filesNames.get(imageEvent_ro.getAttribute_complete_name()) ).processEventImage(imageEvent_ro);
