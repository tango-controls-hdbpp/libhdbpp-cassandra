...
public class NumberSpectrum_RW extends HdbCollector implements
	...
	private void addSourceForEvents(AttributeLightMode attributeLightMode)
		...
		try {
			...
			if (attribute == null) {
				System.out
						.println("\033[1;31mNumberSpectrum_RW.java: the attribute \""
								+ attributeLightMode
	...
	private void removeSourceForEvents(String attributeName) {
		Util.out2.println("NumberSpectrum_RW.removeSource");
		System.out
				.print("\033[0;44mremoving\033[0m source for \033[4mspectrum\033[0m [RW]:\""
						+ attributeName + "\"...\t");
		...
		} catch (ConnectionException e) /* getAttribute() failed */
		{
			...
			print_exception(
					"Error removing tangoArchiveListener for the attribute "
							+ attributeName, f);
		}
		// }
	}
	...
	public void archive(TangoArchiveEvent event) {
		...
		try {
			...
			System.err.println("Spectrum_RW.java: archive : "
					+ GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t"
					+ "Problem while reading "
					+ spectrumEvent_rw.getAttribute_complete_name()
					+ " values...");
	...
	public void errorChange(ErrorEvent errorEvent) {
		int tryNumber = DEFAULT_TRY_NUMBER;
		Util.out3.println("Spectrum_RO.errorChange : "
	...
	public void processEventSpectrum(SpectrumEvent_RW snapSpectrumEvent_RW,
		...
		// lastValue
		{
			...
			Util.out4
					.println("SpectrumEvent_RW.processEventSpectrum/ArchivingException");

			try_number--;
			if (try_number > 0) {
				Util.out2
						.println("SpectrumEvent_RW.processEventSpectrum : \r\n\ttry "
								+ (try_number) + "failed...");
