...
public class DbProxy {
	...
	public DbProxy(ILogger logger) {
		try {
			...
			this.manager.ArchivingConfigure(TdbArchiver.TdbArchiver.myDbHost,
					TdbArchiver.TdbArchiver.myDbName,
					TdbArchiver.TdbArchiver.myDbUser,
					TdbArchiver.TdbArchiver.myDbPassword,
					TdbArchiver.TdbArchiver.myRacConnection, false, false);
			this.logger = logger;
			((ITdbDataExport) ((TDBDataBaseManager) this.manager.getDataBase())
					.getTdbExport()).setLogger(this.logger);
	...
	public void exportToDB_Scalar(String remoteFilePath, String fileName,
		...
		((ITdbDataExport) ((TDBDataBaseManager) this.manager.getDataBase())
				.getTdbExport()).exportToDB_Data(remoteFilePath, fileName,
				tableName, writable);
	}

	public void exportToDB_Spectrum(String remoteFilePath, String fileName,
			String tableName, int writable) throws ArchivingException {
	...
	public void exportToDB_Image(String remoteFilePath, String fileName,
			String tableName, int writable) throws ArchivingException {
	...
	public Vector<AttributeLightMode> getArchiverCurrentTasks(String archiverName) // throws
	// ArchivingException
	// , DevFailed
	{
		...
		try {
			archiverCurrentTasks = this.manager.getDataBase().getMode()
	...
	public void deleteOldRecords(long time, String[] attributeList)
			throws ArchivingException {
		((TDBDataBaseManager) this.manager.getDataBase())
				.getTdbDeleteAttribute().deleteOldRecords(time, attributeList);
	}
}