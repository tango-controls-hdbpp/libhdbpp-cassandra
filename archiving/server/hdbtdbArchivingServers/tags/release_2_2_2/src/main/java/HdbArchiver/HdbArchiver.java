...
/* WithShutdownRunnable */implements TangoConst, Warnable {
	 ...
	 */
	// =========================================================
	HdbArchiver(DeviceClass cl, String s, String d, String version)
			throws DevFailed {
		super(cl, s, d);
		m_version = version;
		init_device();
	...
	public void init_device() throws DevFailed {
		...
		this.dbProxy = new DbProxy(logger);
	...
	private void startArchiving(ArchivingMessConfig archivingMessConfig,
		...
		Thread startArchivingThread = new Thread(startArchivingRunnable,
				"HDBStartArchivingThread_" + device_name);
	...
	private void setControlMode(String _safetyPeriod) throws DevFailed {
		...
		get_logger().info(
				"setControlMode/success/desription/"
	...
	public void get_device_property() throws DevFailed {
		...
		String[] propnames = { "DbHost", "DbName", "DbUser", "DbPassword",
				"RacConnection", "SafetyPeriod", "HasThreadedStartup",
				"IsDedicated", "ReservedAttributes", "DiaryPath", "HasDiary",
				"DiaryLogLevel",
				// --------------------------------------------------------------------------//
				// ELETTRA : Archiving Events
				// --------------------------------------------------------------------------//
		...
		if (dev_prop[++i].is_empty() == false)
			...
			DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
			if (cl_prop.is_empty() == false)
				mySafetyPeriod = cl_prop.extractString();
		}
		...
		System.out.println("The database priority is set to "
				+ eventDBUpdatePolicy);
	 ...
	 */
	// =========================================================
	public void trigger_archive_conf(String[] argin) throws DevFailed {
		set_state(DevState.RUNNING);
		set_status(device_name + " : " + "DevState.RUNNING");
		...
		archivingMessConfig.filter(this.isDedicated, this.reservedAttributes,
				this.logger);
		for (Enumeration<String> my_EnumAttributes = archivingMessConfig
			...
			try {
				...
				if (att_support) {
					try {
						...
						// Some attributes can be archived under 10 s
						try {
							attributeLightMode.getMode().checkMode(
									att_complete_name,
									attributeLightMode.getMode().getModeP()
											.getPeriod());
							if (attributeLightMode.getMode().getModeD() != null) {
								attributeLightMode.getMode().checkMode(
										att_complete_name,
										attributeLightMode.getMode().getModeD()
												.getPeriod());
							}
							if (attributeLightMode.getMode().getModeA() != null) {
								attributeLightMode.getMode().checkMode(
										att_complete_name,
										attributeLightMode.getMode().getModeA()
												.getPeriod());
							}
							if (attributeLightMode.getMode().getModeR() != null) {
								attributeLightMode.getMode().checkMode(
										att_complete_name,
										attributeLightMode.getMode().getModeR()
												.getPeriod());
							}
							if (attributeLightMode.getMode().getModeT() != null) {
								attributeLightMode.getMode().checkMode(
										att_complete_name,
										attributeLightMode.getMode().getModeT()
												.getPeriod());
							}
							if (attributeLightMode.getMode().getModeC() != null) {
								attributeLightMode.getMode().checkMode(
										att_complete_name,
										attributeLightMode.getMode().getModeC()
												.getPeriod());
							}
						} catch (ArchivingException e) {
					...
					if (this.dbProxy.isArchived(att_complete_name)) {
						if (this.dbProxy.isArchived(att_complete_name,
								device_name)) {
							try {
								// collectorFactory.removeAllForAttribute(
								...
								this.dbProxy
										.updateModeRecord(att_complete_name);
							} catch (ArchivingException ae) {
								this.logger
										.trace(
												ILogger.LEVEL_ERROR,
												"Failed while executing HdbArchiver.trigger_archive_conf() method : removeAllForAttribute or updateModeRecord");
								ae.printStackTrace();
								continue;
							}
						} else {
					...
					Integer period = new Integer(attributeLightMode.getMode()
							.getModeP().getPeriod());
					this.dbProxy.setPeriodForAttribute(att_complete_name,
							period);
					...
					if(m_collector == null){
						...
						collectorFactory.createCollectorAndAddSource(attributeLightMode, this.logger,
								this.dbProxy);
						this.logger.trace(ILogger.LEVEL_INFO,
							"===> Exiting createCollectorAndAddSource ");
					}
					else {
						this.logger.trace(ILogger.LEVEL_INFO, "===> Entering addSource ");
						m_collector.addSource(attributeLightMode);
...
//				attributeLightMode.getMode().checkMode(true);
//				mode_valid = true;
//				collector = collectorFactory.get(attributeLightMode,
//						this.logger, true, this.dbProxy);
...
//			String desc = "Unable to archive the attribute named "
//					+ attributeLightMode.getAttribute_complete_name();
//			if (!att_support)
//				desc = desc
//						+ "\r\n"
...
//				// Correction des donnï¿½es de la base : Cas oï¿½ les
//				// ï¿½quipements ne tournent pas au redï¿½marrage de l'archiver.
//				try {
...
//		get_logger().info("Exiting trigger_archive_att()");
	 ...
	 */
	// =========================================================
	public void stop_archive_conf(String[] argin) throws DevFailed {
		set_state(DevState.RUNNING);
		set_status(device_name + " : " + "DevState.RUNNING");
		...
		Vector<AttributeLightMode> myConf = ArchivingManagerApiRef
				.stoppingVector(argin);
	 ...
	 */
	// =========================================================
	public short retry_for_attribute(String attributeToRetry) {
		if (attributeToRetry == null || attributeToRetry.equals("")) {
			return NA;
		}
		try {
			Vector<AttributeLightMode> myCurrentTasks = this.dbProxy
					.getArchiverCurrentTasks(device_name);
			...
			get_logger().warn(
					"Failed during retry_for_attribute ( " + attributeToRetry
							+ " ):");
	 ...
	 */
	// =========================================================
	// A priori la commande est déjà locké au niveau
	...
	public/* synchronized */short retry_for_attributes(String[] attributesToRetry) {
		...
		try {
			Vector<AttributeLightMode> myCurrentTasks = this.dbProxy
					.getArchiverCurrentTasks(device_name);
			Hashtable<String, AttributeLightMode> modes = new Hashtable<String, AttributeLightMode>(myCurrentTasks.size());
			...
			String message = DBTools.getCompleteMessage(e);
			get_logger().warn(
					"Failed during retry_for_attributes ( "
							+ argsBuffer.toString() + " ):");
			get_logger().warn(message);
	 ...
	 */
	// =========================================================
	public short retry_for_ko_attributes() {
		try {
			Vector[] listOfControlledAttributes = this
	 ...
	 */
	// =========================================================
	public boolean is_attribute_correctly_archived_internal(String argin)
			throws DevFailed {
	 ...
	 */
	// =========================================================
	public String[] get_errors_for_domain_internal(String argin)
			throws DevFailed {
		...
		while (keys.hasMoreElements()) {
			...
			ArchivingAttribute attribute = (ArchivingAttribute) _errorAttributes
					.get(key);
	...
	public String[] get_error_domains_internal() throws DevFailed {
		...
		Hashtable<String, Object> _errorDomains = _controlResult.getErrorDomains();
		String[] argout = new String[_errorDomains.size()];
		Enumeration<String> keys = _errorDomains.keys();
		int i = 0;
	 ...
	 */
	// =========================================================
	public String[] get_errors_for_attribute_internal(String argin)
			throws DevFailed {
		ControlResult _controlResult = null;
		// System.out.println ( "CLA--------------------------1" );
		try {
	 ...
	 * @throws DevFailed
	 */
	private Vector[] getControlledAttributes() throws ArchivingException,
			DevFailed {
		try {
			Vector<AttributeLightMode> myCurrentTasks = this.dbProxy
					.getArchiverCurrentTasks(device_name);
			Vector<String> koAttrs = new Vector();
			Vector<String> okAttrs = new Vector();

			for (int i = 0; i < myCurrentTasks.size(); i++) {
				AttributeLightMode mode = (AttributeLightMode) myCurrentTasks
						.elementAt(i);
	...
	public short retry_for_all() {
		try {
			...
			ArchivingMessConfig archivingMessConfig = ArchivingMessConfig
					.basicObjectCreation();
			for (int i = 0; i < myCurrentTasks.size(); i++) {
				archivingMessConfig.add((AttributeLightMode) myCurrentTasks
						.elementAt(i));
			}
	...
	// Call by commands which are synchronized
	private/* synchronized */void computeLoads() {
		// TODO : suppress the synchronization of the HdbArchiver class
	 ...
	 */
	// =========================================================
	public void stop_archive_att(String[] argin) throws DevFailed {
		set_state(DevState.RUNNING);
		set_status(device_name + " : " + "DevState.RUNNING");
		...
		try {
			...
			if (hdbCollector != null) {
				hdbCollector.removeSource(attributeLightMode
						.getAttribute_complete_name());
