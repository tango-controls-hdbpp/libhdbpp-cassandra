...
public class TdbCollectorFactory {
	private Hashtable<SuperMode, TdbCollector> tableCollector = null;
	private ILogger m_logger;

	private String m_currentDbPath = "";
	private String m_currentDsPath = "";

	public TdbCollectorFactory(ILogger logger, String dsPath, String dbPath) {
		tableCollector = new Hashtable<SuperMode, TdbCollector>();
...
//	}
	...
	/**
	 * This method create a new TdbCollector instance if required associated
	 * to an attribute. In this method, attributes are not grouped by mode.
	 *
	 * @param attributeLightMode
	 *            Attribute associated to the looked collector.
	...
	public void createCollectorAndAddSource(
			AttributeLightMode attributeLightMode, DbProxy dbProxy)
			throws ArchivingException {
	 ...
	 *         attribute.
	 */
	private TdbCollector create(AttributeLightMode attributeLightMode)
			throws ArchivingException {
		...
		if (AttributeSupport.checkAttributeSupport(name, data_type,
			...
			case AttrDataFormat._IMAGE: // IMAGE
				collector = new Image_RO(modeHandler, m_currentDsPath,
						m_currentDbPath);
				break;
			default:
	...
	public String factoryAssessment() {
		...
		if (tableCollector != null) {
			if (!tableCollector.isEmpty()) {
				...
				synchronized (tableCollector) {
					Collection<TdbCollector> collectors = tableCollector
							.values();
					int i = 1;
					int size = collectors.size();
					for (Iterator<TdbCollector> iterator = collectors
