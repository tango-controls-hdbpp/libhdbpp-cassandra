...
public class HdbArchiverClass extends DeviceClass implements TangoConst {
    ...
    //
    // ===================================================================
    public static HdbArchiverClass init(String class_name) throws DevFailed {
        if (_instance == null) {
            try {
    ...
    public void command_factory() {
        command_list
                .addElement(new GetErrorDomainsInternal(
                        "GetErrorDomainsInternal",
                        Tango_DEV_VOID,
        ...
        command_list.addElement(new GetErrorsForDomainInternal(
                "GetErrorsForDomainInternal", Tango_DEV_STRING,
                Tango_DEVVAR_STRINGARRAY, "The domain name",
                "The list of all KO attributes for this domain",
                DispLevel.OPERATOR));

        command_list.addElement(new GetErrorsForAttributeInternal(
                "GetErrorsForAttributeInternal", Tango_DEV_STRING,
                Tango_DEVVAR_STRINGARRAY,
                "The attribute name (NOT the complete name)",
                "The list of all KO attributes sharing this name",
                DispLevel.OPERATOR));

        command_list.addElement(new IsAttributeCorrectlyArchivedInternal(
                "IsAttributeCorrectlyArchivedInternal", Tango_DEV_STRING,
                Tango_DEV_BOOLEAN, "The complete name of the attribute",
                "True if this attribute is archiving correctly",
                DispLevel.OPERATOR));
        ...
        command_list.addElement(new TriggerArchiveConfCmd("TriggerArchiveConf",
                ...
                "The group of attributes to archive", "", DispLevel.OPERATOR));
        command_list.addElement(new TriggerArchiveAttCmd("TriggerArchiveAtt",
                Tango_DEVVAR_STRINGARRAY, Tango_DEV_VOID,
                "The attribute to archive", "", DispLevel.OPERATOR));
        command_list
                .addElement(new TriggerArchiveAttCheckCmd(
                        "TriggerArchiveAttCheck", Tango_DEV_STRING,
                        Tango_DEV_VOID, "The name of the attribute to archive",
                        "", DispLevel.OPERATOR));
        ...
        command_list.addElement(new StateDetailedClass("StateDetailed",
                Tango_DEV_VOID, Tango_DEV_STRING, "", "The detailed state",
                DispLevel.EXPERT));
    ...
    public void device_factory(String[] devlist) throws DevFailed {
        ...
        for (int i = 0; i < devlist.length; i++) {
            ...
            device_list.addElement(new HdbArchiver(this, devlist[i],
                    device_version));

            // Export device to the outside world
            // ----------------------------------------------
            if (Util._UseDb == true) export_device(((DeviceImpl) (device_list
                    .elementAt(i))));
            else export_device(((DeviceImpl) (device_list.elementAt(i))),
                    devlist[i]);
        }
    }
    ...
    public void attribute_factory(Vector att_list) throws DevFailed {
        ...
        Attr scalar_charge = new Attr("scalar_charge", Tango_DEV_SHORT,
                AttrWriteType.READ);
        att_list.addElement(scalar_charge);

        // Attribute : spectrum_charge
        Attr spectrum_charge = new Attr("spectrum_charge", Tango_DEV_SHORT,
                AttrWriteType.READ);
        att_list.addElement(spectrum_charge);

        // Attribute : image_charge
        Attr image_charge = new Attr("image_charge", Tango_DEV_SHORT,
                AttrWriteType.READ);
    ...
    public DbDatum get_class_property(String name) {
        for (int i = 0; i < cl_prop.length; i++)
            if (cl_prop[i].name.equals(name)) return cl_prop[i];
    ...
    public void get_class_property() throws DevFailed {
        ...
        if (cl_prop[++i].is_empty() == false) dbHost = cl_prop[i]
                .extractString();
        else cl_prop[i].insert(dbHost);

        // Extract DbName value
        if (cl_prop[++i].is_empty() == false) dbName = cl_prop[i]
                .extractString();
        else cl_prop[i].insert(dbName);

        // Extract Facility value
        if (cl_prop[++i].is_empty() == false) facility = cl_prop[i]
                .extractBoolean();
        else cl_prop[i].insert(facility);

        // Extract RacConnection value
        if (cl_prop[++i].is_empty() == false) RacConnection = cl_prop[i]
                .extractBoolean();
        else cl_prop[i].insert(RacConnection);
