...
public abstract class TdbCollector extends ArchiverCollector {
	...
	public TdbCollector(TdbModeHandler _modeHandler, String currentDsPath, String currentDbPath) {
		super(_modeHandler);
	...
	public String exportFile2Db(String attributeName, boolean isAsynchronous) {
		...
		return ((FileTools) filesNames.get(attributeName))
				.swapFile(isAsynchronous);
	}
...
//	}
	...
	private class KeepingThread extends Thread {
		long keeping_time = m_modeHandler.getMode().getTdbSpec()
				.getKeepingPeriod();
		...
		public void run() {
			...
			while (state.equals("RUNNING")) {
				...
				Util.out4
						.println("KeepingThread.run : Getting attribute list...");
				...
				for (int i = 0; i < m_attributeList.length; i++) {
					m_attributeList[i] = ((IEntity) tmpAttributeList
							.getElementAt(i)).getName();
				}
				try {
					Util.out4
							.println("KeepingThread.run : Sending the attribute list to the database...");
					TdbCollector.this.dbProxy.deleteOldRecords(keeping_time,
							m_attributeList);
	...
	protected boolean isDataArchivableTimestampWise(ArchivingEvent scalarEvent) {
		...
		if (newTime == 0) {
			String message = "TdbCollector/isDataArchivableTimestampWise/Received a zero timestamp ("
					+ newTime
					+ " = "
		...
		if (isAlreadyRegisteredDate) {
			String message = "TdbCollector/isDataArchivableTimestampWise/FALSE for attribute|"
		...
		boolean isValidRegisteredDate = lastTimestampStack.validateDate(
				newTime, this.m_logger);
		// System.out.println (
		// "isDataArchivableTimestampWise/isAlreadyRegisteredDate/"+isAlreadyRegisteredDate);

		if (!isValidRegisteredDate) {
			String message = "TdbCollector/isDataArchivableTimestampWise/Invalid Timestamp for attribute|"
					+ name + "|and timestamp|" + new Timestamp(newTime) + "|";
			this.m_logger.trace(ILogger.LEVEL_DEBUG, message);
			Util.out1.println(message);
