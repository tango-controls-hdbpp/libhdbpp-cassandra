...
public class DbProxy implements IDbProxy {
	...
	public DbProxy() {
		try {
			...
			this.manager.ArchivingConfigure(HdbArchiver.HdbArchiver.myDbHost,
					...
					HdbArchiver.HdbArchiver.myRacConnection);
	...
	private void updateCountersForAttributePreInsert(
		...
		this.lastInsertRequests.put(completeName, new Long(archivingEvent
				.getTimeStamp()));
	}
	...
	private void updateCountersForAttributePostInsert(
		...
		this.lastInserts.put(completeName, new Long(archivingEvent
				.getTimeStamp()));
	}
	...
	/*
	 * public boolean hasBeenArchiving ( String completeName ) { Long
	 * currentCounter_I = (Long) this.counters.get ( completeName ); long
	 * currentCounter = currentCounter_I == null ? 0 :
	 * currentCounter_I.longValue (); boolean ret = ( currentCounter != 0 );
	 * System.out.println (
	 * "hasBeenArchiving/completeName/"+completeName+"/ret/"+ret ); return ret;
	 * }
	 */
