...
public abstract class StringScalar extends TdbCollector implements IStringScalarListener
{
    ...
    public StringScalar ( TdbModeHandler _modeHandler )
    {
        super( _modeHandler );
            ...
            throws ArchivingException
    {
        try
        {
            ...
            {
                /*while ( ( IStringScalar ) attributeList.get(attributeLightMode.getAttribute_complete_name()) == null )
                {*/
                    ...
                    String table_name = super.dbProxy.getDataBase().getDbUtil().getTableName(att_name, super.dbProxy.getDataBase().getAttribute());
    ...
    public synchronized void removeSource ( String attributeName ) throws ArchivingException
    {
        ...
        try
        {
            synchronized ( attributeList )
            {
               /* while ( ( IStringScalar ) attributeList.get(attributeName) != null )
                {*/
                IStringScalar attribute = ( IStringScalar ) attributeList.get(attributeName);
    ...
    public void processEventScalar(ScalarEvent scalarEvent , int try_number)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    super.m_logger.trace(ILogger.LEVEL_WARNING, "StringScalar/processEventScalar/scalarEvent.getReadValue() == null/for attribute/"+scalarEvent.getAttribute_complete_name());
                }
                if ( getLastValue(scalarEvent) == null ) 
                {
                    super.m_logger.trace(ILogger.LEVEL_WARNING, "StringScalar/processEventScalar/getLastValue(scalarEvent) == null/for attribute/"+scalarEvent.getAttribute_complete_name());
                }*/
	...
	public void errorChange(ErrorEvent errorEvent)
	{
	    ...
	    try
	    {
	    	processEventScalar(getNullValueScalarEvent(errorEvent,TangoConst.Tango_DEV_STRING,getWritableValue()), tryNumber);
	    }
	    catch(Exception e)
	    {
