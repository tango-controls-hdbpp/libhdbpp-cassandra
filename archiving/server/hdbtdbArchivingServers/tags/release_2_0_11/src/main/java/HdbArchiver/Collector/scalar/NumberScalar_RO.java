...
//			--------------------------------------------------------------------------//
			switch ( scalarEvent.getData_type() )
			{
				case TangoConst.Tango_DEV_SHORT:
					scalarEvent.setValue(new Short((short)event.getNumberSource().getNumberScalarValue()));
					break;
				case TangoConst.Tango_DEV_USHORT:
					scalarEvent.setValue(new Short((short)event.getNumberSource().getNumberScalarValue()));
					break;
				case TangoConst.Tango_DEV_LONG:
					scalarEvent.setValue(new Integer((int)event.getNumberSource().getNumberScalarValue()));
					break;
				case TangoConst.Tango_DEV_ULONG:
					scalarEvent.setValue(new Integer((int)event.getNumberSource().getNumberScalarValue()));
					break;
				case TangoConst.Tango_DEV_DOUBLE:
					scalarEvent.setValue(new Double(event.getNumberSource().getNumberScalarValue()));
					break;
				case TangoConst.Tango_DEV_FLOAT:
					scalarEvent.setValue(new Float((float)event.getNumberSource().getNumberScalarValue()));
					break;
				default:
					scalarEvent.setValue(new Double(event.getNumberSource().getNumberScalarValue()));
					break;
		...
		{
			System.err.println("NumberScalar_RO.numberScalarChange : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + scalarEvent.getAttribute_complete_name() + " values...");
...
//	--------------------------------------------------------------------------//
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		{
			System.out.println("NUMBER SCALAR READ ONLY: Error getting archive event value");
			System.out.println();
			print_exception(f);
			return;
		}
		...
		{
			System.out.println("NumberScalar_RO.archive.getValue() failed, caught generic Exception, code failure");
			e.printStackTrace();
			return;
		}
		...
		try{
			...
			{
				System.out.println("\033[1;31mNumberScalar_RO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
