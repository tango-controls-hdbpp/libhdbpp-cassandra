...
public class ConfigBuilder
{
	...
	public ArrayList getConfig(String[] argin)
	{
		...
		ArchivingConfig archivingConfig = new ArchivingConfig(argin);
		Mode mode = archivingConfig.getMode();
		String deviceIncharge = archivingConfig.getDevice_in_charge();
		Enumeration attributeKeys = archivingConfig.getAttributeListKeys();

		while ( attributeKeys.hasMoreElements() )
		{
			String attributeKey = ( String ) attributeKeys.nextElement();
			AttributeLight attributeLight = archivingConfig.getAttribute(attributeKey);
			AttributeLightMode attributeLightMode = new AttributeLightMode(attributeLight , deviceIncharge , mode);
			Timestamp timestamp = new Timestamp(System.currentTimeMillis());
			attributeLightMode.setTrigger_time(timestamp);

			try
			{
				DbProxy.getInstance().insertModeRecord(attributeLightMode);
				arrayList.add(attributeLightMode);
			}
