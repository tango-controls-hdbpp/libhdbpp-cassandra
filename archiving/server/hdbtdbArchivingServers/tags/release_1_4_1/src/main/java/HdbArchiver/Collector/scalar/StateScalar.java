...
public abstract class StateScalar extends HdbCollector implements IDevStateScalarListener, ITangoArchiveListener
{
    ...
    public StateScalar ( HdbModeHandler _modeHandler )
    {
        super( _modeHandler );
	...
	{
		Util.out2.println("StateScalar.removeSource");
...
//					informs the mother class that one new attribute must be removed
	...
	private void addSourceForPolling ( AttributeLightMode attributeLightMode ) throws ArchivingException
	{
		try
		{
			...
			synchronized ( attributeList )
			{
				//while ( ( IDevStateScalar ) attributeList.get(att_name) == null )
				//{
...
//	--------------------------------------------------------------------------//	
    private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			 ...
			 */
			attribute = (IDevStateScalar) AttributeFactory.getInstance().
			getAttribute(attributeLightMode.getAttribute_complete_name());
			if(attribute == null)
			{
				System.out.println("\033[1;31mStateScalar.java: the attribute \"" +
						attributeLightMode.getAttribute_complete_name() +
				" is null (StateScalar.java): not adding it!");
				return;
			}
		}
		catch (DevFailed e)
		{
			System.out.println("\033[1;31mStateScalar.java: Failed to get the attribute from the AttributeFactory\033[0m");
...
//	--------------------------------------------------------------------------//	
    public void processEventScalar(ScalarEvent scalarEvent , int try_number)
    {
        ...
        {
            String message = "Problem storing StateScalar value";
