...
/* WithShutdownRunnable */implements TangoConst, Warnable {
	...
	if (dev_prop[++i].is_empty() == false) {
	    ...
	    System.out.println("TdbArchiver/get_device_property/2/" + get_name() + "/isDedicated/"
		    + isDedicated);
	} else {
	    // Try to get value from class property
	    final DbDatum cl_prop = ds_class.get_class_property(dev_prop[i].name);
	...
	for (final Enumeration<String> my_EnumAttributes = archivingMessConfig
		...
		if (att_support) {
			...
			final String desc = "Invalid mode for attribute " + att_complete_name;
			...
			logger.trace(ILogger.LEVEL_ERROR, "Insertion failed in AMT for the att : "
				+ att_complete_name + " with following exception");
		    ...
		    // NON REPRIS m_collector.setWarnable(this);
		}
	    } catch (final ArchivingException e) {
		e.printStackTrace();
		final String message = GlobalConst.ARCHIVING_ERROR_PREFIX + " : ";
	...
	// String reason =
	// "Failed while executing TdbArchiver.trigger_archive_att() method...";
	// String desc = (att_support) ?
	// "Unable to archive the attribute named "
	...
	try {
	    ...
	    if (collector != null) {
		tableName = collector.exportFile2Db(
			attributeLightMode.getAttribute_complete_name(), false);
	    } else {
