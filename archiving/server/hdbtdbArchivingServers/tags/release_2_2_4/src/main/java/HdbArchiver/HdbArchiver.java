...
/* WithShutdownRunnable */implements TangoConst, Warnable {
	...
	if (!dbProxy.is_db_connected()) {
		...
		if (myCurrentTasks.size() > 0) {
		    ...
		    startArchiving(archivingMessConfig, false);
		}
	...
	for (final Enumeration<String> my_EnumAttributes = archivingMessConfig
		...
		if (attSupport) {
			...
			final String desc = "Invalid mode for attribute " + attCompleteName;
			archivingException.addStack(message, reason, ErrSeverity.PANIC, desc, "",
				ae);
			get_logger().error(attributeName + " error " + reason + " " + desc);
		    ...
		    if (dbProxy.isArchived(attCompleteName)) {
			get_logger().debug(attCompleteName + " is already archived, nothing done");
			if (dbProxy.isArchived(attCompleteName, device_name)) {
			    get_logger().debug(
				    attCompleteName + " is already archived by me, remove");
			    try {
				...
				get_logger().error(attributeName + " error " + ae.getMessage());
				logger
					.trace(
						ILogger.LEVEL_ERROR,
			...
			logger.trace(ILogger.LEVEL_ERROR,
				"############################################################");
			logger.trace(ILogger.LEVEL_ERROR, "Insertion failed in AMT for the att : "
				+ attCompleteName + " with following exception");
			...
			get_logger().debug(attCompleteName + " create collector");
    ...
    // =========================================================
    @Deprecated
    public synchronized void trigger_archive_att(final String[] argin) throws DevFailed {
	// set_state(DevState.RUNNING);
	// set_status(device_name + " : " + "DevState.RUNNING");
	//
	...
	// String reason =
	// "Failed while executing HdbArchiver.trigger_archive_att() method...";
    ...
    // =========================================================
    @Deprecated
    public synchronized void trigger_archive_att_check(final String argin) throws DevFailed {
	set_state(DevState.RUNNING);
	set_status(device_name + " : " + "DevState.RUNNING");
     ...
     */
    // =========================================================
    public short retry_for_attribute(final String attributeToRetry) {
	...
	try {
	    ...
	    final DevFailed toThrow = DBTools.getAsDevFailed(t);
	    throw toThrow;
	}
    }
	...
	final Hashtable<String, Domain> _errorDomains = _controlResult.getErrorDomains();
	final String[] argout = new String[_errorDomains.size()];
	final Enumeration<String> keys = _errorDomains.keys();
	int i = 0;
