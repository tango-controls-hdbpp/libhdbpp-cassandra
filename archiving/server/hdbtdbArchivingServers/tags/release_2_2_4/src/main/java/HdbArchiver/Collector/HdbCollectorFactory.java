...
public class HdbCollectorFactory {
	private Hashtable<SuperMode, HdbCollector> tableCollector = null;

	public HdbCollectorFactory() {
		tableCollector = new Hashtable<SuperMode, HdbCollector>();
	}
...
//	 *         attribute.
//	 */
//	public HdbCollector get(AttributeLightMode attributeLightMode,
//			ILogger logger, boolean doCreateCollectorIfMissing, DbProxy dbProxy)
...
//	}
	...
	/**
	 * This method create a new HdbCollector instance if required associated
	 * to an attribute. In this method, attributes are not grouped by mode.
	 *
	 * @param attributeLightMode
	 *            Attribute associated to the looked collector.
	 */
	public void createCollectorAndAddSource(
			AttributeLightMode attributeLightMode, ILogger logger,
			DbProxy dbProxy) throws ArchivingException {
	 ...
	 *         attribute.
	 */
	public HdbCollector create(AttributeLightMode attributeLightMode)
			throws ArchivingException {
		...
		HdbModeHandler modeHandler = new HdbModeHandler(attributeLightMode
				.getMode());
		HdbCollector collector = null;
	...
	public/* synchronized */void destroy(AttributeLightMode attributeLightMode) {
		SuperMode superMode = new SuperMode(
				attributeLightMode.getData_format(), attributeLightMode
						.getData_type(), attributeLightMode.getWritable(),
				attributeLightMode.getMode());
	...
	public/* synchronized */void removeAllForAttribute(
		...
		synchronized (tableCollector)// CLA 15/06/06
		{
			...
			HdbCollector collector = (HdbCollector) tableCollector
					.get(superMode);
			if (collector != null) {
				// collector.
	 ...
	 * (SuperMode)keyIterator.next(); HdbCollector collector = ( HdbCollector )
	 * tableCollector.get(superMode); collector.removeSource(att_name); if
	 * (collector.hasEmptyList()) { toRemove.add(superMode); } } Iterator
	 * toRemoveIterator = toRemove.iterator(); while(toRemoveIterator.hasNext())
	 * { tableCollector.remove(toRemoveIterator.next()); } } }
	 */
	...
	public String factoryAssessment() {
		...
		// synchronized (tableCollector)// CLA 15/06/06
		// sup synchro because an exception will be raised if the tablecollector
		// is null
		// {
		...
		if (tableCollector != null) {
			...
			if (!tableCollector.isEmpty()) {
				...
				synchronized (tableCollector) {
					Collection<HdbCollector> collectors = tableCollector
							.values();
					int i = 1;
					int size = collectors.size();
					for (Iterator<HdbCollector> iterator = collectors
