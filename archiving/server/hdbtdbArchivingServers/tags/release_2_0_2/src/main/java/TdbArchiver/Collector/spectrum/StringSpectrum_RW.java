...
public class StringSpectrum_RW extends TdbCollector implements IStringSpectrumListener
{
	public StringSpectrum_RW(TdbModeHandler modeHandler)
	{
		super(modeHandler);
	}
		...
		try
		{
			synchronized ( attributeList )
			{
				...
				{*/
                    ...
                    String table_name = super.dbProxy.getDataBase().getDbUtil().getTableName(attributeLightMode.getAttribute_complete_name(), super.dbProxy.getDataBase().getAttribute());					FileTools myFile = new FileTools(table_name , attributeLightMode.getData_format() , attributeLightMode.getWritable() , attributeLightMode.getMode().getTdbSpec().getExportPeriod() , super.m_logger , true , super.dbProxy );
					myFile.initialize();
	...
	public void errorChange(ErrorEvent errorEvent)
	{
		...
		spectrumEvent_rw.setAttribute_complete_name(( ( IStringSpectrum ) errorEvent.getSource() ).getName());
