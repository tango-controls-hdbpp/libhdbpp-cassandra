...
//	--------------------------------------------------------------------------//	
    private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			...
			if(attribute == null)
			{
				System.out.println("\033[1;31mSpectrum_RW.java: the attribute \"" +
						attributeLightMode.getAttribute_complete_name() +
		...
		{
			System.out.println("\033[1;31mSpectrum_RW.java: Failed to get the attribute from the AttributeFactory\033[0m");
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		try{
			...
			{
				System.out.println("\033[1;31mBooleanSpectrum_RW.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
