...
public class BooleanScalar_RO extends BooleanScalar
{
	...
	public void booleanScalarChange(BooleanScalarEvent event)
	{
		...
		try
		{
            ...
            //Null pointer ( IBooleanScalar ) event.getSource() ).getAttribute() is null sometimes
...
//	ELETTRA : Archiving Events 
//	--------------------------------------------------------------------------//
			scalarEvent.setValue(new Boolean(( ( IBooleanScalar ) event.getSource() ).getValue()));
...
//	--------------------------------------------------------------------------//
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		try{
			...
			{
				System.out.println("\033[1;31mBooleanScalar_RO: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
