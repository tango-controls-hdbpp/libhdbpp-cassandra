...
public class StringSpectrum_RO extends HdbCollector implements IStringSpectrumListener, ITangoArchiveListener
{
	...
	public StringSpectrum_RO(HdbModeHandler hdbModeHandler)
	{
	...
	{
		Util.out2.println("StringSpectrum_RO.removeSource");
...
//	--------------------------------------------------------------------------//	
    private void addSourceForEvents(AttributeLightMode attributeLightMode) throws ArchivingException
	{
		...
		try
		{
			...
			if(attribute == null)
			{
				System.out.println("\033[1;31mStringStringSpectrum_RO.java: the attribute \"" +
						attributeLightMode.getAttribute_complete_name() +
		...
		{
			System.out.println("\033[1;31mStringStringSpectrum_RO.java: Failed to get the attribute from the AttributeFactory\033[0m");
	...
	private void removeSourceForEvents(String attributeName)
	{
		...
		System.out.print("spectrum [RO]: removing source for \"" + attributeName + "\"...\t");
	...
	public void archive(TangoArchiveEvent event)
	{
		...
		{
			System.out.println("StringSpectrum_RO.java: archive(): Error getting archive event attribute value");
			f.printStackTrace();
		    return;
		}
		catch (Exception e) /* Shouldn't be reached */
		{
			System.out.println("StringSpectrum_RO.java.archive.getValue() failed, caught generic Exception, code failure");
			e.printStackTrace();
			return;
		}
		try{
			...
			{
				System.out.println("\033[1;31mBooleanStringSpectrum_RO.java: event.getValue() or event.getSource() or event.getSource().getEventSupplier()) returned null!\033[0m");
				return;
			}
			int dim = attrib.getDimX(); //??
			String[] svalue = attrib.extractStringArray();
			dim = svalue.length;
			spectrumEvent_ro.setValue(svalue);		
		...
		{
			System.err.println("BooleanStringSpectrum_RO.java: archive() : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + spectrumEvent_ro.getAttribute_complete_name() + " values...");
			//printException(GlobalConst.DATA_TYPE_EXCEPTION , AttrDataFormat._SCALAR , spectrumEvent_ro.getAttribute_complete_name() , devFailed);
		...
		{
			System.err.println("Boolean.StringSpectrum_RO.java: archive : " + GlobalConst.ARCHIVING_ERROR_PREFIX + "\r\n\t" + "Problem while reading " + spectrumEvent_ro.getAttribute_complete_name() + " values...");
			exE.printStackTrace();
			Object value = null;
			spectrumEvent_ro.setValue(value);
			processEventSpectrum(spectrumEvent_ro , tryNumber);
...
//	--------------------------------------------------------------------------//	    	
    ...
    public void stringSpectrumChange (StringSpectrumEvent event)
	{
		...
		spectrumEvent_ro.setDim_x(( ( IStringSpectrum ) event.getSource() ).getXDimension());
	...
	public void processEventSpectrum(SpectrumEvent_RO snapSpectrumEvent_RO , int try_number)
	{
		...
		{
            String message = "Problem (ArchivingException) storing StringSpectrum_RO value";
