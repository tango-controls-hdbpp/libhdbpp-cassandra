...
public abstract class HdbCollector extends ArchiverCollector
{
	...
	/**
	 * An  HdbModeHandler  is associated to each HdbCollector to handle the archiving mode
	 */
...
// --------------------------------------------------------------------------//
	...
	/**
	 * Adds an attribute to the list of the attributes for which is responsible this HdbCollector.
	 * If possible registers for the archive events
	 * @param attributeLightMode
	 */
	public abstract void addSource(AttributeLightMode attributeLightMode) throws ArchivingException;

	/**
	 * Removes an attribute from the list of the attributes for which is responsible this HdbCollector.
	 * If the attribute was registered for the archive events, we unsubscribe from it.
	 * @param attributeName
	...
	{
		if ( HdbArchiver.HdbArchiver.gc_counter < HdbArchiver.HdbArchiver.gc_counter_limit )
		{ // 1x / hour if period time is 1 second
			HdbArchiver.HdbArchiver.gc_counter++;
		}
		else
		{
			Util.out4.println("HdbCollector.checkGC : Launching the garbage collector...");
...
//			new CleanerThread().start();
    ...
    protected boolean isDataArchivableTimestampWise(ArchivingEvent archivingEvent) 
    {
        ...
        if ( newTime == 0 )
        {
            String message = "HdbCollector/isDataArchivableTimestampWise/Received a zero timestamp (" + newTime + " = " + new Timestamp ( newTime ) + ") for attribute|" + name + "|tableName|" + archivingEvent.getTable_name() + "|";
        ...
        boolean isValidDate = lastTimestampStack.validateDate(newTime,  this.m_logger);
        if ( isAlreadyRegisteredDate  || !isValidDate)
        {
            String message = "HdbCollector/isDataArchivableTimestampWise/FALSE for attribute|"+name+"|and timestamp|"+new Timestamp (newTime)+"|";
