...
public class TdbArchiver extends DeviceImpl implements TangoConst {
	...
	final String[] propnames = { "DbHost", "DbName", "DbUser", "DbPassword", "RacConnection", "DsPath", "DbPath",
	// "Cleaner",
		"HasThreadedStartup",
		// "CleanOldFiles",
		// "HasNewExport",
	...
	for (final Enumeration<String> my_EnumAttributes = archivingMessConfig.getAttributeListKeys(); my_EnumAttributes
	    ...
	    final AttributeLightMode attributeLightMode = archivingMessConfig.getAttribute(m_attribute);
		...
		att_support = AttributeSupport.checkAttributeSupport(attCompleteName,
			attributeLightMode.getData_type(), attributeLightMode.getData_format(), attributeLightMode
				.getWritable());
	...
	try {
	    ...
	    if (collector != null) {
		tableName = collector.exportFile2Db(attributeLightMode.getAttribute_complete_name());
	    } else {
    ...
    public/* synchronized */short retry_for_attributes(final String[] attributesToRetry) throws DevFailed {
