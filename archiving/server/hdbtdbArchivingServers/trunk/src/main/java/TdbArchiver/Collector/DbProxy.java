...
public class DbProxy {
	...
	try {
	    ...
	    manager.ArchivingConfigureWithoutArchiverListInit(myDbHost, myDbName, myDbUser,
		    myDbPassword, myRacConnection, false, true);
	    this.logger = logger;
	...
	return manager.is_db_connected();
    }
    ...
    public void exportToDB_Scalar(final String remoteFilePath, final String fileName,
	...
	((TDBDataBaseManager) manager.getDataBase()).getTdbExport().exportToDB_Data(remoteFilePath,
		fileName, tableName, writable);
    }

    public void exportToDB_Spectrum(final String remoteFilePath, final String fileName,
	    final String tableName, final int writable) throws ArchivingException {
    ...
    public void exportToDB_Image(final String remoteFilePath, final String fileName,
	    final String tableName, final int writable) throws ArchivingException {
	...
	try {
	    archiverCurrentTasks = manager.getDataBase().getMode().getArchiverCurrentTasks(
    ...
    public void deleteOldRecords(final long time, final String[] attributeList)
	    throws ArchivingException {
	((TDBDataBaseManager) manager.getDataBase()).getTdbDeleteAttribute().deleteOldRecords(time,
		attributeList);
    }
}