...
public class NumberSpectrum_RO extends TdbCollector implements
		ISpectrumListener {
	/**
	 *
	 */
	private static final long serialVersionUID = 8614195209039885776L;

	public NumberSpectrum_RO(TdbModeHandler _modeHandler, String currentDsPath,
	...
	public void removeSource(String attributeName) throws ArchivingException {
		System.out.println("NumberSpectrum_RO.removeSource");
		try {
			synchronized (attributeList) {
				/*
	...
	public void processEventSpectrum(SpectrumEvent_RO spectrumEvent_ro,
			int try_number) {
		Util.out4.println("NumberSpectrum_RO.processEventSpectrum");

		boolean timeCondition = super
				.isDataArchivableTimestampWise(spectrumEvent_ro);
		if (!timeCondition) {
			return;
		}

		try {
			((FileTools) filesNames.get(spectrumEvent_ro
					.getAttribute_complete_name()))
					.processEventSpectrum(spectrumEvent_ro);
