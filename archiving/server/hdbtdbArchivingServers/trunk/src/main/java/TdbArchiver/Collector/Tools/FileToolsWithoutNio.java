//+======================================================================
// $Source: /cvsroot/tango-cs/tango/jserver/archiving/TdbArchiver/Collector/Tools/FileToolsWithoutNio.java,v $
//
...
// $Revision: 1.1 $
//
// $Log: FileToolsWithoutNio.java,v $
// Revision 1.1  2007/06/04 09:57:21  pierrejoseph
// NIO usage
//
// Revision 1.51  2007/05/09 16:44:51  chinkumo
// minor changes
//
// Revision 1.50  2007/04/18 09:03:24  chinkumo
// corrected a bug in both processEventSpectrum methods and processEventImage: the dimX field isn't a double but an integer
//
// Revision 1.49  2007/04/16 14:57:21  ounsy
// threads are now started via an ExecutorService
//
// Revision 1.48  2007/04/16 12:54:51  ounsy
// added a trace in write() in case of IOException
//
// Revision 1.47  2007/03/20 14:54:24  ounsy
// back to the bufferedWriter
//
...
public class FileToolsWithoutNio {
	...
	public FileToolsWithoutNio(String tableName, int dataFormat, int writable,
			long windowsDuration, ILogger logger, boolean doStart,
			DbProxy dbProxy, String workingDsPath, String workingDbPath) {
	...
	public synchronized void processEventScalar(ScalarEvent scalarEvent)
		...
		try {
			...
			if (_dbProxy.getDataBase().getDbConn().getDbType() == ConfigConst.TDB_ORACLE) {
				if (readValue == null
						|| GlobalConst.ARCHIVER_NULL_VALUE
								.equalsIgnoreCase(readValue.trim())) {
					readValue = GlobalConst.ORACLE_NULL_VALUE;
				}
				if (writeValue == null
						|| GlobalConst.ARCHIVER_NULL_VALUE
								.equalsIgnoreCase(writeValue.trim())) {
				...
				stringBuffer.append("\"");
				stringBuffer.append(DateUtil.milliToString(scalarEvent
						.getTimeStamp(), DateUtil.FR_DATE_PATTERN));
				stringBuffer.append("\"");
				stringBuffer.append(",");
				...
				case AttrWriteType._READ_WITH_WRITE:
					write(new StringBuffer().append(
							toDbTimeStringMySQL(scalarEvent.getTimeStamp()))
							.append(ConfigConst.FIELDS_LIMIT).append(readValue)
							.append(ConfigConst.FIELDS_LIMIT)
							.append(writeValue).append(ConfigConst.LINES_LIMIT)
							.toString());
					break;
				case AttrWriteType._WRITE:
					write(new StringBuffer().append(
	...
	private synchronized void write(String line) throws IOException {
		try {
			...
			String msg = "FileTools Without Nio/write/problem writing for attribute/"
					+ this._tableName;
	...
	public synchronized void processEventSpectrum(
			SpectrumEvent_RO spectrumEvent_ro) throws ArchivingException {
		try {
			if (spectrumEvent_ro.getData_type() == TangoConst.Tango_DEV_STRING) {
				...
				if (value != null) {
					...
					for (int i = 0; i < value.length; i++) {
						transformedValue[i] = StringFormater
								.formatStringToWrite(value[i]);
					}
			...
			if (_dbProxy.getDataBase().getDbConn().getDbType() == ConfigConst.TDB_ORACLE) {
				...
				// write(new StringBuffer().append ( "\""
				// ).append(DateUtil.milliToString(spectrumEvent_ro.getTimeStamp()
				// , DateUtil.FR_DATE_PATTERN)).append ( "\""
				// ).append(",").append ( "\"" ).append(Double.toString(( double
				// ) spectrumEvent_ro.getDim_x())).append ( "\""
				...
				write(new StringBuffer().append("\"").append(
						...
						",").append("\"").append(spectrumEvent_ro.getDim_x())
				...
				buff
						.append(toDbTimeStringMySQL(spectrumEvent_ro
								.getTimeStamp()));
	 ...
	 */
	public synchronized void processEventSpectrum(
			SpectrumEvent_RW spectrumEvent_rw) throws ArchivingException {
		try {
			...
			String readValue = spectrumEvent_rw
					.getSpectrumValueRW_AsString_Read();
			String writeValue = spectrumEvent_rw
					.getSpectrumValueRW_AsString_Write();
			if (_dbProxy.getDataBase().getDbConn().getDbType() == ConfigConst.TDB_ORACLE) {
				...
				buff.append("\"");
				buff.append(DateUtil.milliToString(spectrumEvent_rw
						.getTimeStamp(), DateUtil.FR_DATE_PATTERN));
				buff.append("\"");

				buff.append(",");
				buff.append("\"");
				// buff.append(Double.toString(( double )
				// spectrumEvent_rw.getDim_x()));
				...
				buff
						.append(toDbTimeStringMySQL(spectrumEvent_rw
								.getTimeStamp()));
				buff.append(ConfigConst.FIELDS_LIMIT);
	...
	public synchronized void processEventImage(ImageEvent_RO imageEvent_ro)
		...
		try {
			...
			if (_dbProxy.getDataBase().getDbConn().getDbType() == ConfigConst.TDB_ORACLE) {
				// write(new StringBuffer().append ( "\""
				// ).append(DateUtil.milliToString(imageEvent_ro.getTimeStamp()
				// , DateUtil.FR_DATE_PATTERN)).append ( "\""
				// ).append(",").append ( "\"" ).append(Double.toString(( double
				// ) imageEvent_ro.getDim_x())).append ( "\""
				// ).append(",").append ( "\"" ).append(Double.toString(( double
				// ) imageEvent_ro.getDim_y())).append ( "\""
				// ).append(",").append ( "\""
				// ).append(imageEvent_ro.getValue_AsString()).append ( "\""
				// ).toString());
				write(new StringBuffer().append("\"").append(
						DateUtil.milliToString(imageEvent_ro.getTimeStamp(),
								DateUtil.FR_DATE_PATTERN)).append("\"").append(
						",").append("\"").append(imageEvent_ro.getDim_x())
						.append("\"").append(",").append("\"").append(
								imageEvent_ro.getDim_y()).append("\"").append(
								",").append("\"").append(
								imageEvent_ro.getValue_AsString()).append("\"")
						.toString());
				write(ConfigConst.NEW_LINE);
			} else if (_dbProxy.getDataBase().getDbConn().getDbType() == ConfigConst.TDB_MYSQL) {

				// write(new
				// StringBuffer().append(toDbTimeStringMySQL(imageEvent_ro.getTimeStamp())).append(ConfigConst.FIELDS_LIMIT).append((
				// double )
				// imageEvent_ro.getDim_x()).append(ConfigConst.FIELDS_LIMIT).append((
				// double )
				// imageEvent_ro.getDim_y()).append(ConfigConst.FIELDS_LIMIT).append(imageEvent_ro.getValue_AsString()).append(ConfigConst.LINES_LIMIT).toString());
				write(new StringBuffer().append(
						toDbTimeStringMySQL(imageEvent_ro.getTimeStamp()))
						.append(ConfigConst.FIELDS_LIMIT).append(
								imageEvent_ro.getDim_x()).append(
								ConfigConst.FIELDS_LIMIT).append(
								imageEvent_ro.getDim_y()).append(
								ConfigConst.FIELDS_LIMIT).append(
								imageEvent_ro.getValue_AsString()).append(
	...
	public synchronized String swapFile(boolean isAsynchronous) {
		...
		try {
			...
			t.printStackTrace();
			String message = "Problem (IOException) in swapFile file: get_fileName()|"
	...
	public synchronized void closeFile(boolean isAsynchronous) {
		...
		try {
			...
			t.printStackTrace();
			String message = "Problem (IOException) in closeFile file: get_fileName()|"
					+ get_fileName() + "|isAsynchronous|" + isAsynchronous;
			_logger.trace(ILogger.LEVEL_CRITIC, message);
			_logger.trace(ILogger.LEVEL_CRITIC, t);
		}
	...
	public synchronized void closeFileSimply() {
		try {
			...
			t.printStackTrace();
			String message = "Problem (IOException) in closeFileSimply file: get_fileName()|"
					+ get_fileName();
			_logger.trace(ILogger.LEVEL_CRITIC, message);
			_logger.trace(ILogger.LEVEL_CRITIC, t);
		}
	...
	public class ExportTask {
		...
		public synchronized void execute() {
			ExportThreadRunnable runnable = new ExportThreadRunnable(
					_exportFileName);
		...
		public synchronized void run() {
			try {
				...
				case AttrDataFormat._SCALAR:
					System.out.println("ExportThreadRunnable/_remoteFilePath|"
							+ _remoteFilePath + "|_exportFileName|"
							+ _exportFileName + "|_tableName|" + _tableName
							+ "|_writable|" + _writable);
					_dbProxy.exportToDB_Scalar(_remoteFilePath,
							_exportFileName, _tableName, _writable);
					break;
				case AttrDataFormat._SPECTRUM:
					_dbProxy.exportToDB_Spectrum(_remoteFilePath,
							_exportFileName, _tableName, _writable);
					break;
				case AttrDataFormat._IMAGE:
					_dbProxy.exportToDB_Image(_remoteFilePath, _exportFileName,
							_tableName, _writable);
					break;
				default:
					Util.out2.println("exportThread.run : " + "DataFormat ("
							+ _dataFormat + ") not supported !! ");
					break;
				}

				String message = "Exported file successfully: _remoteFilePath|"
						+ _remoteFilePath + "|_exportFileName|"
						+ _exportFileName + "|_tableName|" + _tableName;
				...
				String message = "Problem (ArchivingException) exporting file: _remoteFilePath|"
						+ _remoteFilePath
						+ "|_exportFileName|"
						+ _exportFileName + "|_tableName|" + _tableName;
				...
				String message = "Problem (Throwable) exporting file: _remoteFilePath|"
						+ _remoteFilePath
						+ "|_exportFileName|"
						+ _exportFileName + "|_tableName|" + _tableName;
				_logger.trace(ILogger.LEVEL_CRITIC, message);
	...
	private class WindowThread implements Runnable {
		...
		public synchronized void run()
		// public /*synchronized CLA*/ void run()
		{
			activate(true);
			while (state.equals("RUNNING")) {
				try {
					...
					/*
					 * synchronized (FileTools.this)//CLA {
					 */
					flushBuffer();
