...
public abstract class TdbCollector extends ArchiverCollector {
	...
	try {
		...
		if (filesNames.get(att_name) == null) {
		    final FileTools myFile = new FileTools(attributeLightMode.getAttribute_complete_name(), table_name,
			    attributeLightMode.getData_format(), attributeLightMode.getWritable(), attributeLightMode
     ...
     * @throws ArchivingException
     */
    public void removeSource(final String attributeName, final boolean closeFile) throws ArchivingException {
	...
	Util.out4.println("\n" + this.getClass().toString() + " (" + temp + ")" + "\n\t Source \t : "
		+ errorEvent.getSource().toString() + "(" + temp + ")" + "\n\t TimeS \t : " + errorEvent.getTimeStamp()
		+ "(" + temp + ")");
	...
	if (newTime == 0) {
	    final String message = "TdbCollector/isDataArchivableTimestampWise/Received a zero timestamp (" + newTime
	...
	if (isAlreadyRegisteredDate) {
	    final String message = "TdbCollector/isDataArchivableTimestampWise/FALSE for attribute|" + name
	...
	if (!isValidRegisteredDate) {
	    final String message = "TdbCollector/isDataArchivableTimestampWise/Invalid Timestamp for attribute|" + name
		    + "|and timestamp|" + new Timestamp(newTime) + "|";
	    m_logger.trace(ILogger.LEVEL_DEBUG, message);
	    Util.out1.println(message);
