...
from PyTangoArchiving.common import getSingletonAPI
...
class SnapAPI(Logger,Singleton):
    ...
    def __init__(self,user='browser',passwd='browser',api=None,SINGLETON=True):
        ...
        if api is None:api=getSingletonAPI(schema='snap')
        self.api=api
...
class SnapDB(FriendlyDB,Singleton):
    ...
    def rename_context(self, ctxid, new_name):
        """ Changes the name of ctxid to new_name.		
        """
        ...
        try:
            ...
            self.error('Exception in DB_rename_context query: %s' % str(e))
            self.db.rollback()
            return False
    ## @}
