            ...
            try {
                ...
                try {
                    ...
                    for (ArchivingConfigurationAttribute a : tree
                        ...
                        // FIXME: See if we have to check for different period types
                        if (getHDBperiod()>0) {
                            this.getLastMode().checkMode(a.getCompleteName(),
                                    getHDBperiod());
                        }
                        if (getHDBAbsolutePeriod()>0) {
                            this.getLastMode().checkMode(a.getCompleteName(),
                                    getHDBAbsolutePeriod());
                        }
                        if (getHDBRelativePeriod()>0) {
                            this.getLastMode().checkMode(a.getCompleteName(),
                                    getHDBRelativePeriod());
                        }
                        if (getHDBThresholdPeriod()>0) {
                            this.getLastMode().checkMode(a.getCompleteName(),
                                    getHDBThresholdPeriod());
                        }
                        if (getHDBDifferencePeriod()>0) {
                            this.getLastMode().checkMode(a.getCompleteName(),
