//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/mambo/models/VCAttributesTreeModel.java,v $
//
...
// Author: XP
// Mantis bug ID: 6961
// Comment: In the VCAttributesTreeModel builder, the historic data is initialized according to the HDB_AVAILABLE property.
//
...
public class VCAttributesTreeModel extends AttributesTreeModel
{
    ...
    {
        TreeMap possibleAttr = VCPossibleAttributesTreeModel.getInstance().getAttributes();
            ...
            for ( int depth = 0 ; depth < CONTEXT_TREE_DEPTH ; depth++ )
            {
                ...
                else
                {
                    ...
                    if ( depth == CONTEXT_TREE_DEPTH - 1 )
                    {
                        ...
                        //ViewConfigurationAttribute attribute = (ViewConfigurationAttribute) possibleAttr.get ( translatePathIntoKey ( path ) );
                        Attribute _attribute = ( Attribute ) possibleAttr.get( translatePathIntoKey( path ) );
                        //System.out.println ( "VCAttributesTreeModel/addSelectedAttibutes/adding|"+translatePathIntoKey ( path )+"|" );
                        ViewConfigurationAttribute attribute = new ViewConfigurationAttribute( _attribute );

                        if ( attribute != null )
                        {
                            htAttr.put( translatePathIntoKey( path ) , attribute );
                            super.addToDomains( _attribute );
                            //System.out.println ( "ACAttributesTreeModel/addSelectedAttibutes/adding|"+translatePathIntoKey ( path )+"|" );
                        }

                        /*ContextAttributesPanel panel = ContextAttributesPanel.getInstance ();
    ...
    public void removeSelectedAttributes ( Vector validSelected )
    {
        ...
        while ( enumeration.hasMoreElements() )
        {
            ...
            String path = dom_s + GUIUtilities.TANGO_DELIM + fam_s + GUIUtilities.TANGO_DELIM + mem_s + GUIUtilities.TANGO_DELIM + att_s;
     ...
     * @param _historic
     */
    public void match ( IAttributeManager manager , Criterions searchCriterions , boolean _historic )
    {
        Vector requestDomains = null;
        setRootName( historic );
        try
        {
        ...
        Vector newDomains = intersectDomains( super.domains , requestDomains );

        super.removeAll();
        super.build( newDomains );
    }

    private static Vector intersectDomains ( Vector before , Vector requestDomains )
    {
        Vector after = before;
        Enumeration enumD = after.elements();
        while ( enumD.hasMoreElements() )
        {
            Domain nextD = ( Domain ) enumD.nextElement();
            ...
            {
                Map fams = nextD.getFamilies();
                Map requestFams = requestDomain.getFamilies();

                Iterator enumF = fams.values().iterator();
                while ( enumF.hasNext() )
                {
                    Family nextF = ( Family ) enumF.next();
                    Family requestFamily = getFamily( requestFams , nextF.getName() );
                    ...
                    {
                        Map mems = nextF.getMembers();
                        Map requestMems = requestFamily.getMembers();

                        Iterator enumM = mems.values().iterator();
                        while ( enumM.hasNext() )
                        {
                            Member nextM = ( Member ) enumM.next();
                            Member requestMember = getMember( requestMems , nextM.getName() );
                            ...
                            {
                                //----------
                                //System.out.println ( "intersectDomains/requestMember.getName()/"+requestMember.getName()+"/" );
                                Map attrs = nextM.getAttributes();
                                Vector attrsToRemove = new Vector();

                                //System.out.println ( "intersectDomains/attrs.size ()/"+attrs.size ()+"/" );
                                Map requestAttrs = requestMember.getAttributes();

                                Iterator enumA = attrs.values().iterator ();
                                while ( enumA.hasNext () )
                                {
                                    Attribute nextA = ( Attribute ) enumA.next ();
                                    //System.out.println ( "intersectDomains/nextA.getName()/"+nextA.getName()+"/" );
                                    Attribute requesteAttribute = getAttribute( requestAttrs , nextA.getName () );
                                ...
                                GUIUtilities.removeVectorElements( attrs , attrsToRemove );
                            }
                        }
                    }
                }
     ...
     * @return
     */
    private static Attribute getAttribute ( Map requestAttrs , String toTest )
    {
        return (Attribute) requestAttrs.get ( toTest );
    }
     ...
     * @return
     */
    private static Member getMember ( Map requestMems , String toTest )
    {
        return (Member) requestMems.get(toTest);
    }
     ...
     * @return
     */
    private static Domain getDomain ( Vector requestDomains , String toTest )
    {
        Enumeration enumD = requestDomains.elements();
        while ( enumD.hasMoreElements() )
        {
    ...
    private static Family getFamily ( Map requestFams , String toTest )
    {
        return (Family) requestFams.get(toTest);
    }
}
