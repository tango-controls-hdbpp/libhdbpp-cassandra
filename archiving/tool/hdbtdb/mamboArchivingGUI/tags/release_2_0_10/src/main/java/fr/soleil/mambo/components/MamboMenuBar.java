...
public class MamboMenuBar extends JMenuBar {
    ...
    private static final long      serialVersionUID = 2803030015338791722L;
    ...
    private MamboMenuBar() {
        ...
        String archivingConfigurationLabel = Messages
                .getMessage("MENU_ARCHIVING_CONFIGURATION");
        String viewConfigurationLabel = Messages
                .getMessage("MENU_VIEW_CONFIGURATION");
        ...
        editACAction = new OpenACEditDialogAction(msg, false, acOptions
                .isAlternateSelectionMode());

        msg = archivingConfigurationLabel;
        newACAction1 = new OpenACEditDialogAction(msg, true, acOptions
                .isAlternateSelectionMode());

        msg = Messages.getMessage("ARCHIVING_ACTION_NEW_BUTTON");
        newACAction2 = new OpenACEditDialogAction(msg, true, acOptions
                .isAlternateSelectionMode());
        ...
        SelectedVCRefreshAction refreshAction = SelectedVCRefreshAction
                .getInstance(msg);

        msg = Messages.getMessage("VIEW_ACTION_VARIATION_BUTTON");
        SelectedVCVariationAction variationAction = SelectedVCVariationAction
                .getInstance(msg);

        msg = Messages.getMessage("VIEW_ACTION_MODIFY_BUTTON");
        OpenSelectedVCEditDialogModifyAction editVCAction = OpenSelectedVCEditDialogModifyAction
                .getInstance(msg);

        msg = Messages.getMessage("VIEW_ACTION_NEW_BUTTON");
        OpenVCEditDialogNewAction newVCAction = OpenVCEditDialogNewAction
                .getInstance(msg);

        msg = Messages.getMessage("VIEW_ACTION_HIDE_GENERAL_VIEW");
        SelectedVCHideGeneralAction hideGeneral = SelectedVCHideGeneralAction
                .getInstance(msg);

        msg = Messages.getMessage("VIEW_ACTION_HIDE_TREE_VIEW");
        SelectedVCHideAction hideTree = SelectedVCHideAction.getInstance(msg);
        // --
        ...
        JMenuItem fileMenu_new_VC = new JMenuItem(newVCAction);
        fileMenu_new_VC.setText(viewConfigurationLabel);
        fileMenu_new.add(fileMenu_new_VC);
        JMenu fileMenu_save = new JMenu(saveLabel);
        JMenuItem fileMenu_save_ac = new JMenuItem(new SaveSelectedACAction(
                archivingConfigurationLabel, false));
        if (Mambo.hasACs()) {
            fileMenu_save.add(fileMenu_save_ac);
        }

        JMenuItem fileMenu_save_vc = new JMenuItem(new SaveSelectedVCAction(
                viewConfigurationLabel, false));
        fileMenu_save.add(fileMenu_save_vc);
        // --
        JMenu fileMenu_saveAs = new JMenu(saveAsLabel);
        JMenuItem fileMenu_saveAs_ac = new JMenuItem(new SaveSelectedACAction(
                archivingConfigurationLabel, true));
        if (Mambo.hasACs()) {
            fileMenu_saveAs.add(fileMenu_saveAs_ac);
        }

        JMenuItem fileMenu_saveAs_vc = new JMenuItem(new SaveSelectedVCAction(
                viewConfigurationLabel, true));
        fileMenu_saveAs.add(fileMenu_saveAs_vc);
        // ---
        JMenu fileMenu_load = new JMenu(loadLabel);
        JMenuItem fileMenu_load_ac = new JMenuItem(new LoadACAction(
                archivingConfigurationLabel, false));
        if (Mambo.hasACs()) {
            fileMenu_load.add(fileMenu_load_ac);
        }

        JMenuItem fileMenu_load_vc = new JMenuItem(new LoadVCAction(
                viewConfigurationLabel, false));
        fileMenu_load.add(fileMenu_load_vc);

        JMenu fileMenu_print = new JMenu(printLabel);
        JMenuItem fileMenu_print_ac = new JMenuItem(new PrintAction(
                archivingConfigurationLabel, null, PrintAction.AC_TYPE));
        if (Mambo.hasACs()) {
            fileMenu_print.add(fileMenu_print_ac);
        }

        JMenuItem fileMenu_print_vc = new JMenuItem(new PrintAction(
                viewConfigurationLabel, null, PrintAction.VC_TYPE));
        fileMenu_print.add(fileMenu_print_vc);

        SaveAllToDiskAction saveAllToDiskAction = new SaveAllToDiskAction(
                saveAllLabel, null, SaveAllToDiskAction.BOTH_TYPE);
        ...
        // BEGIN AC Menu
        // JMenuItem acMenu_new = new JMenuItem ( new NewAction ( newLabel ,
        // null , NewAction.AC_TYPE ) );
        JMenuItem acMenu_save = new JMenuItem(new SaveSelectedACAction(
                saveLabel, false));
        JMenuItem acMenu_saveAs = new JMenuItem(new SaveSelectedACAction(
                saveAsLabel, true));
        JMenuItem acMenu_saveAll = new JMenuItem(new SaveAllToDiskAction(
                saveAllLabel, null, SaveAllToDiskAction.AC_TYPE));
        JMenuItem acMenu_load = new JMenuItem(
                new LoadACAction(loadLabel, false));
        // ---

        // JMenuItem acMenu_start = new JMenuItem( archivingStartAction );
        JMenuItem acMenu_start = new JMenuItem(Messages
                .getMessage("ARCHIVING_ACTION_START_BUTTON"));
        ...
        JMenuItem hideGeneralMenuItem = new JMenuItem(hideGeneral);
        JMenuItem hideTreeMenuItem = new JMenuItem(hideTree);
        ...
        // BEGIN VC Menu
        // JMenuItem vcMenu_new = new JMenuItem ( new NewAction ( newLabel ,
        // null , NewAction.VC_TYPE ) );
        JMenuItem vcMenu_save = new JMenuItem(new SaveSelectedVCAction(
                saveLabel, false));
        JMenuItem vcMenu_saveAs = new JMenuItem(new SaveSelectedVCAction(
                saveAsLabel, true));
        JMenuItem vcMenu_saveAll = new JMenuItem(new SaveAllToDiskAction(
                saveAllLabel, null, SaveAllToDiskAction.VC_TYPE));
        JMenuItem vcMenu_load = new JMenuItem(
                new LoadVCAction(loadLabel, false));
        // JMenuItem vcMenu_view = new JMenuItem ( new LoadVCAction ( loadLabel
        // , false ) );
        ...
        vcMenu.addSeparator();
        vcMenu.add(newVCAction);
        vcMenu.add(editVCAction);
        vcMenu.add(refreshAction);
        vcMenu.add(variationAction);
        vcMenu.addSeparator();
        vcMenu.add(hideGeneralMenuItem);
        vcMenu.add(hideTreeMenuItem);
        // END VC Menu
    ...
    public void setAlternateSelection(boolean isAlternate) {
        editACAction.setAlternateSelectionMode(isAlternate);
        newACAction1.setAlternateSelectionMode(isAlternate);
        newACAction2.setAlternateSelectionMode(isAlternate);
    }
}
