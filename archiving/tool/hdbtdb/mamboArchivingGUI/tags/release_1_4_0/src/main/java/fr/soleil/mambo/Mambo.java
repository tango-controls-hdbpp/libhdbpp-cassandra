...
// added a new role USER_RIGHTS_ADMIN = 3, the only role able to map attributes to specific archivers
//
...
import fr.soleil.mambo.session.Session;
...
public class Mambo
{
    ...
    private static ImageIcon appliStart = new ImageIcon(Mambo.class.getResource("icons/appli_start.jpg"));
    ...
    {
        //String title = "Mambo v" + Messages.getMessage("ABOUT_RELEASE") + " (" + Messages.getMessage("ABOUT_RELEASE_DATE") + ")";
        String title = Messages.getAppMessage("project.name") + " v" + Messages.getAppMessage("project.version") + " (" + Messages.getAppMessage("project.date") + ")";
        ...
        splash.progress(2);
        splash.setMessage( Messages.getLogMessage("APPLICATION_WILL_START_SET_ENVIRONMENT") );
        lifeCycleManager.applicationWillStart( null, splash );

        //Create and set up the window.
        splash.progress(24);
        splash.setMessage( Messages.getLogMessage("APPLICATION_WILL_START_BUILD_FRAME") );
        JFrame frame = MamboFrame.getInstance( lifeCycleManager );
        splash.progress(25);
        splash.setMessage( Messages.getLogMessage("APPLICATION_WILL_START_SET_FRAME") );
        frame.setDefaultCloseOperation( JFrame.DO_NOTHING_ON_CLOSE );

        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                LifeCycleManagerFactory.getCurrentImpl().applicationWillClose( new Hashtable() );
            }
        });

        //Display the window.
        splash.progress(26);
        splash.setMessage( Messages.getLogMessage("APPLICATION_WILL_START_PREPARE_DISPLAY") );
        frame.pack();
    ...
    public static void main ( String[] args )
    {
        ...
        /*try {
            ArchivingManagerApi.getHDBArchiverDedicatedAttributes ();
        ...
        {
            System.out.println( "Incorrect arguments. Correct syntax: java Mambo pathToResources HDBuser HDBpassword TDBuser TDBpassword userRights" );
            // Correct syntax: java Mambo pathToResources HDBuser HDBpassword TDBuser TDBpassword userRights DeployedDB
            System.exit( 1 );
        }
        ...
        if(args.length == 6) {
        	try   	{
        		...
        		else if( argValue.compareToIgnoreCase(DEPLOYED_DB_TDB_ONLY) == 0)
        			hdbAvailable = false;
        		else if ( argValue.compareToIgnoreCase(DEPLOYED_DB_ALL) == 0)
        			hdbAvailable = true;
        		else 		{
        			throw new Exception( "The userRights parameter has to be either " + USER_PROFILE_VIEWER + " or " + USER_PROFILE_EXPERT );
        		}	
            }
            catch ( Exception e )  {
                System.out.println( "Incorrect arguments. The userRights parameter has to be either " + USER_PROFILE_VIEWER + " or " + USER_PROFILE_EXPERT );
                e.printStackTrace();
                System.exit( 1 );
            }
        }
        	...
        	try   	{
        		...
        		argValue = args[ 6 ];
        		if( argValue.compareToIgnoreCase(DEPLOYED_DB_TDB_ONLY) == 0)
        			hdbAvailable = false;
        		else if ( argValue.compareToIgnoreCase(DEPLOYED_DB_ALL) == 0)
        			hdbAvailable = true;
        		else {
        			throw new Exception( "The deployed DB parameter has to be either " + DEPLOYED_DB_TDB_ONLY + " or " + DEPLOYED_DB_ALL );
        		}
            }
            catch ( Exception e )  {
                System.out.println( "Incorrect arguments. The userRights parameter has to be either " + USER_PROFILE_VIEWER + " or " + USER_PROFILE_EXPERT );
        ...
        // else is not useful because the defaults values are used for the variables hdbAvailable and userRights. 
        
       /* String userRights_s = args.length == 6 ? args[ 5 ] : USER_PROFILE_VIEWER;
        try
        {
        	if(userRights_s.compareToIgnoreCase(USER_PROFILE_VIEWER) == 0)
    			...
    			userRights = USER_RIGHTS_ALL;
    		else if( userRights_s.compareToIgnoreCase(USER_PROFILE_ADMIN) == 0)
    			userRights = USER_RIGHTS_ADMIN;
    		else 		{
           	    throw new Exception( "The userRights parameter has to be either " + USER_PROFILE_VIEWER + " or " + USER_PROFILE_EXPERT );
    		}
 ...
 */   
        //NO more true Read the configurating properties in the pathToResources directory
        //readProperties();
        //System.out.println("Is HDB available = " + hdbAvailable );
        
        ProfileManagerFactory.getInstance( pathToResources );
        ProfileManagerFactory.getImpl( ProfileManagerFactory.XML_IMPL_TYPE );
        ...
        {
            e.printStackTrace();
            //we refuse to launch the application without at least those resources
            System.exit( 1 );
        }
    ...
    {
    	File file = new File(pathToResources + File.separator + "mambo.properties");
		try {
			if (file.exists()) {
				...
				try {
					props.load(strm);
					hdbAvailable = Boolean.valueOf(
							props.getProperty("mambo.hdb.available", Boolean.toString(hdbAvailable)));
				} finally {
					strm.close();
				}
				// Initializing system
    ...
    /*private static void test ()
    {
        try
        {
            ArchivingManagerApi.connectArchivingDatabase( true , HDBuser , HDBpassword );
            //System.out.println ( "Name = " + ArchivingManagerApi.getDbName(true) + "/" );

            ArchivingManagerApi.connectArchivingDatabase( true , TDBuser , TDBpassword );
            //System.out.println ( "Name = " + ArchivingManagerApi.getDbName(false) + "/" );
        }
    ...
    protected static void gettingStarted() {
        ProfileSelectionDialog.getInstance().setVisible(true);
        SessionManagerFactory.setPath(ProfileManagerFactory.getCurrentImpl().getSelectedProfilePath());
        try {
            if (SessionManagerFactory.getImpl(SessionManagerFactory.XML_IMPL_TYPE).lockDirectory()) {
                //pathToResources = ProfileManagerFactory.getCurrentImpl().getSelectedProfilePath();
                pathToResources = Session.getInstance().getDirectory();
            }
            else {
                new MamboErrorDialog(ProfileSelectionDialog.getInstance(),
                                        Messages.getMessage("SESSION_ERROR_TITLE"),
                                        Messages.getMessage("SESSION_LOCKED")
                                       ).setVisible(true);
                gettingStarted();
            }
        }
            ...
            new MamboErrorDialog(ProfileSelectionDialog.getInstance(),
                                    Messages.getMessage("SESSION_ERROR_TITLE"),
                                    Messages.getMessage("SESSION_LOCK_ERROR")
                                   ).setVisible(true);
            gettingStarted();
        }
    }
