/*	Synchrotron Soleil 
 *  
 *   File          :  ViewSpectrumTableModel.java
 *  
 *   Project       :  Mambo_CVS
 *  
 *   Description   :  
 ...
 *   Date: 							State:  
 *  
 *   Log: ViewSpectrumTableModel.java,v 
 *
 */
...
public class ViewSpectrumTableModel extends DefaultTableModel {
    ...
    public ViewSpectrumTableModel(int readlength, int writeLength, int dataWritable) {
        super();
    ...
    public void setValueAt ( Object aValue , int rowIndex , int columnIndex )
    {
        if (aValue instanceof Boolean) {
            if (columnIndex == 0) {
                ...
                viewable[rowIndex + readDataLength] = ((Boolean)aValue).booleanValue();
            }
            ...
            case 1:
                switch (writable) {
                    ...
                    case RW:
                        if (rowIndex < readDataLength)
                        {
                            return (rowIndex+1) + " " + Messages.getMessage("VIEW_SPECTRUM_READ");
                        }
                        else return "";
                    case W:
                        if (rowIndex < viewable.length - readDataLength)
                        {
                            return (rowIndex+1) + " " + Messages.getMessage("VIEW_SPECTRUM_WRITE");
                        }
                        else return "";
                    case UNKNOWN:
                        return "" + (rowIndex+1);
