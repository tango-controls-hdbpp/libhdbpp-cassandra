...
public class ACOptions extends ReadWriteOptionBook implements
		PushPullOptionBook {
	...
	public ArchivingConfigurationAttributeProperties getDefaultACProperties() {
		...
		if (selection != null) {
			try {
				...
				ArchivingConfigurationMode acMode = new ArchivingConfigurationMode(
						mode1);
		...
		/*
		 * try { period = ( String ) this.content.get( TDB_KEEPING_PERIOD );
		 * long period_l = Long.parseLong( period );
		 * ret.getTDBProperties().setKeepingPeriod( period_l ); } catch (
	...
	private void fillDefaultValues() {
		...
		/*
		 * l = defaultPanel.getKeepingPeriod(); this.content.put(
		 * TDB_KEEPING_PERIOD , String.valueOf( l ) );
		 */
	...
	private void pushDefaultValues() {
		...
		/*
		 * s = ( String ) this.content.get( TDB_KEEPING_PERIOD ); if ( s != null
		 * && !s.equals ( "" ) ) { l = Long.parseLong( s );
	...
	public void pushSelectionMode() {
		...
		if (plaf_s != null) {
			...
			if (MamboToolBar.getCurrentInstance() != null) {
				MamboToolBar.getCurrentInstance().setAlternateSelection(
						isAlternate);
			}
			if (MamboMenuBar.getCurrentInstance() != null) {
				MamboMenuBar.getCurrentInstance().setAlternateSelection(
						isAlternate);
			}
