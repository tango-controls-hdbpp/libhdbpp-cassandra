...
public class GeneralOptions extends ReadWriteOptionBook implements
    ...
    public void push() {
        ...
        if (bufferTangoAttributes_s != null) {
            ...
            switch (bufferTangoAttributes) {
                case BUFFER_TANGO_ATTRIBUTES_NO:
                    ...
                    tangoManagerType = AttributeTableSelectionBean.SORTED;
                    break;
                case BUFFER_TANGO_ATTRIBUTES_YES:
                default:
                    optionsGeneralTab.setBufferTangoAttributes(true);
                    TangoManagerFactory.setBuffered(true);
                    tangoManagerType = AttributeTableSelectionBean.BUFFERED_AND_SORTED;
                    break;
            }
        }
        else {
        ...
        ACEditDialog.setDefaultTangoManagerType(tangoManagerType);
        if (ACEditDialog.getInstance() != null) {
            ACEditDialog.getInstance().setTangoManagerType(tangoManagerType);
        }
    }
