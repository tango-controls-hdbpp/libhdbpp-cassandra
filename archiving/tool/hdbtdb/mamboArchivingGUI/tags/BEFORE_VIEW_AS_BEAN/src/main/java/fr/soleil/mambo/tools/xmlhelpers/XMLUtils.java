...
public class XMLUtils
{
    ...
    /**
     * @param map 8 juil. 2005
     */
    /*public static void traceTreeMapKeys ( TreeMap map )
    {
        Iterator it = map.keySet().iterator();
        while ( it.hasNext() )
        {
            String s = ( String ) it.next();
            //System.out.println ( "    traceTreeMapKeys/s/"+s+"/" );
        }
    }*/
    ...
    /**
     * @param string
     * @param favoritesResourceLocation 12 juil. 2005
     * @throws Exception
     */
     ...
     * @throws Exception 8 juil. 2005
     */
    public static Hashtable loadAttributes ( Node DOMnode ) throws Exception
    {
        ...
        if ( DOMnode.hasAttributes() )
        {
            ...
            for ( int i = 0 ; i < listAtts.getLength() ; i++ )
            {
                ...
                //Sylvain: dangerous, since other elements can have a "factor" attribute
                if ( ViewConfigurationAttribute.FACTOR_PROPERTY_XML_TAG.equals( nomAtt ) )
                {
                    Double factor;
                    try
                    {
                        factor = Double.valueOf( valueAtt );
                    }
                    catch ( Exception e )
                    {
                        factor = new Double( 1 );
                    }
                    if ( factor.doubleValue() == Double.NaN
                         || factor.doubleValue() == Double.POSITIVE_INFINITY
                         || factor.doubleValue() == Double.NEGATIVE_INFINITY )
                    {
                        factor = new Double( 1 );
                    }
                    retour.put( nomAtt , factor );
                }
