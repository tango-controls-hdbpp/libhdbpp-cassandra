//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/mambo/actions/archiving/SetAttributesGroupProperties.java,v $
//
...
public class SetAttributesGroupProperties extends AbstractAction
{
    ...
    public void actionPerformed ( ActionEvent actionEvent )
    {
        ...
        ArchivingConfiguration currentAC = ArchivingConfiguration.getCurrentArchivingConfiguration();
        ...
        try
        {
            if ( panel.isHistoric () )
            {
                ...
                //System.out.println ( "SetAttributesGroupProperties/HDBPeriod/"+HDBPeriod );    
            }
        }
        catch ( Exception e )
        {
            ...
            String msg = Messages.getLogMessage( "SET_ATTRIBUTES_GROUP_INVALID_VALUES" );
        ...
        try
        {
            if ( ! panel.isHistoric () )
            {
                ...
                //System.out.println ( "SetAttributesGroupProperties/TDBPeriod/"+TDBPeriod );
            }
        }
        catch ( Exception e )
        {
        ...
        ArchivingConfigurationAttributeHDBProperties currentHDBProperties = currentProperties.getHDBProperties();
        ArchivingConfigurationAttributeTDBProperties currentTDBProperties = currentProperties.getTDBProperties();
        currentTDBProperties.setExportPeriod( exportPeriod );
        //currentTDBProperties.setKeepingPeriod( keepingPeriod );

        ACAttributesPropertiesTree tree = ACAttributesPropertiesTree.getInstance();
        ...
        ArchivingConfigurationAttributes currentACAttributes = currentAC.getAttributes();
        while ( enumer.hasMoreElements() )
        {
            ArchivingConfigurationAttribute next = ( ArchivingConfigurationAttribute ) enumer.nextElement();
            ...
            next.setDedicatedArchiver ( currentAC.isHistoric () , dedicatedArchiver );
            
            //System.out.println ( "CLA 0 A/"+exportPeriod+"/keepingPeriod/"+keepingPeriod );
    ...
    private void removeUnwantedProperties ( ArchivingConfigurationAttribute next )
    {
        ...
        next.removeProperty( hasModeHDB_A , ArchivingConfigurationMode.TYPE_A , true );

        boolean hasModeHDB_R = panel.hasModeHDB_R();
        next.removeProperty( hasModeHDB_R , ArchivingConfigurationMode.TYPE_R , true );

        boolean hasModeHDB_T = panel.hasModeHDB_T();
        next.removeProperty( hasModeHDB_T , ArchivingConfigurationMode.TYPE_T , true );

        boolean hasModeHDB_D = panel.hasModeHDB_D();
        next.removeProperty( hasModeHDB_D , ArchivingConfigurationMode.TYPE_D , true );

        boolean hasModeHDB_P = panel.hasModeHDB_P();
        next.removeProperty( hasModeHDB_P , ArchivingConfigurationMode.TYPE_P , true );

        boolean hasModeTDB_A = panel.hasModeTDB_A();
        next.removeProperty( hasModeTDB_A , ArchivingConfigurationMode.TYPE_A , false );

        boolean hasModeTDB_R = panel.hasModeTDB_R();
        next.removeProperty( hasModeTDB_R , ArchivingConfigurationMode.TYPE_R , false );

        boolean hasModeTDB_T = panel.hasModeTDB_T();
        next.removeProperty( hasModeTDB_T , ArchivingConfigurationMode.TYPE_T , false );

        boolean hasModeTDB_D = panel.hasModeTDB_D();
        next.removeProperty( hasModeTDB_D , ArchivingConfigurationMode.TYPE_D , false );

        boolean hasModeTDB_P = panel.hasModeTDB_P();
        next.removeProperty( hasModeTDB_P , ArchivingConfigurationMode.TYPE_P , false );
    }
     ...
     * @return 22 juil. 2005
     */
    /*private DefaultMutableTreeNode getCurrentSelectedNodeInContext ( DefaultMutableTreeNode currentSelectedNode , ACAttributesPropertiesTree tree )
    {
        ...
        while ( e.hasMoreElements() )
        {
            ...
            if ( node.getUserObject().equals( currentSelectedNode.getUserObject() ) )
            {
                return node;
            }
        }
