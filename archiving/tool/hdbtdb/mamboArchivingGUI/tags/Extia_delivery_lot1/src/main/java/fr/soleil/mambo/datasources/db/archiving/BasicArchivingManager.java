...
public class BasicArchivingManager extends DbConnectionManager implements
	...
	public void stopArchiving(ArchivingConfiguration AC)
		...
		for (int i = 0; i < nbOfAttributes; i++) {
			...
			try {
				if (this.hdbManager != null
						&& this.hdbManager.isArchived(nextName)
			...
			try {
				if (this.tdbManager != null
						&& this.tdbManager.isArchived(nextName)
						&& (AC == null || !AC.isHistoric())) {
		...
		try {
			if (this.hdbManager != null && !attributeListFinalHDB.isEmpty()) {
				...
				attributeListFinalHDB_s = (String[]) attributeListFinalHDB
		...
		try {
			if (this.tdbManager != null && !attributeListFinalTDB.isEmpty()) {
				...
				attributeListFinalTDB_s = (String[]) attributeListFinalTDB
	...
	public void exportData2Tdb(String attributeName, String endDate)
			throws ArchivingException {
		try {
			((TDBDataBaseManager) this.tdbManager.getDataBase()).getTdbExport()
