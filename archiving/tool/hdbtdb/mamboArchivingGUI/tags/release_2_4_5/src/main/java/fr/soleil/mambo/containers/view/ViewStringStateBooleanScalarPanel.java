...
public class ViewStringStateBooleanScalarPanel extends MamboCleanablePanel {
	...
	if (attributes != null && attributes.length > 0) {
		...
		viewTables[i] = new MamboFormatableTable(
			new ViewStringStateBooleanScalarTableModel(attributes[i].getCompleteName(),
				viewConfigurationBean));
		if (attributes[i].getDataType(viewConfigurationBean.getViewConfiguration()
			.getData().isHistoric()) == TangoConst.Tango_DEV_STATE) {
		    viewTables[i].setDefaultRenderer(Object.class, new StateTableRenderer());
		} else if (attributes[i].getDataType(viewConfigurationBean.getViewConfiguration()
			.getData().isHistoric()) == TangoConst.Tango_DEV_STRING) {
	...
	if (viewTables != null && tableIndex < viewTables.length) {
	    ...
	    s = s.substring(0, s.lastIndexOf(Options.getInstance().getGeneralOptions()
		    .getSeparator()));
	    s += "\n";
	...
	try {
		...
		try {
		    ...
		    final DbData[] data = ((ViewStringStateBooleanScalarTableModel) viewTables[i]
			    .getModel()).loadData();
