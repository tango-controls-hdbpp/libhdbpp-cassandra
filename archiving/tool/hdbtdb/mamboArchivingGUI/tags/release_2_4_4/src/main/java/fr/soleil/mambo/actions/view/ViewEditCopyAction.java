...
public class ViewEditCopyAction extends AbstractAction {
	 ...
	 * @param name
	 */
	public ViewEditCopyAction(String name, MamboCleanablePanel sourcePanel,
			int reference) {
	...
	public void actionPerformed(ActionEvent arg0) {
		WaitingDialog.changeFirstMessage(Messages
				.getMessage("DIALOGS_WAITING_COPY_TABLE_TITLE"));
		WaitingDialog.changeSecondMessage(Messages
				.getMessage("DIALOGS_WAITING_WAIT_TABLE_TITLE"));
		...
		try {
			...
			if (sourcePanel instanceof ViewStringStateBooleanSpectrumPanel) {
				...
				case ViewStringStateBooleanSpectrumPanel.READ_REFERENCE:
					copyText = ((ViewStringStateBooleanSpectrumPanel) sourcePanel)
							.getReadValue();
					break;
				...
				copyText = ((ViewStringStateBooleanScalarPanel) sourcePanel)
						.getSelectedToString(reference);
			} else {
				copyText = "";
			}
			new ViewEditCopyDialog(MamboFrame.getInstance(), (String) this
					.getValue(Action.SHORT_DESCRIPTION), copyText)
					.setVisible(true);
		} catch (Throwable t) {
