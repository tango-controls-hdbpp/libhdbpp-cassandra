...
public class ViewConfigurationBeanManager implements IViewConfigurationBeanListener {
    ...
    private final static ImageIcon MODIFIED_VC_ICON = new ImageIcon(Mambo.class
	    .getResource("icons/star_red.gif"));

    private final VCTabMouseButtonListener defaultTabMouseButtonListener = new VCTabMouseButtonListener();
	...
	((RootWindow) dockingManager.getDockingArea()).getRootWindow().getRootWindowProperties()
		.getWindowAreaProperties().setBackgroundColor(GUIUtilities.getViewColor());
	((RootWindow) dockingManager.getDockingArea()).getRootWindow().getRootWindowProperties()
		.getTabWindowProperties().getMaximizeButtonProperties().setVisible(false);
	((RootWindow) dockingManager.getDockingArea()).getRootWindow().getRootWindowProperties()
		.getTabWindowProperties().getMinimizeButtonProperties().setVisible(false);
	((RootWindow) dockingManager.getDockingArea()).getRootWindow().getRootWindowProperties()
	...
	if (selectedConfiguration != null) {
		...
		if (view == null) {
		    JScrollPane scrollPane = new JScrollPane(bean.getViewDisplayPanel());
		    ...
		    GUIUtilities.setObjectBackground(scrollPane.getHorizontalScrollBar(),
			    GUIUtilities.VIEW_COLOR);
		    GUIUtilities.setObjectBackground(scrollPane.getVerticalScrollBar(),
			    GUIUtilities.VIEW_COLOR);
		    ImageIcon icon = null;
		    ...
		    view = new InfoNodeView(selectedConfiguration.getDisplayableTitle(), icon,
			    scrollPane, selectedConfiguration);
	    ...
	    if (view != null) {
		view.getWindowProperties().getTabProperties().getTitledTabProperties()
			.getNormalProperties().getComponentProperties().setBackgroundColor(
				GUIUtilities.getViewSelectionColor());
		view.getWindowProperties().getTabProperties().getTitledTabProperties()
			.getHighlightedProperties().getComponentProperties().setBackgroundColor(
				GUIUtilities.getViewSelectionColor());
	...
	final List<IView> views = viewFactory.getViews();
	final Iterator<IView> it = views.iterator();
	while (it.hasNext()) {
	    final InfoNodeView view = (InfoNodeView) it.next();
	    if (enable) {
		view.addTabMouseButtonListener(defaultTabMouseButtonListener);
	    } else {
		view.removeTabMouseButtonListener(defaultTabMouseButtonListener);
	    }
	    if (view.getId() != selectedConfiguration) {
		view.getWindowProperties().getTabProperties().getTitledTabProperties().setEnabled(
			enable);
		view.getWindowProperties().getTabProperties().getTitledTabProperties()
			.setFocusable(enable);
	    }
	...
	if (event != null && event.getSource() != null) {
	    ...
	    final SelectedVCVariationAction selectedVCVariationAction = SelectedVCVariationAction
		    .getCurrentInstance();
