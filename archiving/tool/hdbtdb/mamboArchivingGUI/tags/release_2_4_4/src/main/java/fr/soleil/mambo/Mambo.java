...
public class Mambo {
    ...
    private static ImageIcon appliStart = new ImageIcon(Mambo.class
	...
	final File file = new File(pathToResources + File.separator + "mambo.properties");
	try {
	    if (file.exists()) {

		final InputStream strm = new FileInputStream(file);
		final Properties props = new Properties();
		try {
		    props.load(strm);
		    hdbAvailable = Boolean.valueOf(props.getProperty("mambo.hdb.available", Boolean
    ...
    public static void setHDBpassword(final String bpassword) {
	HDBpassword = bpassword;
    }

    public static void setHDBuser(final String buser) {
	HDBuser = buser;
    }

    public static void setTDBpassword(final String bpassword) {
	TDBpassword = bpassword;
    }

    public static void setTDBuser(final String buser) {
	TDBuser = buser;
    }
	...
	if (args == null || args.length > 6 || args.length < 4) {
	    System.out
	...
	if (args.length == 5) {
		...
		if (argValue.compareToIgnoreCase(DEPLOYED_DB_TDB_ONLY) == 0) {
		    ...
		    throw new Exception("The deployed DB parameter has to be either "
			    + DEPLOYED_DB_TDB_ONLY + " or " + DEPLOYED_DB_ALL);
		}
	    } catch (final Exception e) {
		System.out
			.println("Incorrect arguments. The userRights parameter has to be either "
	...
	setAccountManager(new AccountManager(pathToResources, "Mambo", null, false));
	...
	titleBuffer = null;
	frame.setIconImage(new ImageIcon(Mambo.class.getResource("icons/degrad-all.png"))
		.getImage());
    }
	...
	frame.setTitle(titleBuffer.toString());
	titleBuffer = null;
	frame.setIconImage(new ImageIcon(Mambo.class.getResource("icons/degrad-hdb.png"))
		.getImage());
    }
	...
	frame.setTitle(titleBuffer.toString());
	titleBuffer = null;
	frame.setIconImage(new ImageIcon(Mambo.class.getResource("icons/degrad-tdb.png"))
		.getImage());
    }
}
