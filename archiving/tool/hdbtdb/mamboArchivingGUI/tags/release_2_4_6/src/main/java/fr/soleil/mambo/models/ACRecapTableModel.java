...
public class ACRecapTableModel extends DefaultTableModel {
    ...
    public ACRecapTableModel(final boolean _historic) {
	super();
    ...
    public void load(final ArchivingConfigurationAttribute _attribute, final boolean _historic,
	    final int _mode) {
	attribute = _attribute;
	historic = _historic;
	mode = _mode;
	if (attribute != null) {
	    final ArchivingConfigurationAttributeProperties attrProperties = attribute
		    .getProperties();
	    if (_historic) {
	...
	final Hashtable<String, Mode> modesBuffer = historic ? HDBmodesBuffer : TDBmodesBuffer;
	...
	if (completeName != null) {
	    _mode = modesBuffer.get(completeName);

	    if (_mode == null && completeName != null) {
		try {
		    _mode = ArchivingManagerFactory.getCurrentImpl().getArchivingMode(completeName,
			    historic);
	...
	return getRowCount2(_mode);
    }

    public int getRowCount2(final Mode _mode) {
	switch (mode) {
	case modeP:
