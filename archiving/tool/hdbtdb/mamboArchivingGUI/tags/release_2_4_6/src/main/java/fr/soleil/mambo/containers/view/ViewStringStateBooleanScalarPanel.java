...
public class ViewStringStateBooleanScalarPanel extends MamboCleanablePanel {
    ...
    public ViewStringStateBooleanScalarPanel(final ViewConfigurationAttribute[] stringScalars,
	...
	if (attributes != null && attributes.length > 0) {
		...
		viewTables[i] = new MamboFormatableTable(new ViewStringStateBooleanScalarTableModel(attributes[i]
			.getCompleteName(), viewConfigurationBean));
		if (attributes[i].getDataType(viewConfigurationBean.getViewConfiguration().getData().isHistoric()) == TangoConst.Tango_DEV_STATE) {
		    viewTables[i].setDefaultRenderer(Object.class, new StateTableRenderer());
		} else if (attributes[i].getDataType(viewConfigurationBean.getViewConfiguration().getData()
			.isHistoric()) == TangoConst.Tango_DEV_STRING) {
	...
	final double weighty = 1.0d / viewTables.length;
	for (int i = 0; i < viewTables.length; i++) {
	    ...
	    GUIUtilities.setObjectBackground(titlePanel, GUIUtilities.VIEW_COLOR);
	    copy[i] = new JButton(Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_COPY") + " ("
		    + attributes[i].getName() + ")");
	    copy[i].setEnabled(false);
	    edit[i] = new JButton(Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_EDIT_COPY") + " ("
		    + attributes[i].getName() + ")");
	    ...
	    final Insets defaultGap = new Insets(0, 0, 0, 5);
	    ...
	    final JScrollPane scrollpane = new JScrollPane(viewTables[i]);
	...
	if (viewTables != null && tableIndex < viewTables.length) {
		...
		s += viewTables[tableIndex].getColumnName(i) + Options.getInstance().getGeneralOptions().getSeparator();
	    }
	    s = s.substring(0, s.lastIndexOf(Options.getInstance().getGeneralOptions().getSeparator()));
	    s += "\n";
    ...
    public HashMap<String, DbData[]> loadPanelAndGetAttrMap() {
	HashMap<String, DbData[]> attributeDataMap = new HashMap<String, DbData[]>();
	...
	try {
		...
		try {
		    ...
		    final DbData[] data = ((ViewStringStateBooleanScalarTableModel) viewTables[i].getModel())
			    .loadData();
		    attributeDataMap.put(attributes[i].getCompleteName(), data);
		    copy[i].setEnabled(true);
		    copy[i].addActionListener(new ViewCopyAction(Messages
			    .getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_COPY")
			    + " (" + attributes[i].getName() + ")", this, i));
		    edit[i].setEnabled(true);
		    edit[i].addActionListener(new ViewEditCopyAction(Messages
	...
	HashMap<String, DbData[]> map = loadPanelAndGetAttrMap();
	if (map != null) {
	    map.clear();
	}
	map = null;
	...
	loadingLabel.setForeground(Color.RED);
	for (final MamboFormatableTable viewTable : viewTables) {
