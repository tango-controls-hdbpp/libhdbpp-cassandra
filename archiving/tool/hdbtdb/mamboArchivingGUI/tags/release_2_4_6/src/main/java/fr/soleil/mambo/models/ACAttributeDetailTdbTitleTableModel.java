// +======================================================================
// $Source:
// /cvsroot/tango-cs/tango/tools/mambo/models/ACAttributeDetailTdbTitleTableModel.java,v
// $
//
// Project: Tango Archiving Service
//
// Description: Java source code for the class
// ACAttributeDetailTdbTitleTableModel.
...
// Time formating in assessments + differences between database and archiving
...
public class ACAttributeDetailTdbTitleTableModel extends DefaultTableModel {
	...
	public int getRowCount() {
		...
		if (TDBProperties != null) {
			try {
				...
				if (modes.length == 0 && attribute.getCompleteName() != null) {
	...
	private Object getDBColumnValue(int rowIndex) {
		...
		case 1:
			Mode mode;
			try {
				mode = ArchivingManagerFactory.getCurrentImpl()
						.getArchivingMode(attribute.getCompleteName(), false);
				if (mode != null && mode.getTdbSpec() != null) {
					// ret = mode.getTdbSpec().getExportPeriod() + "ms";
					ret = MamboTimeFormater.formatTime(mode.getTdbSpec()
							.getExportPeriod());
				}
			} catch (Exception e) {
				ret = "";
		...
		/*
		 * case 2: try { mode =
		 * ArchivingManagerFactory.getCurrentImpl().getArchivingMode(
		 * attribute.getCompleteName() , false ); if ( mode != null &&
		 * mode.getTdbSpec() != null ) { //ret =
		 * mode.getTdbSpec().getKeepingPeriod() + "ms"; ret =
		 * MamboTimeFormater.formatTime( mode.getTdbSpec().getKeepingPeriod() );
		 * } } catch ( Exception e ) { ret = ""; e.printStackTrace(); } break;
		 */
		}
		return ret;
	}
	...
	private Object getACColumnValue(int rowIndex) {
		...
		case 1:
			if (modes.length > 0) {
				...
				ret = MamboTimeFormater.formatTime(TDBProperties
						.getExportPeriod());
			}
			break;
		/*
		 * case 2: if ( modes.length > 0 ) { //ret =
		 * TDBProperties.getKeepingPeriod() + "ms"; ret =
		 * MamboTimeFormater.formatTime( TDBProperties.getKeepingPeriod() ); }
		 * break;
		 */
		}
		return ret;
	...
	private Object getSecondColumnValue(int rowIndex) {
		...
		// {
		// case 0:
		// ret = Messages.getMessage(
		// "ARCHIVING_ATTRIBUTES_DETAIL_TITLE_TABLE_CARACTERISTIC" );
		// break;
		// case 1:
		// ret = Messages.getMessage(
		...
		// case 2:
		// ret = Messages.getMessage(
		// "ARCHIVING_ATTRIBUTES_DETAIL_TITLE_TABLE_KEEPING" );
		// break;
		// }
		return Messages
