...
public class BasicArchivingManager extends DbConnectionManager implements IArchivingManager
{
	...
	public void stopArchiving ( ArchivingConfiguration AC ) throws ArchivingException
	{
		...
		for ( int i = 0 ; i < nbOfAttributes ; i++ )
		{
			...
			try
			{
				if (this.hdbManager!=null &&  this.hdbManager.isArchived( nextName ) && ( AC == null || AC.isHistoric() ) )
				{
			...
			try
				{
					if ( this.tdbManager!=null && this.tdbManager.isArchived( nextName   ) && ( AC == null || !AC.isHistoric() ) )
					{
		...
		try
		{
			if ( this.hdbManager!=null && !attributeListFinalHDB.isEmpty() )
			{
				...
				this.hdbManager.ArchivingStop(  attributeListFinalHDB_s );
			}
		}
		catch (ArchivingException e1)
		{
		...
		try
		{
			if (this.tdbManager!=null && !attributeListFinalTDB.isEmpty() )
			{
				...
				this.tdbManager.ArchivingStop(  attributeListFinalTDB_s );
			}
		}
		catch (ArchivingException e2)
		{
	...
	public Mode getArchivingMode ( String completeName , boolean historic ) throws ArchivingException
	{
		Mode mode = null;
		try{
			mode = this.getArchivingManagerApiInstance(historic).GetArchivingMode( completeName  );
		}
		catch(Exception e){
			//e.printStackTrace();
		}
	...
	public boolean isArchived ( String completeName , boolean historic ) throws ArchivingException
	{
		boolean ret = false ;
		try{
			ret  = this.getArchivingManagerApiInstance(historic).isArchived( completeName  );
		}
		catch(Exception e)
		{
			//e.printStackTrace();
	...
	public Archiver getCurrentArchiverForAttribute ( String completeName , boolean _historic ) throws ArchivingException 
	{
		...
		String archiverName = database.getMode().getArchiverForAttributeEvenIfTheStopDateIsNotNull ( completeName );
	...
	{
		try{
		((TDBDataBaseManager)this.tdbManager.getDataBase()).getTdbExport().ExportData2Tdb(attributeName, endDate, this.tdbManager.getDataBase().getMode(),
				this.tdbManager.getDataBase().getAttribute(), this.tdbManager.getDataBase().getDbUtil());
		}
		catch(Exception e){
