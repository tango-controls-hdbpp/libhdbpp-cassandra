...
public class ViewSpectrumStackChartDialog extends JDialog {
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                if (spectrumStackViewer != null) {
                    ...
                    if (ViewSpectrumStackChartDialog.this.specPanel != null) {
                        ViewSpectrumStackChartDialog.this.specPanel
                                .setFileDirectory(spectrumStackViewer.getComponent()
    ...
    public void updateContent() {
        ...
        spectrumStackViewer.addKey(MamboViewDAOFactory.class.getName(), new ViewDAOKey(splitedData,
                checkBoxSelected));
        spectrumStackViewer.switchDAOFactory(MamboViewDAOFactory.class.getName());
        IChartPlayer spectrumStackViewerComponent = spectrumStackViewer.getComponent();
        spectrumStackViewerComponent.setLabelVisible(false);
        spectrumStackViewerComponent.setAxisScaleEditionEnabled(false, IChartViewer.X);
        spectrumStackViewerComponent.setNoValueString(Options.getInstance().getVcOptions()
                .getNoValueString());
        spectrumStackViewerComponent.setFileDirectoryPath(specPanel.getFileDirectory());
        ...
        if (data != null) {
            ...
            for (int i = 0; i < data.size(); i++) {
                for (int j = 0; j < data.get(i).size(); j++) {
                    ...
                    if (name.contains(Messages.getMessage("VIEW_SPECTRUM_READ"))) {
                        // read curves
                        spectrumStackViewerComponent.setDataViewCometeColor(name, CometeColor.RED);
                    }
                    else if (name.contains(Messages.getMessage("VIEW_SPECTRUM_WRITE"))) {
                        // write curves
                        spectrumStackViewerComponent.setDataViewCometeColor(name, CometeColor.BLUE);
                    }
                    else {
                        // should never happen
                        spectrumStackViewerComponent
                                .setDataViewCometeColor(name, CometeColor.BLACK);
                    }
                }
            }
        }
        ...
        if (increment == 0) {
            ...
            spectrumStackViewerComponent.getSlider().setMajorTickSpacing(increment);
    ...
    private void prepareChart() {
        ...
        spectrumStackViewerComponent.getSlider().setCometeForeground(CometeColor.BLACK);
        ...
        spectrumStackViewerComponent.setCometeBackground(AwtColorTool.getCometeColor(GUIUtilities
                .getViewColor()));
    }
}
