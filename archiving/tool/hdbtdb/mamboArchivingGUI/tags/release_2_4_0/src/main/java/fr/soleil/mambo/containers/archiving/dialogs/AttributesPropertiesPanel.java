...
public class AttributesPropertiesPanel extends JPanel {
    ...
    // HDB objects
    private JPanel                           HDBPanel, HDBPeriodPanel,
            HDBModePanel;
    ...
    // TDB objects
    private JPanel                           TDBPanel, TDBPeriodPanel,
            TDBModePanel;
    ...
    private final static Dimension           classicMaxDimension        = new Dimension(
                                                                                Integer.MAX_VALUE,
                                                                                21);
    private final static Dimension           classicPrefDimension       = new Dimension(
                                                                                60,
                                                                                21);
    private final static Dimension           classicFixedDimension      = new Dimension(
                                                                                90,
                                                                                21);
    ...
    public boolean verifyValues() {
        ...
        if (firstTests) {
            try {
                ...
                if (this.isHistoric) {
                    // Check for shortPeriodAttributes property in case of hdb
                    try {
                        ACAttributesPropertiesTree tree = ACAttributesPropertiesTree
                                .getInstance();
                        for (ArchivingConfigurationAttribute a : tree
                                .getListOfAttributesToSet()) {
                            this.getLastMode().checkMode(a.getCompleteName(),
                                    getHDBperiod());
                            if (hasModeHDB_A()) {
                                this.getLastMode().checkMode(
                                        a.getCompleteName(),
                                        getHDBAbsolutePeriod());
                            }
                            if (hasModeHDB_R()) {
                                this.getLastMode().checkMode(
                                        a.getCompleteName(),
                                        getHDBRelativePeriod());
                            }
                            if (hasModeHDB_T()) {
                                this.getLastMode().checkMode(
                                        a.getCompleteName(),
                                        getHDBThresholdPeriod());
                            }
                            if (hasModeHDB_D()) {
                                this.getLastMode().checkMode(
                                        a.getCompleteName(),
                                        getHDBDifferencePeriod());
                            }
                        }
                        return true;
                    } catch (ArchivingException ae) {
                        return treatModeException(ae);
                    }
                } else {
                    // Directly treat exception in case of tdb
    ...
    private boolean treatModeException(ArchivingException ae) {
        ...
        String msgSpecific = ae.getLastExceptionMessage();
