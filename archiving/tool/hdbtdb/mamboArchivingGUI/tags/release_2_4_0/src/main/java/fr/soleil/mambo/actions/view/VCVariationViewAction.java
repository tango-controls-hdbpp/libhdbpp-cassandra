...
public class VCVariationViewAction extends AbstractAction {
     ...
     * @param name
     */
    public VCVariationViewAction(String name, ViewConfigurationBean viewConfigurationBean) {
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        ViewConfigurationData data = svc.getData().cloneData();
        ...
        ViewConfigurationAttributes svcas = svc.getAttributes().cloneAttrs();
        if (viewConfigurationBean.getVariationDialog().getVariationPanel().getWorkingTable()) {
            int[] rows = viewConfigurationBean.getVariationDialog().getVariationPanel()
                    .getResultPanel().getResultTable().getSelectedRows();
            for (int i = 0; i < rows.length; i++) {
                String name = (String) viewConfigurationBean.getVariationDialog()
