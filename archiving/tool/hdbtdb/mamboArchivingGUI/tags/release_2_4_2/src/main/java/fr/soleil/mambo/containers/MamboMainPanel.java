...
public class MamboMainPanel extends JPanel {
    ...
    private MamboMainPanel() {
        ...
        if (Mambo.hasACs()) {
            JSplitPane archivingSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, false);
            ...
            JScrollPane archivingScrollPane = new JScrollPane(ArchivingPanel.getInstance());
            JScrollPane viewScrollPane = new JScrollPane(ViewPanel.getInstance());
    ...
    private void initSplitsPositions() {
        Dimension dim = GraphicsEnvironment.getLocalGraphicsEnvironment().getMaximumWindowBounds()
                .getSize();

        initialViewSplitPosition = (dim.width / 2) - (VIEW_SPLIT_DIVIDER_SIZE / 2);
        initialMessageSplitPosition = dim.height * 4 / 5 - (MESSAGE_SPLIT_DIVIDER_SIZE / 2);
    }
    ...
    public void scrollDownToLatestMessage() {
        SwingUtilities.invokeLater(new Runnable() {
            ...
            @Override
            public void run() {
                synchronized (messageScrollPane) {
