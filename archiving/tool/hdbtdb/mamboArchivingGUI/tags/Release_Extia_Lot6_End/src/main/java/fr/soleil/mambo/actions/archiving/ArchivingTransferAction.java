...
public class ArchivingTransferAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        if (selectedAC != null) {
            ...
            // adding attributes
            TreeMap<String, ArchivingConfigurationAttribute> acTable = acModel
                    .getAttributes();
            ...
            int timePrecision = 0;
            if (selectedAC != null) {
                ArchivingConfigurationAttribute[] attributes = selectedAC
                        .getAttributes().getAttributesList();
                if (attributes.length > 0) {
                    if (selectedAC.isHistoric()) {
                        try {
                            timePrecision = attributes[0].getProperties()
                                    .getHDBProperties().getMode(
                                            ArchivingConfigurationMode.TYPE_P)
                                    .getMode().getModeP().getPeriod() / 2;
                        } catch (Exception e) {
                            timePrecision = 0;
                        }
                    } else {
            ...
            newData.getGeneralChartProperties().setLegendsAreVisible(false);
            ...
            while (it.hasNext()) {
                String key = it.next();
                ViewConfigurationAttribute vca = new ViewConfigurationAttribute(
                        (Attribute) acTable.get(key));
                vcTable.put(key, vca);
            }
