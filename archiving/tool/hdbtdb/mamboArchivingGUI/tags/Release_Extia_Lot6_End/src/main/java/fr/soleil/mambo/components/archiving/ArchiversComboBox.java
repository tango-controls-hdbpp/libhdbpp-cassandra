...
public class ArchiversComboBox extends JComboBox {
	...
	public void setArchivers() {
		...
		for (int i = 0; i < archivers.length; i++) {
			archivers[i].load();
			// System.out.println (
			// "ArchiversComboBox/setArchivers/i|"+i+"|getName|"+archivers[ i
			// ].getName()+"|isDedicated|"+archivers[ i ].isDedicated() );
			// archivers[ i ].trace ();
	...
	public Archiver getSelectedArchiver() {
		...
		if (currentAttributeName == null
				|| currentAttributeName.equals(ArchiversComboBox.NO_SELECTION)) {
			return null;
		}
	...
	private class ArchiversComboBoxActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			Archiver selectedArchiver = ArchiversComboBox.this
					.getSelectedArchiver();
			String selectedArchiverText = selectedArchiver == null ? DEFAULT
					: selectedArchiver.getName();

			AttributesPropertiesPanel attributesPropertiesPanel = AttributesPropertiesPanel
					.getInstance();
			if (attributesPropertiesPanel != null) {
				attributesPropertiesPanel
						.setArchiverChoiceText(selectedArchiverText);
			}
		}
	}
	...
	/*
	 * public Archiver getRunningArchiverForName Archiver archiver = ( Archiver
	 * ) ArchiversComboBox.this.elements.get( archiverName );
	 */
	...
	private class ArchiversComboBoxRenderer extends BasicComboBoxRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			Archiver archiver = (Archiver) ArchiversComboBox.this.elements
					.get(archiverName);
			...
			Color color = archiver
					.getAssociationColor(ArchiversComboBox.this.currentlySelectedAttribute);
