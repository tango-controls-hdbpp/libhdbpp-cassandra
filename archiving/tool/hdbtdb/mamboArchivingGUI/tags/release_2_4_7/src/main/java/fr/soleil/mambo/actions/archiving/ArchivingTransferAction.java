...
public class ArchivingTransferAction extends AbstractAction {
    ...
    public static ArchivingTransferAction getInstance(final String name) {
	if (instance == null) {
    ...
    private ArchivingTransferAction(final String name) {
	...
	final ArchivingConfiguration selectedAC = ArchivingConfiguration
		.getSelectedArchivingConfiguration();

	if (selectedAC != null) {
	    final ACAttributesTreeModel acModel = ACAttributesTreeModel.getInstance();

	    // adding attributes
	    final TreeMap<String, ArchivingConfigurationAttribute> acTable = acModel
		    .getAttributes();
	    final Set<String> attrSet = acTable.keySet();
	    final Iterator<String> it = attrSet.iterator();
	    final TreeMap<String, ViewConfigurationAttribute> vcTable = new TreeMap<String, ViewConfigurationAttribute>(
		    Collator.getInstance());

	    final ViewConfigurationData newData = new ViewConfigurationData();

	    int timePrecision = 0;
	    if (selectedAC != null) {
		final ArchivingConfigurationAttribute[] attributes = selectedAC.getAttributes()
			.getAttributesList();
	    ...
	    final ViewConfiguration transferedVC = new ViewConfiguration();
	    final Timestamp now = GUIUtilities.now();
	    final Timestamp before = new Timestamp(now.getTime() - 3600000);
		...
		final ViewConfigurationAttribute vca = new ViewConfigurationAttribute(acTable
			.get(key));
		vcTable.put(key, vca);
	    }
