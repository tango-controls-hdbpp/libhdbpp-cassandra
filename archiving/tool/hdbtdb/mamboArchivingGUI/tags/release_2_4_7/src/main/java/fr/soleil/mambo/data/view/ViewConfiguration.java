...
public class ViewConfiguration {
     ...
     */
    public ChartViewer getChart(final String idViewSelected) throws ArchivingException, SQLException {
	...
	try {
		...
		chart = attributes.setChartAttributes(chart, range[0].toString(), range[1].toString(), historic, data
			.getSamplingType());
	    } else {
		chart = attributes.setChartAttributes(chart, data.getStartDate().toString(), data.getEndDate()
    ...
    public void loadAttributes(ChartViewer chart) throws ArchivingException, SQLException {
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
		...
		final int choice = JOptionPane.showConfirmDialog(MamboFrame.getInstance(), Messages
			...
			.getMessage("DIALOGS_FILE_CHOOSER_FILE_EXISTS_TITLE"), JOptionPane.YES_NO_OPTION,
	...
	if (lastUpdateDate != null) {
	    openingLine
		    .setAttribute(ViewConfigurationData.LAST_UPDATE_DATE_PROPERTY_XML_TAG, lastUpdateDate.toString());
	}
	if (startDate != null) {
    ...
    public void setChartAttributeExpressionOfString(final HashMap<String, DbData[]> dataMap, final ChartViewer chart)
