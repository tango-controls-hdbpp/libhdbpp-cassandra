...
public class SpectrumDataWriter {
     ...
     */
    public static void writeDataInFile(final File toSave, final DbData readData,
            final DbData writeData, final List<Integer> selectedRead,
            final List<Integer> selectedWrite, final int spectrumType,
                    ...
                    try {
                                ...
                                for (int i = 0; i < timeLengthRead; i++) {
                                    if ((selectedRead == null) || (selectedRead.contains(i))) {
                                        if (readData.getData_timed()[i].time == null) {
                                            ...
                                            calendar
                                                    .setTimeInMillis(readData.getData_timed()[i].time);
                                ...
                                for (int i = 0; i < timeLengthWrite; i++) {
                                    if ((selectedWrite == null) || (selectedWrite.contains(i))) {
                                        if (writeData.getData_timed()[i].time == null) {
                                            ...
                                            calendar
                                                    .setTimeInMillis(writeData.getData_timed()[i].time);
                                            fw.write(DATE_FORMAT.format(calendar.getTime()));
                                        }
                                        fw.write(" ");
                                        fw.write(Messages.getMessage("VIEW_SPECTRUM_WRITE"));
                        ...
                        switch (spectrumType) {
                            case ViewConfigurationAttributePlotProperties.SPECTRUM_VIEW_TYPE_INDEX:
                                ...
                                while ((timeIndexRead < timeLengthRead)
                                        || (timeIndexWrite < timeIndexWrite)) {
                                    boolean canWrite = true;
                                    long minTime;
                                    if ((timeIndexRead < timeLengthRead)
                                            && (timeIndexWrite < timeIndexWrite)) {
                                        // Read and Write Values available
                                        if ((readData.getData_timed()[timeIndexRead].time == null)
                                                && (writeData.getData_timed()[timeIndexWrite].time == null)) {
                                            // Stupid case with no time: Ignore this case
                                            canWrite = false;
                                            timeIndexRead++;
                                            timeIndexWrite++;
                                        }
                                            ...
                                            // Only Write value
                                            appendSecondsToBufferFromMilliseconds(writeData
                                                    .getData_timed()[timeIndexWrite].time, fw);
                                            fillIndexBufferWithNoValueString(fw, indexLengthRead,
                                                    selectedRead);
                                            ...
                                            // Only Read value
                                            appendSecondsToBufferFromMilliseconds(readData
                                                    .getData_timed()[timeIndexRead].time, fw);
                                            appendIndexesToBuffer(fw, readData, timeIndexRead,
                                                    indexLengthRead, selectedRead);
                                            ...
                                            // Both Read and Write values are really available
                                            minTime = Math.min(
                                                    readData.getData_timed()[timeIndexRead].time
                                                            .longValue(),
                                                    writeData.getData_timed()[timeIndexWrite].time
                                                            .longValue());
                                            appendSecondsToBufferFromMilliseconds(minTime, fw);
                                            if (minTime == readData.getData_timed()[timeIndexRead].time
                                                    .longValue()) {
                                                // minTime at least for Read value
                                                appendIndexesToBuffer(fw, readData, timeIndexRead,
                                                        indexLengthRead, selectedRead);
                                                timeIndexRead++;
                                                if (minTime == writeData.getData_timed()[timeIndexWrite].time
                                                        .longValue()) {
                                                    // minTime for both Read and Write values
                                                    appendIndexesToBuffer(fw, writeData,
                                                            timeIndexWrite, indexLengthWrite,
                                                            selectedWrite);
                                                    timeIndexWrite++;
                                                }
                                                else {
                                                    // minTime for Read value only
                                                    fillIndexBufferWithNoValueString(fw,
                                                            indexLengthWrite, selectedWrite);
                                                }
                                            } // end if (minTime ==
                                            // readData.getData_timed()[timeIndexRead].time.longValue())
                                            else {
                                    ...
                                    } // end if ((timeIndexRead < timeLengthRead) && (timeIndexWrite
                                    // < timeIndexWrite))
                                        ...
                                        fillIndexBufferWithNoValueString(fw, indexLengthWrite,
                                                selectedWrite);
                                        timeIndexRead++;
                                    } // end else if (timeIndexRead < timeLengthRead)
                                        ...
                                        appendIndexesToBuffer(fw, writeData, timeIndexWrite,
                                                indexLengthWrite, selectedWrite);
                                        timeIndexWrite++;
                                    } // end else if (timeIndexWrite < timeIndexWrite)
                                    if (canWrite) {
                                        fw.write("\n");
                                        fw.flush();
                                    }
                                } // end while ((timeIndexRead < timeLengthRead) || (timeIndexWrite
                                // < timeIndexWrite))
                                break;
                            case ViewConfigurationAttributePlotProperties.SPECTRUM_VIEW_TYPE_TIME:
                            case ViewConfigurationAttributePlotProperties.SPECTRUM_VIEW_TYPE_TIME_STACK:
                                ...
                                for (int index = 0; index < maxIndexLimit; index++) {
                                    ...
                                    fw.write("\t");
                                    appendTimesToBuffer(fw, readData, timeLengthRead, index,
                                            selectedRead);
                                    appendTimesToBuffer(fw, writeData, timeLengthWrite, index,
                                            selectedWrite);
                                    fw.write("\n");
                                    fw.flush();
                                }
                        ...
                        e.printStackTrace();
                        JOptionPane.showMessageDialog(caller, Messages
                                .getMessage("DIALOGS_FILE_CHOOSER_FILE_SAVING_ERROR")
                                + e.getMessage());
                    }
            ...
            if (index) {
                for (int timeIndex = 0; timeIndex < data.getData_timed().length; timeIndex++) {
                    if ((data.getData_timed()[timeIndex].value != null)
                            && (data.getData_timed()[timeIndex].value.length > limit)) {
    ...
    protected static void fillIndexBufferWithNoValueString(FileWriter fw, int maxIndex,
            List<Integer> selectedIndexes) throws IOException {
        if (fw != null) {
     ...
     * @throws IOException if a problem occurred while file writing
     */
    protected static void appendIndexesToBuffer(FileWriter fw, DbData data, int timeIndex,
            int maxIndex, List<Integer> selectedIndexes) throws IOException {
        if (fw != null) {
            for (int index = 0; index < maxIndex; index++) {
                if ((selectedIndexes == null) || (selectedIndexes.contains(index))) {
                    if ((data.getData_timed()[timeIndex].value != null)
     ...
     * @throws IOException if a problem occurred while file writing
     */
    protected static void appendSecondsToBufferFromMilliseconds(long milli, FileWriter fw)
            throws IOException {
        if (fw != null) {
            long ts = milli / 1000;
            long ms = milli % 1000;
     ...
     * @throws IOException if a problem occurred while file writing
     */
    protected static void appendTimesToBuffer(FileWriter fw, DbData data, int timeLength,
            int currentIndex, List<Integer> selectedTimes) throws IOException {
        if (fw != null) {
            for (int time = 0; time < timeLength; time++) {
                if ((data.getData_timed()[time].value != null)
                        && (currentIndex < data.getData_timed()[time].value.length)
