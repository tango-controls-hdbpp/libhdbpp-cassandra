...
public class MamboTimeFormater {
	/**
	 * Returns a String representing the value of a time represented as days,
	 * hours, minutes, seconds and milliseconds
	 * 
	 * @param totalTime
	 *            the time, in milliseconds, to convert
	 * @return a String representing the value of a time represented as days,
	 *         hours, minutes, seconds and milliseconds. example : totalTime =
	 *         96000000 will return "1d 2hr 40mn"
	 */
	public static String formatTime(long totalTime) {
		...
		if (days > 0) {
			result.append(days).append(
					Messages.getMessage("STANDARD_MESSAGES_DAYS")).append(" ");
		}

		if (hours > 0) {
			result.append(hours).append(
					Messages.getMessage("STANDARD_MESSAGES_HOURS")).append(" ");
		}

		if (minutes > 0) {
			result.append(minutes).append(
					Messages.getMessage("STANDARD_MESSAGES_MINUTES")).append(
					" ");
		}

		if (seconds > 0) {
			result.append(seconds).append(
					Messages.getMessage("STANDARD_MESSAGES_SECONDS")).append(
					" ");
		}

		if (milliseconds > 0) {
			result.append(milliseconds).append(
					Messages.getMessage("STANDARD_MESSAGES_MILLISECONDS"));
