...
public class NumberTextField extends JTextField
{
    ...
    protected class TextFocusAdapter extends FocusAdapter
    {
        ...
        public void focusLost (FocusEvent f)
        {
            ...
            {
                msg = Messages.getMessage("STANDARD_MESSAGES_POSITIVE_INTEGER_ERROR");
                if ( !((NumberTextField) f.getComponent()).getText().matches("[0-9]+") )
                {
                    inError = true;
                }
            }
            ...
            {
                msg = Messages.getMessage("STANDARD_MESSAGES_POSITIVE_FLOAT_ERROR");
                if ( !((NumberTextField) f.getComponent()).getText().matches("[0-9]+")
                     && !((NumberTextField) f.getComponent()).getText().matches("[0-9]+\\.[0-9]+")
                   )
                {
                    inError = true;
                }
            ...
            else if ( ((NumberTextField) f.getComponent()).getNumberType() == NumberTextField.INTEGER_LIKE )
            {
                ...
                if ( !((NumberTextField) f.getComponent()).getText().matches("[0-9]+")
                     && !((NumberTextField) f.getComponent()).getText().matches("\\-[0-9]+")
                   )
                {
                    inError = true;
                }
            ...
            if ( inError )
            {
                JOptionPane.showMessageDialog(MamboFrame.getInstance(),msg,Messages.getMessage("STANDARD_MESSAGES_ERROR"),JOptionPane.ERROR_MESSAGE);
                ((NumberTextField) f.getComponent()).setText(((NumberTextField) f.getComponent()).formerText);
            }
            else
            {
                ((NumberTextField) f.getComponent()).formerText = ((NumberTextField) f.getComponent()).getText();
            }
            msg = null;
            inError = false;
        } // end focusLost()
