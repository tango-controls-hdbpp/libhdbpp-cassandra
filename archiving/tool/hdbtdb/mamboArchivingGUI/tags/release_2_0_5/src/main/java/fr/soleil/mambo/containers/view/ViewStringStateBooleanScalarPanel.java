/*	Synchrotron Soleil 
 *  
 *   File          :  ViewStringStateBooleanScalarPanel.java
 *  
 *   Project       :  Mambo_CVS
 *  
 *   Description   :  
 ...
 *   Author        :  SOLEIL
 *  
 *   Original      :  6 mars 2006 
 *  
 *   Revision:  					Author:  
 *   Date: 							State:  
 *  
 *   Log: ViewStringStateBooleanScalarPanel.java,v 
 *
 */
...
public class ViewStringStateBooleanScalarPanel extends MamboCleanablePanel
{
    ...
    public ViewStringStateBooleanScalarPanel(ViewConfigurationAttribute[] stringScalars)
    {
        super();
        ...
        if (attributes != null && attributes.length > 0)
        {
            ...
            for (int i = 0; i < attributes.length; i++)
            {
                ...
                viewTables[i] = new MamboFormatableTable(new ViewStringStateBooleanScalarTableModel(attributes[i].getCompleteName()));
                if (  attributes[i].getDataType( ViewConfiguration.getSelectedViewConfiguration().getData().isHistoric() ) == TangoConst.Tango_DEV_STATE  )
                {
                    viewTables[i].setDefaultRenderer(Object.class, new StateTableRenderer ());
                }
                else if (  attributes[i].getDataType( ViewConfiguration.getSelectedViewConfiguration().getData().isHistoric() ) == TangoConst.Tango_DEV_STRING  )
                {
    ...
    private void addComponents()
    {
        ...
        for (int i = 0; i < viewTables.length; i++)
        {
            ...
            copy[i] = new JButton(
                    Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_COPY")
                    + " ("
            ...
            edit[i] = new JButton(
                    Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_EDIT_COPY")
                    + " ("
                    + attributes[i].getName()
                    + ")"
            );
            ...
            SpringUtilities.makeCompactGrid(
                    titlePanel,
                    1, titlePanel.getComponentCount(), // rows, columns
                    5, 5, // init x, init y
                    5, 5, // xpad, ypad
    ...
    public void clean ()
    {
        ...
        if (viewTables != null)
        {
            for(int i = 0; i < viewTables.length; i++)
            {
                ...
                {
                    if (viewTables[i].getModel() != null && viewTables[i].getModel() instanceof ViewStringStateBooleanScalarTableModel)
                    {
    ...
    {
        System.out.println("-------loadPanel() :" + new Timestamp(System.currentTimeMillis()).toString());
        ...
        try
        {
            ...
            {
                if (extractingManager.isCanceled() || cleaning) return;
                ( (ViewStringStateBooleanScalarTableModel)viewTables[i].getModel() ).loadData();
                copy[i].setEnabled(true);
                copy[i].addActionListener(
                    new ViewCopyAction(
                        Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_COPY")
                          + " ("
                          + attributes[i].getName()
                ...
                edit[i].addActionListener(
                    new ViewEditCopyAction(
                            Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_EDIT_COPY")
                              + " ("
                              + attributes[i].getName()
                              + ")",
