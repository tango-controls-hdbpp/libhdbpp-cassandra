...
public class ViewSpectrumStackChartDialog extends JDialog {
    ...
    public void updateContent() {
        ...
        if (dao != null) {
            // we affect red color for read curves, and blue color for write
            // curves
                ...
                for (DataArray da : dal) {
                    ...
                    if (da.getName().contains(
                        ...
                        spectrumStackViewer.getViewer().setDataViewColor(
                                da.getName(), Color.RED);
                    }
                    // write curves
                    else if (da.getName().contains(
                            Messages.getMessage("VIEW_SPECTRUM_WRITE"))) {
                        spectrumStackViewer.getViewer().setDataViewColor(
                                da.getName(), Color.BLUE);
                    }
                    // should never happen
                    else {
                        spectrumStackViewer.getViewer().setDataViewColor(
                                da.getName(), Color.DARK_GRAY);
                    }
                }
            }
            ...
            if (increment == 0) {
                spectrumStackViewer.getSlider().setPaintLabels(false);
            } else {
                spectrumStackViewer.getSlider().setPaintLabels(true);
