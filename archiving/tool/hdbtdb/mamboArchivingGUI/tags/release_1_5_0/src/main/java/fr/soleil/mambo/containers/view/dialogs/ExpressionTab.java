...
public class ExpressionTab extends JPanel
{
    ...
    protected JPanel attributesPanel, detailPanel, actionPanel, treePanel;
    protected JLabel nameLabel, expressionLabel, expressionField, variableHeaderLabel;
    protected JTextField nameField;
    protected JButton addButton, applyButton, deleteButton;
    ...
    protected void initComponents()
    {
        ...
        nameField.getDocument().addDocumentListener( new ExpressionNameFieldListener() );
        nameField.setToolTipText(Messages.getMessage("EXPRESSION_ATTRIBUTE_HELP"));
        ...
        variableScrollPane = new JScrollPane(ViewExpressionVariablesDetailPanel.getInstance());
        ...
        propertiesPanel = new AttributesPlotPropertiesPanel( true );
    ...
    protected void initLayout()
    {
        ...
        // end init treePanel layout

        attributesPanel.setLayout(new SpringLayout());
        SpringUtilities.makeCompactGrid(
                attributesPanel,
                attributesPanel.getComponentCount(), 1,
                0, 0,
                0, 0,
                true
        );
    ...
    public void addExpression()
    {
        ...
        else if (expressionField.getText() == null || "".equals(expressionField.getText().trim()))
        {
            errorDialog = new MamboErrorDialog(
                    VCEditDialog.getInstance(),
                    Messages.getMessage("EXPRESSION_ERROR_TITLE"), 
                    Messages.getMessage("EXPRESSION_EXPRESSION_ERROR")
            );
            int width = errorDialog.getWidth();
            int height = errorDialog.getHeight();
            int x = VCEditDialog.getInstance().getX() + 50;
        ...
        else
        {
            ...
            currentAttribute = new ExpressionAttribute(
                    nameField.getText().trim(),
                    expressionField.getText().trim(),
                    ViewExpressionVariablesDetailPanel.getInstance().getVariables(),
                    ViewExpressionVariablesDetailPanel.getInstance().isX()
            );
            
            propertiesPanel.setColorIndex(colorIndex);
            colorIndex = propertiesPanel.rotateCurveColor();
            ...
            ViewConfigurationAttributePlotProperties plotProperties = new ViewConfigurationAttributePlotProperties( 
                    propertiesPanel.getViewType() ,
                    propertiesPanel.getAxisChoice(),
                    propertiesPanel.getBar() , 
                    propertiesPanel.getCurve() ,
                    propertiesPanel.getMarker() ,
                    propertiesPanel.getTransform(),
                    propertiesPanel.isHidden()
            );
            currentAttribute.setProperties(plotProperties);
            if ( !ExpressionTreeModel.getInstance().addAttribute(currentAttribute) )
            {
                errorDialog = new MamboErrorDialog(
                        VCEditDialog.getInstance(),
                        Messages.getMessage("EXPRESSION_ERROR_TITLE"), 
                        Messages.getMessage("EXPRESSION_ATTRIBUTE_NAME_PRESENT_ERROR")
                );
                int width  = errorDialog.getWidth();
                int height = errorDialog.getHeight();
                int x = VCEditDialog.getInstance().getX() + 50;
    ...
    public void delete()
    {
        Vector selectedAttributes = ExpressionTree.getInstance().getSelectedAttributes();
        if (selectedAttributes == null)
        {
            errorDialog = new MamboErrorDialog(
                    VCEditDialog.getInstance(),
                    Messages.getMessage("EXPRESSION_ERROR_TITLE"), 
                    Messages.getMessage("EXPRESSION_SELECTION_ERROR")
            );
            int width = errorDialog.getWidth();
            int height = errorDialog.getHeight();
            int x = VCEditDialog.getInstance().getX() + 50;
        ...
        {
            names[i] = ((ExpressionAttribute)selectedAttributes.get(i)).getName();
        }
    ...
    public void apply()
    {
        ExpressionAttribute attr = ExpressionTree.getInstance().getSelectedAttribute();
        if (attr != null)
        {
            ...
            else
            {
                ...
                currentAttribute.setVariables(ViewExpressionVariablesDetailPanel.getInstance().getVariables());
                currentAttribute.setX(ViewExpressionVariablesDetailPanel.getInstance().isX());
                if ( !ExpressionTreeModel.getInstance().updateAttribute(
                         name,
    ...
    protected void reloadPropertiesPanel()
    {
        ...
        propertiesPanel.setBarFillingMethod(properties.getBar().getFillingMethod());
        ...
        Polynomial2OrderTransform transform = (Polynomial2OrderTransform)properties.getTransform();
        if (transform != null)
        {
    ...
    protected void reloadDetailPanel()
    {
        ...
        ViewExpressionVariablesDetailPanel.getInstance().setVariables(currentAttribute.getVariables());
        ViewExpressionVariablesDetailPanel.getInstance().setX(currentAttribute.isX());
    ...
    public void editExpression()
    {
        ...
        ExpressionEditDialog.getInstance().setExpression(expressionField.getText());
