...
//as many loops as there are attributes in the VC
            {
                ...
                if ( currentAttributeType.equals( ViewConfigurationAttribute.XML_TAG ) )
                {
                    ...
                    currentAttribute = loadCurrentAttribute( currentAttribute , currentAttributeNode );
                    viewConfAttr.addAttribute( currentAttribute );
                }
                else if ( currentAttributeType.equals( ViewConfigurationData.GENERIC_PLOT_PARAMETERS_XML_TAG ) )
                {
                    ViewConfigurationData viewConfData = viewConf.getData();
                    viewConfData = loadGenericPlotParameters( viewConfData , currentAttributeNode );

                    viewConf.setData( viewConfData );
                }
                else if ( currentAttributeType.equals( ViewConfigurationAttributes.COLOR_INDEX_XML_TAG ) )
                {
                    Hashtable colorIndexProperties = XMLUtils.loadAttributes( currentAttributeNode );
                    ...
                    catch(NumberFormatException nfe)
                    {
                        ...
                        //nothing to do : colorIndex automaticly initialized with 0
                    }
                }
                else
                {
