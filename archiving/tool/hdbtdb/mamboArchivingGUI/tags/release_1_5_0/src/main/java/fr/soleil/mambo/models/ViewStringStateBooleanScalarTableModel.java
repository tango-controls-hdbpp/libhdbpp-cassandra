/*	Synchrotron Soleil 
 *  
 *   File          :  ViewStringStateBooleanScalarTableModel.java
 *  
 *   Project       :  Mambo_CVS
 *  
 *   Description   :  
 ...
 *   Author        :  SOLEIL
 *  
 *   Original      :  7 mars 2006 
 *  
 *   Revision:  					Author:  
 *   Date: 							State:  
 *  
 *   Log: ViewStringStateBooleanScalarTableModel.java,v 
 *
 */
...
public class ViewStringStateBooleanScalarTableModel extends DefaultTableModel
{
    ...
    public int getColumnCount() {
        ...
        if (readReference != null && readReference.getData_timed() != null && writeReference != null && writeReference.getData_timed() != null)
        {
            ...
            {
                if (
                        (readReference.getData_timed()[0].value != null && readReference.getData_timed()[0].value.length > 0)
                        && 
                        (writeReference.getData_timed()[0].value != null && writeReference.getData_timed()[0].value.length > 0)
                   )
                {
                    col = 3;
                }
        ...
        switch (columnIndex) {
            case 0:
                if (readReference != null && readReference.getData_timed() != null && readReference.getData_timed().length > 0)
                {
                    ...
                    {
                        return genFormatUS.format(
                                new Date( readReference.getData_timed()[rowIndex].time.longValue() )
                        );
                    }
                }
                if (writeReference != null && writeReference.getData_timed() != null && writeReference.getData_timed().length > 0)
                {
                    ...
                    {
                        return genFormatUS.format(
                                new Date( writeReference.getData_timed()[rowIndex].time.longValue() )
                        );
                    }
                }
                return null;
    ...
    public String getColumnName ( int column )
    {
        switch ( column )
        {
            ...
            case 1:
                ...
                else return Messages.getMessage("DIALOGS_VIEW_TAB_SCALAR_STRING_AND_STATE_VALUE");
            case 2:
                if (writeReference != null && writeReference.getData_timed() != null && writeReference.getData_timed().length > 0)
                {
                    if (writeReference.getData_timed()[0].value != null && writeReference.getData_timed()[0].value.length > 0)
    ...
    public void loadData()
    {
        ...
        ViewConfiguration conf = ViewConfiguration.getSelectedViewConfiguration();
        if (conf != null)
        {
            ...
            try
            {
                startDate = extractingManager.timeToDateSGBD(conf.getData().getStartDate().getTime());
                endDate = extractingManager.timeToDateSGBD(conf.getData().getEndDate().getTime());
            }
            catch ( Exception e )
            {
                e.printStackTrace();
            ...
            try
            {
                Hashtable table = extractingManager.retrieveDataHash(param, conf.getData().isHistoric(),conf.getData ().getSamplingType () );
                if (extractingManager.isCanceled()) return;
                if (table == null)
                {
                    return;
                ...
                if ( table.get(ViewConfigurationAttribute.READ_DATA_VIEW_KEY) != null 
                     && table.get(ViewConfigurationAttribute.READ_DATA_VIEW_KEY) instanceof DbData )
                {
                    readReference = (DbData)table.get(ViewConfigurationAttribute.READ_DATA_VIEW_KEY);
                }

                if ( table.get(ViewConfigurationAttribute.WRITE_DATA_VIEW_KEY) != null 
                     && table.get(ViewConfigurationAttribute.WRITE_DATA_VIEW_KEY) instanceof DbData )
                {
                    writeReference = (DbData)table.get(ViewConfigurationAttribute.WRITE_DATA_VIEW_KEY);
                }
                table.clear();
                table = null;
            }
            ...
            {
                // nothing to do : let "readReference" and "writeReference" null;
            }
        }
