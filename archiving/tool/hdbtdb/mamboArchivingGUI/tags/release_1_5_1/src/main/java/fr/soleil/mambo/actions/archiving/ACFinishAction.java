...
public class ACFinishAction extends AbstractAction
{
     ...
     * @return
     */
    public static ACFinishAction getInstance ( String name )
    {
        if ( instance == null )
        {
     ...
     * @param name
     */
    private ACFinishAction ( String name )
    {
     ...
     * @return
     */
    /*private boolean verifyModes() 
    {
        ArchivingConfiguration currentAC = ArchivingConfiguration.getCurrentArchivingConfiguration();

        if ( containsBadModes () )
        {
            String msgTitle = Messages.getMessage( "DIALOGS_EDIT_AC_BAD_MODES_CONFIRM_TITLE" );
            String msgConfirm = Messages.getMessage( "DIALOGS_EDIT_AC_BAD_MODES_CONFIRM_LABEL_1" );
            msgConfirm += GUIUtilities.CRLF;
            msgConfirm += Messages.getMessage( "DIALOGS_EDIT_AC_BAD_MODES_CONFIRM_LABEL_2" );
            String msgCancel = Messages.getMessage( "DIALOGS_EDIT_AC_BAD_MODES_CONFIRM_CANCEL" );
            String msgValidate = Messages.getMessage( "DIALOGS_EDIT_AC_BAD_MODES_CONFIRM_VALIDATE" );
     ...
     * @return
     */
    /*private boolean containsBadModes() 
    {
        ACAttributesTreeModel model = ACAttributesTreeModel.getInstance();
        Hashtable htAttr = model.getAttributes();
        if ( htAttr == null )
        {
            return false;
        }
        ...
        while ( enum.hasMoreElements() )
        {
            ...
            ArchivingConfigurationAttribute nextValue = ( ArchivingConfigurationAttribute ) htAttr.get( nextKey );
            ...
            ArchivingConfigurationAttributeDBProperties prop;
            long exportPeriod = 0;
            long keepingPeriod = 0;
            if ( currentAC.isHistoric() )
            {
                prop = nextValue.getProperties ().getHDBProperties ();
            }
            else
            {
                prop = nextValue.getProperties ().getTDBProperties ();
                keepingPeriod = nextValue.getProperties ().getTDBProperties ().getKeepingPeriod ();
                exportPeriod = nextValue.getProperties ().getTDBProperties ().getExportPeriod ();
            }
            ArchivingConfigurationMode [] modes =  prop.getModes ();
            ...
            {
                Mode mode = modes [ i ].getMode ();
                try
                {
                    if ( ! currentAC.isHistoric() )
                    {
                        mode.setTdbSpec ( new TdbSpec ( exportPeriod , keepingPeriod ) );
                    }
                    mode.checkMode ( currentAC.isHistoric () );
                }
                catch ( Exception e )
                {
                    e.printStackTrace ();
    ...
    private boolean verifyAttributesAreSet ()
    {
        ...
        {
            String msgTitle = Messages.getMessage( "DIALOGS_EDIT_AC_ATTRIBUTES_NON_SET_CONFIRM_TITLE" );
            ...
            String msgCancel = Messages.getMessage( "DIALOGS_EDIT_AC_ATTRIBUTES_NON_SET_CONFIRM_CANCEL" );
            String msgValidate = Messages.getMessage( "DIALOGS_EDIT_AC_ATTRIBUTES_NON_SET_CONFIRM_VALIDATE" );
            Object[] options = {msgValidate, msgCancel};
