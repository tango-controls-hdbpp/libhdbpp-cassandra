...
public class GlobalStartArchivingManager extends BasicArchivingManager
{
    public void startArchiving ( ArchivingConfiguration AC ) throws ArchivingException
    {
        ...
        for ( int i = 0 ; i < nbOfAttributes ; i++ )
        {
            ...
            ArchivingConfigurationAttributeProperties nextProperties = next.getProperties();
            
            if ( AC.isHistoric () )
            {
                ArchivingConfigurationAttributeHDBProperties nextHDBProperties = nextProperties.getHDBProperties();
                String nextHDBDedicatedArchiver = nextHDBProperties.getDedicatedArchiver ();
                
                if ( !nextHDBProperties.isEmpty() )
                {
                    if ( archivingMessConfigHDB == null )
                    {
                        archivingMessConfigHDB = ArchivingMessConfig.basicObjectCreation();
                    }

                    ArchivingConfigurationMode[] HDBModes = nextHDBProperties.getModes();

                    AttributeLightMode attributeLightMode = new AttributeLightMode();
                    attributeLightMode.setAttribute_complete_name( next.getCompleteName() );
                    Mode mode = ArchivingConfigurationMode.buildCompleteHDBMode( HDBModes );
                    attributeLightMode.setMode( mode );

                    archivingMessConfigHDB.add( attributeLightMode );
                    archivingMessConfigHDB.setDeviceInChargeForAttribute ( attributeLightMode , nextHDBDedicatedArchiver );
                }
            }
            else
            {
                ArchivingConfigurationAttributeTDBProperties nextTDBProperties = nextProperties.getTDBProperties();
                String nextTDBDedicatedArchiver = nextTDBProperties.getDedicatedArchiver ();
                
                if ( !nextTDBProperties.isEmpty() )
                {
                    if ( archivingMessConfigTDB == null )
                    {
                        archivingMessConfigTDB = ArchivingMessConfig.basicObjectCreation();
                    }

                    ArchivingConfigurationMode[] TDBModes = nextTDBProperties.getModes();

                    AttributeLightMode attributeLightMode = new AttributeLightMode();
                    attributeLightMode.setAttribute_complete_name( next.getCompleteName() );
                    //Mode mode = ArchivingConfigurationMode.buildCompleteTDBMode( TDBModes , nextTDBProperties.getExportPeriod() , nextTDBProperties.getKeepingPeriod() );
                    //In order to be compatible with MySQL working, keeping period is set to 3 days = 3 * 24 * 60 * 60 * 1000 = 259200000 ms.
                    Mode mode = ArchivingConfigurationMode.buildCompleteTDBMode( TDBModes , nextTDBProperties.getExportPeriod() , 259200000 );
                    attributeLightMode.setMode( mode );

                    archivingMessConfigTDB.add( attributeLightMode );
                    archivingMessConfigTDB.setDeviceInChargeForAttribute ( attributeLightMode , nextTDBDedicatedArchiver );
                }
            }
        }
        ...
        if ( archivingMessConfigHDB != null )
        {
            ...
            try
            {
                this.manager.ArchivingStart( true , archivingMessConfigHDB );
            }
            catch ( ArchivingException e )
            {
                e.printStackTrace ();
                archivingException.addStack( "Failed when launching historical archiving" , e );
                needsThrow = true;
            }
        }
        if ( archivingMessConfigTDB != null )
        {
            ...
            try
            {
                this.manager.ArchivingStart( false , archivingMessConfigTDB );
            }
            catch ( ArchivingException e )
            {
                e.printStackTrace ();
                archivingException.addStack( "Failed when launching temporary archiving" , e );
                needsThrow = true;
            }
        }
        if ( needsThrow )
