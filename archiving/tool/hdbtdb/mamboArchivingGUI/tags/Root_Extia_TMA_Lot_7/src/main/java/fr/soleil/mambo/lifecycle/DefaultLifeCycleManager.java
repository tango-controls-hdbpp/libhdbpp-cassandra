...
public class DefaultLifeCycleManager implements LifeCycleManager {
	...
	private void connectToDatabase(Splash splash) {
		...
		try {
			...
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_CONNECT_DB"));

			IAttributeManager attributeSource = AttributeManagerFactory
					.getCurrentImpl();
			attributeSource.openConnection();

			IExtractingManager extractingSource = ExtractingManagerFactory
					.getCurrentImpl();
			extractingSource.openConnection();

			splash.progress(17);
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_CONNECT_DB_OK"));
		} catch (Exception e) {
			String msg = Messages
					.getLogMessage("APPLICATION_WILL_START_CONNECT_DB_KO");
			splash.progress(17);
			splash.setMessage(msg);
			logger.trace(ILogger.LEVEL_ERROR, msg);
			logger.trace(ILogger.LEVEL_ERROR, e);
	...
	private void loadOptions(Splash splash) throws Exception {
		...
		try {
			splash.progress(3);
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_PREPARE_OPTIONS"));
			...
			String optionsLoadPath = Mambo.getAccountManager()
					.getSelectedAccountWorkingPath()
					+ "/options/options.xml";

			splash.progress(4);
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_INIT_OPTIONS"));
			...
			splash
					.setMessage(Messages
							.getLogMessage("APPLICATION_WILL_START_LOAD_BASIC_OPTIONS"));
			...
			splash
					.setMessage(Messages
							...
							+ " - "
							+ Messages
									.getLogMessage("APPLICATION_WILL_START_APPLY_BASIC_OPTIONS"));
			...
			splash
					.setMessage(Messages
							...
							+ " - "
							+ Messages
									.getLogMessage("APPLICATION_WILL_START_LOAD_LOG_OPTIONS"));
			...
			splash
					.setMessage(Messages
							...
							+ " - "
							+ Messages
									.getLogMessage("APPLICATION_WILL_START_LOAD_HISTORY_OPTIONS"));
	...
	private void startFactories() {
		...
		ViewConfigurationManagerFactory
				.getImpl(ViewConfigurationManagerFactory.XML_IMPL_TYPE);

		ArchivingManagerFactory
				.getImpl(ArchivingManagerFactory.REAL_IMPL_TYPE_GLOBAL_START_CALL);
		AttributeManagerFactory
				.getImpl(AttributeManagerFactory.BUFFERED_FORMATS_AND_DOMAINS);
		...
		ACDefaultsManagerFactory
				.getImpl(ACDefaultsManagerFactory.PROPERTIES_IMPL_TYPE);
	...
	private void loadHistory(Splash splash) throws Exception {
		...
		try {
			splash.progress(10);
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_PREPARE_HISTORY"));
			...
			String historyLoadPath = Mambo.getAccountManager()
					.getSelectedAccountWorkingPath()
					+ "/history/history.xml";
			...
			splash.progress(12);
			splash.setMessage(Messages
					.getLogMessage("APPLICATION_WILL_START_LOAD_HISTORY"));
			History history = historyManager.loadHistory(historyLoadPath);
			if (history != null) {
				splash.progress(13);
				splash.setMessage(Messages
						.getLogMessage("APPLICATION_WILL_START_APPLY_HISTORY"));
				history.push();
				splash
						.setMessage(Messages
	...
	private void setTraces() {
		...
		fr.soleil.mambo.options.sub.LogsOptions logsOptions = options
				.getLogsOptions();
		logsOptions.push();
	}
