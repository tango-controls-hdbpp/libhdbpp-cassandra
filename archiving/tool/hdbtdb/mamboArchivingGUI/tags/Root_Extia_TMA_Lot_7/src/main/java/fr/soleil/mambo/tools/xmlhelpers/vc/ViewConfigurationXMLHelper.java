...
public class ViewConfigurationXMLHelper {
     ...
     * @throws Exception
     */
    public static ViewConfiguration loadViewConfigurationIntoHash(
     ...
     * @throws Exception
     */
    public static ViewConfiguration loadViewConfigurationIntoHashFromRoot(
        ...
        if (rootNode.hasChildNodes()) {
            ...
            // as many loops as there are attributes in the VC
            {
                ...
                if (currentChildType.equals(ViewConfigurationAttribute.XML_TAG)) {
                    ...
                    currentAttribute = loadCurrentAttribute(currentAttribute,
                            currentChild);
                    // To maintain compatibility with previous versions (< lot6)
                    if (currentAttribute.getProperties().getPlotProperties()
                            .getCurve().getName().equals("null")
                            || currentAttribute.getProperties()
                                    .getPlotProperties().getCurve().getName()
                                    .equals("")) {
                        currentAttribute.getProperties().getPlotProperties()
                                .getCurve().setName(currentAttribute.getName());
                    }
                    ...
                    viewConfData = loadGenericPlotParameters(viewConfData,
                            currentChild);

                    viewConf.setData(viewConfData);
                } else if (currentChildType
                        .equals(ViewConfigurationAttributes.COLOR_INDEX_XML_TAG)) {
                    ...
                    try {
                        viewConfAttr.setColorIndex(Integer
                                .parseInt(colorIndexStringValue));
     ...
     */
    private static ExpressionAttribute loadExpression(Node expressionNode)
            throws Exception {

        // look for node's attributes
        Hashtable<String, String> expressionProperties = XMLUtils
                .loadAttributes(expressionNode);

        String name = expressionProperties
                .get(ExpressionAttribute.NAME_PROPERTY_XML_TAG);
        String id = expressionProperties
                .get(ExpressionAttribute.ID_PROPERTY_XML_TAG);
        String expression = expressionProperties
                .get(ExpressionAttribute.VALUE_PROPERTY_XML_TAG);

        String factorString = expressionProperties
        ...
        String xAttr = expressionProperties
                .get(ExpressionAttribute.X_PROPERTY_XML_TAG);
        ...
        if (expressionNode.hasChildNodes()) {
            ...
            for (int j = 0; j < expressionChildren.getLength(); j++) {
                ...
                if (ViewConfigurationAttributePlotProperties.XML_TAG
                        .equals(currentChild.getNodeName())) {
        ...
        ExpressionAttribute currentExpression = new ExpressionAttribute(name,
                expression, variables.toArray(new String[0]), isX
                        .booleanValue());
        if (id != null) {
    ...
    private static ViewConfigurationData loadGenericPlotParameters(
            ViewConfigurationData viewDataIn, Node genericPlotParametersNode)
            throws Exception {
        if (genericPlotParametersNode.hasChildNodes()) {
            NodeList genericPlotParametersSubNodes = genericPlotParametersNode
                    .getChildNodes();
            int axisGrid = 0;
            int gridStyle = 0;
     ...
     * @return 24 ao�t 2005
     */
    private static Axis loadAxis(Hashtable<String, String> attributeProperties,
            int gridStyle) {
        Axis ret = new Axis();

        String position_s = attributeProperties
                .get(Axis.POSITION_PROPERTY_XML_TAG);
        String scaleMode_s = attributeProperties
                .get(Axis.SCALE_MODE_PROPERTY_XML_TAG);
        String labelFormat_s = attributeProperties
                .get(Axis.LABEL_FORMAT_PROPERTY_XML_TAG);
        String showSubGrid_s = attributeProperties
                .get(Axis.SHOW_SUB_GRID_PROPERTY_XML_TAG);
        String drawOpposite_s = attributeProperties
                .get(Axis.DRAW_OPPOSITE_PROPERTY_XML_TAG);
        String isVisible_s = attributeProperties
                .get(Axis.IS_VISIBLE_PROPERTY_XML_TAG);
        String autoScale_s = attributeProperties
                .get(Axis.AUTO_SCALE_PROPERTY_XML_TAG);
        String color_s = attributeProperties.get(Axis.COLOR_PROPERTY_XML_TAG);
        String scaleMax_s = attributeProperties
                .get(Axis.SCALE_MAX_PROPERTY_XML_TAG);
        String scaleMin_s = attributeProperties
                .get(Axis.SCALE_MIN_PROPERTY_XML_TAG);
        String title_s = attributeProperties.get(Axis.TITLE_PROPERTY_XML_TAG);
        String labelInterval_s = attributeProperties
                .get(Axis.LABEL_INTERVAL_XML_TAG);
        String annotation_s = attributeProperties.get(Axis.ANNOTATION_XML_TAG);
        String gridVisible_s = attributeProperties
                .get(Axis.GRID_VISIBLE_XML_TAG);
    ...
    private static GeneralChartProperties loadGeneralChartProperties(
        ...
        String title_s = attributeProperties
                .get(GeneralChartProperties.TITLE_PROPERTY_XML_TAG);
        ...
        boolean legendsAreVisible = GUIUtilities
                .StringToBoolean(legendsAreVisible_s);
        long displayDuration = GUIUtilities.StringToLong(displayDuration_s);
        CometeFont headerFont = CometeFont.getFont(GUIUtilities
                .StringToFont(headerFont_s));
        CometeFont labelFont = CometeFont.getFont(GUIUtilities
                .StringToFont(labelFont_s));
        boolean dataSendingEnabled = GUIUtilities
                .StringToBoolean(dataSendingEnabled_s);
    ...
    private static ViewConfiguration loadViewConfigurationData(Node rootNode)
            throws Exception {
        ViewConfiguration viewConf = new ViewConfiguration();
        ViewConfigurationData viewConfData = new ViewConfigurationData();

        Hashtable<String, String> VCProperties = XMLUtils
        ...
        String startDate_s = VCProperties
                .get(ViewConfigurationData.START_DATE_PROPERTY_XML_TAG);
        String endDate_s = VCProperties
                .get(ViewConfigurationData.END_DATE_PROPERTY_XML_TAG);
        String historic_s = VCProperties
                .get(ViewConfigurationData.HISTORIC_PROPERTY_XML_TAG);
        String name_s = VCProperties
                .get(ViewConfigurationData.NAME_PROPERTY_XML_TAG);
        String path_s = VCProperties
                .get(ViewConfigurationData.PATH_PROPERTY_XML_TAG);
        String isModified_s = VCProperties
                .get(ViewConfigurationData.IS_MODIFIED_PROPERTY_XML_TAG);
        ...
        String dateRange = VCProperties
                .get(ViewConfigurationData.DATE_RANGE_PROPERTY_XML_TAG);
    ...
    private static ViewConfigurationAttribute loadCurrentAttribute(
            ViewConfigurationAttribute _currentAttribute,
        ...
        if (_currentAttributeNode.hasChildNodes()) {
            NodeList attributeParamsNodes = _currentAttributeNode
                    .getChildNodes();
            ...
            for (int i = 0; i < attributeParamsNodes.getLength(); i++) {
                ...
                String currentAttributeParamType = currentAttributeParamNode
                        .getNodeName().trim();// has to be "plotParams" or
                // "extractParams"
     ...
     * @throws Exception
     */
    private static ViewConfigurationAttributePlotProperties loadPlotParams(
            Node currentNode) throws Exception {
        ...
        if (currentNode.hasChildNodes()) {
            ...
            for (int i = 0; i < subNodes.getLength(); i++) {
                ...
                } else if (currentSubNodeType
                        .equals(Polynomial2OrderTransform.XML_TAG)) {
    ...
    private static Bar loadBar(Node currentSubNode) throws Exception {
        ...
        Hashtable<String, String> barProperties = XMLUtils
                .loadAttributes(currentSubNode);

        String fillColor_s = barProperties.get(Bar.COLOR_PROPERTY_XML_TAG);
        String fillingMethod_s = barProperties
                .get(Bar.FILL_METHOD_PROPERTY_XML_TAG);
        ...
        Bar bar = new Bar(CometeColor.getColor(fillColor), width, fillStyle,
                fillingMethod);
    ...
    private static Curve loadCurve(Node currentSubNode) throws Exception {
        ...
        Hashtable<String, String> curveProperties = XMLUtils
                .loadAttributes(currentSubNode);
    ...
    private static Marker loadMarker(Node currentSubNode) throws Exception {
        ...
        Hashtable<String, String> markerProperties = XMLUtils
                .loadAttributes(currentSubNode);
    ...
    private static Polynomial2OrderTransform loadTransform(Node currentSubNode)
        ...
        Hashtable<String, String> transformProperties = XMLUtils
                .loadAttributes(currentSubNode);

        String a0_s = transformProperties
                .get(Polynomial2OrderTransform.A0_PROPERTY_XML_TAG);
        String a1_s = transformProperties
                .get(Polynomial2OrderTransform.A1_PROPERTY_XML_TAG);
        String a2_s = transformProperties
                .get(Polynomial2OrderTransform.A2_PROPERTY_XML_TAG);
    ...
    private static Interpolation loadInterpolation(Node currentSubNode)
        ...
        Hashtable<String, String> interpolationProperties = XMLUtils
                .loadAttributes(currentSubNode);
        ...
        int interpolationMethod = GUIUtilities
                .StringToInt(interpolationMethod_s);
    ...
    private static Smoothing loadSmoothing(Node currentSubNode)
        ...
        Hashtable<String, String> smoothingProperties = XMLUtils
                .loadAttributes(currentSubNode);

        String extrapolation_s = smoothingProperties
                .get(Smoothing.EXTRAPOLATION_PROPERTY_XML_TAG);
        String gaussSigma_s = smoothingProperties
                .get(Smoothing.GAUSS_SIGMA_PROPERTY_XML_TAG);
        String Method_s = smoothingProperties
                .get(Smoothing.METHOD_PROPERTY_XML_TAG);
        String neighbors_s = smoothingProperties
                .get(Smoothing.NEIGHBORS_PROPERTY_XML_TAG);
    ...
    private static MathPlot loadMathPlot(Node currentSubNode) throws Exception {
        ...
        Hashtable<String, String> mathProperties = XMLUtils
                .loadAttributes(currentSubNode);

        String function_s = mathProperties
                .get(MathPlot.FUNCTION_PROPERTY_XML_TAG);
