//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/mambo/containers/view/VCVariationPanel.java,v $
//
...
public class VCVariationPanel extends JPanel
{
    ...
    private static VCVariationPanel instance = null;
     ...
     * @return 8 juil. 2005
     */
    public static VCVariationPanel getInstance ( boolean forceReload )
    {
        if ( instance == null || forceReload )
        {
            instance = new VCVariationPanel();
    ...
    public VCVariationPanel ()
    {
        ...
        JScrollPane scrollpane1 = new JScrollPane( VCVariationResultPanel.getInstance( true ) );
        GUIUtilities.setObjectBackground( scrollpane1 , GUIUtilities.VIEW_COLOR );
        GUIUtilities.setObjectBackground( scrollpane1.getViewport() , GUIUtilities.VIEW_COLOR );
        JScrollPane scrollpane2 = new JScrollPane( VCVariationRankingPanel.getInstance( true ) );
        GUIUtilities.setObjectBackground( scrollpane2 , GUIUtilities.VIEW_COLOR );
        GUIUtilities.setObjectBackground( scrollpane2.getViewport() , GUIUtilities.VIEW_COLOR );
        ...
        MamboActivableButton viewButton = new MamboActivableButton( action , viewIcon , viewDisabledIcon );
        GUIUtilities.setObjectBackground( viewButton , GUIUtilities.VIEW_COLOR );
        msg = Messages.getMessage( "DIALOGS_VARIATION_COPY" );
        JButton copyButton = new JButton( VCVariationCopyAction.getInstance(msg) );
        ...
        JButton editCopyButton = new JButton( VCVariationEditCopyAction.getInstance(msg) );
        editCopyButton.setMargin(new Insets(0,0,0,0));
        GUIUtilities.setObjectBackground( editCopyButton , GUIUtilities.VIEW_COPY_COLOR );
        ...
        resultRadio = new JRadioButton(Messages.getMessage( "DIALOGS_VARIATION_RESULT" ));
        resultRadio.setSelected(true);
        resultRadio.setOpaque(false);
        rankingRadio = new JRadioButton(Messages.getMessage( "DIALOGS_VARIATION_RANKING" ));
        ...
        SpringUtilities.makeCompactGrid(selectPanel, 
                1, 2, //rows, cols
                0, 0, //initX, initY
                5, 0, //xPad, yPad
        ...
        String dates = "Between dates ["
                       + ViewConfiguration.getSelectedViewConfiguration().getData().getStartDate()
                       + "] and ["
                       + ViewConfiguration.getSelectedViewConfiguration().getData().getEndDate()
                       + "]";
    ...
    public String selectedToString() {
        if (getWorkingTable()) {
            return VCVariationResultTable.getInstance( false ).toString();
        }
        else {
            return VCVariationRankingTable.getInstance( false ).toString();
        }
    }
}