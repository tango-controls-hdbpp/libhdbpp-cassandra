...
{
    protected static ExpressionHelpDialog instance = null;
    ...
    public static ExpressionHelpDialog getInstance()
    {
        if (instance == null)
        {
            instance = new ExpressionHelpDialog();
        }
        return instance;
    }

    protected ExpressionHelpDialog ()
    {
        super(ExpressionEditDialog.getInstance(), Messages.getMessage("EXPRESSION_SUPPORT_TITLE"), true);
        initComponents();
        addComponents();
        initLayout();
    }
    ...
    protected void initComponents()
    {
        ...
        introLabel[0] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_HOWTO"));
        introLabel[1] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_ENTER"));
        introLabel[2] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_VARIABLE_X"));
        introLabel[3] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_VARIABLE_XN"));
        introLabel[4] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_ASSOCIATION"));
        introLabel[5] = new JLabel(Messages.getMessage("EXPRESSION_SUPPORT_INTRODUCTION_EVALUATION"));
        operatorLabel = new JLabel(Messages.getMessage("EXPRESSION_OPERATOR_TITLE"));
        ...
        functionLabel = new JLabel(Messages.getMessage("EXPRESSION_FUNCTION_TITLE"));
    ...
    protected void initBounds()
    {
        ...
        int x = MamboFrame.getInstance().getX() + MamboFrame.getInstance().getWidth() - (bestDim.width + 50);
        if (x < 0) x = 0;
        int y = MamboFrame.getInstance().getY() + MamboFrame.getInstance().getHeight() - (bestDim.height + 50);
        if (y < 0) y = 0;
        int y1 = ExpressionEditDialog.getInstance().getY() + 10;
        if (y > y1 && y != 0) y = y1;
