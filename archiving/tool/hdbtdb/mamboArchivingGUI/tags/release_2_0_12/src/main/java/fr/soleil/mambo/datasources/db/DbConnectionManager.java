	...
	public void openConnection() throws ArchivingException {
		if (!isReady) {
			...
			if (Mambo.isHdbAvailable() == true) {
				...
				String HDBHost = System.getProperty("HDB_HOST") == null ? ""
						: System.getProperty("HDB_HOST");
				String HDBName = System.getProperty("HDB_NAME") == null ? ""
						: System.getProperty("HDB_NAME");
				String racConnection = System.getProperty("HDB_RAC") == null ? "FALSE"
						: System.getProperty("HDB_RAC");
				try {
					if (Mambo.canChooseArchivers()) {

						this.hdbManager
								.ArchivingConfigure(HDBuser, HDBpassword);

					} else {
						this.hdbManager
								.ArchivingConfigureWithoutArchiverListInit(
										HDBHost, HDBName, HDBuser, HDBpassword,
										Boolean.parseBoolean(racConnection));
					}
			...
			try {
				...
				String TDBHost = System.getProperty("TDB_HOST") == null ? ""
						: System.getProperty("TDB_HOST");
				String TDBName = System.getProperty("TDB_NAME") == null ? ""
						: System.getProperty("TDB_NAME");
				String racConnection = System.getProperty("TDB_RAC") == null ? "FALSE"
						: System.getProperty("TDB_RAC");
				if (Mambo.canChooseArchivers()) {
					this.tdbManager.ArchivingConfigure(TDBuser, TDBpassword);
				} else {
					this.tdbManager.ArchivingConfigureWithoutArchiverListInit(
							TDBHost, TDBName, TDBuser, TDBpassword, Boolean
									.parseBoolean(racConnection));
				}
			} catch (ArchivingException ae) {
				System.out
