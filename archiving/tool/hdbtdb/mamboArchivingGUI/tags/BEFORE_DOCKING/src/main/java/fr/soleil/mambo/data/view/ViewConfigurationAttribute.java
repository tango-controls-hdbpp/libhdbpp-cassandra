...
public class ViewConfigurationAttribute extends Attribute {
    ...
    public boolean isEmpty() {
        ...
        // System.out.println (
        // "CLA aaaaaaaaa/name/"+this.getCompleteName()+"/plotProperties.isEmpty ()/"+plotProperties.isEmpty
        // ()+"/" );
     ...
     * @throws DevFailed
     */
    public void addToChart(JLChart chart, String startDate, String endDate,
        ...
        if (plotProperties.getBar() == null
                || plotProperties.getCurve() == null
                || plotProperties.getMarker() == null) {
            // if the properties aren't set we don't add the attribute to the
            // plot
            return;
        }

        Hashtable<String, Object> theViewHash = null;

        try {
            theViewHash = extractingManager.retrieveDataHash(param, historic,
                    samplingType);
        }
        catch (Exception e) {
            e.printStackTrace();
            LoggerFactory.getCurrentImpl().trace(ILogger.LEVEL_ERROR, e);
            // do nothing, we can't know if the attribute is scalar if it isn't
            // referenced in DB
        }

        if (theViewHash != null) {
            JLDataView theReadView = (JLDataView) theViewHash
                    .get(READ_DATA_VIEW_KEY);
            JLDataView theWriteView = (JLDataView) theViewHash
                    .get(WRITE_DATA_VIEW_KEY);
     ...
     *            5 sept. 2005
     */
    private void addReadWriteViewToChart(JLChart chart, int axis,
            JLDataView theView) {
        switch (axis) {
     ...
     * @return 5 sept. 2005
     */
    private JLDataView prepareView(JLDataView theView, boolean isReadValue,
            boolean hasBoth) {
        if (theView != null) {
            ViewConfigurationAttributePlotProperties plotProperties = properties
                    .getPlotProperties();
            String nameCompletion = "/";
            nameCompletion += (isReadValue ? READ_DATA_VIEW_KEY
                    : WRITE_DATA_VIEW_KEY);
