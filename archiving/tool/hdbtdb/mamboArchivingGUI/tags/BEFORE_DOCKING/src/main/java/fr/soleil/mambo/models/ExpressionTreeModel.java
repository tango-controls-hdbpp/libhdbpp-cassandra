...
public class ExpressionTreeModel extends DefaultTreeModel {
    ...
    private static final long                      serialVersionUID = -9084188375650373241L;
    ...
    public ExpressionTreeModel() {
        super(new DefaultMutableTreeNode(Messages
                .getMessage("EXPRESSION_ATTRIBUTES")));
        rootNode = (DefaultMutableTreeNode) this.getRoot();
        setRoot(rootNode);
        expressionAttributes = new TreeMap<String, ExpressionAttribute>(
    ...
    public ExpressionAttribute removeAttribute(String attributeName) {
        ...
        for (int i = 0; i < rootNode.getChildCount(); i++) {
            if (attributeName.equals(((DefaultMutableTreeNode) rootNode
    ...
    public boolean updateAttribute(String formerName,
            ExpressionAttribute attribute) {
        if (expressionAttributes.containsKey(formerName)) {
            if (attribute.getName().equals(formerName)
                    || !expressionAttributes.containsKey(attribute.getName())) {
    ...
    public void setExpressionAttributes(
            TreeMap<String, ExpressionAttribute> expressionAttributes) {
    ...
    protected void build() {
        ...
        while (keyIterator.hasNext()) {
            ...
            this.insertNodeInto(new DefaultMutableTreeNode(key), rootNode,
                    index);
        }
        vect.clear();
        vect = null;
