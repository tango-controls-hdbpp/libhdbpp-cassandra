...
public class ArchivingConfigurationMode {
	...
	// General tags START
	/*
	 * private static final String HDB_MODE_XML_TAG = "modeHDB"; private static
	 * final String TDB_MODE_XML_TAG = "modeTDB";
	 */
	...
	/*
	 * private static final String TDB_SPEC_EXPORT_PERIOD_XML_TAG =
	 * "exportPeriod"; private static final String
	 * TDB_SPEC_KEEPING_PERIOD_XML_TAG = "keepingPeriod";
	 */
	// General tags END
	// modeRoot tags START
	...
	public String toString() {
		String ret = "";
		/*
		 * String modeTag = isTDB ? TDB_MODE_XML_TAG : HDB_MODE_XML_TAG; XMLLine
		 * modeLine = new XMLLine ( modeTag , XMLLine.EMPTY_TAG_CATEGORY );
		 */
		...
		case TYPE_A:
			...
			modeLine.setAttribute(VAL_INF_PROPERTY_XML_TAG, String
					.valueOf(modeA.getValInf()));
			modeLine.setAttribute(VAL_SUP_PROPERTY_XML_TAG, String
					.valueOf(modeA.getValSup()));
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeA
					.getPeriod()));
			modeLine.setAttribute(VAL_ABS_SLOW_DRIFT_XML_TAG, String
					.valueOf(modeA.isSlow_drift()));
			break;

		case TYPE_C:
			ModeCalcul modeC = this.mode.getModeC();
			modeLine.setAttribute(RANGE_PROPERTY_XML_TAG, String.valueOf(modeC
					.getRange()));
			modeLine.setAttribute(CALC_TYPE_PROPERTY_XML_TAG, String
					.valueOf(modeC.getTypeCalcul()));
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeC
					.getPeriod()));
			break;

		case TYPE_D:
			ModeDifference modeD = this.mode.getModeD();
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeD
					.getPeriod()));
			break;

		case TYPE_E:
			ModeExterne modeE = this.mode.getModeE();
			modeLine.setAttribute(DESCRIPTION_PROPERTY_XML_TAG, String
					.valueOf(modeE.getDescription()));
			// modeLine.setAttribute ( PERIOD_PROPERTY_XML_TAG , String.valueOf
			// ( modeE.getPeriod () ) );
			break;
		...
		case TYPE_P:
			...
			// System.out.println (
			// "ArchivingConfigurationMode/toString/TYPE_P/modeP.getPeriod ()/"+modeP.getPeriod
			// ()+"/" );
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeP
					.getPeriod()));
			break;

		case TYPE_R:
			ModeRelatif modeR = this.mode.getModeR();
			modeLine.setAttribute(PERCENT_INF_PROPERTY_XML_TAG, String
					.valueOf(modeR.getPercentInf()));
			modeLine.setAttribute(PERCENT_SUP_PROPERTY_XML_TAG, String
					.valueOf(modeR.getPercentSup()));
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeR
					.getPeriod()));
			modeLine.setAttribute(VAL_REL_SLOW_DRIFT_XML_TAG, String
					.valueOf(modeR.isSlow_drift()));
			break;

		case TYPE_T:
			ModeSeuil modeT = this.mode.getModeT();
			modeLine.setAttribute(THRESHOLD_INF_PROPERTY_XML_TAG, String
					.valueOf(modeT.getThresholdInf()));
			modeLine.setAttribute(THRESHOLD_SUP_PROPERTY_XML_TAG, String
					.valueOf(modeT.getThresholdSup()));
			modeLine.setAttribute(PERIOD_PROPERTY_XML_TAG, String.valueOf(modeT
					.getPeriod()));
			break;
		}

		/*
		 * if ( this.isTDB ) { long exportPeriod =
		 * mode.getTdbSpec().getExportPeriod (); long keepingPeriod =
		 * mode.getTdbSpec().getKeepingPeriod ();
		 * 
		 * modeLine.setAttribute ( TDB_SPEC_EXPORT_PERIOD_XML_TAG ,
		 * String.valueOf ( exportPeriod ) ); modeLine.setAttribute (
		 * TDB_SPEC_KEEPING_PERIOD_XML_TAG , String.valueOf ( keepingPeriod ) );
		 * }
		 */
	 ...
	 * @return 27 juil. 2005
	 */
	public static Mode buildCompleteTDBMode(ArchivingConfigurationMode[] modes,
			long exportPeriod, long keepingPeriod) {
	...
	public void controlValues() throws ArchivingConfigurationException {
		...
		case TYPE_A:
			...
			if (modeA.getValInf() >= modeA.getValSup()) {
				throw new ArchivingConfigurationException(null,
						MODE_A_VAL_INF_BIGGER_THAN_VAL_SUP);
			}
		...
		case TYPE_R:
			...
			if (modeR.getPercentInf() >= modeR.getPercentSup()) {
				throw new ArchivingConfigurationException(null,
						MODE_R_PERCENT_INF_BIGGER_THAN_PERCENT_SUP);
			}
