...
public class ViewImagePanel extends MamboCleanablePanel {
    ...
    private static final long          serialVersionUID = 4708894995143267423L;
    ...
    private Vector<ImageData>          partialImageData;
    ...
    private void initComponents() {
        ...
        startDateLabel = new JLabel(Messages
                .getMessage("DIALOGS_EDIT_VC_GENERAL_START_DATE"));
        endDateLabel = new JLabel(Messages
                .getMessage("DIALOGS_EDIT_VC_GENERAL_END_DATE"));
    ...
    private void addComponents() {
        ...
        if (startDateLabel == null) return;
        topBox.add(startDateLabel);
        topBox.add(Box.createHorizontalStrut(5));
        if (startDateField == null) return;
        topBox.add(startDateField);
        topBox.add(Box.createHorizontalStrut(10));
        if (endDateLabel == null) return;
        topBox.add(endDateLabel);
        topBox.add(Box.createHorizontalStrut(5));
        if (endDateField == null) return;
    ...
    public void initLayout() {
        ...
        SpringUtilities.makeCompactGrid(rowsPanel, rowsPanel
                .getComponentCount(), 1, 0, 0, 0, 0, false);
    ...
    private void initData() {
        ...
        partialImageData = new Vector<ImageData>();
        try {
            partialImageData = extractingManager.retrieveImageDatas(param,
                    this.isHistoric, selectedVC.getData().getSamplingType());
        }
        catch (DevFailed e1) {
            e1.printStackTrace();
        }
    ...
    public void loadPanel() {
        ...
        try {
            if (rowsPanel == null) return;
            if (partialImageData == null) return;
            if (image == null) return;
            rowsPanel.remove(scrollPane);
            PartialImageDataTable table = new PartialImageDataTable(
                    partialImageData, image.getDisplayFormat(this.isHistoric));
            table.setImageName(param[0]);

            if (table.getRowCount() == 0) {
                JLabel noData = new JLabel(Messages
                        .getMessage("VIEW_ATTRIBUTES_NO_DATA"));
                JPanel emptyPanel = new JPanel();
                GUIUtilities.setObjectBackground(emptyPanel,
                        GUIUtilities.VIEW_COLOR);
                emptyPanel.add(noData);
                emptyPanel.add(Box.createVerticalGlue());
                emptyPanel.setLayout(new SpringLayout());
                SpringUtilities.makeCompactGrid(emptyPanel, emptyPanel
                        .getComponentCount(), 1, 0, 0, 0, 0, false);
                scrollPane = new JScrollPane(emptyPanel);
            }
