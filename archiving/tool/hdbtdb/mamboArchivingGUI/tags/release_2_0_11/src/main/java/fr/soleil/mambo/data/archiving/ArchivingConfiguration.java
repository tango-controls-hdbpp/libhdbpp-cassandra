...
public class ArchivingConfiguration {
    ...
    private boolean                          isHistoric                    = Mambo
                                                                                   .isHdbAvailable();
    ...
    public ArchivingConfiguration(ArchivingConfigurationData _data,
            ArchivingConfigurationAttributes _attributes) {
        this.data = _data;
    ...
    public boolean containsAttribute(String completeName) {
        return this.attributes.getAttributesMap().containsKey(completeName);
    }

    public String toString() {
        String ret = "";

        XMLLine openingLine = new XMLLine(ArchivingConfiguration.XML_TAG,
                XMLLine.OPENING_TAG_CATEGORY);

        openingLine.setAttribute(
                ArchivingConfigurationData.IS_HISTORIC_PROPERTY_XML_TAG,
                this.isHistoric + "");
        openingLine.setAttribute(
                ArchivingConfigurationData.IS_MODIFIED_PROPERTY_XML_TAG,
                this.isModified + "");

        if (this.getCreationDate() != null) {
            openingLine.setAttribute(
                    ArchivingConfigurationData.CREATION_DATE_PROPERTY_XML_TAG,
                    this.getCreationDate().toString());
        }
        if (this.getLastUpdateDate() != null) {
            openingLine
                    .setAttribute(
                            ArchivingConfigurationData.LAST_UPDATE_DATE_PROPERTY_XML_TAG,
                            this.getLastUpdateDate().toString());
        }
        if (this.getName() != null) {
            openingLine.setAttribute(
                    ArchivingConfigurationData.NAME_PROPERTY_XML_TAG, this
                            .getName());
        }
        if (this.getPath() != null) {
            openingLine.setAttribute(
                    ArchivingConfigurationData.PATH_PROPERTY_XML_TAG, this
                            .getPath());
        }

        XMLLine closingLine = new XMLLine(ArchivingConfiguration.XML_TAG,
    ...
    public static ArchivingConfiguration[] findArchivingConfigurations(
            Criterions searchCriterions, boolean forceGlobalReload)
            throws Exception {
        ...
        if (openedArchivingConfigurations == null || forceGlobalReload) {
            ...
            ret = manager.findArchivingConfigurations(
                    openedArchivingConfigurations, searchCriterions);
        }
    ...
    public static void setCurrentArchivingConfiguration(
            ArchivingConfiguration currentArchivingConfiguration) {
    ...
    public void push() {
        ...
        if (Mambo.hasACs()) {
            ArchivingAttributesDetailPanel.getInstance().refresh(
                    this.isHistoric);

            ACAttributesTreeModel model = ACAttributesTreeModel.getInstance();
            model.removeAll();
            if ((ACEditDialog.getInstance() != null)
                    && (ACEditDialog.getInstance()
                            .getAttributeTableSelectionBean() != null)) {
                ACEditDialog.getInstance().getAttributeTableSelectionBean()
                        .getSelectionPanel().getAttributesSelectTable()
                        .getModel().reset();
            }
            ...
            ArchivingGeneralPanel archivingGeneralPanel = ArchivingGeneralPanel
                    .getInstance();
     ...
     */
    public static void updateCurrentModes(boolean currentACIsHistoric)
            throws ArchivingException {
    ...
    private static void updateCurrentAbsoluteMode(boolean historic) {
        AttributesPropertiesPanel panel = AttributesPropertiesPanel
                .getInstance();
        boolean selected = historic ? panel.hasModeHDB_A() : panel
                .hasModeTDB_A();
        ...
        if (!selected) {
            ...
            if (historic) {
                ArchivingConfigurationAttributeHDBProperties HDBProperties = currentProperties
                        .getHDBProperties();
                HDBProperties.removeMode(ArchivingConfigurationMode.TYPE_A);
                currentProperties.setHDBProperties(HDBProperties);
            }
            else {
                ArchivingConfigurationAttributeTDBProperties TDBProperties = currentProperties
                        .getTDBProperties();
            ...
            int absolutePeriod = historic ? panel.getHDBAbsolutePeriod()
                    : panel.getTDBAbsolutePeriod();
            double upperLimit = historic ? Double.parseDouble(panel
                    .getHDBAbsoluteUpper()) : Double.parseDouble(panel
                    .getTDBAbsoluteUpper());
            double lowerLimit = historic ? Double.parseDouble(panel
                    .getHDBAbsoluteLower()) : Double.parseDouble(panel
                    .getTDBAbsoluteLower());
            boolean slowDrift = historic ? panel.isHDBAbsoluteDLCheck() : panel
                    .isTDBAbsoluteDLCheck();
            upperLimit = Math.abs(upperLimit);
            lowerLimit = Math.abs(lowerLimit);

            ModeAbsolu ModeA = new ModeAbsolu(absolutePeriod, lowerLimit,
                    upperLimit, slowDrift);
     ...
     * @throws ArchivingException
     */
    private static void updateCurrentRelativeMode(boolean historic)
            throws ArchivingException {
        AttributesPropertiesPanel panel = AttributesPropertiesPanel
                .getInstance();
        boolean selected = historic ? panel.hasModeHDB_R() : panel
                .hasModeTDB_R();
        ...
        if (!selected) {
            ...
            int relativePeriod = historic ? panel.getHDBRelativePeriod()
                    : panel.getTDBRelativePeriod();
            double upperLimit = historic ? Double.parseDouble(panel
                    .getHDBRelativeUpper()) : Double.parseDouble(panel
                    .getTDBRelativeUpper());
            double lowerLimit = historic ? Double.parseDouble(panel
                    .getHDBRelativeLower()) : Double.parseDouble(panel
                    .getTDBRelativeLower());
            boolean slowDrift = historic ? panel.isHDBRelativeDLCheck() : panel
                    .isTDBRelativeDLCheck();
            upperLimit = Math.abs(upperLimit);
            lowerLimit = Math.abs(lowerLimit);

            ModeRelatif ModeR = new ModeRelatif(relativePeriod, lowerLimit,
                    upperLimit, slowDrift);
    ...
    private static void updateCurrentThresholdMode(boolean historic)
            throws ArchivingException {
        AttributesPropertiesPanel panel = AttributesPropertiesPanel
                .getInstance();
        boolean selected = historic ? panel.hasModeHDB_T() : panel
                .hasModeTDB_T();
        ...
        if (!selected) {
            ...
            int thresholdPeriod = historic ? panel.getHDBThresholdPeriod()
                    : panel.getTDBThresholdPeriod();
            double upperLimit = historic ? Double.parseDouble(panel
                    .getHDBThresholdUpper()) : Double.parseDouble(panel
                    .getTDBThresholdUpper());
            double lowerLimit = historic ? Double.parseDouble(panel
                    .getHDBThresholdLower()) : Double.parseDouble(panel
                    .getTDBThresholdLower());
            ModeSeuil ModeT = new ModeSeuil(thresholdPeriod, lowerLimit,
                    upperLimit);
    ...
    private static void updateCurrentPeriodicMode(boolean historic)
            throws ArchivingException {
        AttributesPropertiesPanel panel = AttributesPropertiesPanel
                .getInstance();
        // System.out.println (
        // "CLA/updateCurrentPeriodicMode/historic/"+historic );
        boolean selected = historic ? panel.hasModeHDB_P() : panel
                .hasModeTDB_P();
        // System.out.println (
        ...
        if (!selected) {
            ...
            int periodicPeriod = historic ? panel.getHDBperiod() : panel
                    .getTDBperiod();
            // System.out.println (
    ...
    private static void updateCurrentDifferenceMode(boolean historic)
            throws ArchivingException {
        AttributesPropertiesPanel panel = AttributesPropertiesPanel
                .getInstance();
        boolean selected = historic ? panel.hasModeHDB_D() : panel
                .hasModeTDB_D();
        ...
        if (!selected) {
            ...
            int differencePeriod = historic ? panel.getHDBDifferencePeriod()
                    : panel.getTDBDifferencePeriod();
    ...
    public ArchivingConfigurationAttribute getAttribute(String name) {
        return this.attributes.getAttributesMap().get(name);
    }
     ...
     * @return
     */
    private String getPathToUse(IArchivingConfigurationManager manager,
            boolean saveAs) {
