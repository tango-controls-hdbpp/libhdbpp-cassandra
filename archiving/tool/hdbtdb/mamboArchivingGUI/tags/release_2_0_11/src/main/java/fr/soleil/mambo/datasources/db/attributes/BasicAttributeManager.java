...
public class BasicAttributeManager extends DbConnectionManager implements
        IAttributeManager {
    ...
    public Vector<Domain> loadDomains(Criterions searchCriterions,
        ...
        try {
            ...
            String domains_s[] = database.getAttribute().getDomains()
                    .get_domains_by_criterion(domain_criterions);
            ...
            for (int domainsIndex = 0; domainsIndex < numberOfDomains; domainsIndex++) {
                ...
                String[] families_s = database.getAttribute().getFamilies()
                        .get_families_by_criterion(currentDomainName,
                                family_criterions);
                ...
                for (int familiesIndex = 0; familiesIndex < numberOfFamilies; familiesIndex++) {
                    ...
                    String[] members_s = database.getAttribute().getMembers()
                            .get_members_by_criterion(currentDomainName,
                                    currentFamilyName, member_criterions);
                    ...
                    for (int membersIndex = 0; membersIndex < numberOfMembers; membersIndex++) {
                        ...
                        if (loadCompletely) {
                            String[] attributes_s = database.getAttribute()
                                    .getNames().getAttributes_by_criterion(
                                            currentDomainName,
                                            currentFamilyName,
                                            currentMemberName,
                                            attribute_criterions);
    ...
    public Vector<Domain> loadDomains(boolean _historic, boolean forceReload) {
        ...
        try {
            ...
            String completeNames[] = database.getAttribute().getNames()
                    .get_attributes_complete_names();
            if (completeNames == null) {
                return new Vector<Domain>();
            }
            for (int i = 0; i < completeNames.length; i++) {
                String[] dfma = this
                        .parseCompleteNameIntoDFMA(completeNames[i]);
                ...
                domains.addAttribute(domainName, familyName, memberName,
                        attributeName);
            }
        }
        catch (ArchivingException e) {
        ...
        Vector<Domain> ret = domains.getList();
        // Collections.sort( ret, new MamboDomainsComparator() );
        return ret;
    }
    ...
    public ArrayList<String> getAttributes(String domain, String family,
            ...
            ArchivingException {
        DataBaseManager database = getDataBaseApi(_historic);
        String[] list = database.getAttribute().getNames().getAttributes(
        ...
        ArrayList<String> ret = new ArrayList<String>(list.length);
    ...
    public int getDataWritable(String completeName, boolean _historic) {
        try {
            ...
            int writable = database.getAttribute().getAttDataWritable(
                    completeName);
    ...
    public String getDisplayFormat(String completeName, boolean _historic) {
        try {
            ...
            String format = database.getAttribute().getProperties()
                    .getDisplayFormat(completeName);
