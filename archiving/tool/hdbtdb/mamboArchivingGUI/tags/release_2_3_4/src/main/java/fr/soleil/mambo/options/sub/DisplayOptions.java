...
public class DisplayOptions extends ReadWriteOptionBook implements
	...
	public void fillFromOptionsDialog() {
		...
		// "DisplayOptions/fillFromOptionsDialog/selectedActionCommand/"+selectedActionCommand+"/"
		// );
	...
	public void push() {
		 ...
		 * null ) { int plaf = Integer.parseInt ( plaf_s ); MamboFrame frame =
		 * MamboFrame.getInstance (); OptionsDialog.resetInstance ();
		 * frame.dispose ();
		 * 
		 * switch ( plaf ) { case PLAF_WIN : UIManager.setLookAndFeel (
		 * "com.sun.java.swing.plaf.windows.WindowsLookAndFeel" );
		 * frame.setUndecorated ( false ); break;
		 * 
		 * case PLAF_JAVA : UIManager.setLookAndFeel (
		 * UIManager.getCrossPlatformLookAndFeelClassName () );
		 * frame.setUndecorated ( true ); break; }
		 * 
		 * SwingUtilities.updateComponentTreeUI ( frame ); frame.setVisible (
		 * true );
		 * 
		 * OptionsDisplayTab displayTab = OptionsDisplayTab.getInstance ();
