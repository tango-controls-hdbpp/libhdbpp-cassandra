...
public class ViewConfigurationXMLHelper {
		...
		if (rootNode.hasChildNodes()) {
			...
			// as many loops as there are attributes in the VC
			{
				...
				if (currentChildType.equals(ViewConfigurationAttribute.XML_TAG)) {
					...
					// Sylvain: unknown tag, should be correctly reported to the
					// user
					throw new Exception();
				}
	 ...
	 * @return 24 ao�t 2005
	 */
	private static YAxis loadYAxis(Hashtable<String, String> attributeProperties) {
		YAxis ret = new YAxis();

		String position_s = attributeProperties
				.get(YAxis.POSITION_PROPERTY_XML_TAG);
		String scaleMode_s = attributeProperties
				.get(YAxis.SCALE_MODE_PROPERTY_XML_TAG);
		String labelFormat_s = attributeProperties
				.get(YAxis.LABEL_FORMAT_PROPERTY_XML_TAG);
		String showSubGrid_s = attributeProperties
				.get(YAxis.SHOW_SUB_GRID_PROPERTY_XML_TAG);
		String drawOpposite_s = attributeProperties
				.get(YAxis.DRAW_OPPOSITE_PROPERTY_XML_TAG);
		String isVisible_s = attributeProperties
				.get(YAxis.IS_VISIBLE_PROPERTY_XML_TAG);
		String autoScale_s = attributeProperties
				.get(YAxis.AUTO_SCALE_PROPERTY_XML_TAG);
		String color_s = attributeProperties.get(YAxis.COLOR_PROPERTY_XML_TAG);
		String scaleMax_s = attributeProperties
				.get(YAxis.SCALE_MAX_PROPERTY_XML_TAG);
		String scaleMin_s = attributeProperties
				.get(YAxis.SCALE_MIN_PROPERTY_XML_TAG);
		String title_s = attributeProperties.get(YAxis.TITLE_PROPERTY_XML_TAG);
	...
	private static GeneralChartProperties loadGeneralChartProperties(
		...
		String timePrecision = attributeProperties
	...
	private static ViewConfigurationAttributePlotProperties loadPlotParams(
		...
		if (currentNode.hasChildNodes()) {
			...
			// as many loops as there are sub nodes types for a plotParams Node
			// (4)
			{
