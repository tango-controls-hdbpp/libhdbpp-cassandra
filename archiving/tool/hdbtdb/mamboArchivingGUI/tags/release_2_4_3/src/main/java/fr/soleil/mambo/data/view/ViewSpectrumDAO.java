...
public class ViewSpectrumDAO extends AbstractDAO<List<DataArray>> {
    ...
    public ViewSpectrumDAO(DbData[] splitedData, ViewSpectrumTableModel model, int spectrumViewType) {
        this.model = model;
        if (splitedData != null) {
                        ...
                        if (extractingManager != null) {
                            if (extractingManager.isShowRead()) {
                                DataArray readDataArray = extractDataArray(row, readNameColumn,
                                        readBooleanColumn, spectrumViewType, splitedData[0]);
                            ...
                            if (extractingManager.isShowWrite()) {
                                DataArray writeDataArray = extractDataArray(row, writeNameColumn,
                                        writeBooleanColumn, spectrumViewType, splitedData[1]);
