...
public class ViewStringStateBooleanScalarTableModel extends DefaultTableModel {
    ...
    public ViewStringStateBooleanScalarTableModel(final String attributeName,
	...
	if (readReference != null && readReference.getData_timed() != null
		...
		if (readReference.getData_timed()[0].value != null
			...
			&& writeReference.getData_timed()[0].value != null
	...
	case 0:
		...
		if (readReference.getData_timed()[0].value != null
			&& readReference.getData_timed()[0].value.length > 0) {
		    return genFormatUS.format(new Date(readReference.getData_timed()[rowIndex].time
			    .longValue()));
		}
	    }
	    if (writeReference != null && writeReference.getData_timed() != null
	...
	if (conf != null) {
	    ...
	    param[2] = endDate;
	    try {
		final DbData retrievedData = extractingManager.retrieveData(param, conf.getData()
