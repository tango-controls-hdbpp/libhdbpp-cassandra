...
import fr.soleil.comete.dao.DAOListener;
...
public class ViewScalarDAO extends AbstractDAO<List<DataArray>> {
    ...
    public void addData(DbData data, boolean showRead, boolean showWrite) {
        if (data != null) {
    ...
    public void clean() {
        List<DAOListener<List<DataArray>>> listeners = new ArrayList<DAOListener<List<DataArray>>>();
        List<DAOListener<List<DataArray>>> temp = getDAOListeners();
        if (temp != null) {
            listeners.addAll(temp);
        }
        temp = null;
        for (DAOListener<List<DataArray>> listener : listeners) {
            removeDAOListener(listener);
        }
        listeners.clear();
        listeners = null;
        if (arrayList != null) {
