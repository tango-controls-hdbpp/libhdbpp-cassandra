...
public class ViewSpectrumPanel extends MamboCleanablePanel {
    ...
    public ViewSpectrumPanel(ViewConfigurationAttribute theSpectrum) {
        ...
        if (spectrum == null
                ...
                || !spectrum.isSpectrum(ViewConfiguration
                        .getSelectedViewConfiguration().getData().isHistoric()))
            return;
    ...
    private void initComponents() {
        ...
        if (readData.length == 0 && writeData.length == 0) {
            ...
            model = new ViewSpectrumTableModel(readData.length,
                    writeData.length, writable);
        }
    ...
    private void initData() {
        ...
        if (cleaning) return;
        ViewConfiguration selectedViewConfiguration = ViewConfiguration
    ...
    public JLDataView[] getSelectedWriteView() {
        ...
        JLDataView[] selection = new JLDataView[model
                .getSelectedWriteRowCount()];
        int index = 0;
        switch (writable) {
            case ViewSpectrumTableModel.W:
                for (int i = 0; i < model.getRowCount(); i++) {
