...
public class MamboViewDAOFactory extends AbstractDAOFactory {
    ...
    @Override
    public AbstractDAO<StackDataArray> createStackNumberDataArrayDAO(AbstractKey key) {
        if (key != null) {
                    ...
                    if (checkBoxSelected != null) {
                        if (checkBoxSelected instanceof Integer) {
                            ViewSpectrumStackDAO dao = new ViewSpectrumStackDAO(
                                    (DbData[]) splitedData, (Integer) checkBoxSelected);
                            return dao;
                        }
                    }
                }
    ...
    @Override
    public AbstractDAO<StackNumberMatrix> createStackNumberImageDAO(AbstractKey key) {
        return null;
    }
    ...
    private AbstractDAO<List<DataArray>> createNumberDataArrayDAOForSpectrum(AbstractKey key) {
            ...
            if (spectrumType instanceof Integer) {
                ...
                if (model != null) {
                    if (model instanceof ViewSpectrumTableModel) {
                        ...
                        if (splitedData != null) {
                            if (splitedData instanceof DbData[]) {
                                ViewSpectrumDAO dao = new ViewSpectrumDAO((DbData[]) splitedData,
                                        (ViewSpectrumTableModel) model, (Integer) spectrumType);
                                return dao;
                            }
                        }
                    }
    ...
    private AbstractDAO<List<DataArray>> createNumberDataArrayDAOForScalar(AbstractKey key) {
    ...
    public void removeKeyForDaoScalar(String idViewSelected) {
        ViewScalarDAO previous = getDAOScalar(idViewSelected);
        if (previous != null) {
            previous.clean();
        }
        previous = null;
