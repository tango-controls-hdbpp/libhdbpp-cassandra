...
public abstract class AttributesTreeModel<A extends Attribute> extends DefaultTreeModel {
    ...
    protected TreeMap<String, A> attributesHT;
	...
	attributesHT = new TreeMap<String, A>(Collator.getInstance());
    }
     ...
     *            8 juil. 2005
     */
    public void build(final Vector<Domain> _domains) {
	attributesHT = new TreeMap<String, A>(Collator.getInstance());
	domains = _domains;
	if (_domains != null) {
		...
		final Domain domain = domains.get(domIndex);
		final DefaultMutableTreeNode domainNode = new DefaultMutableTreeNode(domain
			.getName());
		((DefaultMutableTreeNode) getRoot()).add(domainNode);
		final Vector<Family> familiesToSort = new Vector<Family>();
		...
		for (int famIndex = 0; famIndex < familiesToSort.size(); famIndex++) {
		    ...
		    final Family family = familiesToSort.get(famIndex);
		    final DefaultMutableTreeNode familyNode = new DefaultMutableTreeNode(family
			    .getName());
		    domainNode.add(familyNode);

		    final Vector<Member> membersToSort = new Vector<Member>();
			...
			final Member member = membersToSort.get(membIndex);
			final DefaultMutableTreeNode memberNode = new DefaultMutableTreeNode(member
				.getName());
			familyNode.add(memberNode);

			final Collection<Attribute> coll = member.getAttributes().values();
			final Vector<A> attributesToSort = new Vector<A>();
			...
			for (int attrIndex = 0; attrIndex < attributesToSort.size(); attrIndex++) {
			    ...
			    // --
			    attribute.setCompleteName(domain.getName() + "/" + family.getName()
				    + "/" + member.getName() + "/" + attribute.getName());
     ...
     * @return 8 juil. 2005
     */
    public TreeMap<String, A> getAttributes() {
	return attributesHT;
    }
     ...
     *            14 sept. 2005
     */
    public void addToDomains(final Attribute _attribute) {
	final String dom_s = _attribute.getDomainName();
	final String fam_s = _attribute.getFamilyName();
	final String mem_s = _attribute.getMemberName();
	final String attr_s = _attribute.getName();
	// System.out.println (
