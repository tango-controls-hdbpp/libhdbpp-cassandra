...
public class ArchivingStartAction extends AbstractAction {
    ...
    public static ArchivingStartAction getInstance(final String name) {
	if (instance == null) {
    ...
    private ArchivingStartAction(final String name) {
	...
	try {
		...
		msg = Messages.getLogMessage("START_ARCHIVING_ACTION_KO_TIMEOUT");
	    ...
	    // If the caught exception is an AttributeArchivingException, we
	    // iterate over its bad attributes collection, and trace it
		...
		if (aae.getFaultingAttributes().size() > 0) {
		    logger.trace(level, Messages
			    .getLogMessage("START_ARCHIVING_ACTION_EXCEPTION_TRACE_MESSAGE"));
		    for (int i = 0; i < aae.getFaultingAttributes().size(); i++) {
			StringBuffer buffer = new StringBuffer("\t");
			buffer
				.append(Messages
					.getLogMessage("START_ARCHIVING_ACTION_EXCEPTION_TRACE_MESSAGE_ATTRIBUTE"));
			buffer.append(i + 1).append(":");
			buffer
				.append("\n\t\t")
				.append(
					Messages
						.getLogMessage("START_ARCHIVING_ACTION_EXCEPTION_TRACE_MESSAGE_ATTRIBUTE_NAME"));
			buffer.append(aae.getFaultingAttributes().get(i).getName());
			buffer
				.append("\n\t\t")
				.append(
					Messages
						.getLogMessage("START_ARCHIVING_ACTION_EXCEPTION_TRACE_MESSAGE_ATTRIBUTE_REASON"));
