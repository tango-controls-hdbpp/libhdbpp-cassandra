// +======================================================================
// $Source:
// /cvsroot/tango-cs/tango/tools/mambo/containers/archiving/dialogs/AttributesPropertiesPanel.java,v
// $
//
...
public class AttributesPropertiesPanel extends JPanel {
     ...
     * @return 8 juil. 2005
     */
    public static AttributesPropertiesPanel getInstance(final boolean isHistoric) {
     ...
     * 
     */
    private AttributesPropertiesPanel(final boolean isHistoric) {
	this.isHistoric = isHistoric;

	initComponents(isHistoric);
	addComponents();
	initLayout();
	...
	SpringUtilities.makeCompactGrid(this, getComponentCount(), 1, 0, 0, 0, 0, true);
    }
     ...
     * 19 juil. 2005
     */
    private void initComponents(final boolean isHistoric) {
	...
	final long millisecondsInOneHour = 60000 * 60;
     ...
     * ]; reverseExportPeriodsTable = new Hashtable( 7 ); long
     * millisecondsInOneMinute = 60000; exportPeriodsTable[ 0 ] = 5 *
     ...
     * millisecondsInOneMinute; reverseExportPeriodsTable.put( new Long(
     * exportPeriodsTable[ 0 ] ) , new Integer( 0 ) );
     * reverseExportPeriodsTable.put( new Long( exportPeriodsTable[ 1 ] ) , new
     * Integer( 1 ) ); reverseExportPeriodsTable.put( new Long(
     * exportPeriodsTable[ 2 ] ) , new Integer( 2 ) );
     * reverseExportPeriodsTable.put( new Long( exportPeriodsTable[ 3 ] ) , new
     * Integer( 3 ) ); reverseExportPeriodsTable.put( new Long(
     * exportPeriodsTable[ 4 ] ) , new Integer( 4 ) );
     * reverseExportPeriodsTable.put( new Long( exportPeriodsTable[ 5 ] ) , new
     * Integer( 5 ) ); reverseExportPeriodsTable.put( new Long(
     * exportPeriodsTable[ 6 ] ) , new Integer( 6 ) ); }
     */

    /**
     * 20 juil. 2005
     */
    private void initCenterPanel(final boolean isHistoric) {
	initCenterComponents(isHistoric);
	centerPanel.setLayout(new SpringLayout());
	SpringUtilities.makeCompactGrid(centerPanel, centerPanel.getComponentCount(), 1, 0, 0, 0,
		0, true);
    ...
    private void initCenterComponents(final boolean isHistoric) {
	...
	final JPanel warningPanel = getWarningPanel();
	...
	SpringUtilities.makeCompactGrid(warningPanel, 1, warningPanel.getComponentCount(), // rows,
		// cols
		0, 0, // initX, initY
		0, 0, // xPad, yPad
		true);
	...
	final TitledBorder tb = BorderFactory.createTitledBorder(BorderFactory
		.createEtchedBorder(EtchedBorder.LOWERED), archiverTitleLabelMessage,
     ...
     *            22 juil. 2005
     */
    public void doSelectMode(final boolean _historic, final int _type) {
     ...
     *            25 juil. 2005
     */
    public void setSelectedModes(final boolean _historic, final Vector modesTypes) {
	if (_historic) {
	...
	final Enumeration enumeration = modesTypes.elements();
	while (enumeration.hasMoreElements()) {
	    final Integer _nextModeType = (Integer) enumeration.nextElement();
	...
	final String text = period == -1 ? "" : String.valueOf(period / 1000);
	...
	final String text = period == -1 ? "" : String.valueOf(period);
     ...
     */
    @Deprecated
    public void setKeepingPeriod(final long period) {
	/*
	 * int idx = 2; //default 4 hours try { Integer key = ( Integer )
	 * reverseKeepingPeriodsTable.get( new Long( period ) ); if ( key ==
	 * null ) { idx = 2; } else { idx = key.intValue(); } } catch (
	 * Exception e ) { e.printStackTrace(); }
	...
	if (firstTests) {
		...
		for (final ArchivingConfigurationAttribute a : tree.getListOfAttributesToSet()) {
		    getLastMode().checkMode(isHistoric, a.getCompleteName());
		}
		return true;
	...
	final String msgGeneral = Messages
	...
	if (isHistoric) {
		...
		final long exportPeriod = getExportPeriod();
		final long keepingPeriod = getKeepingPeriod();
		final TdbSpec tdbSpec = new TdbSpec(exportPeriod, keepingPeriod);
	...
	if (isHistoric) {
	    final boolean hasHDBNonPeriodicMode = HDBAbsoluteCheck.isSelected()
		    || HDBThresholdCheck.isSelected() || HDBDifferenceCheck.isSelected();
	    ...
	    final boolean hasTDBNonPeriodicMode = TDBAbsoluteCheck.isSelected()
		    || TDBThresholdCheck.isSelected() || TDBDifferenceCheck.isSelected();
 ...
 */
	...
	if (isDefault) {
	    ...
	    archiverChoiceNameTextField.setText("");
	}
	...
	final boolean archiverDefaultIsEmpty = dedicatedArchiver == null
    ...
    public void pushCurrentArchiver(final Archiver currentArchiver,
	    final String attributeCompleteName) {
	final String name = currentArchiver == null ? "" : currentArchiver.getName();
	// System.out.println (
	...
	// );
	String text = "";
	final Hashtable attributesToDedicatedArchiver = Archiver
		.getAttributesToDedicatedArchiver(isHistoric);
	if (attributesToDedicatedArchiver != null && attributeCompleteName != null) {
	    // System.out.println ( "CLA/pushDedicatedArchiver/1" );
	    final Archiver dedicatedArch = (Archiver) attributesToDedicatedArchiver
	...
	public ArchiverChoiceNameTextField(final String initialValue) {
	    ...
	    setMaximumSize(classicMaxDimension);
	}
	...
	public void setColorForAttribute(final String completeName) {
	    final String archiver = this.getText();
