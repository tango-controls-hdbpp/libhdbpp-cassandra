...
public class VCAttributesPropertiesTreeSelectionListener implements
        ...
        if (currentVC != null) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                ViewConfigurationAttributePlotProperties currentPlotProperties = currentProperties
                        .getPlotProperties();
                if (!(plotProperties.getViewType() == currentPlotProperties
                        .getViewType()))
                    currentPlotProperties.setViewType(plotProperties
                            .getViewType());
                if (!plotProperties.getBar().equals(
                        currentPlotProperties.getBar()))
                    currentPlotProperties.setBar(plotProperties.getBar());
                if (!plotProperties.getCurve().equals(
                        currentPlotProperties.getCurve()))
                    currentPlotProperties.setCurve(plotProperties.getCurve());
                if (!plotProperties.getMarker().equals(
                        currentPlotProperties.getMarker()))
                    currentPlotProperties.setMarker(plotProperties.getMarker());
                if (!plotProperties.getTransform().equals(
                        currentPlotProperties.getTransform()))
                    currentPlotProperties.setTransform(plotProperties
                            .getTransform());
                if (!plotProperties.getInterpolation().equals(
                        currentPlotProperties.getInterpolation()))
                    currentPlotProperties.setInterpolation(plotProperties
                            .getInterpolation());
                if (!plotProperties.getSmoothing().equals(
                        currentPlotProperties.getSmoothing()))
                    currentPlotProperties.setSmoothing(plotProperties
                            .getSmoothing());
                if (!plotProperties.getMath().equals(
                        currentPlotProperties.getMath()))
                ...
                next.getProperties().getPlotProperties().setAxisChoice(
                        currArribute.getProperties().getPlotProperties()
                                .getAxisChoice());
                next.getProperties().getPlotProperties().setSpectrumViewType(
                        currArribute.getProperties().getPlotProperties()
                                .getSpectrumViewType());
    ...
    public void treeSelectionAttributesPush() {
        ...
        PlotPropertyComboListener plotPropertyComboListener = panel
                .getPlotPropertyChoiceComboListener();

        DefaultComboBoxModel spectrumViewTypeComboModel = panel
                .getSpectrumViewTypeComboModel();
        JComboBox spectrumViewTypeCombo = panel.getSpectrumViewTypeCombo();
        ...
        if (attributes.size() == 1) {// Select one attribut
            ...
            spectrumViewTypeCombo
                    .removeActionListener(plotPropertyComboListener);
