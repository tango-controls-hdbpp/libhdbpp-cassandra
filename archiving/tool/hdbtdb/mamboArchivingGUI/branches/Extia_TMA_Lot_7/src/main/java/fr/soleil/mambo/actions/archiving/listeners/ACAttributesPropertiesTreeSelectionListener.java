...
public class ACAttributesPropertiesTreeSelectionListener implements
	...
	public void valueChanged(TreeSelectionEvent event) {
		// System.out.println (
		// "ACAttributesPropertiesTreeSelectionListener/valueChanged/START" );

		ACAttributesPropertiesTree tree = (ACAttributesPropertiesTree) event
				.getSource();
		...
		ArchivingConfigurationAttribute selectedAttribute = currentArchivingConfiguration
		...
		if (selectedAttribute == null) {
			...
			if (!currentArchivingConfiguration
					.containsAttribute(selectedAttributeName)) {
				theSelectedAttributeBelongsToTheCurrentAC = true;
			} else if (currentArchivingConfiguration.getAttributes()
					.getAttribute(selectedAttributeName).isEmpty()) {
				theSelectedAttributeBelongsToTheCurrentAC = true;
			}
		}
		...
		if (theSelectedAttributeBelongsToTheCurrentAC) {
			...
			ArchivingConfigurationAttributeProperties properties = selectedAttribute
					.getProperties();
	...
	private void applyDefaults(String completeName, boolean isLeafNode) {
		...
		try {
			...
			// if there is a problem, we push empty properties (fields are
			// resetted)
			ArchivingConfigurationAttributeHDBProperties HDBProperties = new ArchivingConfigurationAttributeHDBProperties();
			ArchivingConfigurationAttributeTDBProperties TDBProperties = new ArchivingConfigurationAttributeTDBProperties();
