...
public class ViewStringStateBooleanScalarPanel extends MamboCleanablePanel {
    ...
    public DbData getDataAttribute(String name, boolean isRead) {
        if (attributes != null) {
            for (int i = 0; i < attributes.length; i++) {
                if (attributes[i].getCompleteName().equals(name)) {
                    TableModel model = viewTables[i].getModel();
                    if (model != null && model instanceof ViewStringStateBooleanScalarTableModel) {
                        if (isRead) {
                            return ((ViewStringStateBooleanScalarTableModel) model)
                                    .getReadRefence();
                        }
                        else {
                            return ((ViewStringStateBooleanScalarTableModel) model)
