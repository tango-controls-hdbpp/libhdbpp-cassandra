...
public class ViewStringStateBooleanScalarTableModel extends DefaultTableModel {
    ...
    public int getColumnCount() {
        int col = 2;
        if (readReference != null && readReference.getData_timed() != null
                && writeReference != null && writeReference.getData_timed() != null) {
                ...
                if (readReference != null && readReference.getData_timed() != null
                    ...
                    if (readReference.getData_timed()[0].value != null
                        ...
                        return genFormatUS.format(new Date(
                                readReference.getData_timed()[rowIndex].time.longValue()));
                    }
                }
                if (writeReference != null && writeReference.getData_timed() != null
                        && writeReference.getData_timed().length > 0) {
                    if (writeReference.getData_timed()[0].value != null
                        ...
                        return genFormatUS.format(new Date(
                                writeReference.getData_timed()[rowIndex].time.longValue()));
                    }
                }
                return null;
            case 1:
    ...
    public String getColumnName(int column) {
        switch (column) {
            ...
            case 1:
                ...
                else if (writeReference != null && writeReference.getData_timed() != null
                        && writeReference.getData_timed().length > 0) {
                    if (writeReference.getData_timed()[0].value != null
                            && writeReference.getData_timed()[0].value.length > 0) {
                        return Messages
    ...
    public void loadData() {
        ...
        ViewConfiguration conf = viewConfigurationBean.getViewConfiguration();
        if (conf != null) {
            ...
            String endDate = "";
            try {
                startDate = extractingManager.timeToDateSGBD(conf.getData().getStartDate()
                        .getTime());
            ...
            param[2] = endDate;
            try {
                DbData retrievedData = extractingManager.retrieveData(param, conf.getData()
                        .isHistoric(), conf.getData().getSamplingType());
                if (retrievedData != null) {
                    DbData[] splitedData = retrievedData.splitDbData();
                    if (splitedData != null) {
    ...
    public DbData getReadRefence() {
        return readReference;
    }

    public DbData getWriteReference() {
