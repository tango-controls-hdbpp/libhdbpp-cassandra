...
public class ViewNumberScalarPanel extends MamboCleanablePanel implements ChartActionListener {
                ...
                if (title != null) {
                    if (title.contains(" - " + Messages.getMessage("VIEW_ATTRIBUTES_START_DATE"))
                        ...
                        newChartProperties.setTitle(newTitle);
                            ...
                            && title.contains(Messages.getMessage("VIEW_ATTRIBUTES_END_DATE"))) {
                        int index = title
                                .indexOf(Messages.getMessage("VIEW_ATTRIBUTES_START_DATE"));

                        String newTitle = title.substring(0, index);
                        newChartProperties.setTitle(newTitle);
                    }
                }
                ViewConfigurationData data = viewConfigurationBean.getViewConfiguration().getData();
                data.getGeneralChartProperties().setGeneralChartProperties(newChartProperties);
            }
        }

        // if these are plotProperties that have changed
        if (evt.getName().startsWith(IChartViewer.DATAVIEW_PROPERTY_ACTION_NAME)) {
            if (chart != null) {
                int startInd = IChartViewer.DATAVIEW_PROPERTY_ACTION_NAME.length();
                String nameIdPlot = evt.getName().substring(startInd);

                PlotProperties newPlotProperties = chart.getComponent().getDataViewPlotProperties(
                        nameIdPlot);
                if ((newPlotProperties != null) && (newPlotProperties.getCurve() != null)
                    ...
                    if (newName.endsWith(ViewConfigurationAttribute.WRITE_DATA_VIEW_KEY)) {
                        newName = newName.substring(0, newName.length()
                                - ViewConfigurationAttribute.WRITE_DATA_VIEW_KEY.length() - 1);
                        isWrite = true;
                    }

                    ViewConfigurationAttribute attr = viewConfigurationBean.getViewConfiguration()
    ...
    public void setChartAttributeExpressionOfString(
            ViewStringStateBooleanScalarPanel otherScalarsPanel) {
        viewConfigurationBean.getViewConfiguration().setChartAttributeExpressionOfString(
                otherScalarsPanel, chart);
