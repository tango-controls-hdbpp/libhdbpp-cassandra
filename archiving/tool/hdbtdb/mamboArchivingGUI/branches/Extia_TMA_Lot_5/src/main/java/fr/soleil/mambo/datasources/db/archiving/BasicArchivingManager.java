...
public class BasicArchivingManager extends DbConnectionManager implements
    ...
    public void stopArchiving(ArchivingConfiguration AC)
        ...
        ArrayList<String> attributeListFinalHDB = new ArrayList<String>();
        ArrayList<String> attributeListFinalTDB = new ArrayList<String>();
        ...
        for (int i = 0; i < nbOfAttributes; i++) {
            ...
            try {
                if (hdbManager != null && hdbManager.isArchived(nextName)
                        && (AC == null || AC.isHistoric())) {
            ...
            try {
                if (tdbManager != null && tdbManager.isArchived(nextName)
                        && (AC == null || !AC.isHistoric())) {
        ...
        try {
            if (hdbManager != null && !attributeListFinalHDB.isEmpty()) {
                String[] attributeListFinalHDB_s = new String[0];
                attributeListFinalHDB_s = attributeListFinalHDB
                        .toArray(attributeListFinalHDB_s);
                hdbManager.ArchivingStop(attributeListFinalHDB_s);
            }
        }
        catch (ArchivingException e1) {
            exceptionMessage = errorSeparator;
        ...
        try {
            if (tdbManager != null && !attributeListFinalTDB.isEmpty()) {
                String[] attributeListFinalTDB_s = new String[0];
                attributeListFinalTDB_s = attributeListFinalTDB
                        .toArray(attributeListFinalTDB_s);
                tdbManager.ArchivingStop(attributeListFinalTDB_s);
            }
        }
        catch (ArchivingException e2) {
            exceptionMessage = errorSeparator;
