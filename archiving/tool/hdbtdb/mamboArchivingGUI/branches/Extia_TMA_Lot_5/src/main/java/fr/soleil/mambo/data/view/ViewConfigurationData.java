...
public class ViewConfigurationData {
    ...
    public ViewConfigurationData(
            GeneralChartProperties _generalChartProperties, YAxis _y1, YAxis _y2) {
    ...
    private void setAxisDefaultProperties() {
        y1.setAutoScale(true);
        y1.setVisible(true);
        y1.setColor(Color.BLACK);
        y1.setDrawOpposite(false);
        y1.setShowSubGrid(false);
        y1.setScaleMode(JLAxis.LINEAR_SCALE);
        y1.setTitle("");
        y2.setAutoScale(true);
        y2.setVisible(false);
        y2.setColor(Color.BLACK);
        y2.setDrawOpposite(false);
        y2.setShowSubGrid(false);
        y2.setScaleMode(JLAxis.LINEAR_SCALE);
        y2.setTitle("");
    }
    ...
    public String toString() {
        ...
        if (y1 != null) {
            ret += y1.toString(true);
            ret += GUIUtilities.CRLF;
        }
        if (y2 != null) {
            ret += y2.toString(false);
            ret += GUIUtilities.CRLF;
        }
    ...
    public ChartViewer getChart() {
        ...
        // chart.setParentForTableDialog(MamboFrame.getInstance());
        ...
        if (axisGrid == GeneralChartProperties.AXISGRID_ONX
                || axisGrid == GeneralChartProperties.AXISGRID_ONXANDY1
                || axisGrid == GeneralChartProperties.AXISGRID_ONXANDY2) {
            xHasAxisGrid = true;
        }
        if (axisGrid == GeneralChartProperties.AXISGRID_ONY1
                || axisGrid == GeneralChartProperties.AXISGRID_ONXANDY1) {
            y1HasAxisGrid = true;
        }
        if (axisGrid == GeneralChartProperties.AXISGRID_ONY2
                || axisGrid == GeneralChartProperties.AXISGRID_ONXANDY2) {
            y2HasAxisGrid = true;
        }
        ...
        y1.setupJLAxis(chartViewerComponent, IChartViewer.Y1, y1HasAxisGrid,
                generalChartProperties.getAxisGridStyle());

        y2.setupJLAxis(chartViewerComponent, IChartViewer.Y2, y2HasAxisGrid,
                generalChartProperties.getAxisGridStyle());

        chartViewerComponent.setGridVisible(xHasAxisGrid, IChartViewer.X);
        chartViewerComponent.setGridStyle(chartViewerComponent
                .getGridStyle(IChartViewer.X), IChartViewer.X);
