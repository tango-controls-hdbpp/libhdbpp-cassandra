...
public class ViewActionPanel extends JPanel implements PropertyChangeListener {
	...
	private static final long serialVersionUID = 4865593596749119212L;
	...
	private final static ImageIcon variationIcon = new ImageIcon(Mambo.class
	...
	private VCVariationAction variationAction;
	private OpenVCEditDialogModifyAction modifyAction;
	private ViewConfigurationBean viewConfigurationBean;

	/**
     *
     */
	public ViewActionPanel(ViewConfigurationBean viewConfigurationBean,
			ViewAttributesPanel attributesPanel) {
		super(new GridBagLayout());
		this.viewConfigurationBean = viewConfigurationBean;

		initComponents(attributesPanel);
		addComponents();
		initBorder();

		attributesPanel.getViewAttributesGraphPanel()
	 ...
	 * 19 juil. 2005
	 */
	private void initComponents(ViewAttributesPanel attributesPanel) {
		GUIUtilities.setObjectBackground(this, GUIUtilities.VIEW_COLOR);

		String msg = Messages.getMessage("VIEW_ACTION_REFRESH_BUTTON");

		refreshAction = new VCRefreshAction(msg, viewConfigurationBean);
		...
		variationAction = new VCVariationAction(msg, viewConfigurationBean);
		...
		modifyAction = new OpenVCEditDialogModifyAction(msg,
	...
	public void putRefreshButton() {
		synchronized (this) {
			for (int i = 0; i < getComponentCount(); i++) {
				if (getComponent(i) == refreshButton)
					return;
			}
			remove(cancelButton);
			...
			variationAction.setEnabled(true);
			refreshAction.setEnabled(true);
			revalidate();
			repaint();
		}
	...
	public void putCancelButton() {
		synchronized (this) {
			for (int i = 0; i < getComponentCount(); i++) {
				if (getComponent(i) == cancelButton)
					return;
			}
			...
			variationAction.setEnabled(false);
			refreshAction.setEnabled(false);
			revalidate();
			repaint();
		}
	...
	public VCRefreshAction getRefreshAction() {
