...
public class ACAttributeDetailTableModel extends DefaultTableModel {
	...
	private ACAttributeDetailTableModel(String[] _columnNames, int rowCount) {
		...
		isArchived = Messages
				.getMessage("ARCHIVING_ATTRIBUTES_DETAIL_IS_ARCHIVED_LABEL");
	...
	public void load(ArchivingConfigurationAttribute _attribute) {
		...
		if (TDBProperties != null) {
			...
			if (TDBProperties.getExportPeriod() >= 0) {
				tdbArea.setText(tdbArea.getText() + GUIUtilities.CRLF
						+ "Export Period : " + TDBProperties.getExportPeriod()
						+ "ms");
			}
			/*
	...
	private void fillArea(
		...
		if (ACRMode != null) {
			...
			textArea.setText(textArea.getText() + GUIUtilities.CRLF
					+ "Relative Mode Slow Drift : "
					+ ACRMode.getMode().getModeR().isSlow_drift());
		}
		ArchivingConfigurationMode ACTMode = properties
				.getMode(ArchivingConfigurationMode.TYPE_T);
		if (ACTMode != null) {
			textArea.setText(textArea.getText() + GUIUtilities.CRLF
					+ "Threshold Mode Period : "
		...
		ArchivingConfigurationMode ACDMode = properties
				.getMode(ArchivingConfigurationMode.TYPE_D);
		if (ACDMode != null) {
			textArea.setText(textArea.getText() + GUIUtilities.CRLF
					+ "On Difference Mode Period : "
					+ ACDMode.getMode().getModeD().getPeriod() + "ms");
		}
		ArchivingConfigurationMode ACCMode = properties
				.getMode(ArchivingConfigurationMode.TYPE_C);
		if (ACCMode != null) {
			textArea.setText(textArea.getText() + GUIUtilities.CRLF
					+ "Calculation Mode Period : "
	...
	private Object getHDBColumnValue(int rowIndex) {
		...
		if (rowIndex == 0) {
			...
			try {
				ret = new Boolean(ArchivingManagerFactory.getCurrentImpl()
						.isArchived(attribute.getCompleteName(), true));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else if (rowIndex == 9) {
	...
	private Boolean hasMode(
			ArchivingConfigurationAttributeDBProperties _properties,
			int rowIndex) {
	...
	private Object getTDBColumnValue(int rowIndex) {
		...
		if (rowIndex == 0) {
			...
			try {
				ret = new Boolean(ArchivingManagerFactory.getCurrentImpl()
						.isArchived(attribute.getCompleteName(), false));
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else if (rowIndex == 9) {
