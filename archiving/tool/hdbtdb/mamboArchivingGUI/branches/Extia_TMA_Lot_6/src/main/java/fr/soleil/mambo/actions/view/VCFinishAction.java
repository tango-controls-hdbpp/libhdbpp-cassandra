...
public class VCFinishAction extends AbstractAction {
     ...
     * @param name
     */
    public VCFinishAction(String name,
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if (oldValue == 2) {
            viewConfigurationBean.getEditDialog()
        ...
        if (oldValue == 3) {
            viewConfigurationBean.getEditDialog().getExpressionTab()
                    .getExpressionTree().saveCurrentSelection();
            viewConfigurationBean.getEditDialog().getExpressionTab()
                    .getExpressionTree().getExpressionTreeListener()
                    .treeSelectionSave();
        }

        if (verifyVC(viewConfigurationBean.getEditDialog().getGeneralTab()
                .getDateRangeBox())) {
            // --setting the generic plot data
            ChartGeneralTabbedPane generalTabbedPane = viewConfigurationBean
                    .getEditDialog().getGeneralTab()
            ...
            if (currentVC != null) {
                ...
                boolean dynamic = viewConfigurationBean.getEditDialog()
                        .getGeneralTab().getDateRangeBox().isDynamicDateRange();
                if (dynamic) {
                    Timestamp[] range = viewConfigurationBean.getEditDialog()
                            .getGeneralTab().getDateRangeBox()
                            .getDynamicStartAndEndDates();
                    newData.setStartDate(range[0]);
                    newData.setEndDate(range[1]);
                } else {
                    newData.setStartDate(viewConfigurationBean.getEditDialog()
                            .getGeneralTab().getDateRangeBox().getStartDate());
                    newData.setEndDate(viewConfigurationBean.getEditDialog()
                            .getGeneralTab().getDateRangeBox().getEndDate());
                }
                newData.setDynamicDateRange(dynamic);
                newData.setDateRange(viewConfigurationBean.getEditDialog()
                        .getGeneralTab().getDateRangeBox().getDateRange());
                // --setting the start and end dates
                GeneralTab generalTab = viewConfigurationBean.getEditDialog()
                        .getGeneralTab();
    ...
    public boolean verifyVC(DateRangeBox dateRangeBox) {
        return ViewConfigurationData.verifyDates(dateRangeBox)
                && verifyAttributesAreSet();
    }
    ...
    private boolean verifyAttributesAreSet() {
        if (containsNonSetAttributes()) {
            ...
            msgConfirm += Messages
                    .getMessage("DIALOGS_EDIT_VC_ATTRIBUTES_NON_SET_CONFIRM_LABEL_2");
            ...
            int confirm = JOptionPane.showOptionDialog(null, msgConfirm,
                    msgTitle, JOptionPane.DEFAULT_OPTION,
    ...
    private boolean containsNonSetAttributes() {
        VCAttributesTreeModel model = viewConfigurationBean.getEditDialog()
                .getAttributesTab().getSelectedAttributesTree().getModel();
        TreeMap<String, ViewConfigurationAttribute> htAttr = model
                .getAttributes();
        if (htAttr == null) {
            return false;
        }
        ...
        while (keyIterator.hasNext()) {
            ViewConfigurationAttribute nextValue = htAttr.get(keyIterator
                    .next());
