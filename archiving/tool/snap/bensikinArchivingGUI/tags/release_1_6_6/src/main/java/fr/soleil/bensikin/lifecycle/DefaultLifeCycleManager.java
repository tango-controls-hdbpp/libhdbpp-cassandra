...
public class DefaultLifeCycleManager implements LifeCycleManager {
    ...
    /**
     * Called before the GUI graphics containers are instantiated. Loads from files :
     * <UL>
    ...
    /**
     * Sets the trace level early so that the preloading logs respect the log level. (ie. if the log
     * level was ERROR we don't want to display DEBUG logs )
     */
    ...
    private void startFactories() {
        ...
        System.out
                .println("DefaultLifeCycleManager/getImpl/user/" + user + "/password/" + password);
    ...
    /**
     * Called just after the GUI graphics containers are instantiated. Is used for operations that
     * need the containers to already exist: -pushing the pre loaded history and options to the
     * display components -setting the window size
     * 
     * @param startParameters Not used
     */
    ...
    /**
     * Called when the application detects a shutdown request, be it through the close icon or
     * through the menu's Exit option. Is used to: -save everything that has to be saved -close
     * resources And finally, shutdowns the application.
     * 
