 ...
 * <p/>
 * A Context object has two main attributes: - a ContextData attribute representing all
 * non-attribute-dependant information - a ContextAttributes attribute representing all
 * attribute-dependant information It also has a Snapshot[] attribute, representing the Snapshots
 * loaded for this context.
 * <p/>
 * The Context class also saves static references to the currently opened contexts, and the selected
 * context.
 * 
 * @author CLAISSE
 */
public class Context {
    ...
    /**
     * Default constructor. The contextData and contextAttributes attributes are initialized but
     * empty.
     */
    public Context() {
     ...
     */
    public Context(int _contextId, ContextAttributes _contextAttributes) {
     ...
     * @param _contextData
     */
    public Context(int _contextId) {
        contextId = _contextId;
    }

    /**
     * Looks up in database the referenced attributes for this context, provided they match the
     * search criterions. The join condition on context id is automatically added to
     * searchCriterions. Once the attributes are found ,they are converted to the ContextAttributes
     * attribute.
     * 
    ...
    public void loadAttributes(Criterions searchCriterions) throws SnapshotingException {
        ...
        searchCriterions.addCondition(new Condition(GlobalConst.TAB_CONTEXT[0],
                GlobalConst.OP_EQUALS, "" + contextId));
    ...
    /**
     * Looks up in database the referenced snapshots for this context, provided they match the
     * search criterions. The join condition on context id is automatically added to
     * searchCriterions. Once the contexts are found ,they are converted to the Snapshot []
     * attribute.
     * 
     * @param searchCriterions The object containing the search criterions
     * @throws Exception
     */
    public void loadSnapshots(Criterions searchCriterions) throws SnapshotingException {
        searchCriterions.addCondition(new Condition(GlobalConst.TAB_SNAP[1], GlobalConst.OP_EQUALS,
                "" + contextId));
    ...
    /**
     * Saves a SnapContext object, and results the resulting Context object (which unlike the
     * SnapContext has a known id and date)
     * 
     * @param context The context to save
     * @return A Context object that has the same properties as the context pamareter, except for
     *         its id and date
     * @throws Exception
     */
    public static Context save(SnapContext context) throws Exception {
        ...
        Context savedContext = new Context(savedContextData.getId());
        try {
    ...
    /**
     * Looks up in database the contexts matching the search criterions. Only loads their
     * ContextData part.
     * 
    ...
    /**
     * Looks up in database a particular Context, defined by its id. Loads both its ContextData and
     * ContextAttributes part.
     * 
    ...
    public static Context findContext(String id) throws Exception {
        Context ret = new Context(Integer.parseInt(id));
    ...
    /**
     * @return The Hashtable which values are the currently opened Contexts, and which keys are
     *         those contexts' ids.
     */
    ...
    public static void addOpenedContext(Context openedContext) {
        int id = openedContext.getContextId();
     ...
     */
    public static void setOpenedContexts(Hashtable<String, Context> openedContexts) {
        Context.openedContexts = openedContexts;
    }

    /**
     * Empties all references to opened and selected contexts, and resets the context displays, list
    ...
    public static void reset() {
        ...
        ContextDetailPanel.getInstance().getAttributeTableSelectionBean().getSelectionPanel()
                .getAttributesSelectTable().getModel().reset();
    }

    /**
     * Launches a snapshot on this context. If necessary (defined in SnapshotOptions), automatically
     * updates this snapshot's comment. Also refreshes the snapshots list.
     */
    ...
    /**
     * Builds a SnapContext object from parameters. Verifies the parameters and attributes list are
     ...
     * @throws Exception
     */
    public static SnapContext fillSnapContext(String name, String author, String reason,
            String description, ContextAttributesTreeModel model) {
     ...
     * @throws Exception
     */
    public static Context fillContext(String id_s, String creationDate_s, String name,
            String author, String reason, String description, ContextAttributesTreeModel model) {
        Iterator<String> enumer = model.getTreeAttributes().iterator();

        int id = (id_s == null || id_s.equals("")) ? -1 : Integer.parseInt(id_s);
        Date creationDate = (creationDate_s == null || creationDate_s.equals("")) ? null : Date
                .valueOf(creationDate_s);

        ContextData data = new ContextData(id, creationDate, name, author, reason, description);
        Context ret = new Context(data.getId());
     ...
     */
    private static boolean verifyAttributes(Iterator<String> enumer) {
        while (enumer.hasNext()) {
            return true;
        }
    ...
    public String toString() {
        ...
        if (this.contextData == null) {
            this.contextData = ContextDataManager.getInstance().getContextData(this.contextId);
        }

        int id = this.contextData.getId();
        Date creationDate = this.contextData.getCreation_date();
        String author = this.contextData.getAuthor_name();
        String description = this.contextData.getDescription();
        String name = this.contextData.getName();
        String reason = this.contextData.getReason();
        String path = this.contextData.getPath();
        ...
        if (creationDate != null) {
            openingLine.setAttribute(ContextData.CREATION_DATE_PROPERTY_XML_TAG, creationDate
                    .toString());
        }
        if (author != null) {
    ...
    public void push() {
        // }
        if (this.contextData == null) {
            this.contextData = ContextDataManager.getInstance().getContextData(contextId);
        }
        if (this.contextData != null) {
    ...
    public static Context buildContextToBeSaved() {
        ...
        Context ret = Context.fillContext(id_s, creationDate_s, name, author, reason, description,
                model);
    ...
    public void save(IContextManager manager, boolean saveAs) {
        ...
        try {
            Context newContext = new Context(this.getContextId());
            ContextData newData = new ContextData(this.contextData.getId(), this.contextData
                    .getCreation_date(), this.contextData.getName(), this.contextData
                    .getAuthor_name(), this.contextData.getReason(), this.contextData
                    .getDescription());
            newContext.setContextData(newData);
            newContext.setContextAttributes(this.getContextAttributes());
            newContext.setSnapshots(this.getSnapshots());
    ...
    private String getPathToUse(IContextManager manager, boolean saveAs) {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (f.exists()) {
                int choice = JOptionPane.showConfirmDialog(BensikinFrame.getInstance(), Messages
    ...
    public void setContextId(int _contextId) {
        contextId = _contextId;
    }

    public void setContextFile(boolean isContextFile) {
