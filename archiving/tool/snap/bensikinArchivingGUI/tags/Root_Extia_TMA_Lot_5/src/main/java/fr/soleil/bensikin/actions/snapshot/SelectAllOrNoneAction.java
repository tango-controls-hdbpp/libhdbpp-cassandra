...
public class SelectAllOrNoneAction extends BensikinAction {
	 ...
	 * @param name
	 */
	public SelectAllOrNoneAction(String name, int _type,
			SnapshotDetailTable _table) {
		super(name);

		if (_type != SELECT_REVERSE_TYPE && _type != SELECT_ALL_TYPE
				&& _type != SELECT_NONE_TYPE) {
			throw new IllegalArgumentException("Expected either of "
					+ SELECT_REVERSE_TYPE + "," + SELECT_ALL_TYPE + ","
					+ SELECT_NONE_TYPE + " as a parameter. Received " + type
					+ " instead.");
		}
