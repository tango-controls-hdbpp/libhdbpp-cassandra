...
public class XMLFavoritesManager implements IFavoritesManager {
	 ...
	 * 
	 * @see
	 * bensikin.bensikin.favorites.IFavoritesManager#saveFavorites(bensikin.
	 * bensikin.favorites.Favorites, java.lang.String)
	 */
	...
	public Favorites loadFavorites(String favoritesResourceLocation)
		...
		// BEGIN OPEN BOOKS
		FavoritesContextSubMenu contextSubMenu = new FavoritesContextSubMenu(
				FavoritesContextSubMenu.XML_TAG);
		if (favoritesHt != null) {
			TreeMap contextsBook = (TreeMap) favoritesHt
					.get(FavoritesContextSubMenu.XML_TAG);
	...
	private Hashtable loadFavoritesIntoHash(String location) throws Exception {
		...
		try {
			...
			// if there is no favorites file yet,we return null to have an empty
			// menu
			return null;
		}
	 ...
	 * @throws Exception
	 */
	private Hashtable loadFavoritesIntoHashFromRoot(Node rootNode)
			throws Exception {
		try {
	 ...
	 */
	private TreeMap getChapter(TreeMap mapIn, Node currentChapterNode)
			throws Exception {
		...
		for (int i = 0; i < bookNodes.getLength(); i++) {
			...
			if (XMLUtils.hasRealChildNodes(nextNode)) {
				...
				if (attributes == null) {
					...
					XMLLine line = new XMLLine(nextNodeName,
