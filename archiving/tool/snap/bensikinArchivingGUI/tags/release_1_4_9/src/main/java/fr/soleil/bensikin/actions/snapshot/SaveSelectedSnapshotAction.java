 ...
 * <UL>
 * <LI> If the action's snapshot attribute hasn't been filled yet, gets the current snapshot from the selected tab; if it's null too, does nothing
 * <LI> Opens a file chooser dialog specialised in snapshot saving, gets the path to save to from it (if non-default)
 * <LI> Uses the application's ISnapshotManager to save the context
 * <LI> If the saved snapshot was a file snapshot, removes the "modified" mark (if there was one)
...
public class SaveSelectedSnapshotAction extends BensikinAction
{
    ...
    /**
     * Standard action constructor that sets the action's name, plus sets the isDefault attribute and the reference to the <code>Snapshot</code> to save on <code>actionPerformed</code>
     *
     * @param name       The action name
     * @param _snapshot  The snapshot to save on <code>actionPerformed</code>
     * @param _isDefault True if the save action is a quick save to the working directory and default file, false otherwise
     */
    public SaveSelectedSnapshotAction ( String name , Snapshot _snapshot , boolean _isSaveAs )
    {
        super( name );
    ...
    */
    public void actionPerformed ( ActionEvent actionEvent )
    {
        ...
        if ( snapshot == null )
        {
            ...
            if ( snapshot == null )
            {
                JOptionPane.showMessageDialog( BensikinFrame.getInstance() ,
                                               Messages.getMessage( "DIALOGS_POPUP_SNAPSHOT_NO_SNAPSHOT" ) ,
                                               Messages.getMessage( "DIALOGS_POPUP_SNAPSHOT_WARNING" ) ,
        ...
        /*if ( !this.isDefault )
        {
            ...
            {
                location = manager.getDefaultSaveLocation();
            }
            ...
            String title = Messages.getMessage( "DIALOGS_FILE_CHOOSER_SAVE_SNAPSHOT_TITLE" );
            ...
            if ( returnVal == JFileChooser.APPROVE_OPTION )
            {
                ...
                if ( f != null )
                {
                    ...
                    if ( f.exists() )
                    {
                        int ok = JOptionPane.showConfirmDialog( BensikinFrame.getInstance() ,
        ...
        else
        {
            ...
            try
            {
                System.out.println( "---------SNAPSHOT TO BE SAVED--------------" );
