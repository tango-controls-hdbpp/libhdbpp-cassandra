...
public class Snapshot {
	...
	if (contextId < 0) {
	    contextId = ContextDataManager.getInstance().getContextIdBySnapshotId(
		    snapshotData.getId());
	}
	if (getSnapshotAttributes() == null
		...
		final SnapAttributeExtract currentSnapAttributeExtract = sae[i];
	...
	final SnapShot transformedSnap = toSnapShot();
     ...
     * @throws Exception
     */
    public String setEquipmentWithCommand(final String cmd_name, final String option)
	    throws SnapshotingException {
     ...
     */
    public static void reset(final boolean hasToRemoveSelectedSnapshots,
	    final boolean resetContextFields) {
	...
	final Condition condition = new Condition(GlobalConst.TAB_SNAP[0], "=", String.valueOf(id));
	...
	final SnapshotAttribute[] attributeList = snapAttrs.getSnapshotAttributes();
	...
	final SnapShot ret = new SnapShot(argin);
	...
	final XMLLine openingLine = new XMLLine(Snapshot.XML_TAG, XMLLine.OPENING_TAG_CATEGORY);
	final XMLLine closingLine = new XMLLine(Snapshot.XML_TAG, XMLLine.CLOSING_TAG_CATEGORY);
    ...
    public static void setSecondSnapshotOfComparisonTitle(
	    final String secondSnapshotOfComparisonTitle) {
     ...
     */
    public static void setOpenedSnapshots(final Hashtable<String, Snapshot> openedSnapshots) {
     ...
     */
    public static void setSelectedSnapshots(final Hashtable<String, Snapshot> selectedSnapshots) {
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
		...
		final int choice = JOptionPane.showConfirmDialog(BensikinFrame.getInstance(),
			Messages.getMessage("DIALOGS_FILE_CHOOSER_FILE_EXISTS"), Messages
	...
	if (contextData != null) {
	    _context = new SnapContext();
	    _context.setAuthor_name(contextData.getAuthor_name());
	    _context.setCreation_date(contextData.getCreation_date());
	    _context.setDescription(contextData.getDescription());
	    _context.setId(contextData.getId());
	    _context.setName(contextData.getName());
	    _context.setReason(contextData.getReason());
	    _context.setAttributeList(getAsAttributeList());
	} else {
	    ...
	    final Context ctx = Context.save(_context);
	...
	for (final SnapshotAttribute next : attrs) {
	    final String attribute_complete_name = next.getAttribute_complete_name();
    ...
    public StringBuffer snapshotInfoToUserFriendlyStringBuffer(StringBuffer infoBuffer) {
	if (infoBuffer == null) {
    ...
    public StringBuffer snapshotAttributesToUserFriendlyStringBuffer(StringBuffer attrBuffer) {
	if (attrBuffer == null) {
