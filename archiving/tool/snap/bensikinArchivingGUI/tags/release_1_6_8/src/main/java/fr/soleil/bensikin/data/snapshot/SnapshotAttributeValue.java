...
public class SnapshotAttributeValue {
	...
	case SPECTRUM_DATA_FORMAT:
		...
		case TangoConst.Tango_DEV_LONG:
		    final Integer[] lval = (Integer[]) spectrumValue;
		    dimX = lval.length;
		    for (int i = 0; i < dimX; i++) {
			ret += "" + lval[i];
			if (i < dimX - 1) {
	...
	case IMAGE_DATA_FORMAT:
	     ...
	     * int i = 0 ; i < dimX ; i++ ) { Object[] y = imageValue[ i ]; int
	     * dimY = y.length; for ( int j = 0 ; j < dimX ; j++ ) { ret += ( ""
	     * + imageValue[ i ][ j ] ); if ( j < dimY - 1 ) { ret +=
	     * Y_SEPARATOR; }
	     * 
	     * }
	     * 
    ...
    public void appendToString(final StringBuffer buffer) {
	if (buffer != null) {
    ...
    /**
     * Sets the value attribute, and depending on the data format, the
     * scalarValue, imageValue, or spectrumValue.
     * 
     * @param _value
     *            The raw value
     */
	...
	case SCALAR_DATA_FORMAT:
		...
		try {
		    ...
		    case TangoConst.Tango_DEV_DOUBLE:
			scalarValue = doubleValue;
			break;

		    case TangoConst.Tango_DEV_FLOAT:
			scalarValue = new Float(doubleValue.floatValue());
			break;

		    case TangoConst.Tango_DEV_USHORT:
		    case TangoConst.Tango_DEV_SHORT:
			scalarValue = new Short(doubleValue.shortValue());
			break;

		    case TangoConst.Tango_DEV_STATE:
		    case TangoConst.Tango_DEV_ULONG:
		    case TangoConst.Tango_DEV_LONG:
			scalarValue = new Integer(doubleValue.intValue());
			break;

		    case TangoConst.Tango_DEV_BOOLEAN:
			scalarValue = Boolean.valueOf(stringValue);
			break;
	...
	case SPECTRUM_DATA_FORMAT:
		...
		try {
			...
			spectrumValue = shArray;
			break;

		    case TangoConst.Tango_DEV_STATE:
		    case TangoConst.Tango_DEV_ULONG:
		    case TangoConst.Tango_DEV_LONG:
			final Integer[] intArray = new Integer[length];
			for (int i = 0; i < length; i++) {
			    String token = tokenizer.nextToken();
			    if ("".equals(token) || "null".equals(token)) {
				intArray[i] = null;
	...
	case IMAGE_DATA_FORMAT:
		...
		setImageValue(new Object[][] { { value } });
	    }
	    break;
     ...
     * 
     * @param _dataFormat
     *            The data format ( scalar, spectrum, or image )
     * @param _dataType
     *            The Tango data type
     * @param _value
     *            The value content
     * @throws IllegalArgumentException
     *             if dataFormat is not in
     *             (SCALAR_DATA_FORMAT,SPECTRUM_DATA_FORMAT
     *             ,IMAGE_DATA_FORMAT,NOT_APPLICABLE_DATA_FORMAT)
     */
    public SnapshotAttributeValue(final int _dataFormat, final int _dataType, final Object _value)
	    throws IllegalArgumentException {
     ...
     */
    public void setImageValue(final Object[][] imageValue) {
     ...
     */
    public void setScalarValue(final Object scalarValue) {
     ...
     */
    public void setSpectrumValue(final Object[] spectrumValue) {
