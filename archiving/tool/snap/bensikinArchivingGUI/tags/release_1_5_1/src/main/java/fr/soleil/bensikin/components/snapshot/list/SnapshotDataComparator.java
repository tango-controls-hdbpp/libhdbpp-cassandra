...
/**
 * Implements a Comparator on SnapshotData objects, useful to sort the snapshots list.
 * The comparison can be on any one of the SnapshotData fields, depending on the type specified on constructing the comparator.
...
public class SnapshotDataComparator extends BensikinComparator
{
	 ...
	 */
	public SnapshotDataComparator(int _fieldToCompare) throws IllegalArgumentException
	{
		super ( _fieldToCompare );

		if
		(
		        _fieldToCompare != COMPARE_ID &&
		        _fieldToCompare != COMPARE_TIME &&
