...
public class ContextXMLHelper
{
     ...
     *
     * @param location The complete path to the context file to load
     ...
     * @throws Exception
     */
    private static Context loadContextIntoHashFromRoot ( Node rootNode ) throws Exception
    {
        ...
        if ( rootNode.hasChildNodes() )
        {
                    ...
                    // as many loops as there are attributes in the VC
            {
                ...
                String currentAttributeType = currentAttributeNode.getNodeName().trim();//has to be "attribute" or "genericPlotParameters"
                if ( currentAttributeType.equals( ContextAttribute.XML_TAG ) )
                {
                    ...
                    String attributeCompleteName = ( String ) attributeProperties.get( ContextAttribute.COMPLETE_NAME_PROPERTY_XML_TAG );
                    String device = ( String ) attributeProperties.get( ContextAttribute.DEVICE_PROPERTY_XML_TAG );
                    String domain = ( String ) attributeProperties.get( ContextAttribute.DOMAIN_PROPERTY_XML_TAG );
                    String family = ( String ) attributeProperties.get( ContextAttribute.FAMILY_PROPERTY_XML_TAG );
                    String member = ( String ) attributeProperties.get( ContextAttribute.MEMBER_PROPERTY_XML_TAG );
                    String name = ( String ) attributeProperties.get( ContextAttribute.NAME_PROPERTY_XML_TAG );
     ...
     *
     * @param node The XML Context node
    ...
    private static Context loadContextData ( Node node ) throws Exception
    {
        ...
        String creationDate_s = ( String ) VCProperties.get( ContextData.CREATION_DATE_PROPERTY_XML_TAG );
        String author = ( String ) VCProperties.get( ContextData.AUTHOR_PROPERTY_XML_TAG );
        String description = ( String ) VCProperties.get( ContextData.DESCRIPTION_DATE_PROPERTY_XML_TAG );
        String id_s = ( String ) VCProperties.get( ContextData.ID_PROPERTY_XML_TAG );
        String name = ( String ) VCProperties.get( ContextData.NAME_PROPERTY_XML_TAG );
        String reason = ( String ) VCProperties.get( ContextData.REASON_PROPERTY_XML_TAG );
        
        String path = ( String ) VCProperties.get( ContextData.PATH_PROPERTY_XML_TAG );
        String isModified_s = ( String ) VCProperties.get( ContextData.IS_MODIFIED_PROPERTY_XML_TAG );
