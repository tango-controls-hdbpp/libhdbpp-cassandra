 ...
 * <LI> If the event is on a Member node which attributes are already loaded, nothing happens
 * <LI> Otherwise, that is in the case of a yet unloaded Member node, the attributes are loaded, and the tree model refreshed
 * </UL>
 *
 * @author CLAISSE
 */
public class PossibleAttributesTreeSelectionListener implements TreeSelectionListener
{
    ...
    /**
     * Called whenever a Member node's attributes needs to be loaded.
     * Uses the current implementation of <code>ITangoManager</code> to load them.
     *
    ...
    private void query ( String node , int level )
    {
        if ( level == 3 )
        {
            ...
            String device_name =
                    grdparent.toString() + GUIUtilities.TANGO_DELIM +
                    parent.toString() + GUIUtilities.TANGO_DELIM +
            ...
            if ( attribute_list != null )
            {
                for ( int i = 0 ; i < attribute_list.size() ; i++ )
                {
                    ...
                    ContextAttribute attribute = new ContextAttribute( ( String ) attribute_list.get( i ) );
                    attribute.setCompleteName( grdparent.toString() + GUIUtilities.TANGO_DELIM + parent.toString() + GUIUtilities.TANGO_DELIM + currentNode.toString() + GUIUtilities.TANGO_DELIM + attributeNode.toString() );
