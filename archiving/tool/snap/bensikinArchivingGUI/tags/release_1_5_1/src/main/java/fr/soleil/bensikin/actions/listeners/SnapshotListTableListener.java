...
/**
 * Listens to double clicks on the snapshots list table body. When a row is
 * clicked, the selected snapshot is added to the snapshots selection.
 * <UL>
 ...
 * @author CLAISSE
 */
public class SnapshotListTableListener extends MouseAdapter implements
        ActionListener {
    ...
    public SnapshotListTableListener() {
        ...
        selectItem = new JMenuItem(Messages
                .getMessage("SNAPSHOT_LIST_VIEW_DETAILS"));
        ...
        if (event.getButton() == MouseEvent.BUTTON1
            ...
            if (event.getButton() == MouseEvent.BUTTON3
                ...
                table.addRowSelectionInterval(table
                        ...
                        .getPoint()));
                tableMenu.show(SnapshotListTable.getInstance(), event.getX(),
                        event.getY());
            }
        }
    }
    ...
    private void selectSnapshot() {
        ...
        SnapshotListTableModel sourceModel = (SnapshotListTableModel) source
                .getModel();
        SnapshotData selectedSnapshotData = sourceModel
                .getSnapshotDataAtRow(row);

        Context selectedContext = Context.getSelectedContext();

        Snapshot selectedSnapshot = new Snapshot(selectedContext,
                selectedSnapshotData);
        ILogger logger = LoggerFactory.getCurrentImpl();
        try {
            selectedSnapshot.loadAttributes();
            String msg = Messages.getLogMessage("LOAD_SNAPSHOT_ATTRIBUTES_OK");
            ...
            if (e.computeIsDueToATimeOut()) {
                msg = Messages
                        .getLogMessage("LOAD_SNAPSHOT_ATTRIBUTES_TIMEOUT");
            }
            else {
                msg = Messages.getLogMessage("LOAD_SNAPSHOT_ATTRIBUTES_KO");
            }
            ...
            String msg = Messages.getLogMessage("LOAD_SNAPSHOT_ATTRIBUTES_KO");
            logger.trace(ILogger.LEVEL_ERROR, msg);
            logger.trace(ILogger.LEVEL_ERROR, e);
            return;
        }
