...
public class TangoManagerImpl implements ITangoManager {
	 ...
	 * <UL>
	 * <LI>Gets the criterions on the respective domain, family, member, and
	 * attribute parts from <code>searchCriterions</code>; if one of them is
	 * null, it is replaced with a joker
	 * <LI>Uses the Database API to get the domains matching the domain
	 * criterion
	 * <LI>For each such domain, uses the Database API to get the families
	 * rattached to this domain and matching the family criterion
	 * <LI>For each such family, uses the Database API to get the members
	 * rattached to this family and matching the member criterion
	 * <LI>The loading stops there, the attributes are not loaded yet lest Tango
	 * be overloaded
	 * </UL>
	 * 
	...
	public Vector loadDomains(Criterions searchCriterions) {
		...
		if (searchCriterions != null) {
			...
			Condition domainCondition = searchCriterions
					.getConditions(GlobalConst.TAB_DEF[4])[0];
			domain_criterions = domainCondition.getValue();

			Condition familyCondition = searchCriterions
					.getConditions(GlobalConst.TAB_DEF[5])[0];
			family_criterions = familyCondition.getValue();

			Condition memberCondition = searchCriterions
					.getConditions(GlobalConst.TAB_DEF[6])[0];
			member_criterions = memberCondition.getValue();

			// Condition attributeCondition = searchCriterions.getConditions(
			// GlobalConst.TAB_DEF[ 7 ] )[ 0 ];
		...
		try {
			...
			String domains[] = database.get_device_domain(domain_criterions
					+ GUIUtilities.TANGO_DELIM + GUIUtilities.TANGO_JOKER);
			...
			for (int domainsIndex = 0; domainsIndex < numberOfDomains; domainsIndex++) {
				...
				String[] families = database
						.get_device_family(currentDomainName
								+ GUIUtilities.TANGO_DELIM + family_criterions
								+ GUIUtilities.TANGO_DELIM
								+ GUIUtilities.TANGO_JOKER);
				...
				for (int familiesIndex = 0; familiesIndex < numberOfFamilies; familiesIndex++) {
					...
					String[] members = database
							.get_device_member(currentDomainName
									+ GUIUtilities.TANGO_DELIM
									+ currentFamilyName
									+ GUIUtilities.TANGO_DELIM
									+ member_criterions);
	...
	public ArrayList dbGetAttributeList(String device_name) {
		...
		try {
			...
			device_data_argin.insert(GUIUtilities.TANGO_JOKER + device_name
					+ GUIUtilities.TANGO_JOKER);
