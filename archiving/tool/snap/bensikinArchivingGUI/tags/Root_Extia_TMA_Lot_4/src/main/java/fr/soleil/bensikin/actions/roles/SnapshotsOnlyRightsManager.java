...
public class SnapshotsOnlyRightsManager implements IRightsManager {
	...
	public boolean isGrantedToOperator(Action action) {
		...
		/*
		 * if ( action instanceof LaunchSnapshotAction ) { return false; }
		 */
		...
		/*
		 * if ( action instanceof EditCommentAction ) { return false; } if (
		 * action instanceof OpenEditCommentAction ) { return false; } if (
		 * action instanceof SelectAllOrNoneAction ) { return false; } if (
		 * action instanceof SetEquipmentsAction ) { return false; } if ( action
		 * instanceof SnapSpectrumSetAction ) { return false; } if ( action
		 * instanceof SnapSpectrumSetAllAction ) { return false; }
		 */
		return true;
	}
	...
	public void disableUselessFields() {
		...
		ContextDetailPanel.getInstance().getAttributeTableSelectionBean()
				.getSelectionPanel().setEditionEnabled(false);
		ContextDetailPanel.getInstance().getAttributeTableSelectionBean()
				.stop();
		ContextDetailPanel.getInstance().getAttributeTableSelectionBean()
				.getSelectionPanel().stepDone(0, 0);
	}
