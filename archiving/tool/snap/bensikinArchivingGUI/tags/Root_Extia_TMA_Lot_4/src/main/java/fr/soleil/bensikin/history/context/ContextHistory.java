...
public class ContextHistory {
	 ...
	 * 
	 * @param _selectedContextRef
	 *            A reference to the selected context
	 * @param _openedContextRefs
	 *            Reference to the opened contexts
	 */
	...
	public ContextHistory(Context selectedContext, Context[] openedContexts) {
		if (selectedContext != null) {
			if (selectedContext.getContextData() != null) {
				this.selectedContextRef = new SelectedContextRef(
						selectedContext);
			}
		}
		...
		if (openedContexts != null) {
			...
			for (int i = 0; i < openedContexts.length; i++) {
				this.openedContextRefs[i] = new OpenedContextRef(
						openedContexts[i]);
			}
		}
	...
	public String toString() {
		...
		if (openedContextRefs != null) {
			ret += new XMLLine(History.OPENED_CONTEXTS_KEY,
	...
	/**
	 * @param openedContextRefs
	 *            The openedContextRefs to set.
	 */
	...
	/**
	 * @param selectedContextRef
	 *            The selectedContextRef to set.
	 */
