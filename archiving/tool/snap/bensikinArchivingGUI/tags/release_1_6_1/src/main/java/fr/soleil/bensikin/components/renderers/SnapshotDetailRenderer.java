	...
	public Component getTableCellRendererComponent(JTable table, Object value,
		...
		if (value instanceof SnapshotAttributeValue) {
				...
				if (((SnapshotAttributeValue) value).getScalarValue() == null) {
					...
					ret = new BensikinTableCellRenderer()
							.getTableCellRendererComponent(table,
									(SnapshotAttributeValue) value, isSelected,
									hasFocus, row, column);
				} else {
					ret = new BensikinTableCellRenderer()
							.getTableCellRendererComponent(table,
			...
			case SnapshotAttributeValue.SPECTRUM_DATA_FORMAT:
				...
				if (table.getColumnName(column).equals(
					...
					name += Messages
						...
						Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_DELTA"))) {
					name += Messages
				...
				if (value instanceof SnapshotAttributeWriteValue) {
					button = new SpectrumWriteValueButton(table.getValueAt(row,
							0).toString()
					...
					button = new SpectrumDeltaValueButton(table.getValueAt(row,
							0).toString()
							+ " " + name, (SnapshotAttributeValue) value,
							table, row, column);
				}
			...
			case SnapshotAttributeValue.IMAGE_DATA_FORMAT:
				...
				if (value instanceof SnapshotAttributeWriteValue) {
					imageButton = new ImageWriteButton(
							table,
							row,
					...
					imageButton = new ImageButton(table.getModel().getValueAt(
							row, 0).toString(),
	...
	public static String getTextFor(SnapshotAttributeValue value) {
		...
		case SnapshotAttributeValue.SCALAR_DATA_FORMAT:
			if (value.getScalarValue() == null) {
				...
				return TangoStateTranslation.getStrStateValue(valS.intValue());
		...
		case SnapshotAttributeValue.SPECTRUM_DATA_FORMAT:
			if (value instanceof SnapshotAttributeReadValue) {
				return Messages.getMessage("DIALOGS_SPECTRUM_ATTRIBUTE_READ");
			} else if (value instanceof SnapshotAttributeWriteValue) {
				return Messages.getMessage("DIALOGS_SPECTRUM_ATTRIBUTE_WRITE");
			} else if (value instanceof SnapshotAttributeDeltaValue) {
				return Messages.getMessage("DIALOGS_SPECTRUM_ATTRIBUTE_DELTA");
			}
		case SnapshotAttributeValue.IMAGE_DATA_FORMAT:
			if (value instanceof SnapshotAttributeWriteValue) {
