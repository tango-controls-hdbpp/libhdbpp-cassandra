//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/history/History.java,v $
//
...
/**
 * Contains the application's history, eg. what contexts/snashots were opened/selected at the time of
 * the latest shutdown.
 *
 * @author CLAISSE
 */
public class History
{
     ...
     *
     * @param _snapshotHistory the Snapshot history
     * @param _contextHistory  the Context history
     */
    public History ( SnapshotHistory _snapshotHistory , ContextHistory _contextHistory )
    {
     ...
     *
     * @return A Hashtable which keys are the opened contexts' ids and which values are those contexts.
     * @throws Exception
     */
    private Hashtable loadOpenedContextsFromHistory () throws SnapshotingException , Exception
    {
        Hashtable openedContexts = new Hashtable();

        ContextHistory contextHistory = this.getContextHistory();
        OpenedContextRef[] openedContextRefs = contextHistory.getOpenedContextRefs();
        ...
        for ( int i = 0 ; i < openedContextRefs.length ; i++ )
        {
            ...
            Condition condition = new Condition( GlobalConst.TAB_CONTEXT[ 0 ] , "=" , String.valueOf( id ) );

            searchCriterions = new Criterions();
            searchCriterions.addCondition( condition );

            //Condition[] cond = searchCriterions.getConditions( GlobalConst.TAB_CONTEXT[ 0 ] );
            ...
            if (snapContexts.length > 0)
            {
                ...
                openedContexts.put( String.valueOf( id ) , new Context( contextData ) );
            }
        }
     ...
     * @throws Exception
     */
    private Context loadSelectedContext () throws SnapshotingException , Exception
    {
        ContextHistory contextHistory = this.getContextHistory();
        SelectedContextRef selectedContextRef = contextHistory.getSelectedContextRef();
        ...
        Condition condition = new Condition( GlobalConst.TAB_CONTEXT[ 0 ] , "=" , String.valueOf( selectedContextRef.getId() ) );
    ...
    /**
     * Builds and return a History object representing the current state of the application
     *
    ...
    public static History getCurrentHistory ()
    {
        ...
        Hashtable htOpenedContexts = Context.getOpenedContexts();
        Context[] openedContexts = new Context[ htOpenedContexts.size() ];
        Enumeration e = htOpenedContexts.keys();
        int i = 0;
        ...
        //SNAPSHOTS
        Hashtable htOpenedSnapshots = Snapshot.getOpenedSnapshots();
        Snapshot[] openedSnapshots = new Snapshot[ htOpenedSnapshots.size() ];
        Enumeration e1 = htOpenedSnapshots.keys();
        int i1 = 0;
        ...
        Hashtable htSelectedSnapshots = Snapshot.getSelectedSnapshots();
        Snapshot[] selectedSnapshots = new Snapshot[ htSelectedSnapshots.size() ];
        Enumeration e2 = htSelectedSnapshots.keys();
        int i2 = 0;
     ...
     *
     * @param selectedContext The context to which the snapshots are rattached
     * @return A Hashtable which keys are the opened snapshots' ids and which values are those snapshots.
     * @throws SnapshotingException
     * @throws Exception
     */
    private Hashtable loadOpenedSnapshotsFromHistory ( Context selectedContext ) throws SnapshotingException , Exception
    {
        Hashtable openedSnapshots = new Hashtable();

        SnapshotHistory snapshotHistory = this.getSnapshotHistory();
        OpenedSnapshotRef[] openedSnapshotRefs = snapshotHistory.getOpenedSnapshotRefs();
        ...
        for ( int i = 0 ; i < openedSnapshotRefs.length ; i++ )
        {
            ...
            //Condition[] cond = searchCriterions.getConditions( GlobalConst.TAB_SNAP[ 0 ] );
            ...
            if (snapshots.length > 0)
            {
                ...
                openedSnapshots.put( String.valueOf( id ) , new Snapshot( selectedContext , snapshotData ) );
            }
        }
     ...
     * @return A Hashtable which keys are the selected snapshots' ids and which values are those snapshots.
     * @throws SnapshotingException
     * @throws Exception
     */
    private Hashtable loadSelectedSnapshotsFromHistory ( Context selectedContext ) throws SnapshotingException , Exception
    {
        Hashtable selectedSnapshots = new Hashtable();

        SnapshotHistory snapshotHistory = this.getSnapshotHistory();
        SelectedSnapshotRef[] selectedSnapshotRefs = snapshotHistory.getSelectedSnapshotRefs();
    ...
    /**
     * Applies the opened/selected state of snapshots/contexts contained in this History to the static references in Context/Snapshot.
     *
     * @throws SnapshotingException
     * @throws Exception
     */
    public void setOpenedAndSelectedEntities () throws SnapshotingException , Exception
    {
        Hashtable openedContexts = this.loadOpenedContextsFromHistory();
        ...
        {
            return;
            //we don't load the snapshots because they don't have an associated context
        }
        Context.setSelectedContext( selectedContext );

        Hashtable openedSnapshots = this.loadOpenedSnapshotsFromHistory( selectedContext );
        Snapshot.setOpenedSnapshots( openedSnapshots );

        Hashtable selectedSnapshots = this.loadSelectedSnapshotsFromHistory( selectedContext );
        Snapshot.setSelectedSnapshots( selectedSnapshots );
    }

    /**
     * Displays onscreen the opened/selected state of snapshots/contexts contained in this History.
     */
    public void push ()
    {
    ...
    /**
     * Displays onscreen the selected state of snapshots contained in this History.
     */
    private void pushSelectedSnapshots ()
    {
        Hashtable selectedSnapshots = Snapshot.getSelectedSnapshots();
        if ( selectedSnapshots == null )
        {
            return;
        }

        Iterator it = selectedSnapshots.values().iterator();
        //int size = selectedSnapshots.values().size();

        int i = 0;
        SnapshotDetailTabbedPane tabbedPane = SnapshotDetailTabbedPane.getInstance();
        while ( it.hasNext() )
        {
            Snapshot nextSnapshot = ( Snapshot ) it.next();
    ...
    /**
     * Displays onscreen the opened state of snapshots contained in this History.
     */
    private void pushOpenedSnapshots ()
    {
        Hashtable openedSnapshots = Snapshot.getOpenedSnapshots();
        if ( openedSnapshots == null )
        {
            return;
        }

        Iterator it = openedSnapshots.values().iterator();
    ...
    {
        Hashtable openedContexts = Context.getOpenedContexts();
        if ( openedContexts != null )
        {
            Iterator it = openedContexts.values().iterator();
    ...
    /**
     * Displays onscreen the selected state of contexts contained in this History.
     */
    ...
    /**
     * @param contextHistory The contextHistory to set.
     */
    ...
    /**
     * @param snapshotHistory The snapshotHistory to set.
     */
