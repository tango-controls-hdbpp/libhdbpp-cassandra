...
/**
 * Saves the current context in a file.
 * This action can be of 2 type: default (quick save to the default directory and file), or non-default (save to the user-defined directory and file, with prepositionning on the default directory).
 * <UL>
 * <LI> Builds the current context from the display area
 * <LI> Opens a file chooser dialog specialised in context saving, gets the path to save to from it (if non-default)
...
public class SaveSelectedContextAction extends BensikinAction
{
    ...
    public void actionPerformed ( ActionEvent actionEvent )
    {
        ...
        //System.out.println ( "SaveSelectedContextAction/selectedContext/"+selectedContext.getPath()+"/" );
    }
    ...
    /*public void actionPerformed ( ActionEvent actionEvent )
    {
        ...
        if ( !this.isDefault )
        {
            ...
            ContextFileFilter filter = new ContextFileFilter();
            chooser.addChoosableFileFilter( filter );

            String title = Messages.getMessage( "DIALOGS_FILE_CHOOSER_SAVE_CONTEXT_TITLE" );
