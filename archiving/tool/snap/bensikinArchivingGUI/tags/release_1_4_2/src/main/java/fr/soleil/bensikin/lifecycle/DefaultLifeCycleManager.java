//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/lifecycle/DefaultLifeCycleManager.java,v $
//
...
public class DefaultLifeCycleManager implements LifeCycleManager
{
     ...
     * </UL>
     *
     * @param startParameters Not used
     */
...
//        	 No exception can be raised
    ...
    /**
     * Sets the trace level early so that the preloading logs respect the log level.
     * (ie. if the log level was ERROR we don't want to display DEBUG logs )
     */
    ...
    private void loadOptions (Splash splash)
    {
        ...
        try
        {
            ...
            setTraces();
            splash.progress(8);
            splash.setMessage("loading log options done. Loading history options");
    ...
    /**
     * Sets the load history boolean early so that the history isn't loaded if it doesn't have to.
     */
    ...
    private void loadFavorites ()
    {
        ...
        try
        {
            ...
            IFavoritesManager favoritesManager = FavoritesManagerFactory.getCurrentImpl();

            favoritesManager.loadFavorites( favoritesPath );

            String msg = Messages.getLogMessage( "APPLICATION_WILL_START_LOAD_FAVORITES_OK" );
            logger.trace( ILogger.LEVEL_DEBUG , msg );
        }
        catch ( FileNotFoundException fnfe )
        {
            String msg = Messages.getLogMessage( "APPLICATION_WILL_START_LOAD_FAVORITES_WARNING" );
            logger.trace( ILogger.LEVEL_WARNING , msg );
            logger.trace( ILogger.LEVEL_WARNING , fnfe );
            return;
        }
        ...
        {
            String msg = Messages.getLogMessage( "APPLICATION_WILL_START_LOAD_FAVORITES_KO" );
            logger.trace( ILogger.LEVEL_ERROR , msg );
            logger.trace( ILogger.LEVEL_ERROR , e );
            return;
        }
    ...
    /**
     * Tries to use WindowsLookAndFeel. If it fails, uses the JAVA LookAndFeel instead.
     */
    private void setUpLookAndFeel ()
    {
        ...
        /*try {
            ...
            //do nothing and stay in JAVA plaf
        }*/
    }
    ...
    private void startFactories ()
    {
        ...
        System.out.println("DefaultLifeCycleManager/getImpl/user/"+user+"/password/"+password);
     ...
     * Is used for operations that need the containers to already exist:
     * -pushing the pre loaded history and options to the display components
    ...
    public void applicationStarted ( Hashtable startParameters )
    {
        ...
        //We have to push the selected context data only after the data panel is created
        //Otherwise the fields don't exist yet, hence we can't set their values
     ...
     * Is used to:
     * -save everything that has to be saved
     * -close resources
     * And finally, shutdowns the application.
     *
     * @param endParameters Not used
     */
    ...
    private void saveFavorites () throws Exception
    {
        ...
        try
        {
            ...
            String msg = Messages.getLogMessage( "APPLICATION_WILL_STOP_SAVE_FAVORITES_OK" );
            logger.trace( ILogger.LEVEL_DEBUG , msg );
        }
        catch ( Exception e )
        {
            String msg = Messages.getLogMessage( "APPLICATION_WILL_STOP_SAVE_FAVORITES_KO" );
            logger.trace( ILogger.LEVEL_ERROR , msg );
            logger.trace( ILogger.LEVEL_ERROR , e );
            return;
        }
