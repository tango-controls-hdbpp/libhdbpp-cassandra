...
{
    protected final static ImageIcon imageIcon = new ImageIcon(Bensikin.class.getResource("icons/viewImage.gif"));
	 ...
	 *
	 */
	public ImageAttibuteDialog(String attributeName, Object _value, int data_type, String _displayFormat)
	{
		super(BensikinFrame.getInstance() , Messages.getMessage("DIALOGS_IMAGE_ATTRIBUTE_TITLE") + ": " +  attributeName, true);
    ...
    protected void initComponents()
	{
        ...
        if (hasNoData())
        {
            noData = new JLabel(Messages.getMessage("DIALOGS_IMAGE_ATTRIBUTE_NAN"), JLabel.CENTER);
            GUIUtilities.setObjectBackground(noData, GUIUtilities.SNAPSHOT_COLOR);
        }
        else
        {
            initTable();
    ...
    {
        headerTable = new JTable(new ColumnHeaderModel(imageTable.getRowCount()));
        headerTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        for (int i = 0; i < headerTable.getColumnCount(); i++)
        {
            headerTable.getColumn(headerTable.getColumnName(i)).setMinWidth(initColumnWidth);
            headerTable.getColumn(headerTable.getColumnName(i)).setWidth(initColumnWidth);
            headerTable.getColumn(headerTable.getColumnName(i)).setPreferredWidth(initColumnWidth);
            headerTable.getColumn(headerTable.getColumnName(i)).setMaxWidth(initColumnWidth);
        }
        headerTable.setDefaultRenderer(Object.class, new JTableHeader().getDefaultRenderer());
        headerTable.setMaximumSize(new Dimension(initColumnWidth, Integer.MAX_VALUE));
        viewButton = new JButton(new ViewImageAction(attributeName, value, data_type, displayFormat));
    ...
    protected void addWhenData()
    {
        ...
        rowHeader.setMaximumSize(new Dimension(initColumnWidth, Integer.MAX_VALUE));
        rowHeader.setPreferredSize(new Dimension(initColumnWidth, Integer.MAX_VALUE));
        rowHeader.setSize(new Dimension(initColumnWidth, Integer.MAX_VALUE));
        rowHeader.setMinimumSize(new Dimension(initColumnWidth, Integer.MAX_VALUE));
        ...
        else
        {
            switch (data_type)
            {
                ...
                case TangoConst.Tango_DEV_ULONG:
                    if ( ((Integer[][])value).length == 0 || ((Integer[][])value)[0].length == 0 )
                    {
                        return true;
                    }
                    break;
