//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/actions/context/SearchContextsAction.java,v $
//
...
/**
 * Looks up contexts according to the filtering criterions filled in the search popup.
 * <UL>
 * <LI> Gets the SearchContextsInDBDialog instance where the action was called from
 * <LI> If the action is called from the reset button, resets the criterions fields and returns
...
{
	private SearchContextsInDBDialog dialogFrom;
	 ...
	 * @param _dialogFrom The dialog where the user filled the filtering criterions
	 */
	public SearchContextsAction(String name , SearchContextsInDBDialog _dialogFrom)
	{
	...
	{
		SearchContextsInDBDialog source = SearchContextsInDBDialog.getInstance();
		JButton buttonFrom = ( JButton ) actionEvent.getSource();
		...
		Criterions searchCriterions = this.getContextsSearchCriterions();
		...
		try
		{
			...
			String msg = Messages.getLogMessage("LOAD_CONTEXTS_ACTION_OK");
			logger.trace(ILogger.LEVEL_DEBUG , msg);
		}
		catch ( Exception e )
		{
			String msg = Messages.getLogMessage("LOAD_CONTEXTS_ACTION_KO");
			logger.trace(ILogger.LEVEL_ERROR , msg);
			logger.trace(ILogger.LEVEL_ERROR , e);
			return;
		}

		ContextListTable tableToUpdate = ContextListTable.getInstance();
		ContextListTableModel modelToUpdate = ( ContextListTableModel ) tableToUpdate.getModel();
	...
	private Criterions getContextsSearchCriterions()
	{
		...
		{
			String msg = "ERROR";
			String title = Messages.getLogMessage("SEARCH_CONTEXTS_FIELD_ERROR");
		...
		cond = getCondition(source.selectId.getSelectedItem() , source.textId.getText() , GlobalConst.TAB_CONTEXT[ 0 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectStartTime.getSelectedItem() , source.textStartTime.getText() , GlobalConst.TAB_CONTEXT[ 1 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectEndTime.getSelectedItem() , source.textEndTime.getText() , GlobalConst.TAB_CONTEXT[ 1 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectName.getSelectedItem() , source.textName.getText() , GlobalConst.TAB_CONTEXT[ 2 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectAuthor.getSelectedItem() , source.textAuthor.getText() , GlobalConst.TAB_CONTEXT[ 3 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectReason.getSelectedItem() , source.textReason.getText() , GlobalConst.TAB_CONTEXT[ 4 ]);
		ret.addCondition(cond);

		cond = getCondition(source.selectDescription.getSelectedItem() , source.textDescription.getText() , GlobalConst.TAB_CONTEXT[ 5 ]);
		ret.addCondition(cond);
	 ...
	 */
	private void verifyFields(SearchContextsInDBDialog source) throws FieldFormatException
	{
		...
		{
			BensikinDate.stringToTimestamp(startTime , false , FieldFormatException.SEARCH_CONTEXTS_START_TIME);
		}

		String endTime = source.textEndTime.getText();
		if ( endTime != null && !endTime.trim().equals("") )
		{
			BensikinDate.stringToTimestamp(startTime , false , FieldFormatException.SEARCH_CONTEXTS_END_TIME);
		}
	}
