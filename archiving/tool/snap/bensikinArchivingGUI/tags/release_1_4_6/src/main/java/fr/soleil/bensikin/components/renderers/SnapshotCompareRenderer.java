//+======================================================================
//$Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/components/renderers/SnapshotCompareRenderer.java,v $
//
...
//Author: XP
//Mantis bug ID: 5629
//Comment : In the getTableCellRendererComponent method, one apply the renderer of the header of the table to the first row if the first row contains the header (before printing).
//
...
/**
 * A cell renderer used for SnapshotCompareTable. It does the same thing as its father class SnapshotDetailRenderer
 * but for one difference, in how it displays the first row and column as if they were table headers.
 * (SnapshotCompareTable's first line is filled with columns title to go around a printing bug)
 *
 * @author CLAISSE
 */
public class SnapshotCompareRenderer extends SnapshotDetailRenderer
{
    ...
    public Component getTableCellRendererComponent ( JTable table , Object value , boolean isSelected , boolean hasFocus , int row , int column )
    {
        ...
        if ( row == 0 && SnapshotCompareTablePrintModel.getInstance().isTitlesDisplayed() )
        {
            TableCellRenderer renderer = table.getTableHeader().getDefaultRenderer();
            ret = renderer.getTableCellRendererComponent( table , value , isSelected , hasFocus , row , column );
        ...
        { 
            ...
            SnapshotComparison snapshotComparison = tableModel.getSnapshotComparisonAtRow ( row );
            ...
            if ( column == 0 )
	        {
	            switch ( comparisonType )
                {
                	case SnapshotComparison.ATTR_BELONGS_TO_SN1_ONLY :
                	    ret.setBackground( SnapshotComparison.getFirstSnapshotColor () );
                	break;
                	
                	case SnapshotComparison.ATTR_BELONGS_TO_SN2_ONLY :
                	    ret.setBackground( SnapshotComparison.getSecondSnapshotColor () );
                	break;
                	
                	case SnapshotComparison.ATTR_BELONGS_TO_BOTH :
                	    ret.setBackground( SnapshotComparison.getDiffSnapshotColor () );
                	break;
