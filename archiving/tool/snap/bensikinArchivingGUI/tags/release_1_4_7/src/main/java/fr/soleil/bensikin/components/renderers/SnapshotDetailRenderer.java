//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/components/renderers/SnapshotDetailRenderer.java,v $
//
	    ...
	    if ( value instanceof SnapshotAttributeValue )
		{
			...
			switch ( dataFormat )
			{
				case SnapshotAttributeValue.SCALAR_DATA_FORMAT:
                    ...
                    {
                        ret = new JLabel( Messages.getMessage("SNAPSHOT_DETAIL_NO_DATA") );
                    ...
                    else
                    {
                        ret = new BensikinTableCellRenderer()
                                .getTableCellRendererComponent( table,
                                        ( (SnapshotAttributeValue) value ).toFormatedString(),
                                        isSelected, hasFocus, row, column );
                    }
				...
				case SnapshotAttributeValue.SPECTRUM_DATA_FORMAT:
			        ...
			        else if ( table.getColumnName(column).equals(Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_W")) ) 
			        {
			            name += Messages.getMessage("DIALOGS_SPECTRUM_ATTRIBUTE_WRITE");
			        }
			        else if ( table.getColumnName(column).equals(Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_DELTA")) ) 
			        {
			        ...
			        {
			            button = new SpectrumWriteValueButton(table.getValueAt(row,0).toString() + " " + name, (SnapshotAttributeValue)value, table, row, column);
			        }
			        else if (value instanceof SnapshotAttributeReadValue)
			        {
				        button = new SpectrumButton( table.getValueAt(row,0).toString() + " " + name, (SnapshotAttributeValue)value );
			        }
			        else if (value instanceof SnapshotAttributeDeltaValue)
			        {
				        button = new SpectrumDeltaValueButton(table.getValueAt(row,0).toString() + " " + name, (SnapshotAttributeValue)value, table, row, column);
			        }
				...
				case SnapshotAttributeValue.IMAGE_DATA_FORMAT:
                    ...
                    if (value instanceof SnapshotAttributeWriteValue)
                    {
                        imageButton = new ImageWriteButton( table, row, table.getModel().getValueAt(row,0).toString(), ((SnapshotAttributeWriteValue)value).getImageValue(),((SnapshotAttributeWriteValue)value).getDataType(), ((SnapshotAttributeWriteValue)value).getDisplayFormat() );
                    ...
                    {
                        imageButton = new ImageButton( table.getModel().getValueAt(row,0).toString(), ((SnapshotAttributeReadValue)value).getImageValue(),((SnapshotAttributeReadValue)value).getDataType(), ((SnapshotAttributeReadValue)value).getDisplayFormat() );
                        Border border2 = new JLabel().getBorder();
                        if (isSelected) 
                        {
                            imageButton.setOpaque(true);
