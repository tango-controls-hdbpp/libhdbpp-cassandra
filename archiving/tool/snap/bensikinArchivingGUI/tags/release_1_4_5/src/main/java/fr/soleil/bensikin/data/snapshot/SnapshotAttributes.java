//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/data/snapshot/SnapshotAttributes.java,v $
//
...
public class SnapshotAttributes
{
    ...
    /**
     * Initializes both the list of snapshot attributes and the reference to the mother Snapshot.
     *
     * @param _snapshotAttributes The list of attributes
     * @param _snapshot           The mother Snapshot
     */
    public SnapshotAttributes ( SnapshotAttribute[] _snapshotAttributes , Snapshot _snapshot )
    {
    ...
    Hashtable intersectWith ( SnapshotAttributes this2 )
    {
        ...
        ret.put ( new Integer ( SnapshotComparison.ATTR_BELONGS_TO_SN1_ONLY ) ,  c1Only );
        ret.put ( new Integer ( SnapshotComparison.ATTR_BELONGS_TO_SN2_ONLY ) ,  c2Only );
        ret.put ( new Integer ( SnapshotComparison.ATTR_BELONGS_TO_BOTH ) ,  cBoth );
        
        
        /*int size1Only = size1 - intersectionSize;
        int size2Only = size2 - intersectionSize;*/
        
        
/*
    These algorithms test some aspect of the composition of one or more collections:
 ...
 */
    ...
    ArrayList toArrayList()
    {
        ...
        for (int i = 0; i < nbOfAttributes; i++)
        {
            ...
            {
                Object[] stupidVal = (Object[]) transformedAttr.getValue();
                ...
                for (int j = 0; j < stupidVal.length; j++) {
                    if (stupidVal[j] == null) {
                        System.out.println(transformedAttr
                                .getAttribute_complete_name()
                                + "a une value null !!!! : c'est val["
                                + j
                                + "]");
                    }
                    else {
                        System.out.println(transformedAttr
                                .getAttribute_complete_name()
                                + "a une value non null *** : c'est val["
                                + j
                                + "]");
                    }
                }
     ...
     *
     * @param snapshotAttributes2 The attributes to compare to
     * @return True if both groupss have the same attributes (including a null list)
     */
    boolean hasSameAttributesAs ( SnapshotAttributes snapshotAttributes2 )
    {
        SnapshotAttribute[] snapshotAttributesTab1 = this.getSnapshotAttributes();
        ...
        {
            String completeName = snapshotAttributesTab1[ i ].getAttribute_complete_name();
            v1.add( completeName );
        }

        for ( int i = 0 ; i < snapshotAttributesTab2.length ; i++ )
        {
            String completeName = snapshotAttributesTab2[ i ].getAttribute_complete_name();
    ...
    /**
     * Sets the modified state of the attributes group, by calling setModified on each attribute
     *
     * @param b True if the attributes are to be marked modified
     */
