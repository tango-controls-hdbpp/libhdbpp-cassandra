...
/**
 * A generic type for all sub-menus of the "Favorites" menu.
 * Implements recursive methods to add:
 * <UL>
 * <LI> A sub-menu
...
public class FavoritesSubMenu
{
	...
	/**
	 * A recursive (ie. containing other TreeMaps) list of favorites directories. A TreeMap is used for automatic alphabetic ordering.
	 */
	...
	/**
	 * Builds a sub-menu of type <code>_type</code> and name <code>_menuName</code>
	 *
	 * @param _menuName The name as it will appear in the favorites menu.
	 * @param _type     The type of sub-menu
	 * @throws IllegalArgumentException If type is not in (CONTEXT_TYPE, SNAPSHOT_TYPE, CONFIGURATION_TYPE)
	 */
	 ...
	 *
	 * @param favoritesSubMenuElements2 The current zoom level in the sub-menu tree
	 * @return An XML representation of the current zoom level in the sub-menu tree
	 */
	 ...
	 *
	 * @param _favoritesSubMenuElements The recursive map to build from
	 */
	 ...
	 *
	 * @param menuIn                    The current level of sub-menu
	 * @param favoritesSubMenuElements2 The current zoom level in the sub-menu tree
		...
		while ( it.hasNext() )
		{
			...
			else if ( nextVal instanceof XMLLine )
			{
				...
				switch ( this.type )
				{
					...
					case SNAPSHOT_TYPE:
						//ret.add ( new JMenuItem ( new SelectFavoriteSnapshotAction ( itemName ) ) );
						break;

					case CONFIGURATION_TYPE:
						//ret.add ( new JMenuItem ( new SelectFavoriteConfigurationAction ( itemName ) ) );
						break;
				}
			}
		}
	 ...
	 *
	 * @param _favoritesSubMenuElements The recursive TreeMap describing the structure
	 */
	 ...
	 *
	 * @param nodeIn                    The current level of tree
	 * @param favoritesSubMenuElements2 The current zoom level in the sub-menu tree
	...
	/**
	 * Builds both the associated JMenu and Tree from the given recursive TreeMap
	 *
	 * @param contextsBook The recursive TreeMap describing the structure
	 */
	...
	/**
	 * A recursive method used by the addContext, addXXX .. methods of daughter classes
	 * to add an object (context,XXX) reference to the existing structure.
	 *
	 * @param in   The TreeMap before addition of the new object reference
	 * @param path The path in the recursive TreeMap where this object reference will be added
	 * @param id   The id to use for this object reference
	...
	/**
	 * A recursive method used by addDirectory to add a menu file to the existing structure.
	 *
	 * @param in                 The TreeMap before addition of the new sub directory
	 * @param path               The path in the recursive TreeMap where this sub directory will be added
	 * @param nameOfNewDirectory The name of the new sub directory
    ...
    public void removeItem ( TreePath selectedTreePath ) 
    {
		...
		else
		{
			...
			{
				DefaultMutableTreeNode currentPathElement = ( DefaultMutableTreeNode ) path_o[ i + 1 ];
				//System.out.println ( "CLA/currentPathElement/"+currentPathElement.getUserObject().getClass().toString() );
				...
				//System.out.println ( "CLA/currentPathElementName/"+currentPathElementName );
				path[ i ] = currentPathElementName;
			}

			TreeMap currentFavoritesSubMenuElements = this.favoritesSubMenuElements;
			/*System.out.println ( "CLA/currentFavoritesSubMenuElements-------------------------------" );
			trace ( currentFavoritesSubMenuElements );
			System.out.println ( "CLA/currentFavoritesSubMenuElements-------------------------------" );
			*/
