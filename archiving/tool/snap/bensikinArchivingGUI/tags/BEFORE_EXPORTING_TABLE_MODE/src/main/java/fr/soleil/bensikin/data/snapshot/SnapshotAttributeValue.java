//+======================================================================
// $Source: /cvsroot/tango-cs/tango/tools/bensikin/bensikin/data/snapshot/SnapshotAttributeValue.java,v $
//
...
/**
 * All snapshot attributes values, be it read write or delta, extend this class that implements basic
 * cast operations, value tests, and String representation.
 *
 * @author CLAISSE
 */
public class SnapshotAttributeValue
{
	...
	/**
	 * The data format used for "not applicable" values, ie. values that are meaningless (such as the delta value of a read-only attribute)
	 */
	...
	/**
	 * True if the value is a "not applicable" value, ie. a value that is meaningless (such as the delta value of a read-only attribute)
	 */
	...
	public String toFormatedString()
	{
		...
		switch ( this.dataFormat )
		{
			case SCALAR_DATA_FORMAT:
				if ( scalarValue != null )
				{
					...
					else
					{
						try
						{
							formatedValue = Format.sprintf(display_format , valTab);
							if ( formatedValue == null
							     || "".equals(formatedValue.trim()) || (scalarValue instanceof Double && scalarValue.toString().contains("E")))
			...
			case IMAGE_DATA_FORMAT:
				/*if ( imageValue != null )
				{
					int dimX = imageValue.length;
					for ( int i = 0 ; i < dimX ; i++ )
					{
						Object[] y = imageValue[ i ];
						int dimY = y.length;
    ...
    public void appendToString(StringBuffer buffer) {
        if (buffer != null) {
            ...
            switch ( this.dataFormat )
            {
                ...
                case SPECTRUM_DATA_FORMAT:
                    if ( spectrumValue != null && spectrumValue.length > 0 && !"NaN".equals(spectrumValue[0]) && !"[NaN]".equals(spectrumValue[0]) )
                    {
                        int dimX = 0;
                        switch ( dataType )
                        {
		...
		switch ( dataFormat )
		{
			...
			case SPECTRUM_DATA_FORMAT:
                if ( value instanceof String )
                {
                    ...
                    else this.spectrumValue = new Object[]{value};
                }
				break;
			...
			case IMAGE_DATA_FORMAT:
                ...
                else this.setImageValue(new Object[][]{{value}});
				break;
	 ...
	 * @throws IllegalArgumentException if dataFormat is not in (SCALAR_DATA_FORMAT,SPECTRUM_DATA_FORMAT,IMAGE_DATA_FORMAT,NOT_APPLICABLE_DATA_FORMAT)
	 */
	public SnapshotAttributeValue(int _dataFormat , int _dataType , Object _value) throws IllegalArgumentException
	{
	...
	{
        if ( imageValue != null && !"NaN".equals(imageValue[0][0]) && !"[NaN]".equals(imageValue[0][0]) )
        {
            this.imageValue = imageValue;
        }
        else this.imageValue = null;
	}
	...
	{
        if ( spectrumValue != null && !"NaN".equals(spectrumValue[0]) && !"[NaN]".equals(spectrumValue[0]) )
        {
            this.spectrumValue = spectrumValue;
        }
        else this.spectrumValue = null;
	}
