...
public class SearchContextsAction extends BensikinAction {
     ...
     */
    public SearchContextsAction(String name,
            SearchContextsInDBDialog _dialogFrom) {
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        SearchContextsInDBDialog source = SearchContextsInDBDialog
                .getInstance();
        ...
        ContextListTableModel modelToUpdate = (ContextListTableModel) tableToUpdate
                .getModel();
    ...
    private Criterions getContextsSearchCriterions() {
        ...
        try {
            ...
            String title = Messages
                    .getLogMessage("SEARCH_CONTEXTS_FIELD_ERROR");

            switch (e.getCode()) {
                case FieldFormatException.SEARCH_CONTEXTS_ID:
                    msg = Messages
                            .getLogMessage("SEARCH_CONTEXTS_FIELD_ERROR_ID");
                    break;

                case FieldFormatException.SEARCH_CONTEXTS_START_TIME:
                    msg = Messages
                            .getLogMessage("SEARCH_CONTEXTS_FIELD_ERROR_START_TIME");
                    break;

                case FieldFormatException.SEARCH_CONTEXTS_END_TIME:
                    msg = Messages
                            .getLogMessage("SEARCH_CONTEXTS_FIELD_ERROR_END_TIME");
                    break;
            }
        ...
        cond = getCondition(source.selectId.getSelectedItem(), source.textId
                .getText(), GlobalConst.TAB_CONTEXT[0]);
        ret.addCondition(cond);

        cond = getCondition(source.selectStartTime.getSelectedItem(),
                source.textStartTime.getText(), GlobalConst.TAB_CONTEXT[1]);
        ret.addCondition(cond);

        cond = getCondition(source.selectEndTime.getSelectedItem(),
                source.textEndTime.getText(), GlobalConst.TAB_CONTEXT[1]);
        ret.addCondition(cond);

        cond = getCondition(source.selectName.getSelectedItem(),
                source.textName.getText(), GlobalConst.TAB_CONTEXT[2]);
        ret.addCondition(cond);

        cond = getCondition(source.selectAuthor.getSelectedItem(),
                source.textAuthor.getText(), GlobalConst.TAB_CONTEXT[3]);
        ret.addCondition(cond);

        cond = getCondition(source.selectReason.getSelectedItem(),
                source.textReason.getText(), GlobalConst.TAB_CONTEXT[4]);
        ret.addCondition(cond);

        cond = getCondition(source.selectDescription.getSelectedItem(),
                source.textDescription.getText(), GlobalConst.TAB_CONTEXT[5]);
        ret.addCondition(cond);
     ...
     */
    private void verifyFields(SearchContextsInDBDialog source)
        ...
        if (id != null && !id.trim().equals("")) {
            try {
                ...
                throw new FieldFormatException(
                        FieldFormatException.SEARCH_CONTEXTS_ID);
            }
        }

        String startTime = source.textStartTime.getText();
        if (startTime != null && !startTime.trim().equals("")) {
            BensikinDate.stringToTimestamp(startTime, false,
     ...
     */
    public Condition getCondition(Object selectedItem, String text,
            String id_field_key2) {
        String _selectedItem = (String) selectedItem;
        String _text = (String) text;
        boolean isACriterion = true;

        if (selectedItem == null
                || _selectedItem.equals(OperatorsList.NO_SELECTION)) {
            isACriterion = false;
        }
