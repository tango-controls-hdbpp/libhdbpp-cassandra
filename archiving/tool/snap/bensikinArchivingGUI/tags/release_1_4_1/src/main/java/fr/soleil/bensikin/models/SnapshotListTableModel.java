...
/**
 * The table model used by SnapshotListTable, this model lists the current list of snapshots.
 * Its rows are SnapshotData objects.
 * A singleton class.
 *
 * @author CLAISSE
 */
public class SnapshotListTableModel extends DefaultTableModel
{
	...
	public void removeRows(int[] indexesToRemove)
	{
		...
		SnapshotData[] newRows = new SnapshotData[ rows.length - numberOfLinesToRemove ];

		Vector idsToRemoveList = new Vector(numberOfLinesToRemove);
		for ( int i = 0 ; i < numberOfLinesToRemove ; i++ )
		{
			int idOfLineToRemove = this.getSnapshotDataAtRow(indexesToRemove[ i ]).getId();
		...
		{
			Integer idOfCurrentLine = new Integer(this.getSnapshotDataAtRow(i).getId());
	...
	/**
	 * Initializes the columns titles, and adds a SnapshotTableModelListener to itself.
	 */
	private SnapshotListTableModel()
	{
		...
		String msgComment = Messages.getMessage("SNAPSHOT_LIST_COLUMNS_COMMENT");
	 ...
	 *
	 * @param snapshots The model's new list of snapshots
	 */
	...
	/**
	 * Adds a line at the end of the current model for the specified snapshot, and refreshes the list.
	 *
	 * @param snapshot The snapshot to add to the current list
	 */
	 ...
	 * Changes the current model's list of snapshots, and refreshes the list.
	 *
	 * @param _rows The model's new list of snapshots data
	 */
	 ...
	 *
	 * @param snapshotData The Snapshot to refresh, with its modified comment field
	 */
