...
public class DefaultLogger extends AbstractLogger {
        ...
        try {
            if (o instanceof String) {
                ...
                for (int i = 0; i < stack.length; i++) {
                    GUIUtilities.write2(this.writer, "        at "
                            + stack[i].toString(), true);
                }
            }
            else {
                throw new IllegalArgumentException(
                        "DefaultLogger/addToDiary/other/"
                                + o.getClass().toString() + "/");
            }
        }
        catch (Exception e) {
    ...
    protected void log(String msg) {
        MessagesPanel messagesPanel = MessagesPanel.getCurrentInstance();
        if (messagesPanel == null) {
            ...
            synchronized (messageBuffer) {
                ...
                if (size > 0) {
                    for (int i = 0; i < size; i++) {
                        messagesPanel.getMessagesArea().append(
                                messageBuffer.get(i));
                        messagesPanel.getMessagesArea().append(
                                GUIUtilities.CRLF);
                    }
            ...
            messagesPanel.getMessagesArea().append(msg);
