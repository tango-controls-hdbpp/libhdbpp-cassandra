...
public class Context {
     ...
     */
    public void loadAttributes(Criterions searchCriterions)
            throws SnapshotingException {
        ...
        SnapAttributeHeavy[] sah = source.findContextAttributes(cont,
                searchCriterions);
     ...
     * @throws Exception
     */
    public void loadSnapshots(Criterions searchCriterions)
            throws SnapshotingException {
        searchCriterions.addCondition(new Condition(GlobalConst.TAB_SNAP[1],
                GlobalConst.OP_EQUALS, "" + contextId));
    ...
    public static Context save(SnapContext context) throws Exception {
        ...
        ContextListTableModel modelToRefresh = ContextListTableModel
                .getInstance();
    ...
    private void setContextAttributes(SnapAttributeHeavy[] sah) {
        ...
        for (int i = 0; i < numberOfAttributes; i++) {
            ...
            ContextAttribute currentOutAttribute = new ContextAttribute(
                    _contextAttributes);

            currentOutAttribute.setName(currentInAttribute.getAttribute_name());
            currentOutAttribute.setCompleteName(currentInAttribute
                    .getAttribute_complete_name());
            currentOutAttribute.setDevice(currentInAttribute
    ...
    public SnapContext getAsSnapContext() {
        ...
        if (contextData == null) {
            contextData = ContextDataManager.getInstance().getContextData(
                    contextId);
        }
     ...
     * @throws Exception
     */
    public static ContextData[] loadContexts(Criterions searchCriterions)
            throws Exception {
        ISnapManager source = SnapManagerFactory.getCurrentImpl();
     ...
     */
    public static void setOpenedContexts(
            Hashtable<String, Context> openedContexts) {
     ...
     * @throws Exception
     */
    public static SnapContext fillSnapContext(String name, String author,
            String reason, String description, ContextAttributesTreeModel model) {
        if (!verifyData(name, author, reason, description)) {
            String title = Messages
                    .getMessage("REGISTER_CONTEXT_EMPTY_DATA_TITLE");
            String msg = Messages
                    .getMessage("REGISTER_CONTEXT_EMPTY_DATA_MESSAGE");
        ...
        if (!verifyAttributes(enumer)) {
            String title = Messages
                    .getMessage("REGISTER_CONTEXT_EMPTY_ATTRIBUTES_TITLE");
            String msg = Messages
                    .getMessage("REGISTER_CONTEXT_EMPTY_ATTRIBUTES_MESSAGE");
        ...
        SnapContext context = new SnapContext(author, name, -1, null, reason,
                description);
        ...
        while (enumer.hasNext()) {
            ...
            ContextAttribute ca = (ContextAttribute) model
                    .getAttribute(nextKey_s);
     ...
     * @throws Exception
     */
    public static Context fillContext(String id_s, String creationDate_s,
            String name, String author, String reason, String description,
            ContextAttributesTreeModel model) {
        Iterator<String> enumer = model.getTreeAttributes().iterator();

        int id = (id_s == null || id_s.equals("")) ? -1 : Integer
                .parseInt(id_s);
        Date creationDate = (creationDate_s == null || creationDate_s
                .equals("")) ? null : Date.valueOf(creationDate_s);

        ContextData data = new ContextData(id, creationDate, name, author,
                reason, description);
        ...
        while (enumer.hasNext()) {
            ...
            ContextAttribute nextContextAttribute = new ContextAttribute(
                    attributes);
     ...
     */
    private static boolean verifyData(String name, String author,
    ...
    public String toString() {
        ...
        XMLLine openingLine = new XMLLine(Context.XML_TAG,
                XMLLine.OPENING_TAG_CATEGORY);
        XMLLine closingLine = new XMLLine(Context.XML_TAG,
                XMLLine.CLOSING_TAG_CATEGORY);
        if (this.contextData == null) {
            this.contextData = ContextDataManager.getInstance().getContextData(
                    this.contextId);
        }
        ...
        openingLine.setAttribute(ContextData.ID_PROPERTY_XML_TAG, String
                .valueOf(id));
        openingLine.setAttribute(ContextData.IS_MODIFIED_PROPERTY_XML_TAG,
                isModified);
        if (creationDate != null) {
            openingLine.setAttribute(
                    ContextData.CREATION_DATE_PROPERTY_XML_TAG, creationDate
                            .toString());
        }
        if (author != null) {
            openingLine.setAttribute(ContextData.AUTHOR_PROPERTY_XML_TAG,
                    author);
        }
        if (description != null) {
            openingLine.setAttribute(
                    ContextData.DESCRIPTION_DATE_PROPERTY_XML_TAG, description);
        }
        if (name != null) {
            openingLine.setAttribute(ContextData.NAME_PROPERTY_XML_TAG, name);
        }
        if (reason != null) {
            openingLine.setAttribute(ContextData.REASON_PROPERTY_XML_TAG,
                    reason);
        }
        if (path != null) {
    ...
    public static Context buildContextToBeSaved() {
        ...
        String creationDate_s = contextDataPanel.getCreationDateField()
                .getText();
        ...
        Context ret = Context.fillContext(id_s, creationDate_s, name, author,
                reason, description, model);

        // System.out.println (
        // "Context/buildContextToBeSaved IN 3/"+Context.getSelectedContext().getPath()+"/"
        // );
        if (Context.getSelectedContext() != null) {
            Context.getSelectedContext().setContextAttributes(
                    ret.getContextAttributes());
            ret.getContextData()
                    .setPath(Context.getSelectedContext().getPath());
    ...
    public void save(IContextManager manager, boolean saveAs) {
        ...
        try {
            ...
            ContextData newData = new ContextData(this.contextData.getId(),
                    this.contextData.getCreation_date(), this.contextData
                            .getName(), this.contextData.getAuthor_name(),
                    this.contextData.getReason(), this.contextData
                            .getDescription());
