...
public abstract class AbsRefreshAction extends BensikinAction {
     ...
     */
    private void verifyFields(SnapshotFilterPanel source)
            throws FieldFormatException {
        String id = source.getTextId().getText();
        if (id != null && !id.trim().equals("")) {
            try {
                ...
                throw new FieldFormatException(
                        FieldFormatException.FILTER_SNAPSHOTS_ID);
            }
        }

        String startTime = source.getTextStartTime().getText();
        if (startTime != null && !startTime.trim().equals("")) {
            BensikinDate.stringToTimestamp(startTime, true,
                    FieldFormatException.FILTER_SNAPSHOTS_START_TIME);
        }

        String endTime = source.getTextEndTime().getText();
        if (endTime != null && !endTime.trim().equals("")) {
            BensikinDate.stringToTimestamp(endTime, true,
                    FieldFormatException.FILTER_SNAPSHOTS_START_TIME);
        }
    ...
    private Criterions getSnapshotsSearchCriterions() {
        ...
        try {
            ...
            String title = Messages
                    .getLogMessage("FILTER_SNAPSHOTS_FIELD_ERROR");

            switch (e.getCode()) {
                case FieldFormatException.FILTER_SNAPSHOTS_ID:
                    msg = Messages
                            .getLogMessage("FILTER_SNAPSHOTS_FIELD_ERROR_ID");
                    break;

                case FieldFormatException.FILTER_SNAPSHOTS_START_TIME:
                    msg = Messages
                            .getLogMessage("FILTER_SNAPSHOTS_FIELD_ERROR_START_TIME");
                    break;

                case FieldFormatException.FILTER_SNAPSHOTS_END_TIME:
                    msg = Messages
                            .getLogMessage("FILTER_SNAPSHOTS_FIELD_ERROR_END_TIME");
                    break;
            }
        ...
        cond = getCondition(source.getSelectId().getSelectedItem(), source
                .getTextId().getText(), GlobalConst.TAB_SNAP[0]);
        ret.addCondition(cond);

        cond = getCondition(source.getSelectStartTime().getSelectedItem(),
                source.getTextStartTime().getText(), GlobalConst.TAB_SNAP[2]);
        ret.addCondition(cond);

        cond = getCondition(source.getSelectEndTime().getSelectedItem(), source
                .getTextEndTime().getText(), GlobalConst.TAB_SNAP[2]);
        ret.addCondition(cond);

        cond = getCondition(source.getSelectComment().getSelectedItem(), source
                .getTextComment().getText(), GlobalConst.TAB_SNAP[3]);
        ret.addCondition(cond);
    ...
    public void refreshSnapList() {
        ...
        SnapshotListTableModel modelToUpdate = (SnapshotListTableModel) SnapshotListTable
        ...
        if (selectedContext != null) {
            try {
                ...
                String msg = Messages
                        .getLogMessage("FILTER_SNAPSHOTS_ACTION_OK");
                logger.trace(ILogger.LEVEL_DEBUG, msg);
            } catch (SnapshotingException e) {
                String msg;
                if (e.computeIsDueToATimeOut()) {
                    msg = Messages
                            .getLogMessage("FILTER_SNAPSHOTS_ACTION_TIMEOUT");
                } else {
                ...
                String msg = Messages
                        .getLogMessage("FILTER_SNAPSHOTS_ACTION_KO");
                logger.trace(ILogger.LEVEL_ERROR, msg);
                logger.trace(ILogger.LEVEL_ERROR, e);
            }
        }
