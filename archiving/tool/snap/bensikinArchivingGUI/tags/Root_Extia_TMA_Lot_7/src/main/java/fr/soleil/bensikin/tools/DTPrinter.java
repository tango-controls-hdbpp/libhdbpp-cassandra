...
public class DTPrinter {
			...
			try {
				...
				if (fitToPage) {
					...
					if (ratioW > ratioH) {
						...
						double cH = nResolution / 72.0 * (double) hp
								- (double) h;
						...
						double cW = nResolution / 72.0 * (double) wp
								- (double) w;
	 ...
	 * component.setContentType ("text/xml"); component.setText (content); int h
	 * = component.getPreferredSize().height; component.setSize ( 500 , h );
	 * ATKGraphicsUtils.printComponent(component,title,false,printerResolution);
	 * }
	 */
	/**
	 * @param title
	 *            The title of the document to print.
	 * @param content
	 *            The content of the document to print.
	 */
	 ...
	 * @param printerResolution
	 */
	private static void printComponent(Vector contentParts, String title,
	 ...
	 *            Printer resolution when fitToPage is not enabled.
	 */
	public static void printComponent(JComponent comp, String title,
			boolean fitToPage, int printerResolution) {
		PageAttributes pa = new PageAttributes();
		JobAttributes ja = new JobAttributes();
		pa.setPrintQuality(PageAttributes.PrintQualityType.HIGH);
		...
		if (parent instanceof Frame) {
			printJob = java.awt.Toolkit.getDefaultToolkit().getPrintJob(
					(Frame) parent, title, ja, pa);
		} else {
			Frame dummy = new Frame();
			printJob = java.awt.Toolkit.getDefaultToolkit().getPrintJob(dummy,
					title, ja, pa);
		}
