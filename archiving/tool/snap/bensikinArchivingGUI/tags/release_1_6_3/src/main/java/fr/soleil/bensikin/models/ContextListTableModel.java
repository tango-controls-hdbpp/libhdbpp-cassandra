...
public class ContextListTableModel extends DefaultTableModel {
	...
	private ContextListTableModel() {
		...
		String msgDescription = Messages
				.getMessage("CONTEXT_LIST_COLUMNS_DESCRIPTION");
	...
	public void removeRows(int[] indexesToRemove) {
		...
		for (int i = 0; i < numberOfLinesToRemove; i++) {
			int idOfLineToRemove = this.getContextAtRow(indexesToRemove[i])
					.getId();
		...
		for (int i = 0; i < rows.length; i++) {
			Integer idOfCurrentLine = new Integer(this.getContextAtRow(i)
					.getId());
