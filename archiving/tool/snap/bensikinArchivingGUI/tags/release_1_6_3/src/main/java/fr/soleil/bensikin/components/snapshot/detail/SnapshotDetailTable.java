...
public class SnapshotDetailTable extends JXTable {
    ...
    private static final long serialVersionUID = -3313090567413872748L;
    ...
    public void applyChange() {
        ...
        SnapshotDetailEditor editor = (SnapshotDetailEditor) this
                .getCellEditor();
        if (this.isEditing()) {
            model.setValueAt(editor.getCellEditorValue(), this.getEditingRow(),
    ...
    public String snapshotToCsvString() {
        ...
        if (snapshot != null) {
            outputBuffer = snapshot
                    .snapshotInfoToUserFriendlyStringBuffer(outputBuffer);
            if (outputBuffer.length() > 0) {
                outputBuffer.append('\n').append('\n');
            }
            outputBuffer = snapshotAttributesToStringBuffer(outputBuffer);
        }
        return outputBuffer.toString();
    }

    private StringBuffer snapshotAttributesToStringBuffer(StringBuffer buffer) {
        if (buffer == null) {
        ...
        for (int row = 0; row < getRowCount(); row++) {
            ...
            for (int col = 0; col < getColumnCount(); col++) {
                ...
                if (columnMap.get(col)) {
                    ...
                    if (value instanceof SnapshotAttributeValue) {
                        buffer.append(SnapshotDetailRenderer
                                .getTextFor((SnapshotAttributeValue) value));
                    } else {
                        buffer.append(value);
                    }
                    value = null;
    ...
    private HashMap<Integer, Boolean> computeExportableColumnsMap() {
        ...
        for (int i = 0; i < getColumnCount(); i++) {
            if (Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_NAME").equals(
                    getColumnName(i))) {
                columnMap.put(i, true);
            } else if (Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_R").equals(
                    getColumnName(i))) {
                columnMap.put(i, Options.getInstance().getSnapshotOptions()
                        .isCSVRead());
            } else if (Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_W").equals(
                    getColumnName(i))) {
                columnMap.put(i, Options.getInstance().getSnapshotOptions()
                        .isCSVWrite());
            } else if (Messages.getMessage("SNAPSHOT_DETAIL_COLUMNS_DELTA")
                    .equals(getColumnName(i))) {
                columnMap.put(i, Options.getInstance().getSnapshotOptions()
                        .isCSVDelta());
            } else {
