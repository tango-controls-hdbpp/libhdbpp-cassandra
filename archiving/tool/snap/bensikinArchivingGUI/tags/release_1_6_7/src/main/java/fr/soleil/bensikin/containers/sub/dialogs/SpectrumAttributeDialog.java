...
public class SpectrumAttributeDialog extends JDialog {
    ...
    public SpectrumAttributeDialog(String name, int row, int column,
        ...
        if ((value.getDataType() == AttrData.Tango_DEV_CHAR)
            ...
            if (chart != null) {
                chart.clearDAO();
                chart.removeKey(BensikinDAOFactory.class.getName());
                if (chart.getComponent() != null) {
                    chart.getComponent().resetAll();
                }
                chart = null;
            }
            ...
            if (key == null) {
                ...
                chart.addKey(BensikinDAOFactory.class.getName(), key);
            }
            chart.switchDAOFactory(BensikinDAOFactory.class.getName());
            if (chart.getData().isEmpty()) {
                JLabel label = new JLabel(Messages.getMessage("DIALOGS_SPECTRUM_ATTRIBUTE_NAN"),
                        JLabel.CENTER);
                ...
                component.setDataViewMarkerCometeColor(dataName, CometeColor.RED);
                component.setDataViewLineStyle(dataName, 1);
                component.setDataViewMarkerSize(dataName, 5);
                component.setDataViewMarkerStyle(dataName, IChartViewer.MARKER_DIAMOND);
                component.setDataViewLineStyle(dataName, IChartViewer.STYLE_SOLID);

                component.setAutoScale(true, IChartViewer.Y1);
                component.setAutoScale(true, IChartViewer.X);

                component.setAnnotation(IChartViewer.VALUE_ANNO, IChartViewer.X);
                ...
                component.setLabelVisible(false);
                component.setCometeBackground(AwtColorTool.getCometeColor(GUIUtilities
                        .getSnapshotColor()));
                ...
                case AttrData.Tango_DEV_STATE:
                    ...
                    if (!"Nan".equals(valsta) && valsta != null && valsta instanceof Integer[]) {
                        ...
                        if (stateArray.length > 0) {
                            spectrumTable = new JTable(new SpectrumReadValueTableModel(value
        ...
        this.setSize(dim);
        this.setLocation((BensikinFrame.getInstance().getX() + BensikinFrame.getInstance()
                .getWidth())
                - (this.getWidth() + 50), (BensikinFrame.getInstance().getY() + BensikinFrame
