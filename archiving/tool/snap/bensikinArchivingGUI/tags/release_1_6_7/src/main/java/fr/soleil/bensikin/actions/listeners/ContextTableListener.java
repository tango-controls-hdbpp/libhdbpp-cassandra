// +======================================================================
// $Source:
// /cvsroot/tango-cs/tango/tools/bensikin/bensikin/actions/listeners/ContextTableListener.java,v
// $
//
...
// ();
// not to wipe out everything snapshot-related so that the user can compare
// snapshots
...
/**
 * Listens to double clicks on the contexts list table body. When a row is
 * clicked, the selected context becomes the current context.
 * <UL>
 ...
 * @author CLAISSE
 */
public class ContextTableListener extends MouseAdapter implements
        ActionListener {
    ...
    public ContextTableListener() {
        ...
        selectItem = new JMenuItem(Messages
                .getMessage("CONTEXT_LIST_VIEW_DETAILS"));
    ...
    public void mousePressed(MouseEvent event) {
        if (event.getButton() == MouseEvent.BUTTON1
            ...
            if (event.getButton() == MouseEvent.BUTTON3
                ...
                table.addRowSelectionInterval(table
                        .rowAtPoint(event.getPoint()), table.rowAtPoint(event
                        .getPoint()));
                tableMenu.show(ContextListTable.getInstance(), event.getX(),
                        event.getY());
            }
        }
    }
    ...
    private void selectContext() {
        ...
        ContextListTableModel sourceModel = (ContextListTableModel) source
                .getModel();
        ContextData selectedContextData = sourceModel.getContextAtRow(row);

        Context selectedContext = new Context(selectedContextData.getId());
    ...
    public static void transferFilter() {
        ...
        String dayStartString = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                .format(today.getTime());
        today = null;
        // starting filter
        SnapshotFilterPanel.getInstance().resetFields();
        SnapshotFilterPanel.getInstance().getSelectStartTime()
                .setSelectedIndex(1);
        SnapshotFilterPanel.getInstance().getTextStartTime().setText(
                dayStartString);
        dayStartString = null;
