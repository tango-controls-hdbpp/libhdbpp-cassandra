		...
		if (columnIndex == 0) {
			...
			if (values != null && !"NaN".equals(values)) {
				...
				case TangoConst.Tango_DEV_STATE:
					value = new SnapshotAttributeValue(
							SnapshotAttributeValue.SCALAR_DATA_FORMAT, type,
							((Integer[]) values)[rowIndex]);
					return value;
				case TangoConst.Tango_DEV_STRING:
					value = new SnapshotAttributeValue(
							SnapshotAttributeValue.SCALAR_DATA_FORMAT, type,
							new String(((String[]) values)[rowIndex]));
					return value;
				case TangoConst.Tango_DEV_BOOLEAN:
					value = new SnapshotAttributeValue(
							SnapshotAttributeValue.SCALAR_DATA_FORMAT, type,
							((Boolean[]) values)[rowIndex]);
					return value;
