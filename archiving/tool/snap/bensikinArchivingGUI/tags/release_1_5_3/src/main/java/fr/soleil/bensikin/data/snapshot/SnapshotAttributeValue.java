		...
		case SCALAR_DATA_FORMAT:
			if (scalarValue != null) {
				...
				if (scalarValue instanceof String) {
					ret += StringFormater.formatStringToRead(new String(
							(String) scalarValue));
					...
					try {
						formatedValue = String.format(Locale.US,
								display_format, valTab);
						if (formatedValue == null
								|| "".equals(formatedValue.trim())
								|| (scalarValue instanceof Double && scalarValue
										.toString().contains("E")))
			...
			case SPECTRUM_DATA_FORMAT:
				if (spectrumValue != null && spectrumValue.length > 0
					...
					case TangoConst.Tango_DEV_STRING:
						...
						dimX = stval.length;
						for (int i = 0; i < dimX; i++) {
							buffer.append(StringFormater
									.formatStringToRead(new String(stval[i])));
							if (i < dimX - 1) {
					...
					case TangoConst.Tango_DEV_SHORT:
						...
						for (int i = 0; i < length; i++) {
							...
							if ("".equals(token) || "null".equals(token)) {
								...
								shArray[i] = new Short(Double.valueOf(token)
										.shortValue());
							}
							token = null;
						}
					...
					case TangoConst.Tango_DEV_LONG:
						...
						for (int i = 0; i < length; i++) {
							...
							if ("".equals(token) || "null".equals(token)) {
								...
								intArray[i] = new Integer(Double.valueOf(token)
										.intValue());
							}
							token = null;
						}
