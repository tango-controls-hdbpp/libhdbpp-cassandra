...
/**
 * Listens to changes in the content of the selected context's ID field. If the
 * value changes to an empty or null value, all actions (launch snapshot, look
 * up snapshots) on the current context are disabled while the context
 * registering action is enabled. If the value changes to an non empty value,
 * the opposite happens: all actions (launch snapshot, look up snapshots) on the
 * current context are enabled while the context registering action is disabled.
 * 
 * @author CLAISSE
 */
public class SelectedContextListener implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (ID_TEXT_PROPERTY.equals(prop)) {
			LaunchSnapshotAction launchSnapshotAction = LaunchSnapshotAction
					.getInstance();
			FilterSnapshotsAction filterSnapshotsAction = FilterSnapshotsAction
					.getInstance();
			RegisterContextAction registerContextAction = RegisterContextAction
					.getInstance();
			...
			if (id == null || id.trim().equals("")) {
				...
				ContextDataPanel contextDataPanel = ContextDataPanel
						.getInstance();
