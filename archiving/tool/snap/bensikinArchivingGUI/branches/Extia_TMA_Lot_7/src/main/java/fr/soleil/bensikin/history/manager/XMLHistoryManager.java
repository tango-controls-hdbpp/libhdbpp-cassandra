...
public class XMLHistoryManager implements IHistoryManager {
    ...
    public History loadHistory(String historyResourceLocation) throws Exception {
        try {
            ...
            // BEGIN CONTEXTS
            // BEGIN OPEN BOOKS
            Hashtable contextsBook = (Hashtable) historyHt
                    .get(History.CONTEXTS_KEY);
            // END OPEN BOOKS

            // BEGIN OPEN CHAPTERS
            Hashtable selectedContextChapter = (Hashtable) contextsBook
                    .get(History.SELECTED_CONTEXT_KEY);
            Vector openedContextsChapter = (Vector) contextsBook
                    .get(History.OPENED_CONTEXTS_KEY);
            // END OPEN CHAPTERS
            ...
            // BEGIN OPEN PAGES
            OpenedContextRef[] openedContexts = new OpenedContextRef[openedContextsChapter
                    .size()];
            ...
            // BEGIN SNAPSHOTS
            // BEGIN OPEN BOOKS
            Hashtable snapshotsBook = (Hashtable) historyHt
                    .get(History.SNAPSHOTS_KEY);
            // END OPEN BOOKS

            // BEGIN OPEN CHAPTERS
            // Hashtable selectedSnapshotChapter = (Hashtable) snapshotsBook.get
            // ( History.SELECTED_SNAPSHOT_KEY );
            Vector selectedSnapshotsChapter = (Vector) snapshotsBook
                    .get(History.SELECTED_SNAPSHOTS_KEY);
            Vector openedSnapshotsChapter = (Vector) snapshotsBook
                    .get(History.OPENED_SNAPSHOTS_KEY);
            // END OPEN CHAPTERS

            // BEGIN OPEN PAGES
            OpenedSnapshotRef[] openedSnapshots = new OpenedSnapshotRef[openedSnapshotsChapter
                    .size()];
            ...
            SelectedSnapshotRef[] selectedSnapshots = new SelectedSnapshotRef[selectedSnapshotsChapter
                    .size()];
            ...
            while (enum2.hasMoreElements()) {
                ...
                // We insert a special attribute for the tag referencing the
                // focused tab
                SelectedSnapshotRef selectedsnapshot;
                String highlightedState = (String) nextPage
                        .get(History.HIGHLIGHTED_KEY);
                if (highlightedState != null) {
                    boolean isHighlight = Boolean
                            .parseBoolean(highlightedState);
     ...
     * @return A Hashtable which keys are History.CONTEXTS_KEY and
     *         History.SNAPSHOTS_KEY, and which values are respectively
     *         Context/Snapshot Hashtables
     * @throws Exception
     */
    ...
    /**
     * Loads the Contexts history into a Hashtable, given the DOM node of the
     * XML file contexts part.
     * 
     * @param contextsNode
     *            The DOM node of the XML file contexts part
     * @return A Hashtable which keys are History.SELECTED_CONTEXT_KEY and
     *         History.OPENED_CONTEXTS_KEY, and which values are respectively
     *         selected/opened Contexts Hashtables
     * @throws Exception
     */
    private static Hashtable loadContextsBook(Node contextsNode) {
        ...
        if (contextsNode.hasChildNodes()) {
            ...
            // as many loops as there are contexts types, ie. selected or
            ...
            // (which is normally three: contexts, snapshots, and options)
            {
                ...
                String currentContextChapterType = currentContextChapterNode
                        .getNodeName().trim();
                if (History.SELECTED_CONTEXT_KEY
                        .equals(currentContextChapterType)) {
                    Hashtable currentChapter = null;
                    try {
                        currentChapter = XMLUtils
                                .loadAttributes(currentContextChapterNode);
                        if (currentChapter != null) {
                            book.put(History.SELECTED_CONTEXT_KEY,
                                    currentChapter);
                        }
                ...
                } else if (History.OPENED_CONTEXTS_KEY
                        .equals(currentContextChapterType)) {
    ...
    /**
     * Loads the opened Contexts history into a Vector, given the DOM node of
     * the XML file opened contexts part.
     * 
    ...
    private static Vector loadOpenedContextsPages(Node openedContextsNode) {
        ...
        if (openedContextsNode.hasChildNodes()) {
            ...
            for (int i = 0; i < openedContextNodes.getLength(); i++) {
                ...
                String currentOpenedContextType = currentOpenedContextNode
                        .getNodeName().trim();
                Hashtable currentPage = null;
                if (History.OPENED_CONTEXT_KEY.equals(currentOpenedContextType)) {
                    try {
                        currentPage = XMLUtils
                                .loadAttributes(currentOpenedContextNode);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    /**
     * Loads the Snapshots history into a Hashtable, given the DOM node of the
     * XML file snapshots part.
     * 
     * @param snapshotsNode
     *            The DOM node of the XML file snapshots part
     * @return A Hashtable which keys are History.SELECTED_SNAPSHOTS_KEY and
     *         History.OPENED_SNAPSHOTS_KEY, and which values are respectively
     *         selected/opened Snapshots Hashtables
     * @throws Exception
     */
    private static Hashtable loadSnapshotsBook(Node snapshotsNode) {
        ...
        if (snapshotsNode.hasChildNodes()) {
            ...
            // opened, in the current snapshot
            {
                ...
                String currentSnapshotChapterType = currentSnapshotChapterNode
                        .getNodeName().trim();

                if (History.SELECTED_SNAPSHOTS_KEY
                        .equals(currentSnapshotChapterType)) {
                ...
                } else if (History.OPENED_SNAPSHOTS_KEY
                        .equals(currentSnapshotChapterType)) {
    ...
    /**
     * Loads the opened Snapshots history into a Vector, given the DOM node of
     * the XML file opened snapshots part.
     * 
    ...
    private static Vector loadOpenedSnapshotsPages(Node openedSnapshotsNode) {
        ...
        if (openedSnapshotsNode.hasChildNodes()) {
            ...
            for (int i = 0; i < openedSnapshotNodes.getLength(); i++) {
                ...
                String currentOpenedSnapshotType = currentOpenedSnapshotNode
                        .getNodeName().trim();
                Hashtable currentPage = null;

                if (History.OPENED_SNAPSHOT_KEY
                        .equals(currentOpenedSnapshotType)) {
                    try {
                        currentPage = XMLUtils
                                .loadAttributes(currentOpenedSnapshotNode);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    pages.add(currentPage);
    ...
    /**
     * Loads the selected Snapshots history into a Vector, given the DOM node of
    ...
    private static Vector loadSelectedSnapshotsPages(Node selectedSnapshotsNode) {
        ...
        if (selectedSnapshotsNode.hasChildNodes()) {
            NodeList selectedSnapshotNodes = selectedSnapshotsNode
                    .getChildNodes();

            for (int i = 0; i < selectedSnapshotNodes.getLength(); i++) {
                Node currentSelectedSnapshotNode = selectedSnapshotNodes
                        .item(i);
                ...
                String currentSelectedSnapshotType = currentSelectedSnapshotNode
                        .getNodeName().trim();
                Hashtable currentPage = null;

                if (History.SELECTED_SNAPSHOT_KEY
                        .equals(currentSelectedSnapshotType)) {
                    try {
                        currentPage = XMLUtils
                                .loadAttributes(currentSelectedSnapshotNode);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    pages.add(currentPage);
