...
public class SpectrumAttributeDialog extends JDialog {
	...
	public SpectrumAttributeDialog(String name, SnapshotAttributeValue value) {
		...
		case AttrDataFormat._SPECTRUM:
			...
			case AttrData.Tango_DEV_UCHAR:
				...
				if (!"Nan".equals(valc) && valc != null
					...
					for (int i = 0; i < byteArray.length; i++) {
						...
						if (byteArray[i] == null)
							spectrumView.add(i + 1, Double.NaN);
						else
							spectrumView.add(i + 1, byteArray[i].doubleValue());
					}
				} else {
					spectrumView = null;
				}
				...
				if (!"Nan".equals(vall) && vall != null
					...
					for (int i = 0; i < intArray.length; i++) {
						...
						if (intArray[i] == null)
							...
							spectrumView.add(i + 1, intArray[i].doubleValue());
					}
				} else {
					spectrumView = null;
				}
				...
				if (!"Nan".equals(vals) && vals != null
					...
					for (int i = 0; i < shortArray.length; i++) {
						...
						if (shortArray[i] == null)
							...
							spectrumView
									.add(i + 1, shortArray[i].doubleValue());
					}
				} else {
					spectrumView = null;
				}
				...
				if (!"Nan".equals(valf) && valf != null
					...
					for (int i = 0; i < floatArray.length; i++) {
						...
						if (floatArray[i] == null)
							...
							spectrumView
									.add(i + 1, floatArray[i].doubleValue());
					}
				} else {
					spectrumView = null;
				}
				...
				if (!"Nan".equals(vald) && vald != null
					...
					for (int i = 0; i < doubleArray.length; i++) {
						...
						if (doubleArray[i] == null)
							...
							spectrumView.add(i + 1, doubleArray[i]
									.doubleValue());
					}
				} else {
					spectrumView = null;
			...
			case AttrData.Tango_DEV_STRING:
				...
				if (!"Nan".equals(valstr) && valstr != null
					...
					spectrumTable = new JTable(new SpectrumReadValueTableModel(
							value.getDataType(), stringArray));
		...
		if (spectrumView != null && (spectrumView.getData() != null)
			...
			//chart = new JLChart();
			chart = new JLChart();
			chart.setDAO(ATKToCometUtilities.createSpectrumDAO(spectrumView));
